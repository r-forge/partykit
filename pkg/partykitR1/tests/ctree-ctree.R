...
fun <- function(args) {
    ...
    ct <- partykitR1:::ctree_control
    ctrl <- do.call("ct", args[-(1:2)])
    ctrl$majority <- TRUE
    nt <- system.time(newmod <- try(partykitR1:::ctree(y ~ ., data = learn, control = ctrl)))[1]
...
ct11 <- partykitR1::ctree(y ~ x, data = d)
ct12 <- partykitR1::ctree(y ~ x, data = d, 
                        scores = list(y = c(1, 4, 5)))
ct13 <- partykitR1::ctree(y ~ x, data = d, 
...
ctrl = partykitR1::ctree_control(maxdepth=3, minbucket = min(d$n) + 1)
tree = partykitR1::ctree(y ~ Age, weights=n, data=d, control=ctrl)
tree

(w1 <- predict(tree, type = "node"))

detach(package:partykitR1)
library("party")
