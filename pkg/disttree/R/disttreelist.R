...
disttreelist <- function(formula, data, na.action, cluster, family = NO,
  control = mob_control(...), ocontrol = list(), ...)
{
  ...
  ## glue code for calling distfitlist() with given family in mob()
  dist_family_fit <- function(y, x = NULL, start = NULL, weights = NULL, offset = NULL,
    cluster = NULL, ..., vcov = TRUE, estfun = FALSE, object = FALSE, type.hessian = "analytic")
  {
    if(!(is.null(x) || NCOL(x) == 0L)) warning("x not used")
    if(!is.null(offset)) warning("offset not used")
    rval <- distfitlist(y, family = family, weights = weights, start = start,
      vcov = vcov, estfun = estfun, type.hessian = type.hessian, ...)
    rval <- list(
      coefficients = rval$par,
      objfun = -rval$loglik,
      estfun = if(estfun) rval$estfun else NULL,   ## rva$estfun contains the scores of the positive loglik  (FIX ME: *(-1) ?)
...
if(FALSE){
  tr <- disttreelist(dist ~ speed, data = cars)
  print(tr)
...
if(FALSE){
  ...
  test <- disttreelist(y~x, data = d, family = dist_list_normal)
  print(test)
  plot(test)
