...
varimp.disttree <- function(object, nperm = 1L, risk = c("loglik", "misclassification"),
  ...
  risk <- match.arg(risk)
  if(risk == "misclassification") stop("Risk 'missclassification' is not feasible for class 'disttree'...")

  ### risk is _NEGATIVE_ log-likelihood
  risk <- function(object, perm = NULL, ...) {
    ...
    if (is.null(perm)) {
      ...
      -as.numeric(logLik(object, newdata = newdata))
    }
  ...
  ret <- ret / nperm - risk(object, perm = NULL, ...)
