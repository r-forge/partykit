...
make_censored_dist_list <- function(distribution = c("normal","logistic"), 
                                     ...
                                     censpoint = 0)
{
  ...
  family.name <- if(distribution == "normal") "censored Normal Distribution" else "censored Logistic Distribution"
  ...
  ddist <-  function(y, eta, log = TRUE, weights = NULL, sum = FALSE, left = left, right = right) {     
    par <- c(eta[1], exp(eta[2]))
  ...
  sdist <- function(y, eta, weights = NULL, sum = FALSE, left = left, right = right) {   
    par <- c(eta[1], exp(eta[2]))
    # y[y==0] <- 1e-323
  ...
  hdist <- function(y, eta, weights = NULL, left = left, right = right) {    
    ny <- length(y)
    if(is.null(weights)) weights <- rep.int(1, ny)
  ...
  startfun <- function(y, weights = NULL){
    ...
    if(type == "left") yc <- max(censpoint,y) 
    if(type == "right") yc <- min(censpoint,y) 
