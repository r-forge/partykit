...
\author{Lisa Schlosser, Torsten Hothorn, Reto Stauffer, Achim Zeileis}
...
# setwd("~/svn/partykit/pkg/disttree/inst/eeecon_workshop")
library("ggplot2")
...
# 10 split variables (x1, ..., x10) are available (5 numeric, 2 binary, 3 categorical)
#
...
\node[font=\scriptsize] at (-1.9,-2) {2018-03-15};
}
\visible<2->{
...
\node[font=\scriptsize] at (5.4,-2) {2018-03-16};
}
\end{tikzpicture}
\end{center}
\visible<3, 5->{
...
Summary:
\begin{itemize}
\item Stochastic model to approximate data generating process.\\
\only{$\Rightarrow$ fixed $f$.\\}
\item Disadvantage: Risk of incorrect assumptions.
\end{itemize}
}
\only<1>{\vspace{1cm}}
...
nobs <- 120
x <- c(1:nobs)/nobs
ytrue <- ytree <- ytree2 <- ytree3 <- ytree4 <- 
  ytree_up <- ytree_up2 <- ytree_up3 <- ytree_up4 <- 
  yforest <- numeric(length = length(x))
...
for(i in 1:nobs) ytree2[i] <- if(x[i]<0.63) 4.3 else 9.4
for(i in 1:nobs) ytree3[i] <- if(x[i]<0.75) 3.7 else 9.2
for(i in 1:nobs) ytree4[i] <- if(x[i]<0.55) 4.4 else 8.7
ytree_up <- rep.int(15, length(x))
ytree_up2 <- rep.int(14.5, length(x))
ytree_up3 <- rep.int(15.3, length(x))
ytree_up4 <- rep.int(15.5, length(x))
for(i in 1:nobs) yforest[i] <- if(x[i]<0.55) 4 else {if(x[i]<0.63) 5 else {if(x[i]<0.7) 6.5 else {if(x[i]<0.75) 8 else 9}}}
@
...
%\vspace{-0.5cm}
\textbf{Example:} \only<3>{Tree model}\only<4-10>{Subsampling}\only<3>{\\}\only<11->{Forest model}\\
%\vspace{-0.8cm}
\begin{center}
\only<3>{
\vspace{-0.96cm}
...
\vspace{-0.7cm}
\begin{flushleft}
\noindent {\small X = rain now, humidity\\
Y = rain 24h}
\end{flushleft}}
\only<4->{
\vspace{-0.5cm}
...
\draw[gray,rounded corners=10pt] (-1,4.3) -- (4,4.3) -- (4,2.7) -- (-2,2.7) -- (-2,4.3) -- (0,4.3);
...
\end{scope}
\visible<5->{
\node[ellipse, draw=black!50!green, align=center, minimum width=80pt, minimum height = 40pt, line width = 3pt] (subset1) at (0.1, 3.5) {};
}
\visible<7->{
\node[ellipse, draw = blue, align=center, minimum width=100pt, minimum height = 30pt, line width = 3pt] (subset2) at (1.2, 3.6) {};
}
\visible<9->{
\node[ellipse, draw = orange, align=center, minimum width=120pt, minimum height = 25pt, line width = 3pt] (subset3) at (1.6, 3.3) {};
}
\visible<6->{
...
lines(x = x, y = ytree_up, col = pal["forest"], lwd=7)
...
lines(x = x, y = ytree_up2, col = pal["tree"], lwd=7)
mtext("rain", side = 4, las = TRUE, at = 16, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("now", side = 4, las = TRUE, at = 14.7, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("no", side = 4, las = TRUE, at = 10.3, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("rain", side = 4, las = TRUE, at = 9, line = 0.5, cex = 2.5, col = pal["forest"])
...
lines(x = x, y = ytree_up2, col = pal["tree"], lwd=7)
lines(x = x, y = ytree_up3, col = "blue", lwd=7)
mtext("rain", side = 4, las = TRUE, at = 16, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("now", side = 4, las = TRUE, at = 14.7, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("no", side = 4, las = TRUE, at = 10.3, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("rain", side = 4, las = TRUE, at = 9, line = 0.5, cex = 2.5, col = pal["forest"])
...
lines(x = x, y = ytree_up4, col = "orange", lwd=7)
mtext("rain", side = 4, las = TRUE, at = 16, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("now", side = 4, las = TRUE, at = 14.7, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("no", side = 4, las = TRUE, at = 10.3, line = 0.5, cex = 2.5, col = pal["forest"])
mtext("rain", side = 4, las = TRUE, at = 9, line = 0.5, cex = 2.5, col = pal["forest"])
...
\begin{itemize}
  ...
  Automatic detection of abrupt changes,
  and selection of variables/interactions.
  \item Random forests:\\
...
$\Rightarrow$ Distributional trees and forests
...
kappa <- 12
x <- c(1:nobs)/nobs
ytrue <- ytree <- ytree2 <- ytree3 <- ytree4 <- ytree_up <- yforest <- var <- var_up <- numeric(length = length(x))
for(i in 1:nobs) ytrue[i] <- if(x[i]<0.2) 4 else 4+5*plogis(kappa*(3*(x[i]-0.33)-1))
ytrue_up <- rep.int(15, length(x))
for(i in 1:nobs) var_up[i] <- if(x[i]<0.15) 3 else (if(x[i]<0.35) 2 else (if(x[i]<0.5) 2 else (if(x[i]<0.7) 1 else (if(x[i]<0.8) 1.5 else (if(x[i]<0.9) 2 else (2.5))))))
for(i in 1:nobs) var[i] <- if(x[i]<0.15) 4 else (if(x[i]<0.2) 3 else (if(x[i]<0.25) 2 else (if(x[i]<0.4) 1 else (if(x[i]<0.8) 2 else (if(x[i]<0.9) 2.5 else (3))))))
y <- ytrue + rnorm(nobs,0, sqrt(var))
y_up <- ytrue_up + rnorm(nobs,0, sqrt(var_up))
for(i in 1:nobs) ytree[i] <- if(x[i]<0.7) 4 else 9
ytree_up <- rep.int(15, length(x))
...
lines(x = x, y = ytree_up, col = pal["forest"], lwd=10)
@
...
\visible<2->{
<<plot_motivation_distforest, fig=TRUE, echo=FALSE, width=6>>=
par(mar=c(0,0,3,0))
plot(x = x, y = y, xaxt="n", yaxt="n", ann=FALSE, col = "slategray", pch = 19,
     ylim = c(min(c(y,y_up)), max(c(y,y_up))))
polygon(c(x, rev(x)), c(yforest + var, rev(yforest - var)),
  col = pallight["forest"], border = "transparent")
polygon(c(x, rev(x)), c(ytrue_up + var_up, rev(ytrue_up - var_up)),
  col = pallight["forest"], border = "transparent")
points(x=x, y = y_up, col = "slategray", pch = 19)
box(lwd=5)
lines(x = x, y = ytree_up, col = pal["forest"], lwd=10)
...
%\textbf{Idea:} Capture covariates through adaptive weights.
%\[
%\hat{\theta}(\bold{x}) = \argmax_{\theta \in \Theta} \sum_{i=1}^n w_i(\bold{x}) \cdot \ell%(\theta; y_i).
%\]
...
%\textbf{Possible answers:} Based on learning sample $\{(y_i, \bold{x}_i)\}_{i=1, \dots, n}$ and (possibly new) observation $\bold{x}$.
%\begin{itemize}
%\item \emph{Tree:} $w_i(\bold{x}) \in \{0,1\}$ indicates whether $\bold{x}$ and $\bold{x}_i$ are classified into the same subgroup.
%\item \emph{Forest:} $w_i(\bold{x}) \in  [0, 1]$ averages the weights for $\bold{x}$ and $\bold{x}_i$ across trees.
%\end{itemize}
...
Parameter estimator for a \only<1-3>{global}\only<4->{local} model with learning data \only<1>{$\{y_i\}_{i=1,\ldots,n}$}\only<2->{$\{(y_i,\bold{x}_i)\}_{i=1,\ldots,n}$}:
...
\textbf{Weights:}
\begin{eqnarray*}
w^{\text{base}}_i(\bold{x})   & = & 1 \\[0.25cm]
\visible<4->{
w^{\text{tree}}_i(\bold{x})   & = & \sum_{b=1}^B I((\bold{x}_i \in \mathcal{B}_b) \land (\bold{x} \in \mathcal{B}_b)) \\[0.2cm]
\visible<5->{
...
plot( GFS_rain24[,1], screen=1, xaxs="i", xaxt="n", yaxs="i", ylim=range(GFS_rain24)*c(0,1.05), type = "o", col = "slategray" )
...
plot( GFS_t2m[,1], screen=1, xaxs="i", xaxt="n", 
      ylim = range(GFS_t2m)*1.05, col = "slategray" )
lines( GFS_t2m[,"obs"], col = "red", lwd = 2)
abline( v = main, lty = 2 )
mtext( side = 3, line = 1, cex = 1.2, font = 2, title )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Temperature [°C]" )
plot( GFS_rain24[,1], screen=1, xaxs="i", xaxt="n", yaxs="i", 
      ylim=range(GFS_rain24)*c(0,1.05), type = "o", col = "slategray" )
...
\frametitle{Probabilistic Forecasting of Precipitation}
\vspace*{0.2cm}
\textbf{Application for one station:}
\begin{itemize}
...
and 
  \emph{Working Papers, Faculty of Economics and Statistics, University of Innsbruck},
  \url{https://EconPapers.repec.org/RePEc:inn:wpaper:2018-08}
