...
\author{\small{Lisa Schlosser\\
\vspace{0.4cm}
Supervisors: Achim Zeileis, Torsten~Hothorn, Michael Oberguggenberger\\}
\vspace{0.4cm}
\footnotesize{PhD thesis defense, 2020-04-24}
%Committee: Achim Zeileis, Michael Oberguggenberger, Georg Moser
}
...
\definecolor{forestred}{rgb}{0.8078431, 0.2862745, 0.3176471}
\definecolor{treegreen}{rgb}{0.0000000, 0.5607843, 0.0000000}
...
\end{center}
\vspace{0.3cm}
\textbf{Goal:} Approximation of reality based on a set of observations.\\
\pause
...
\begin{itemize}
\item Prediction.
\item Capturing dependencies and interactions.
\end{itemize}
\pause

\vspace{0.5cm}
\textbf{Two cultures:} Breiman (2001, \emph{Statistical Science})
\begin{itemize}
\item Data modeling.
...
\textbf{Example:} 
\only<2-3>{Linear model}\only<4>{Generalized additive model}\only<5>{Generalized additive model\\ for location, scale, and shape}
\\
\only<2-3>{
...
\begin{align*}
\visible<5>{\mathbb{E}(Y|X) = }\mu(X) = \alpha_{\mu} + s_{\mu}(X)\\  
\visible<5>{\sqrt{Var(Y|X)} = }\sigma(X) = \alpha_{\sigma} + s_{\sigma}(X)
\end{align*}
}
\end{minipage}
%\vspace{-0.6cm}
...
for(i in 1:(nobs*100)) yforest[i] <- if(x100[i]<=0.33) 0.52 else {if(x100[i]<=0.34) 1.5 else {if(x100[i]<=0.54) 2 else {if(x100[i]<=0.6) 1.9 else {if(x100[i]<=0.64) 1.7 else {if(x100[i]<=0.7) 1.4 else {if(x100[i]<=0.74) 1.2 else 1}}}}}}
@


\subsubsection{Algorithmic modeling}
...
\vspace{0.25cm}
\visible<3->{
\begin{minipage}{0.5425\textwidth}
\textbf{Example:} \only<3>{Tree model}\only<4-10>{Subsampling}\only<11->{Forest model}
%\vspace{-0.5cm}
%\vspace{-0.8cm}
\begin{center}
\only<3>{
...
\visible<11->{
\draw[-, line width=1pt] (-0.5, -0.1) -- (0.2,-0.5);
\draw[-, line width=1pt] (1, -0.1) -- (1,-0.5);
\draw[-, line width=1pt] (2.5, -0.1) -- (1.8,-0.5);
\node[rectangle, align=center] (combine) at (1, -0.9) {\Large average};
}
\end{tikzpicture}
}
}
...
@
% mtext("rain", side = 4, las = TRUE, at = 16, line = 0.5, cex = 2.5, col = pal["forest"])
% mtext("now", side = 4, las = TRUE, at = 14.7, line = 0.5, cex = 2.5, col = pal["forest"])
% mtext("no", side = 4, las = TRUE, at = 10.3, line = 0.5, cex = 2.5, col = pal["forest"])
% mtext("rain", side = 4, las = TRUE, at = 9, line = 0.5, cex = 2.5, col = pal["forest"])
% mtext("now", side = 4, las = TRUE, at = 7.7, line = 0.5, cex = 2.5, col = pal["forest"])
% mtext("rain 24h", side = 2, las = FALSE, line = 1, cex = 3)
% mtext("humidity", side = 1, line = 1.5, cex = 3)
}
\only<6-7>{
%\vspace{0.01cm}
%\hspace{-0.1cm}
...
\textbf{Parameter estimator} for \only<1-3>{\textbf{a global}}\only<4->{\textbf{an adaptive local}} model\\
with learning data \only<1>{$\{y_i\}_{i=1,\ldots,n}$}\only<2->{$\{(y_i,\bold{x}_i)\}_{i=1,\ldots,n}$} :
...
\visible<5->{
w^{\text{forest}}_i(\bold{x}) & = & \frac{1}{T} \sum_{t=1}^T \sum_{b=1}^{B^t} \frac{I((\bold{x}_i \in \mathcal{B}^t_b) \land (\bold{x} \in \mathcal{B}^t_b))}{\left|\mathcal{B}^t_b\right|}
\end{eqnarray*}
}}}
\end{frame}
...
\subsection{Unbiased recursive partitioning}

\begin{frame}%[fragile]
\frametitle{}
\vspace{1.5cm}
\center
{\bf \large The Power of Unbiased Recursive Partitioning: \\
\smallskip
...
Preprint version at \url{https://arxiv.org/abs/1906.10179}}
...
\begin{itemize}
\smallskip
  \item Separation of split variable and split point selection.
\smallskip
  \item Application of a statistical test to assess associations between the response~$Y$ (or corresponding scores) and each possible split variable~$X_j$.
\smallskip
  \item Split variable selection based on resulting $p$-values.
\end{itemize}
\end{frame}
% \begin{frame}%[fragile]
% \frametitle{Unbiased recursive partitioning}
% 
% {\bf General testing strategy for split variable selection:}
% \begin{enumerate}
% \item Evaluate a discrepancy measure capturing the observation-wise goodness of fit of
% the considered model.
% %$\mathcal{M}(Y,X;\hat{\beta})$.
% \item Apply a statistical test assessing dependency of the discrepancy measure to each possible split variable $X_j$.
% \item Select the split variable $X$ %$X_j^\ast$ 
% showing the smallest $p$-value.
% \end{enumerate}
% 
% \bigskip
% 
% {\bf Discrepancy measures:} (Model-based) transformations of $Y$, possibly for each model parameter.
% \begin{itemize}
% \item (Ranks of) $Y$.
% \item (Absolute) deviations $Y - \bar Y$.
% \item Residuals of the fitted model. %$\mathcal{M}(Y,X;\hat{\beta})$.
% \item Score matrix of the fitted model. %$\mathcal{M}(Y,X;\hat{\beta})$.
% \item \ldots
% \end{itemize}
% \end{frame}
...
\begin{frame}
\frametitle{Unbiased recursive partitioning algorithms}

\textbf{GUIDE:} Loh (2002, \emph{Statistica Sinica}).
\begin{itemize}
  \item First \emph{unbiased} algorithm for recursive partitioning of linear models.
  \item Based on categorical association tests. 
\end{itemize}

\medskip

\textbf{CTree:} Hothorn, Hornik, Zeileis (2006, \emph{JCGS}).
\begin{itemize}
  \item Proposed as unbiased recursive partitioning for \emph{nonparametric} modeling.
  \item Can be model-based via model scores as the response transformation.
  \item Based on conditional permutation tests.
\end{itemize}

\medskip

\textbf{MOB:} Zeileis, Hothorn, Hornik (2008, \emph{JCGS}).
\begin{itemize}
  \item Model-based unbiased recursive partitioning. % using M-estimation (ML, OLS, CRPS, \ldots)
  \item Based on asymptotic parameter instability tests.
\end{itemize}
...
\textbf{Algorithms:} CTree, MOB, GUIDE are all `flavors' of a general framework.
...
\begin{tabular}{llccl}
\hline 
      & Scores  & Binarization  & Categorization  & Test statistic \\ 
\hline
...
% Internal stopping criterion based on Bonferroni-corrected $p$-values of the underlying tests. Stop splitting when there is no significant association.
...
% \vspace{0.2cm} 
% 
% \textbf{Based on evaluations in our simulation study:}
% 
% \vspace{0.2cm} 
% 
% \begin{itemize}
% \setlength\itemsep{0.2cm}
% \item Full model scores better than residuals only. %: Using scores allows for capturing changes in all model parameters.
...
% \item Maximally-selected statistics (as in MOB) more powerful for abrupt shifts.
% \item Linear statistics (default in CTree) more powerful for linear changes.
...
\item Best performance with full model scores, without binarization, and without categorization of possible split variables.
\item Adapting GUIDE to employing full model scores improves its performance.
\item Preferable test statistic depends on the type of effects (maximally-selected for abrupt shifts, linear statistics for linear changes).
%\item If the significance test performs well pre-pruning works well,\\
%otherwise post-pruning might be needed.
%\item Including full model scores is advantageous over using only residuals.
%\item Binarizing scores/residuals does not improve the performance.
...
\vspace{1.5cm}
\center
{\bf \large Distributional Regression Forests for Probabilistic\\
\smallskip
...
\subsection{Weather forecasting: precipitation}
...
\subsection{Weather forecasting: wind direction}
...
\center
{\bf \large 
Circular Regression Trees and Forests with an \\
\smallskip
...
\small{
Moritz N. Lang, Lisa Schlosser, Torsten Hothorn, \\
...
Preprint version at \url{https://arxiv.org/abs/2001.00412}}

\end{frame}


\begin{frame}[fragile]
\frametitle{Weather forecasting: wind direction}
...
\begin{itemize}
  ...
  \item Base variables: Wind direction, wind (gust) speed, temperature, \\
...
\begin{figure}
\includegraphics[center]{circtree_ibk.pdf}
\end{figure}

%\vspace{-0.3cm}
\textbf{Conclusions:}
\begin{itemize}
\item Circular trees are an easy-to-apply method with high interpretability. 
\item A single circular tree provides reliable predictions which can be improved by employing a circular forest model.
\item Competitive performance: better than persistence and linear model.
\end{itemize}
...
% \item Circular GLM: Based on current wind speed and wind vectors $(u, v)$.
% \end{itemize}
% 
% \end{frame}
% 
...
\emph{arXiv 1906.10179}, arXiv.org E-Print Archive.
\url{https://arxiv.org/abs/1906.10179}.
Submitted to \emph{Computational Statistics \& Data Analysis}.
...
Minor revision in \emph{Journal of the Royal Statistical Society C}.
Preprint version at \url{https://arxiv.org/abs/2001.00412}
