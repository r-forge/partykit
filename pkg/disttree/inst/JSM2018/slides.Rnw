...
\item Choose the breakpoint along this variable with highest improvement in log-likelihood and split the sample.
\item Repeat steps 2--5 recursively in the subgroups
    until some stopping criterion is met, yielding $B$ subgroups $\mathcal{B}_b$ with $b = 1, \dots, B$.
...
# predictions
pred1 <- c(testdata[pdays,"robs"][1]^(1.6), crch::dcnorm(testdata[pdays,"robs"][1], mean = df_mu[1], sd = df_sigma[1], left = 0))
pred2 <- c(testdata[pdays,"robs"][2]^(1.6), crch::dcnorm(testdata[pdays,"robs"][2], mean = df_mu[2], sd = df_sigma[2], left = 0))
pred3 <- c(testdata[pdays,"robs"][3]^(1.6), crch::dcnorm(testdata[pdays,"robs"][3], mean = df_mu[3], sd = df_sigma[3], left = 0))
pred4 <- c(testdata[pdays,"robs"][4]^(1.6), crch::dcnorm(testdata[pdays,"robs"][4], mean = df_mu[4], sd = df_sigma[4], left = 0))
...
# add labels
text(x = -1.7, y = lh1, labels = "2009", col = "red", cex = 0.8)         # -0.8
text(x = -1.7, y = lh2, labels = "2010", col = "blue", cex = 0.8)        # -0.8
text(x = -1.7, y = lh3, labels = "2011", col = "darkgreen", cex = 0.8)   # -0.8
text(x = -1.7, y = lh4, labels = "2012", col = "purple", cex = 0.8)      # -0.8
@
\end{center}
\end{frame}
...
%BMLFUW (2016).
%  \dquote{{B}undesministerium f\"ur {L}and und {F}orstwirtschaft, {U}mwelt und {W}asserwirtsch%aft ({BMLFUW}), {A}bteilung {IV/4} -- {W}asserhaushalt}
