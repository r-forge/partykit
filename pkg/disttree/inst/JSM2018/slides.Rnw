...
# setwd("~/svn/partykit/pkg/disttree/inst/JSM2018")
library("ggplot2")
...
\minipage{0.285\textwidth}
  ...
  \begin{center}
  \begin{tikzpicture}
  \draw[line width=0.2mm, black] (0,0) rectangle (2.44,2.12);
  \node[rectangle, align=center] (sq) at (1.22, 1.06) {\Huge{?}};
  \end{tikzpicture}
  \end{center}}
\endminipage
...
{\bf Tree:}
\begin{itemize}
  \item Automatic variable selection and detection of steps and abrupt changes. %(data driven)
...
\item Test for associations or instabilities of the scores $\frac{\partial \ell}{\partial \theta}(\hat{\theta};y_i)$ and each partitioning variable.
...
\item Choose the partitioning variable with the strongest association or instability as split variable.
\item Choose the breakpoint along this variable leading to the highest improvement in log-likelihood and split the sample.
\item Repeat steps 2--5 recursively until some stopping criterion is met, yielding $B$ subgroups $\mathcal{B}_b$ with $b = 1, \dots, B$.
...
\textbf{Parameter estimator} for \only<1-2>{\textbf{a global}}\only<3->{\textbf{an adaptive local}}\\ model with learning data \only<1>{$\{y_i\}_{i=1,\ldots,n}$}\only<2->{$\{(y_i,\bold{x}_i)\}_{i=1,\ldots,n}$} :
...
\textbf{Weights:}
\begin{eqnarray*}
w^{\text{global}}_i(\bold{x})   & = & 1 \\[0.2cm]
\visible<3->{
...
\medskip
\textbf{Input:} Numerical Weather Prediction (NWP) based on a global physical model numerically simulating atmospheric processes on a $50\times 50 \text{km}^2$ grid.\\
\medskip
...
\begin{itemize}
\item Ensemble predictions of various meteorological quantities related to precipitation, temperature,
air pressure, \dots.\\
...
# plot predictions
points(x = pred1[1], y = pred1[2], col = "red", pch = 4, cex = 1.4)
points(x = pred2[1], y = pred2[2], col = "blue", pch = 4, cex = 1.4)
points(x = pred3[1], y = pred3[2], col = "darkgreen", pch = 4, cex = 1.4)
points(x = pred4[1], y = pred4[2], col = "purple", pch = 4, cex = 1.4)
...
# add labels
text(x = -0.8, y = lh1, labels = "2009", col = "red", cex = 0.8)         # -1.7 
text(x = -0.8, y = lh2, labels = "2010", col = "blue", cex = 0.8)        # -1.7
text(x = -0.8, y = lh3, labels = "2011", col = "darkgreen", cex = 0.8)   # -1.7
text(x = -0.8, y = lh4, labels = "2012", col = "purple", cex = 0.8)      # -1.7
@
\end{center}
\end{frame}
...
\end{itemize}
%\begin{itemize}
%\item Prespecified GAMLSS: variable selection based on meteorological expert knowledge.
%\item Boosted GAMLSS: automatic variable selection.
