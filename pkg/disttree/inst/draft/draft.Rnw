...
\begin{cases}
    ...
    \log\left\{\sigma^{-1} \cdot \phi\left((Y - \mu) \cdot \sigma^{-1}\right) \right\}, & \text{if } Y > 0\\[0.2cm]
    \log\left\{\Phi\left((-\mu) \cdto \sigma^{-1}\right)\right\}, & \text{if } Y = 0
\end{cases}
$$
...
$$
where $s(\bm{\hat \theta}, y_i)$ is the associated score function
$$ \label{eq:score}
s(\bm{\hat{\theta}}, y_i) = \frac{\partial \ell}{\partial \bm{\theta}}(\bm{\theta}; y_i) \mid_{\bm{\theta} = \bm{\hat{\theta}}}.
$$ 
...
Distributional random forests share some concepts and algorithmic aspects 
...
or generalized forests). Distributional random forests, in contrast to these 
...
(zero left-censored normal for precipitation) have a clear meaning but 
...
\item \emph{Prespecified GAMLSS:} Smooth additive splines are selected for
  ...
  submodel and six in the scale submodel. Both involve an interaction of
...
The models are learned on six~folds (= 24~years) and evaluated on the 7-th fold
(= 4~years) using the average CRPS across all observations.
The resulting ten~CRPS skill scores are displayed by boxplots in
...
\code{replication.R} provided in the package \textbf{disttree}.
...
can be plugged in the score function $s(\bm{\hat{\theta}}, Y)$.
