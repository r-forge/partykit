...
Less specifically, generalized additive models for location, scale, and shape (GAMLSS) provide a framework where each distribution parameter is modeled separately capturing smooth linear or nonlinear effects.
%
...
$$
\hlc{Even though this parameter depends on the given $z$ it has to be noted that
it is the same for all other observations that are in the same terminal node of 
the tree as $z$ and therefore does not necessarily have to be recalculated for
each of them.}
...
correlation among the trees and to stabilizes the variance of the model.
...
\hlc{\textit{FIXME: notation with or without power transformation parameter?}}
...
from local meteorological observations or an NWP model. Unfortunately, it is typically unknown 
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  for variable selection} (\code{alpha}) \hlc{is kept at the default value $1$.}
  % stopping criteria:
...
complexity and accuracy''} \citep{Biau+Scornet:2016}.
...
left-censored at zero. However, comparing the two censored distributions,
at many stations the models perform not as well under the logistic distribution 
as under the Gaussian distribution. The hurdle model leads to slight improvements 
in some settings, but this comes with more variability (especially for EMOS and the 
boosted GAMLSS).
Altogether, the distributional forest has proven to be very robust against
...
QQ~plots (Figure~\ref{fig:axams-qq}).
