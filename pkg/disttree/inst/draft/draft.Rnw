...
As illustrated in Figure~\ref{Devel_parmod} a wide range of tools have been developed in the field of parametric modeling (which can be applied in research areas such as probabilistic forecasting). The shift from modeling only the expected value of the response to probabilistic modeling allows for much broader inference. In many applications further features such as the ability to capture non-additive effects or interactions are desirable which demands for more flexible models. Since tree-structured models provide these required features combining them with parametric models can offer a framework that is able to deal with a wide variety of different modeling situations. 
...
From these results we can conclude that a distributional forest provides a powerful alternative framework requiring only little effort to set up a model which is competitive to commonly used methods or even outperforms them. 
