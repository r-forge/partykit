...
interpretability and a high goodnes-of-fit. For distributional forests, the focus
...
criteria} \code{minsplit}, \code{minbucket} and \code{alpha}.
...
\hlc{Following this advice we chose to build forests consisting of 100 trees.}
...
shows slightly better results, however, only within a range of less than $2\%$
and not for all stations. Altogether, the distributional forest has proven to be 
