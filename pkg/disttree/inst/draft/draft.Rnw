...
\begin{tabular}{| l  c  l |}
\hline
\thead{basic covariates} & \thead{nr} & \thead{variations}\\
\hline
\textbf{tp{\_}pow}: total precipitation    & 12 variations:  & ensemble mean of sum, \\
\hspace*{\fill} power transformed (by 1.6) &    & ensemble std dev of sum, \\ 
\textbf{cape{\_}pow}: convective           &    & ensemble min of sum, \\
\hspace*{0.4cm} available potential energy &    & ensemble max of sum\\
...
\hspace*{\fill}                            &    & \qquad 6--12, 12--18, 18--24, 24--30 UTC\\
\hline
\textbf{dswrf}: downwards short ware      & 6 variations: & ensemble mean of mean, \\
...
\hline
\textbf{tdiff500850}: temperature         & 3 variations: & ensemble mean of diff in mean,\\
\hspace*{\fill} difference 500 to 850 hPA &   & ensemble min of diff in mean,\\
\textbf{tdiff500700}: temperature         &   & ensemble max of diff in mean\\
...
Fit one model on the first 25 years (1985--2009, both included)\\
