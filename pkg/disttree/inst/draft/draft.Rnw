...
\Abstract{
In the field of probabilistic forecasting classical linear models such as ensemle model output statistics (EMOS) are still commonly and widely used. This is due to their simplicity and their interpretability. However, the restriction to modelling only linear effects required further extensions. Generalized additive models (GAM) which can also capture non-linear effects allow for more flexibilty.\\
only location\\
GAMLSS\\
trees and forests not yet applied\\
less effort\\
