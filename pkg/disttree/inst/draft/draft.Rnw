...
To prove these statements distributional trees and forests are evaluated on generated data sets. Their results are then compared to those of a GAMLSS (\cite{stasinopoulos2005}) which fits a separate generalized additive model to each of the distribution models and in that way specifies a whole distribution and is a very efficient option for smooth effects of the covariates on the parameters.\\
The generated data sets consist of the values of a response variable $Y$ and six covariates $X_1, \ldots, X_6$ which are all independent and distributed as follows:
$$
...
The location parameter $\mu$ depends on $X_1, X_4$ and $\kappa$ and the scale parameter $\sigma$ depends on $X_1, X_4$ and $\kappa$. The other covariates $X_2, X_3, X_5$ and $X_6$ are used as noise variables.\\
...
and
$$
\mu(X_1, X_4, \kappa) = 0.2 + 7 \cdot \exp{(-(3 \cdot X_1-1.2)^{(2\cdot \kappa)})} \cdot \mathbf{1}(X_1 < 0.4) + 
                              7 \cdot \exp{(-(3 \cdot X_1-1.2)^{(2\cdot \kappa)})} \cdot \frac{1}{2} + \frac{7}{2} \cdot \mathbf{1}(X_1 \geq 0.4) + 
                              3 \cdot \exp{(-(3 \cdot X_1-1.2)^{(2\cdot \kappa)})} \cdot X_4
$$

$$
\sigma(X_2,\kappa) = 0.5 + \frac{\mu}{5} + 2 \cdot \mathbf{1}(X_1 < -0.7)
$$
...
To illustrate the parameter functions and the impact of $\kappa$ together with the results of the considered methods they are first applied on one generated data set only where $\kappa$ is set to 1 (top plots in Figure \ref{}) and then to a second data set where $\kappa$ is set to 10 (bottom plots in Figure \ref{}).\\
...
if(file.exists("oneset10.rda")){Hans-Brenner-Platz
  load("oneset10.rda")
} else {
...
In the case of smooth effects of the coefficients on the parameter function the GAMLSS is closest to the true parameter while the steps of the tree are cleary too rough. By combining various trees the forest can smooth out these steps, however, not as well as the GAMLSS.\\
Looking at the other extreme situation in which the parameter functions include abrupt changes the tree captures them very well wich also leads to a good fit of the forest model. In contrast to the two tree structured models the GAMLSS has its difficulties with these steps and therefore fluctuates around the true parameter function.\\
\\


Now, to get a more profound impression, the methods are evaluated on various data sets. The value of $\kappa$ is increasing from 1 to 10 by a stepsize of 1 and for each step ... data sets are generated.\\
\\
...
\subsection{Models}
Next to distributional forests other methods are applied to this data set and their results are compared based on RMSE, log-likelihood values and CRPS values. The chosen methods and their specifications are listed in Table \ref{model_specification}.
\begin{table}[t!]
%\resizebox{1.4\textwidth}{!}{
%\begin{minipage}{\textwidth}
\begin{center}
