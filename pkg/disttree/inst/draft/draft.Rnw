...
\Plainauthor{Lisa Schlosser, Torsten Hothorn, Achim Zeileis, Reto Stauffer}
...
In particular, 14 basic forecast variables are considered with up to 12 variations of these variables being included as covariates. Since each forecast variable consists of an ensemble of individual predictions the possible variations include the mean, the standarad deviation, the minimum and the maximum over all these individual predictions. Another variation refers to the time periods which can be over 24 hours or over time windows of 6 hours. Another possible option is to consider the mean over 24 hours for each individual prediction and then take the mean over the whole ensemble. In the following table listing all covariates this last option is referred to as the ensemble mean of mean.\\
...
\begin{tabular}{| l | c | l |}
\hline
basic covariates & nr & variations\\
\hline
\textbf{tp{\_}pow}: total precipitation    & 12 & ensemble mean, ensemble std dev, \\
\hspace*{\fill} power transformed (by 1.6) &    & ensemble min, ensemble max\\
\textbf{cape{\_}pow}: convective           &    & (all for 6--30 UTC)\\
\hspace*{0.4cm} available potential energy &    & ensemble mean: 6--12, 12--18,\\
\hspace*{\fill}  power transformed (by 1.6)&    & \hspace{2.5cm} 18--24, 24--30 UTC\\
\hspace*{\fill}                            &    & ensemble std dev: 6--12, 12--18,\\
\hspace*{\fill}                            &    & \hspace{2.85cm}  18--24, 24--30 UTC\\
\hline
\textbf{dswrf}: downwards short ware & 6 & ensemble mean of mean, \\
\hspace*{\fill} radiation flux (sunshine) &     & ensemble mean of min,\\
\textbf{msl}: mean sea level pressure  &     &  ensemble mean of max,\\
\textbf{pwat}: preciptable water &   & ensemble std dev of mean,\\
\textbf{tmax}: 2m maximum        &   & ensemble std dev of min,\\
\hspace*{1.15cm} temperature  &   & ensemble std dev of max,\\
\textbf{tcolc}: total column-integrated &   & (all for 6--30 UTC)\\
...
\hline
\textbf{tdiff500850}: temperature         & 3 & ensemble mean,\\
\hspace*{\fill} difference 500 to 850 hPA &   & ensemble min,\\
\textbf{tdiff500700}: temperature         &  & ensemble max\\
\hspace*{\fill} difference 500 to 700 hPA &   & (all for 6--30 UTC)\\
...
\hspace{1.65cm} difference            &   & (for 6--30 UTC)\\
\hline
\end{tabular}
...
\hline
 & \textbf{mu link} & \textbf{sigma link} & \textbf{mu covariates} & \textbf{sigma covariates} \\
\hline
\textbf{disttree} & identity & log & all covariates & all covariates \\ 
\hline
\textbf{distforest} & identity & log & all covariates & all covariates \\
\hline
\textbf{gamlss} & identity & log & tppow{\_}mean, & tppow{\_}sprd,\\
& & & tppow{\_}max, & dswrf{\_}sprd{\_}mean, \\
& & & tppow{\_}mean1218, & tppow{\_}sprd1218,\\ 
& & & capepow{\_}mean1218, & capepow{\_}mean1218,\\
...
& & & msl{\_}diff & \\
\hline
\textbf{gamboostLSS} & identity & log & all covariates & all covariates \\
\hline
\textbf{EMOS} & identity & log & tppow{\_}mean & tppow{\_}spread \\
\hline
\end{tabular}
\\
manuall selected covariates in gamlss\\
mention interactions:\\
tppow{\_}mean1218 $\cdot$ capepow{\_}mean1218,\\
tppow{\_}sprd1218 $\cdot$ capepow{\_}mean1218\\
\\
(advantage: no sepcification needed in disttree/distforest)
...
one station \\
Axams, closest to Ibk\\
ca 800 observations\\

\subsubsection{Predictions}
Fit one model on the first 25 years.\\
predictions for the 26th, 27th and 28th year\\
plot predicted distributions together with obersvations for one day in July of 26th, 27th and 28th year  
<<echo=FALSE, results=hide>>=
...
if(file.exists("vimp100_crps.rda")){
  #load("~/svn/partykit/pkg/disttree/inst/draft/vimp100_crps.rda")
...
# plot effect of most important variables on parameter
if(FALSE){
...
@
\end{figure}
marginal effect of covariate with highest variable importance in distforest\\
(other covariates? no effects for 4th variable, probably because only splits after several other splits)
...
\hspace{-1cm}
\begin{subfigure}{0.3\textwidth}
<<rain_cross_axams_crps, fig=TRUE, echo=FALSE>>=
boxplot(rain_crps)
@
\end{subfigure}
\hspace{-1cm}
...
@
\end{subfigure}
\caption{\label{boxplots_rain}Results 10x10 cross validation at the station Axams (RMSE, log-likelihood and CRPS)\\
left: continuous lines represent the difference between the true and the estimated expected value, dotted lines represent the difference between the observations and the estimated expected value\\
right: continuous lines represent the location parameter, dotted lines the scale parameter}
\end{figure}
...
\begin{figure}[t!]
\begin{subfigure}{0.3\textwidth}
<<rain_pred_rmse, fig=TRUE, echo=FALSE>>=
boxplot(rmse)
@
\end{subfigure}
\hspace{-1cm}
\begin{subfigure}{0.3\textwidth}
<<rain_pred_ll, fig=TRUE, echo=FALSE>>=
boxplot(ll, ylim = c(-2.8,-1.3))
#boxplot(ll)
@
\end{subfigure}
\hspace{-1cm}
\begin{subfigure}{0.3\textwidth}
<<rain_pred_crps, fig=TRUE, echo=FALSE>>=
boxplot(crps)
@
\end{subfigure}
\hspace{-1cm}
...
@
\end{subfigure}
\caption{\label{boxplots_rain}Results of predictions over all stations (RMSE, log-likelihood and CRPS)\\
left: continuous lines represent the difference between the true and the estimated expected value, dotted lines represent the difference between the observations and the estimated expected value\\
right: continuous lines represent the location parameter, dotted lines the scale parameter}
\end{figure}
