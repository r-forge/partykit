...
@
\caption{\label{fig:axams-dist}\hlc{Total precipitation predictions by a 
...
\item \emph{EMOS:} \hcl{The basic} ensemble model output statistics
...
Axams in Supplement 2 (Stationwise Evaluation) which contains a more 
detailed explanation of residual QQ plots and PIT histograms.}
...
%% choice of distribution mit Verweis auf Supplement distributions
...
In Supplement 1 (Different Response Distributions) the exact
...
criteria} \code{minsplit} and \code{minbucket} \hcl{together with a high significance 
level} \code{alpha}.
...
(Stationwise Evaluation) where the exact same evaluations as in} 
...
the extended analysis in Supplement 2 (Stationwise Evaluation)
where the 15 considered stations also include some of those where 
...
The results presented in Supplement 1 (Different Response Distributions) and 
Supplement 2 (Stationwise Evaluation) can be reproduced using
