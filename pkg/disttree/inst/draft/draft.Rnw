...
distributions using standard maximum likelihood in in Section~\ref{sec:distfit}.
...
in the empirical case study in Section~\ref{sec:precipitation} to model
...
as the closest in the data set to Innsbruck, capital of Tyrol that three of the
authors are based in). As for all other stations, daily precipitation observations and
...
The models learned on 6~folds (= 24~years) and evaluated on the 7-th fold
...
\fixme{still not finished}

As shown in Table~\ref{tab:models} the distributional forest 
...
To get an overview of which covariates have the most influential 
...
Figure~\ref{fig:axams-varimp}. As expected, the numerical forecast for total precipitation
(\emph{tp}) is the most influential variable. In particular, five variations of 
...
for each method at each station. In Figure~\ref{fig:all-crps} the boxplots 
...
@
\caption{\label{fig:all-crps}CRPS skill score for each station (grey lines) 
...
Figure~\ref{fig:all-map} shows a map of Tyrol (based on the SRTM 90m Digital Elevation 
...
@
\caption{\label{fig:all-map}Map of Tyrol showing which model performed best at 
...
From the results in Section~\ref{sec:precipitation} we can conclude 
...
In Section~\ref{sec:precipitation} the prespecified GAMLSS were 
...
\cite{Gneiting+Balabdaoui+Raftery:2007} are a commonly used to 
assess how well-calibrated distributional fits are.
Figure~\ref{fig:pit} shows out-of-sample PIT histograms for the same
probabilistic forecasts as Figure~\ref{fig:qq}, confirming that
