...
popular in weather forecasting. More generally, the GAMLSS framework
...
modeled by a Gaussian distribution, left-censored at 0.
...
knowledge about the athmospheric processes which drive formation of precipitation
...
\hlc{In case of precipitation forecasting EMOS typically uses the ensemble mean of 
``total precipitation'' (\emph{tp}) forecasts as predictor for the location 
...
of the response following a left-censored at zero Gaussian distribution.
This distributional approach of modeling both parameters allows to correct 
for possible errors of the ensemble in both, the expectation but also the 
...
\hlc{(Hereafter, this model is going to be referred to as a basic EMOS model.)}

\hlc{\textit{FIXME: notation with or withoug power transformation parameter?}}
...
compared to three other zero-censored Gaussian models: (a)~a standard basic
...
\hlc{alltogether 80 variables are available. Even if only paired interactions were
...
(Stationwise Evaluation). There, the exact same evaluations as in} 
...
decision should be made thoughtfuly. 
In the presented application we applied a Gaussian left-censored at
zero distribution as this has proven to be a well working choice in
earlier literature} \citep{Stauffer+Mayr+Messner:2017}. 
\hlc{But to test for robustness against distributional misspecifation 
...
with a left-censored at zero logistic distribution in order to account
for heavier tails. Under both assumptions the obtained results 
are similar to those presented in the paper applying a left-censored
at zero Gaussian distribution. However, comparing the two censored distributions,
at many stations the models perform not as well under the logisctic distribution 
as under the Gaussian distribution. The hurdle model leads to slight improvements 
in some settings, but this comes with more variablity (especially for EMOS and the 
boosted GAMLSS).
Alltogether, the distributional forest has proven to be very robust against
...
methods implemented in the} \textsf{R} package \textbf{gamlss} \hlc{``can be unstable, 
especially when it comes to selecting possibly different sets of variables for 
...
framework as it did not succeed in fitting a model. In this cases the method
...
QQ~plots (Figure~\ref{fig:axams-qq}) and
...
<<rain_axams_pit_oosample, fig=TRUE, echo=FALSE, width = 7>>=
