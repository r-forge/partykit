...
for individual observations $y_i$ and parameters $\bm{\theta}$.
...
for reasonably representing the data and covariates $\bold{Z} = Z_1, \dots, Z_m \in \mathcal{Z}$ are
...
them as regressors so that an additive predictor
$g_j(\theta_j) = f_{j,1}(\bold{Z}) + f_{j,2}(\bold{Z}) + \dots$
with link function $g_j(\cdot)$ is used for every parameter $\theta_j$
...
additive terms and their functional forms in advance which is often difficult in
...
including covariates in a data-driven way that also allows for abrupt
...
$$
and a local distributional model $\mathcal{D}(Y, \bm{theta}^(b))$
(i.e., with segment-specific parameters $\bm{theta}^(b)$) is fitted to the
...
Equation~\ref{eq:mle}) and then assess the goodness of fit by looking at the
scores $s(\bm{\hat \theta}; y_i)$ (see Equation~\ref{eq:score}).

\fixme{still more to do}
...
$$
\hat{\theta}(\bold{z}) = \argmax_{\theta \in \Theta} \sum_{i=1}^n w^{\text{tree}}_i(\bold{z}) \cdot \ell(\theta; y_i).
$$
...
$$
w^{\text{forest}}_i(\bold{z}) = \frac{1}{T} \sum_{t=1}^T \sum_{b=1}^{B^t} I((\bold{z}_i \in \mathcal{B}^t_b) \land (\bold{z} \in \mathcal{B}^t_b))
$$
...
$$
\hat{\theta}(\bold{z}) =  \argmax_{\theta \in \Theta} \sum_{i=1}^n w^{\text{forest}}_i(\bold{z}) \cdot \ell(\theta; y_i)
$$
