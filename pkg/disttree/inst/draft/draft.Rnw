...
\title{\texttt{disttree}: Distributional Trees and Forests}
...
if(file.exists("mcycle_plotdata.rda")){
  ...
  #b <- bamlss(list(accel ~ s(times), accel ~ s(times)), data = mcycle)save(df, file = "mcycle_plotdata.rda")
  
  
  ## get 'fitted.sigma.rf'
  rf_sd <- numeric(length(NROW(mcycle)))
  
  for(i in 1:NROW(mcycle)){
    rf_sd[i] <- rf_getsd(rf, newobs = mcycle[i,], rfdata = mcycle)
  }
...
if((file.exists("mcycle_cross_mean.rda")) & (file.exists("mcycle_cross_all.rda"))){
  ...
  for(k in 1:nrep_cross) {
    ...
    for(i in 1:10){
      ...
      rf.sd <- numeric(length(NROW(testdata)))
      for(l in 1:NROW(testdata)){
        rf.sd[l] <- rf_getsd(rf, newobs = testdata[l,], rfdata = learndata)
      }
...
Within this simulation study the performances of distributional trees and forests are tested and compared to other models, showing situations where one or the other method is more advantageous than others. By doing so the following hypotheses are investigated:
\begin{itemize}%[leftmargin=0.5cm]
\item When applying a tree algorithm the use of a fitting function which returns scores is advantageous to find correct splits. In particular it is crucial to find splits in parameters other than the location parameter. Hence a forest consisting of trees which were built using scores for all distribution parameters leads to better results than a simple forest considering only the mean.
\item In the case of smooth parameter functions without any abrupt changes a distributional forest performs quite well, even though other models such as GAMLSS might be more successful in these situations. However, once the smooth parameter functions include very steep sections the distributional forest outperforms other models (and in extreme cases a distributional tree does as well).
\item Once non-additive effects or interactions appear within the parameter functions distributional trees and forests clearly show their advantages over other models.
\end{itemize}


The following methods are compared to distributional trees and forests:
\begin{itemize}
...
The different methods are tested on generated data sets consisting of the values of a response variable $Y$ and one or both of the two covariates $X_1$ and $X_2$.\\
The covariates are generated based on the following distribution assumptions:
\begin{center}
\begin{tabular}{ l  c  l }
$X_1$ & $\sim$ & \textit{Unif}([-0.4,1]) \\ 
$X_2$ & $\sim$ & \textit{Unif}([-10,10])
\end{tabular}
\end{center}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1 covariate
In a first example the distribution of the response variable is a left censored normal distribution with censoring point 0 where the corresponding distribution parameters are defined by a parameter function $f$ depending on one covariate only. 

\begin{center}
$$ Y \sim \text{c}\mathcal{N} (f(x_1, \kappa))$$
with
$$f(x_1, \kappa) = \left(\mu(x_1,\kappa) , \sigma(x_2,\kappa)\right)$$ 
\end{center}

An additional factor $\kappa$ defines the steepness of certain sections of the parameter function. For low values of $\kappa$ the resulting parameters change rather slowly over varying values of the covariates while high values of $\kappa$ lead to abrupt changes. \\
...
\mu(x_1,\kappa) = 
\begin{cases}
    4 + 8 \cdot \exp{(-(4 \cdot x_1-0.7)^{(2\cdot \kappa)})},& \text{if } x_1 \geq 0\\
    0,              & \text{if } x_1 < 0
\end{cases}
$$
...
\sigma(x_1,\kappa) = 
\begin{cases}
    1+3 \cdot \|x_1\|,& \text{if } x_1 \geq 0\\
    0.00001,              & \text{if } x_1 < 0
\end{cases}
$$
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2 covariates
In a second example we consider a normaly distributed response variable with the following parameter function depending on two covariates.
%\vspace{-1cm}
\begin{center}
$$ Y \sim \mathcal{N} (f(x_1, x_2, \kappa))$$
with
$$f(x_1, x_2, \kappa) = \left(\mu(x_1,x_2,\kappa) , \sigma(x_1,x_2,\kappa)\right)$$ 
\end{center}

Including non-additive effects and interactions creates a situation in which the main advantages of distributional trees and in particular distributional forests can be investigated. Here a non-additive effects is created by separating the generated data into two subgroups depending on the value of $X_2$. In particular, within the function $\mu$ each observation is assigned with a probability of being in subgroup 1. This probability is specified by a smooth function and used to generate a binary variable indicating in which subgroup the observation is. Depending on the subgroup the actual smooth parameter function is defined.
\vspace{-1cm}
\begin{center}
$$\mu(x_1,x_2,\kappa)  = 10 \cdot \Big(1-s(x_1,x_2,\kappa)+ \big(2\cdot s(x_1,x_2,\kappa) - 1\big) \cdot \text{\code{rbinom}}\left(n,1,P_1\right)\Big)
$$
with the distribution function of the logistic distribution as a smooth function
$$
s(x_2,\kappa) = \text{\code{plogis}}\left(\kappa^2 \cdot \frac{x_2 - 3}{10}\right)
$$
and the probability of being in subgroup 1
$$
P_1(x_1,\kappa) = e^{-(10\cdot x_1 - 5)^{2 \cdot \kappa}}.
$$
\end{center}
In this scenerio the scale parameter $\sigma$ depends on the location parameter $\mu$ in the following way:
\vspace{-1cm}
\begin{center}
$$\sigma(x_1,x_2,\kappa)  = 2 + \frac{2 \cdot \mu(x_1,x_2,\kappa)}{5}$$
\end{center}
The  methods are compared by the out-of-sample RMSE for the location parameter $\mu$ and the shape parameter $\sigma$ as well as by the out-of-sample log-likelihood of the fitted models.\\
\\
Generally, a smooth parametric model such as GAMLSS should be the most successful method if the parameter function is smooth with no rapid changes. However, if there are any jumps or non-additive effects or interactions, the trees should be more effective in detecting these steps and therefore also lead to a better fitting forest. But this situation might also appear for smooth functions which contain sections with very steep slopes. Therefore, it is now of great interest, whether the concept of GAMLSS or of trees and forests covers a broader range of functions where it is superior to the other one.\\
\\
...
In this section the results of the three methods are compared over an increasing value of $\kappa$. For each step they are applied on various data sets and the resulting values of the RMSE and the log-likelihood are averaged within each step. In that way it can be investigated how the results change for a changing value of $\kappa$.\\
Again, a normally distributed response variable and the same smooth parameter function are considered.\\
...
if(file.exists("simres.rda")){
  ...
  #source("~/svn/partykit/pkg/disttree/paper/gensim.R")
  source("gensim.R")
  simres <- gensim(seedconst = 7423, nrep = 20, ntree = 100L,
                   cov.sep = TRUE,
                   fix.mu = FALSE,
                   fix.sigma = FALSE,
