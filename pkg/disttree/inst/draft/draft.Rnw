...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  in each split of the tree (\hlc{specified by the argument} \code{mtry} 
  \hlc{in the applied function} \code{distforest}; 
  \hlc{more information on the employed software is provided in Section}
...
\hlc{In particular, the figure displays the point mass at zero (i.e., the probability 
of a dry day) depicted by the model together with the forecasted probability 
...
\hlc{Since} \cite{Strasser+Weber:1999} \hlc{showed that the asymptotic conditional 
distribution of a linear statistics with conditional expectation} $\mu$ 
\hlc{and covariance} $\Sigma$ 
\hlc{is a multivariate normal distribution with parameters}
$\mu$ and $\Sigma$ 
\hlc{the asymptotic conditional distribution of the standardized test statistic}
$c(t_{l},\mu_{l},\Sigma_{l})$ 
\hlc{is either normal}
($c_{\text{max}}$) 
\hlc{or follows a} 
$\chi^2$ distribution ($c_{\text{quad}}$).
