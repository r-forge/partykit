...
local effects within a single grid cell.
...
models. In that way, they offer features that could be particularly 
valuable in probabilistic weather forecasting in order to tackle 
the above mentioned challenges. Among these features are the ability 
to deal with a wide variety of effects and an automatic variable 
selection and detection of interactions. 
...
\item Ensemble model output statistics 
(EMOS, \citealp{Gneiting+Raftery+Westveld:2005}): The 
...
\item Prespecified GAMLSS: For each distribution parameter the selection 
...
\cite{Stauffer+Umlauf+Messner:2017}. Among the 
available 82 regressors, 9 are selected for the location parameter 
...
\item Boosted GAMLSS: Non-homogeneous boosting 
\citep{Messner+Mayr+Zeileis:2017} is used for an automatic 
selection of covariates \citep{Hofner+Mayr+Schmid:2016}. 
...
%\fixme{Adding a reference to the above paragraph would be good, justifying our knowledge/experience regarding the prespecified GAMLSS. Probably Stauffer+Umlauf+Messner:2017 as I have done in the intro.}
...
\textbf{Distributional forest} & recursive    & all & all \\ 
                & partitioning &     &     \\ 
\hline
\textbf{Prespecified GAMLSS} & spline  & \emph{tp{\_}mean}, & \emph{tp{\_}sprd},\\
 ...
 & & \emph{msl{\_}diff} & \\
\hline
\textbf{Boosted GAMLSS} & spline  & all & all \\
                       & in each &     &     \\
\hline
...
and compare its results to those of the prespecified and the boosted GAMLSS 
...
shows that, in this setting, both GAMLSS and the distributional forest perform 
better than the EMOS model. While the two GAMLSS lead to an improvement of 
...
and \code{distforest}: 
\begin{itemize}
\item a GAMLLS family object from the \textsf{R} package \textbf{gamlss.dist} 
