...
employ a zero-censored Gaussian distribution and 82~predictor variables
...
selection of smooth additive terms based on all 82~predictor variables.
...
provided in Table~\ref{covariates}, yielding 82~predictor variables in total.
...
\item \emph{Distributional forest:} All 82~predictor variables are
...
\item \emph{Prespecified GAMLSS:} Smooth additive splines are selected for
  ...
  on the 82~available variables, 8 terms are included in the location
...
\item \emph{Boosted GAMLSS:} Smooth additive splines are selected
  automatically from all 82~available variables, using non-cyclic boosting
...
scoring rule \fixme{citation} and lower values indicate a better fit.
