...
obtained. \hlc{Thus, in practice $\bm{\hat \theta}(\bold{z})$ does not have to be recalculated
for each new $\bold{z}$ but one can simply extract the parameter estimates for
the corresponding segment which have been computed already while
learning the tree. However, to understand this estimator conceptually it is useful
to denote it as a weighted MLE} where the weights
...
only a random subset of the covariates $\bold{Z}$ is considered for splitting to reduce the 
...
%with power transformation parameter $\frac{1}{p}$
...
solve this problem without requiring prior expert knowledge about the meteorological covariates.
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  \hlc{The stopping criteria are the minimal number of observations to perform a split 
...
distributional fits. \hlc{More details of this assessment as well as
...
\hlc{(and thus not necessarily consecutive)} years.
...
clr <- colorspace::diverge_hcl(5, h = c(130, 320), c = 70, l = c(50, 90), power = 1.3)
...
\hlc{However, if the number of covariates is high, including all (or many)
...
\hlc{In the precipitation forecasting application, as presented in} Section~\ref{sec:precipitation},
\hlc{80~covariates are considered which corresponds to 3160~potential pairwise
interactions (and even more higher-order interactions). 
...
\hlc{Choosing an adequate distributional family is an important step for
...
for precipitation modeling in earlier literature} \citep[e.g.,][]{Stauffer+Mayr+Messner:2017}. 
...
in Supplement A} \citep{Schlosser+Hothorn+Stauffer:2018a}: \hlc{Using the
same evaluations as in} Section~\ref{sec:all}, \hlc{all models are
...
similar results as for the zero-censored Gaussian distribution. For some statons
...
\hlc{Moreover, one could consider a distribution including an additional parameter
for capturing skewness} \citep[as in][]{Scheuerer+Hamill:2015, Baran+Nemoda:2016}. 
\hlc{However, this would go beyond the mean/variance specification of the NGR
...
\hlc{are extensions of interest for future research.}


\subsubsection*{Axams vs.~other meteorological stations}

\hlc{Axams was chosen as the meteorological station for the more extensive evaluations
in} Section~\ref{sec:axams} \hlc{as it yields fairly typical results
...
\hlc{carries out the same evaluation for 14~further stations. These
...
\hlc{The supplement shows that some of the differences in forecast skill from}
Figure~\ref{fig:all-map} \hlc{even out in the cross-validation with distributional
...
\hlc{Selecting tuning parameters for flexible regression models is important
not only in terms of predictive accuracy but also computational complexity. 
For the application in} Section~\ref{sec:precipitation} \hlc{tuning parameters
are selected based on advice from the literature as well as our own experiences.}
As \cite{Hastie+Tibshirani+Friedman:2001} and \cite{Breiman:2001} \hlc{recommend to 
...
\hlc{Applying the Law of Large Numbers it can be shown that random forests do not overfit 
as the number of trees increases}
\citep{Breiman:2001,Hastie+Tibshirani+Friedman:2001,Biau+Scornet:2016}.
\hlc{Therefore, in principle, forests can be built with a very large number of trees
...
\hlc{Following this advice, we decided to build forests consisting of 100 trees.}
...
applying} \textbf{gamlss} \hlc{in certain settings within the cross-validation
