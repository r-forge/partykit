...
The distribution of the response variable is a left censored normal distribution with censoring point 0 where the corresponding distribution parameters are defined by a parameter function $f$ depending each on one covariate. The location parameter $\mu$ depends on $X_1$ and the scale parameter $\sigma$ depends on $X_2$. The other covariates $X_3, \ldots, X_{10}$ are noise variables.\\
