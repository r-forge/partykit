...
plot_crps(simres)
@
\caption{\label{plot_sim_crps}CRPS of the three models evaluated on various data sets over increasing $\kappa$} %\code{disttree} vs. \code{distforest} vs. \code{gamlss} vs. \code{randomForest} vs. %\code{bamlss} vs. \code{gamboostLSS}\\
...
palone <- hcl(250, 100, c(90, 60, 30))
#pie(rep(1, 3), col = palone)
...
diff_crps_df <- numeric(length = length(crps_ssc_df))
transp_col_crps_ssc <- numeric(length = length(crps_ssc_df))
level_col_crps_ssc <- numeric(length = length(crps_ssc_df))
pch_crps_ssc <- numeric(length = length(crps_ssc_df))
pch_intensity_crps_ssc <- numeric(length = length(crps_ssc_df))
reference_crps_ssc <- numeric(length = length(crps_ssc_df))

# get best model (loop over all stations)
for(i in 1: length(crps_ssc_df)){
  ...
  pch_crps_ssc[i] <- c(21, 24, 25, 22)[which.max(c(crps_ssc_df[i], crps_ssc_g[i], crps_ssc_gb[i], crps_ssc_ml[i]))]
  
  crps_ssc_best[i] <- which.max(crps_ssc[i,])
  diff_crps_ssc[i] <- crps_ssc[i,crps_ssc_best[i]] - crps_ssc[i,2]
  
  pch_intensity_crps_ssc[i] <- 1
  if(diff_crps_ssc[i] > 0.02) pch_intensity_crps_ssc[i] <- 2
  if(diff_crps_ssc[i] > 0.05) pch_intensity_crps_ssc[i] <- 3
    
  
  # falls forest am besten
  if(crps_ssc_best[i] == 2){
    reference_crps_ssc[i] <- max(c(crps_ssc_g[i], crps_ssc_gb[i], crps_ssc_ml[i]), na.rm = TRUE)
    diff_crps_df[i] <- crps_ssc_df[i] - reference_crps_ssc[i]
    level_col_crps_ssc[i] <- pal0[1]
    pch_intensity_crps_ssc[i] <- 1
    if(diff_crps_df[i] > 0.02) {
      level_col_crps_ssc[i] <- pal2[1]
      pch_intensity_crps_ssc[i] <- 2
    }
    if(diff_crps_df[i] > 0.05) {
      level_col_crps_ssc[i] <- pal5[1]
      pch_intensity_crps_ssc[i] <- 3
    }
  } else {
...
<<map, fig=TRUE, echo=FALSE, height=6, width=9>>=
  ...
  if(FALSE){
    ...
    legend("topleft", fill=c(pal["forest"], pal["gamlss"], pal["gamboostLSS"], pal["EMOS"]), legend = c("Forest", "prespecified GAM", "boosted GAM", "EMOS"), cex = 1, bty = "n")
    ...
    raster::image(tirol.dem, col = rev(gray.colors(100)),
                  ...
                  ylim = c(46.6, 47.8))
    plot(tirol.gadm, add = TRUE)
    points(sp, pch = pch_crps_ssc, bg = palone[pch_intensity_crps_ssc], col = 1, cex = 1.5)
    #points(sp, pch = 21, bg = col_crps_ssc, col = 1)
    #points(sp, pch = 21, bg = transp_col_crps_ssc, col = 1)
