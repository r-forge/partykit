...
$$
where \(\mathbf{1}\) is the indicator function. \\
In this application the CRPS is evaluated out of sample on a data set that has not been used to build the model. The mean CRPS over thi data set is used for validation. Since the CRPS can be seen as a generalization of the mean absolute error, a low value indicates a good fit. In applications, one big advantage in terms of interpretability is that the resulting value is on the scale of the observation. \\
