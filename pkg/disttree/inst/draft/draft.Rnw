...
\begin{itemize}
  \item Generalized linear models (GLMs, \citealp{McCullagh+Nelder:1986})
...
For these reasons, regression trees and particularly random forests provide
valuable features which can contribute to improving models for probabilistic
forecasting. Therefore, we are going to introduce a novel general distributional
framework for these methods by presenting distributional trees and forests which
embed the idea of flexible parametric modeling into the framework of regression
trees and random forests. In Figure~\ref{Devel_treeforest} the developments in
the field of tree-structured models together with the novel approaches of
distributional trees and forests are illustrated for the case of one covariate.
In the top left picture the step function modeling the mean of the response
variable represents the division into three terminal subgroups by a regression
tree. This is extended in the bottom left picture where a distributional tree
models not only the mean but also the variance of the response variable by its
own step function. The top middle picture shows that combining an ensemble of
trees to a random forest can lead to smoother effects while the tree structure
assures that abrupt changes are captured as well. Again, an extension of the
random forest model considering only the mean is made in the top right picture.
Here a distributional forest models the mean and the variance capturing abrupt
changes but also smooth effects. 

However, for example in complex
terrain in mountainous regions various factors can change rapidly and their
effects are difficult to be represented by a smooth parametric model. In this
situation a different approach which can deal with these abrupt changes would be
to apply tree-structured models such as regression trees
(\citealt{breiman1984classification}) or random forests such as Breiman and
Cutler's randomForest (\citealt{breiman2001random}). Moreover, these models are
...
specification of the model is required in advance. However, even though their
main advantage of being able to model non-additive effects and interactions
without any necessary prespecification can play to their strength, regression
trees are not an adequate choice if (additional) smooth effects appear as well.
In this situation a random forest provides a good compromise as it combines a
set of trees such that the steps imposed by the trees are smoothed out in case
of smooth effects while abrupt changes are still captured by the tree structure
of the model.
