...
The goal is to fit a distributional model $D(Y, \theta)$ to a response variable 
...
$$
where $\{y_i\}_{i=1,\ldots,n}$ are the observations of the response 
variable $Y$.
...
$D(Y, \theta = g(\bold{Z}))$. In order to avoid the restriction to linear 
effects one might also use a set of smooth functions $\{f_i\}_{i = 1,\ldots,p}$ 
to specify a more flexible model 
$D(Y, \theta = f_1(\bold{Z}) + \ldots + f_p(\bold{Z}))$ applying a GAM structure. 
...
\label{disttree}
Given the fit of a global distributional model $D(Y, \bold{\hat{\theta}})$ 
...
Common forest models (such as Breiman and Cutler's randomForest) consist of 
trees only considering the mean of the distribution of the response variable. 
For that reason, changes in other parameters can only be detected if they 
are connected to changes in the location parameter, e.g. if they depend on 
the location parameter. As explained before, a distributional tree considers
scores for all distribution parameters. Therefore, a forest consisting of 
...
In weather forecasting, various factors can make it challenging to 
build a well fitting model. Complex physical processes in the 
atmosphere depend on possibly rapidly changing circumstances. 
Often it is hard to tell which of these circumstances have a 
relevant impact on the quantity of interest and what type of 
effect that can be or how they might interact. A profound 
basis of information can be obtained from numerical weather 
prediction (NWP) models as they provide predictions for a 
large number of meteorological quantities. However, while 
including too many variables in a model can lead to overfitting, 
excluding variables in advance can result in a loss of valuable 
information. Therefore, the task of selecting variables and 
specifying interactions forms the basis of building an adequate 
model. Moreover, this model has to be able to deal with a wide 
range of effects as abrupt changes can appear as well as smooth 
transitions. Especially if the considered region includes complex 
terrain, a large grid spacing of NWP models can lead to abrupt 
local effects within a single NWP grid cell.

As explained in the introduction, distributional forests provide 
a new approach in probabilistic forecasting by embedding the ideas 
of flexible parametric modeling into the framework of tree-structured 
models. The ability to deal with a wide variety of effects and an 
automatic variable selection and detection of interactions are 
features provided by distributional forests that could be particularly 
valuable in probabilistic weather forecasting in order to tackle 
the above mentioned challenges.
To show their strengths in practice, distributional forests are 
now applied to probabilistic precipitation forecasts in a 
mountainous region based on a large number of numerical weather 
prediction quantities. Their results are compared to those of 
other zero-censored Gaussian models including a standard ensemble 
model output approach and two GAMLSS. For one of the GAMLSS the 
regressors have been selected and interactions have been specified 
in advance based on subject-matter knowledge and experience from 
meteorology. The other one is based on a computationally more 
demanding boosting approach.

\subsection{Data}
\label{data}
The data set used for training and validation consists of observed 
precipitation sums provided by the National Hydrographical Service 
(\citealt{ehyd}) and numerical weather forecasts from the {U.S.} 
National Oceanic and Atmospheric Administration (NOAA).
Both, observations and weather forecasts are available from 1985 
trough the year 2012, focusing on July only in this article.

Observations are available on a daily basis, each representing the 
total precipitation sum fallen within 24~hours observed at 6~UTC 
at 95 stations all over Tyrol and its close surrounding. The 
observations are undergoing a rigorous quality check before made 
available to the public.  As covariates the second generation 
reforecast data set of the global ensemble forecast system (GEFS) 
is used (\citealt{Hamill+Bates+Whitaker:2013}).  This data set 
consists of an 11-member ensemble based on a fixed version of the 
numerical model and a horizontal grid-spacing of about $50~\times~50 
\text{km}^2$ initialized daily at 0~UTC from December 1984 to present 
...
In particular, 14 basic forecast variables are considered with up 
to 12 variations of each of these variables being included as 
covariates. Since an ensemble of individual predictions is provided 
for each forecast variable, the possible variations include the mean, 
the standard deviation, the minimum and the maximum over all ensemble 
members, noted as ensemble mean, ensemble std. deviation, ensemble 
minimum and ensemble maximum. All of these values can be calculated 
on the sums over 24 hours (6--6 UTC of the following day) or a 6 hour 
time window. Another possible option is to consider the mean, minimum 
or maximum over 24 hours for each individual prediction and then take 
the ensemble mean or ensemble standard deviation, noted as ensemble 
mean of mean/minimal/maximal values or ensemble std. deviation of 
mean/minimal/maximal values respectively. Table~\ref{covariates} 
shows all variables together with the number of variations and 
the provided variations listed in the last column. Altogether 
82~covariates are available in this setting. 
\fixme{Mention that dswrf\_mean/sprd\_min is not included?}
...
a power transformation has been used frequently (Box and Cox 1964). 
In literature, cubic (\citealt{Stidd:1973}) or square root 
(\citealt{Hutchinson:1998}) transformations have often been 
suggested but may vary for different climatic zones or temporal 
aggregation periods. In \cite{Stauffer+Mayr+Messner:2017} 
optimization methods have been applied to find the best transformation 
factor which resulted in a value of $\frac{1}{1.6}$. Since the 
observation data set used for this optimization contains the 
observations considered in this article the same factor is 
chosen here for the power transformation of the response variable 
\emph{tp} (total precipitation). The resulting transformed 
variable is expected to be normally distributed and left-censored 
at censoring point 0. For simplicity the same transformation has 
been chosen for the variable \emph{cape} (convective available 
potential energy).
...
\item An ensemble model output statistics 
(EMOS, \citealp{Gneiting+Raftery+Westveld:2005}) model where the 
location parameter and the logarithm of the scale parameter depend 
linearly on the ensemble mean forecast of the total daily precipitation 
sum and the corresponding ensemble standard deviation respectively.
The choice of the logarithmic function as link function for the scale 
parameter has been made based on the results of 
\cite{Gebetsberger+Messner+Mayr:2017} where different link functions 
have been compared. Moreover, testing the identity, quadratic and 
logarithmic function as link functions in this setting supported 
this choice as the logarithmic function led to the best performance 
of the EMOS model in this application.
\item A GAMLSS where for each distribution parameter the selection 
of covariates and the specification of interactions is done in 
advance, both based on meteorological expert knowledge following 
\cite{Stauffer+Umlauf+Messner:2017} (prespecified GAM). Among the 
available 82 regressors, 9 where selected for the location parameter 
and 6 for the scale parameter. Additionally, for each distribution 
parameter one interaction term is specified in which convective 
available energy is interacting with total precipitation (with the 
ensemble mean for the location parameter and with the ensemble 
standard deviation for the scale parameter). These interactions 
might capture events with a high potential for moderate or severe 
thunderstorms with large amounts of precipitation. By putting 
extra effort in the specification of the formulas the prespecified 
GAM is provided with additional information and is therefore 
expected to profit from this advantage.
\item A boosted GAMLSS \citep{Hofner+Mayr+Schmid:2016} using 
non-homogeneous boosting \citep{Messner+Mayr+Zeileis:2017} 
for an automatic selection of covariates (boosted GAM). 
The idea of this boosting approach is to iteratively minimize 
a loss function where in each step only the coefficient leading 
to the highest improvement of the current model is updated. As 
the initial coefficients are all zero, only the most important 
covariates are included with nonzero coefficients while less 
relevant variables are ignored if the procedure is stopped before 
convergence.
\end{itemize}
...
In order to compare the performance of the chosen methods, the continuous 
ranked probability score (CRPS) is calculated for each model. 
\fixme{Adding reference for CPRS}

The CRPS measures the accuracy of a probabilistic model. 
For one observation $y$ with the predicted distribution 
function $F$ the CRPS value is calculated by
$$
...
In this application the CRPS is evaluated out of sample, 
e.g. on a part of the data set that has not been used to 
build the model. The mean CRPS over this subset of the data 
is used for validation. Since the CRPS can be seen as a 
generalization of the mean absolute error, a low value indicates 
a good fit. In applications, one big advantage in terms of 
interpretability is that the resulting value is on the scale of 
the observations.

For the purpose of comparing the models' performances with respect 
to a reference model the so-called skill score of the CRPS can be 
calculated. In that way a measure of how much a method improves the
model fit in terms of CRPS compared to the reference method is obtained.
$$
...
station was selected due to its geographical closeness to Innsbruck, 
...
example for a probabilistic forecast. Staying with this fitted model we are 
now going to analyze variable importance of the distributional forest model 
and compare its results to those of the GAMs and the EMOS model based on 
residual QQ-plots.
...
Figure~\ref{varimp}. As expected, the numerical forecast for total precipitation 
is the most influential variable. In particular, five variations of this variable 
are among the top ten of the list of covariates ordered by variable importance.  
...
visualization is easier, we now want to get a more profound impression 
of the performances of the different methods. For that reason the four 
methods are evaluated in a cross validation framework for the station Axams. 
...
shows that, in this setting, both GAMs and the distributional forest perform 
better than the EMOS model. While the two GAMs lead to an improvement of 
around 4 percent with the boosted version being slightly ahead, the 
...
compare the four methods under different geographical circumstances we are 
now going to apply them on all 95 stations. At each of them, the first 
...
of the CRPS skill score values are plotted and show that the distributional 
...
three models (prespecified GAM, boosted GAM and EMOS).
Even though for some stations, especially in East Tyrol, one of the two GAMs 
and for a few others the EMOS model leads to the best results, for the 
...
other three models (prespecified GAM, boosted GAM and EMOS).}
\end{figure}
...
with a wide range of modelling situations in a reasonable way.
...
\code{disttree} uses either the \code{mob} or \code{ctree} function 
as a framework to build a tree. Similarly, the function 
\code{distforest} is based on the \code{cforest} function which 
provides a framework to build a distributional forest. All three 
...
and \code{distforest}: 
\begin{itemize}
\item a GAMLLS family object from the \textsf{R} package \textbf{gamlss} 
...
In Section~\ref{ProbabilisticForecasting} the prespecified GAMs were 
built using the function \code{gamlss} from the \textsf{R} package 
\textbf{gamlss.dist} (\citealt{Stasinopoulos+Rigby:2007}). For the 
boosted GAMs the function \code{gamboostLSS} from the \textsf{R} 
...
is explained in the following. For notational simplicity, the testing 
...
\begin{itemize}
\item A distributional model $D(Y, \theta)$ is fit to the set of 
