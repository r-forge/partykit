...
shape. Notably, so-called non-homogeneous Gaussian regression (NGR) models both
mean and variance of a Gaussian response and is particularly
popular in weather forecasting. 
Moreover, generalized additive models for location, scale, and shape (GAMLSS)
...
obtained. Thus, in practice $\bm{\hat \theta}(\bold{z})$ does not have to be 
recalculated for each new $\bold{z}$ but one can simply extract the parameter estimates 
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  (\hlc{argument} \code{mtry} \hlc{in our implementation} \code{distforest},
  \hlc{with more ``computational details'' provided at the end of the manuscript}).
...
\hlc{In particular, the figure depicts the forecasted point mass at zero
...
(2009--2012).
\hlc{Note that differences in the results of East Tyrol compared to the rest of 
Tyrol could be (partly) due to East Tyrol lying in a different climate zone but 
could also be caused by the specific selection of learning and validation period
as only one single time split is considered. This will be discussed more
...
\hlc{Beyond the task of modeling precipitation it is of interest how well distributional
...
\hlc{and} \citet[][Figure~1]{Hothorn+Zeileis:2017} \hlc{demonstrated (independently) that their
respective score-based random forest algorithms outperform the mean-based quantile
regression forests of} \cite{Meinshausen:2006} \hlc{in a setup where only the variance of a normal
...
\hlc{Similarly, the score-based distributional forests introduced in this manuscript
...
\hlc{as well as transformation model specifications} \citep[e.g., as in][]{Hothorn+Zeileis:2017}
\hlc{are of interest for future research.}
...
\hlc{The differences are greatly reduced for several stations in East Tyrol by considering 
more splits in the full cross-validation. However, part of the performance difference can also 
be attributed to varying climatological characteristics in East Tyrol that, unlike North Tyrol,
is located south of the main Alpine Ridge. Therefore, effects could be more additive
and with less interactions than in North Tyrol and thus be captured more precisely by
an additive model such as GAMLSS, particularly with an appropriate prespecification of covariates.}
...
The function \code{distforest} learns the distributional forests proposed
in this manuscript by combining the general \code{cforest} function from
\textbf{partykit} with the function \code{distfit} for fitting distributional
models by maximum likelihood. Analogously, \code{disttree} can learn a single
distributional tree by combining \code{ctree} with \code{distfit}.
...
\citep[version~\Sexpr{packageVersion("crch")}]{Messner+Mayr+Zeileis:2016} for the EMOS 
models, \textbf{gamlss}
\citep[version~\Sexpr{packageVersion("gamlss")}]{Stasinopoulos+Rigby:2007} for the 
...
is reproducible using \code{demo("RainAxams", package = "disttree")}.
...
(version~\Sexpr{packageVersion("RainTyrol")}),
...
\cite{Strasser+Weber:1999} \hlc{showed that the asymptotic conditional 
distribution of the linear statistic} $t_l$ \hlc{is a multivariate normal with parameters}
$\mu$ and $\Sigma$. \hlc{Hence, the asymptotic conditional distribution of}
$c(t_{l},\mu_{l},\Sigma_{l})$ 
\hlc{is either normal}
(for $c_{\text{max}}$) 
\hlc{or} 
...
\subsection*{Supplement~B: Stationwise Evaluation}
  ...
  this supplement presents the same analysis as in Section~\ref{sec:axams} of the main
