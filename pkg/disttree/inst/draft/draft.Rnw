...
Next to distributional forests three other methods are applied to this data set and their results are compared based on CRPS values. One of the reference models is an EMOS models as this is a common choice in this type of setting and research field. The other two models are both GAMs, once in a prespecified form in terms of variable selection for each distribution parameters and once applying a boosting algorithm). 
...
par(mfrow = c(2,2))
pithist(pit_df_t, nsim = 1000, breaks = seq(0, 1, length.out = 9), main = "Forest", ylim = c(0,1.4))
pithist(pit_g_t, nsim = 1000, breaks = seq(0, 1, length.out = 9), main = "prespecified GAM", ylim = c(0,1.4))
pithist(pit_gb_t, nsim = 1000, breaks = seq(0, 1, length.out = 9), main = "boosted GAM", ylim = c(0,1.4))
pithist(pit_ml_t, nsim = 1000, breaks = seq(0, 1, length.out = 9), main = "EMOS", ylim = c(0,1.4))
