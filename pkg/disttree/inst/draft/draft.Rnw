...
\hspace*{\fill}                            &    & \qquad for 6--12, 12--18, 18--24, 24--30 UTC\\
\hline
\textbf{dswrf}: downwards short ware      & 6 & ensemble mean of mean values, \\
...
\hline
\textbf{msl{\_}diff}: mean sea level  & 1 & msl{\_}mean{\_}max - msl{\_}mean{\_}min\\
...
%@
%\end{subfigure}
\caption{\label{varimp} Variable importance based on mean decrease in accuracy regarding the CRPS: Top 10 list for the forest model.}
\end{center}
\end{figure}
