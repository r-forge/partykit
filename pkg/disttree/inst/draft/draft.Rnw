...
linearly on the ensemble mean forecast of total daily precipitation 
sums and the corresponding ensemble standard deviation respectively.
...
For one observation $y$ with predicted distribution 
...
e.g. on a part of the data set that has not been considered to 
