...
given day (see Figure~\ref{fig:axams-dist}). Forecasts for July are
...
In the case of precipitation forecasting, EMOS explains past precipitation
...
in the empirical case study, see also Table~\ref{tab:models} for
further details:
%
\begin{itemize}
...
as the closest in the data set to Innsbruck, capital of Tyrol and base of three of the
...
to 2012. In Figure~\ref{fig:axams-dist} in the introduction the probabilistic
...
showing that these differ considerably in location, scale, and the amount of
censoring while conforming quite well with the actual precipitation observations
from those days. But, of course, this is just one selected day and we are interested
in assessing how well the distributional fits are calibrated and what the
relative predictive performance is.
...
theoretical standard normal quantile (i.e., form a straight line), indicating
that all four models are sufficiently well calibrated. This is also 
supported by the corresponding PIT (probability integral transform) histograms 
in Appendix~\ref{app:pit}.
...
To assess predictive performance, a full cross-validation is carried out
rather than relying on just the one test set for the years 2009--2012.
To do so, 10 times 7-fold cross-validation is carried out where each
...
line at a CRPSS of 0). Clearly, both GAMLSS models and the distributional forest 
...
Figure~\ref{fig:axams-varimp} shows 10~covariates with the highest permutation
...
After considering only one observation station up to now, a second step
evaluates and compares the competing methods on all 95~stations. As in the previous
...
by looking at the boxplots and also in the green line representing the results
...
Finally, because some CRPSS become negative, it is also clear that the simple
EMOS performs better than the other models for a few of the stations.
...
in the topology, Figure~\ref{fig:all-map} shows a brief summary for each station
on a map of Tyrol (based on the SRTM 90m Digital Elevation 
Data, \cite{srtm}). Each station is illustrated by a symbol that conveys which
...
the prespecified EMOS performs particularly well.
...
up changes in the location but also the scale or shape of a distributional
family.

Distributional forests are an attractive alternative when prespecifying or boosting
all effects and interactions in a GAMLSS model is challenging. They are
rather straightforward to specify requiring only little prior subject
matter knowledge and also work well in the presence of many potential
covariates. The application to precipitation forecasting in complex terrain
illustrates that distributional forests often perform on par or even
better than their GAMLSS counterparts. Hence, they form a useful addition
to the toolbox of probabilistic forecasts, especially in meteorology.
...
\fixme{Shorten/omit parts that are in the main paper already. Focus in
the description on what is actually used in the application, i.e.,
reduce the emphasis of the modularity of the framework.}

\fixme{Adapt notation.}
...
In that way a meassurement of goodness-of-fit is obtained for each
...
assess how well-calibrated distributional fits are.
Figure~\ref{fig:pit} shows out-of-sample PIT histograms for the same
probabilistic forecasts as Figure~\ref{fig:axams-qq}, confirming that
despite some deviations the overall calibration of all models is acceptable.
