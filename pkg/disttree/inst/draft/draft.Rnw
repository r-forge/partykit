...
The so far discussed parametric models with the response depending on additive linear or 
smooth nonlinear terms of the covariates are powerful tools as long as only smooth effects 
are to be considered. A different approach that can also deal with non-additive effects and 
possibly unknown interactions of covariates is to apply tree-based methods. 
A regression tree (\citealt{breiman1984classification}) works on the idea of separating 
the data into more homogeneous subgroups based on covariates. However, the resulting step 
function, as illustrated in the left panel of Figure \ref{Devel_treeforest}, can often be too 
rough and impose abrupt changes on the model even in case of smooth transitions. Random 
forest models such as Breiman and Cutler's randomForest (\citealt{breiman2001random}) 
solve this problem by combining an ensemble of trees such that steps are smoothed 
out in case of smooth effects while abrupt changes are still captured by its tree-based 
structure (see the top middle panel of Figure \ref{Devel_treeforest}).
...
made in the field of flexible parametric modeling. In particular, the development from mean regression
...
forecasting. For example in weather forecasting in mountainous regions various influencing factors 
...
complex terrain can lead to abrupt changes within just one cell of the grid.
