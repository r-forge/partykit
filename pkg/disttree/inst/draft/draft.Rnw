...
\hl{To do so, a 10 times 7-fold cross-validation is carried out where each
splits the available 28~years into 7~subsets of 4~randomly selected and 
...
However, in the application presented in Section~\ref{sec:precipitation}
alltogether 80 variables are available. Even if only paired interactions were
...
Setting tuning parameters of a forest model can have a major influence
not only on the accuracy of the model but also on the computational 
complexity. 
For the application in Section~\ref{sec:precipitation} we have 
...
\cite{Hastie+Tibshirani+Friedman:2001} and \cite{Breiman:2001} recommend to 
...
However, in a distributional model it also has to be considered that a 
...
50 (\code{minbucket}) and the minimal number of observations on which
...
of a large number of trees} (\code{ntree}) \hl{as this generally leads to an increase 
in accuracy. However, this also comes along with increasing computational costs. 
...
However, increasing this tuning parameter could improve the performance
of the distributional forest without the problem of overfitting.}

%% TO DOs:
%% East Tyrol
\subsubsection*{East Tyrol}
Figure~\ref{fig:all-map} shows that, contrary to the rest of Tyrol, 
...
the stations in East Tyrol. These results are presented in the online supplement 
which comes along with our manuscript. There, more extensive evaluations
are carried out for 15 observation stations, including a cross-validation
as presented for station Axams in this paper.

\subsubsection*{Choice of station Axams}
%% choice of station Axams mit Verweis auf Supplement stationwise
Axams was chosen for the more extensive evaluations presented
...
There, the same evaluations as for station Axams have been carried out for 14 other
stations, including some of those where the forest does not perform better than
the other models in the setting as presented in Figure~\ref{fig:all-map}.  
It shows that the differences in performance even out in the cross-validation 
framework. There, the forest performs at least equally well compared to the 
best of the other models at most stations.
...
methods implemented in the \textsf{R} package \textbf{gamlss} ``can be unstable, 
...
applying the function \textbf{gamlss} in certain settings within the cross-validation
