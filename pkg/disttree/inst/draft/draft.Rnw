...
@
\caption{\label{fig:axams-dist}\hl{Total precipitation predictions by a 
...
$$
\hl{Even though this parameter depends on the given $z$ it has to be noted that
...
\hl{The idea of random forests is to learn an ensemble of trees, each on a different
...
\cite{Breiman:2001}).
\hl{This idea is now taken one step further by embedding it in a distributional 
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  \hl{As stopping criteria the minimal number of observations to perform a split} 
  (\code{minsplit}) \hl{is set to $50$, the minimal number of observations in a 
  terminal node} (\code{minbucket})\hl{is set to $20$ and the significance level 
  for variable selection} (\code{alpha}) \hl{is kept at the default value $1$.}
  % stopping criteria:
...
\hl{This is also supported by the corresponding probability integral transform}
(PIT, \citealt{Gneiting+Balabdaoui+Raftery:2007}) \hl{histograms for station
...
\hl{For each setting in the considered 10 times 7-fold cross-validation the 
...
\hl{Generally, there are many possibilities how to specify the variables that are
...
\hl{However, in the application presented in} Section~\ref{sec:precipitation}
\hl{alltogether 80 variables are available. Even if only paired interactions were
...
\hl{Setting tuning parameters of a forest model can have a major influence
not only on the accuracy of the model but also on the computational 
complexity. 
For the application in} Section~\ref{sec:precipitation} \hl{we have 
...
\cite{Hastie+Tibshirani+Friedman:2001} and \cite{Breiman:2001} \hl{recommend to 
...
\hl{However, in a distributional model it also has to be considered that a 
...
50 (\code{minbucket}) \hl{and the minimal number of observations on which
...
\hl{random forests do not overfit, not even for an increasing number of 
trees} (\code{ntree}). \hl{Hence, it is recommended to build forests consisting 
...
\hl{Following this advice we chose to build forests consisting of 100 trees.}


\subsubsection*{East Tyrol}
Figure~\ref{fig:all-map} \hl{shows that, contrary to the rest of Tyrol, 
...
Section~\ref{sec:axams} \hl{for station Axams have been carried out 
for 14 other stations.}

\subsubsection*{Choice of station Axams}
%% choice of station Axams mit Verweis auf Supplement stationwise
\hl{Axams was chosen for the more extensive evaluations presented
...
presented in} Figure~\ref{fig:all-map}. \hl{It shows that the differences in 
...
\hl{Choosing an adequate distributional family is a crucial step and forms
...
\hl{But to test for robustness against distributional misspecifation 
...
same evaluations as in} Section~\ref{sec:all} \hl{are carried out for 15
...
As stated by \cite{Hofner+Mayr+Schmid:2016} \hl{the AIC-based variable selection 
methods implemented in the} \textsf{R} package \textbf{gamlss} \hl{``can be unstable, 
...
applying the function} \textbf{gamlss} \hl{in certain settings within the cross-validation
...
$$
\hl{where $s(\bm{\hat{\theta}}, Y_i) \in \mathbb{R}^{1\times k}$ and the 
...
\hl{The observed multivariate linear statistic $t_l$ which is either an
...
$$
\hl{where $\Sigma_l^+$ is the Moore-Penrose inverse of $\Sigma_l$.} Alternatively,
...
is chosen as splitting variable. \hl{Otherwise no further split is made 
...
$$
\hl{for $q \in \{1,2\}$ where $\mathcal{B}_{1r}$ and $\mathcal{B}_{2r}$ are the 
