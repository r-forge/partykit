...
\newcommand{\I}{\mathbf{I}}\documentclass[nojss]{jss}
\shortcites{Hamill+Bates+Whitaker:2013}
...
statistic $t$ is mapped onto the real line by a  univariate test 
...
This is measured by the linear statistic
$$
T_{l^{\ast}}^r = \min_{s=1,2}\sum_{i \in \mathcal{B}_{sr}} s(\bm{\hat{\theta}}, Y_i)
$$
where $\mathcal{B}_{1r}$ and $\mathcal{B}_{2r}$ are the two new subgroups, without 
any particular ordering, that 
are defined by splitting in split point $r$ of variable $Z_{l^{\ast}}$. 
The split point is then chosen as follows:
$$
r^{\ast} = \argmin_{r} c(t_{l^{\ast}}^r,\mu_{j^{\ast}}^r,\Sigma_{l^{\ast}}^r).
