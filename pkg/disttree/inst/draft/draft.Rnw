...
%\VignetteKeywords{parametric models, regression trees, random forests, recursive partitioning, probabilistic forecasting, GAMLSS}
...
models this only captures the location of the distribution but
...
popular in weather forecasting. 
% More generally, the GAMLSS framework allows to establish generalized additive 
% models for location, scale, and shape with smooth linear or nonlinear effects.
\hlc{Less specifically, generalized additive models for location, scale, and shape (GAMLSS) provide a framework where each distribution parameter is modeled separately capturing smooth linear or nonlinear effects.}
%
...
a framework for distributional regression trees and forests is proposed that blends
...
probabilistic precipitation forecasts at numerous sites in a mountainous
region (Tyrol, Austria) based on a large number of numerical weather
prediction quantities. It is shown that the novel distributional regression forests
...
box(lwd=5)
lines(x = x, y = ytrue, col = pal[1], lwd = 7, main = "")
@
\endminipage};
\node (b) at (5,0) 
{
...
polygon(c(x, rev(x)), c(ytrue + 0.1 + var, rev(ytrue - 0.1 - var)),
  col = pallight[1], border = "transparent")
lines(x, ytrue, col = pal[1], lwd=7)
...
lines(x = x, y = ytree, col = pal[1], lwd=7)
@
\endminipage};
\node (b) at (5,0) 
{
...
box(lwd=5)
#lines(x = x, y = ytrue, col = "gray", lwd=5, main = "")
lines(x = x, y = yforest, col = pal[1], lwd=7, main = "")
@
\endminipage};
\node (c) at (10,0) 
{
...
usr <- par("usr")
rect(usr[1], usr[3], usr[2], usr[4], col = gray(0.94), border = "transparent")
lines(x = x, y = ytree, col = pal[1], lwd=7, main = "")
...
\hlc{However, distributional forests should not be considered as a 
replacement of GAMLSS but rather as a complementing method in the field of
distributional models, similar to the way how GAM and random forests are complements 
in the conditional mean regression toolbox.}

In the following, particular focus is given to distributional forests as a method for
obtaining probabilistic forecasts by leveraging the strengths of random forests: the 
ability to capture both smooth and abruptly changing functions along with simultaneous selection of variables and possibly complex interactions. Thus, these properties make the method particularly appealing in case of many covariates with unknown effects and 
interactions where it would be challenging to specify a distributional regression model 
like GAMLSS.
...
# observations
obs1 <- c(Axams_24to4$testdata[pdays,"robs"][1]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][1], mean = df_mu[1], sd = df_sigma[1], left = 0))
obs2 <- c(Axams_24to4$testdata[pdays,"robs"][2]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][2], mean = df_mu[2], sd = df_sigma[2], left = 0))
obs3 <- c(Axams_24to4$testdata[pdays,"robs"][3]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][3], mean = df_mu[3], sd = df_sigma[3], left = 0))
obs4 <- c(Axams_24to4$testdata[pdays,"robs"][4]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][4], mean = df_mu[4], sd = df_sigma[4], left = 0))
...
if(file.exists("vimp_crps.rda")){
  ...
  meancrps <- function(permute = NULL, newdata = Axams_24to4$testdata) {
    if(!is.null(permute)) newdata[[permute]] <- sample(newdata[[permute]])
    p <- predict(Axams_24to4$df, newdata = newdata, type = "parameter")
  ...
  risklist <- applyfun(1:nperm, 
                       function(i){
                         set.seed(i)
                         sapply(c(5:29, 31, 32, 34: ncol(Axams_24to4$testdata)), meancrps)
                       })
  risk <- Reduce("+", risklist) / length(risklist)
  
  names(risk) <- names(Axams_24to4$testdata)[c(5:29, 31, 32, 34: ncol(Axams_24to4$testdata))]
  vimp_crps <- risk - meancrps(newdata = Axams_24to4$testdata)
...
<<rain_Axams_pred, fig=TRUE, echo=FALSE, width=6.5, height=3.8>>=
...
@
\caption{\label{fig:axams-dist}\protect\hlc{Total precipitation predictions by a 
...
More specifically, a Gaussian distribution left-censored at zero, is employed to model 24-hour total precipitation so that the
...
\citep{Messner+Mayr+Zeileis:2017} as an alternative technique for variable selection among all
...
Gaussian distribution left-censored at zero. The latter is employed in the empirical 
...
\textit{k}-dimensional parameter vector $\bm{\theta} \in \bold{\Theta}$ and
...
rain), a Gaussian distribution left-censored at zero (for short: zero-censored Gaussian)
...
\end{equation}
\hlc{Even though this parameter depends on the given $\bold{z}$ it has to be noted that
it is the same for all other observations that are in the same terminal node of 
the tree as $\bold{z}$ and therefore does not necessarily have to be recalculated for
each of them.}
...
\hlc{The idea of random forests is to learn an ensemble of trees, each on a different
learning data obtained through resampling (bootstrap or subsampling). In each node 
only a random subset of the covariates $\bold{Z}$ is considered to reduce the 
...
\hlc{This idea is now taken one step further by embedding it in a distributional 
framework based on loglikelihood estimation. \emph{Distributional forests} 
employ an ensemble of $T$~\emph{distributional trees} which pick up changes in 
the ``direction'' of any distribution parameter by considering the full score 
vector for choosing splitting variables and split points.}
...
\begin{equation}
w^{\text{forest}}_i(\bold{z}) = \frac{1}{T} \sum_{t=1}^T \sum_{b=1}^{B^t}
\frac{\mathbf{1}((\bold{z}_i \in \mathcal{B}^t_b) \land (\bold{z} \in \mathcal{B}^t_b))}{|\mathcal{B}^t_b|},
\end{equation}
...
or generalized forests). Distributional forests, in contrast to these 
...
\hlc{EPSs not only predict the most likely future state of the atmosphere but provide
...
the initial state and the model and its parametrizations.
...
\cite{Gneiting+Raftery+Westveld:2005} \hlc{to produce high-quality forecasts for
specific quantities and sites.  
In case of precipitation forecasting EMOS typically uses the ensemble mean of 
``total precipitation''} (\emph{tp}) \hlc{forecasts as predictor for the location 
parameter $\mu$ and the corresponding ensemble standard deviation for the 
scale parameter $\sigma$ of the statistical model based on the assumption
of the response following a Gaussian distribution left-censored at zero.
...
uncertainty of a specific forecast. In order to assure positivity of the 
scale parameter a log-link is applied. Hence, the two distribution parameters 
are modeled by the linear functions} 
$\mu = \beta_0 + \beta_1 \cdot \emph{mean}(\emph{tp})$ and  
$\log(\sigma) = \gamma_0 + \gamma_1 \cdot \emph{sd}(\emph{tp})$
with regression coefficients $\beta_0$, $\beta_1$, $\gamma_0$ and $\gamma_1$,  
\hlc{hereafter referred to as a basic EMOS model} \citep{Gebetsberger+Messner+Mayr:2017}.

% FIXME: notation with or without power transformation parameter?

% with:
%$\mu = \beta_0 + \beta_1 \cdot \mbox{mean}(\mbox{tp}^{\frac{1}{p}})$ and  
%$\log(\sigma) = \gamma_0 + \gamma_1 \cdot \mbox{sd}(\mbox{tp}^{\frac{1}{p}})$
%with power transformation parameter $\frac{1}{p}$ and 
%regression coefficients $\beta_0$, $\beta_1$, $\gamma_0$ and $\gamma_1$,  
%\hlc{hereafter referred to as a basic EMOS model \citep{Gebetsberger+Messner+Mayr:2017}.}
...
effects in the \hlc{NWP system} \citep{Bauer+Thorpe+Brunet:2015}. 
...
solve this problem without requiring prior expert knowledge.  For fitting the
forest only the response distribution and the list of potential predictor
...
in their choice of $1.6^{-1}$ as a suitable power parameter for the
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  in each split of the tree (\code{mtry}). Parameters are estimated by
  adaptive local likelihood based on the forest weights as described
  in Section~\ref{sec:methods}. 
  \hlc{As stopping criteria the minimal number of observations to perform a split} 
  (\code{minsplit}) \hlc{is set to $50$, the minimal number of observations in a 
  terminal node} (\code{minbucket}) \hlc{is set to $20$ and the significance level 
  for variable selection} (\code{alpha}) \hlc{is kept at the default value $1$ such 
  that no early-stopping (sometimes also referred to as ``pre-pruning'') is applied
  based on results of the statistical tests.}
  % stopping criteria:
  % forest_minsplit = 50, forest_minbucket = 20, forest_mincrit = 0
  
  % default in ctree_control within distforest:
  % mincriterion = 0
  
  % default values in ctree_control:
  % alpha = 0.05, mincriterion = 1 - alpha,
  % minsplit = 20L, minbucket = 7L,
  % minprob = 0.01

\item \emph{EMOS:} \hlc{The basic} ensemble model output statistics
...
\item \emph{Boosted GAMLSS:} Smooth additive splines are selected
  ...
  the number of boosting iterations (\code{mstop}) is: $50, 75, \dots, 975, 1000$.
...
see Figure~\ref{fig:all-map}). As for all other stations, daily precipitation observations and
...
The four sample forecasts differ considerably in location $\mu$, scale $\sigma$, and the amount of
...
\hlc{This is also supported by the corresponding probability integral transform}
(PIT, \citealp{Gneiting+Balabdaoui+Raftery:2007}) \hlc{histograms for station
Axams in Supplement B} \citep{Schlosser+Hothorn+Stauffer:2018b} \hlc{which 
...
\hlc{For each setting in the considered 10 times 7-fold cross-validation the 
available 28~years are split into 7~subsets of 4~randomly selected and 
therefore mostly not subsequent years.}
...
<<rain_axams_varim, fig=TRUE, echo=FALSE, width=6.5, height=4.2>>=
par(mar = c(4,10,1,2))
...
lines(s[70,], col = pal[2], type = "o", pch = 19, lwd = 2)  
...
\hlc{Generally, there are many possibilities how to specify the variables that are
to be included in a model. Especially for a low number of variables all types
of interactions can be employed and mixed effect models and separations into 
main effects and interaction effects can be considered allowing for easier 
interpretations} (see for example \citealp{Wood+Scheipl+Faraway:2013}; 
\citealp{Goicoa+Adin+Ugarte:2018}; \citealp{Ugarte+Adin+Goicoa:2017}).
\hlc{However, in the application presented in} Section~\ref{sec:precipitation}
\hlc{altogether 80 variables are available. Even if only paired interactions were
considered this would already lead to 3160 additional covariates in the model
making it very difficult to keep the model manageable regarding computational
costs but also interpretability. 
Therefore, only additive effects are considered for the boosted GAMLSS which
automatically selects covariates while meteorological expert knowledge was 
applied to select covariates and specify interactions for the prespecified 
GAMLSS. For the distributional forest no prespecification is required as it
automatically selects covariates and also detects interaction effects.

As mentioned before, distributional forests should not be seen as a replacement
for GAMLSS but rather as a complementary method. GAMLSS provide a powerful framework
in the field of distributional modeling as they allow for the combination of 
interpretability and a high goodness-of-fit. For distributional forests, the focus
is mainly on forecasting skill, automatic variable selection and capturing also 
non-additive effects rather than on interpretability. Therefore, both approaches
provide valuable features and advantages that can complement each other in many 
applications as it can not be expected that one of them is in general better than 
the other. 
}


\subsubsection{Distributional specifications}
\hlc{Choosing an adequate distributional family is a crucial step and forms
the basis required to build a well fitting model. Therefore, this
decision should be made thoughtfully. 
In the presented application we applied a Gaussian distribution left-censored 
at zero as this has proven to be a well working choice in
earlier literature} \citep{Stauffer+Mayr+Messner:2017}. 
\hlc{To test for robustness against distributional misspecifation 
different distributional assumptions have been considered as well. 
In Supplement A} \citep{Schlosser+Hothorn+Stauffer:2018a} \hlc{the exact
same evaluations as in} Section~\ref{sec:all} \hlc{are carried out for 15
observation stations, once employing a Gaussian hurdle model and once
with a logistic distribution left-censored at zero in order to account
for heavier tails. Under both assumptions the obtained results 
are similar to those presented in the paper applying a Gaussian distribution
left-censored at zero. In some settings the hurdle model leads to slight 
improvements, but this comes with more variability (especially for EMOS and the 
boosted GAMLSS). For the boosted GAMLSS the left-censored logistic distribution
shows slightly better results regarding CRPS skill score, however, only within 
a range of less than $2\%$ and not for all stations. 
Altogether, the distributional forest has proven to be 
very robust against distributional misspecifications.}

\hlc{Alternatively, a distribution including an additional parameter capturing 
skewness could be applied} \citep{Scheuerer+Hamill:2015, Baran+Nemoda:2016}. 
\hlc{For example, in transformation forests} \citep{Hothorn+Zeileis:2017} 
\hlc{a very flexible parametric distribution characterized by the transformation 
function is employed such that higher moments can be modeled implicitly.
Evaluating further distributions might be of interest but this would go beyond the
scope of this manuscript.}

\subsubsection*{Tuning parameters}
\hlc{Setting tuning parameters of a forest model can have a major influence
not only on the accuracy of the model but also on the computational 
complexity. 
For the application in} Section~\ref{sec:precipitation} \hlc{we have 
selected the values of these parameters} (listed in Section~\ref{sec:evaluation}) 
based on advice from literature and our own experience.

\cite{Hastie+Tibshirani+Friedman:2001} and \cite{Breiman:2001} \hlc{recommend to 
build full-grown trees. Therefore, low values should be chosen for the stopping 
criteria} \code{minsplit} and \code{minbucket} \hlc{together with a high significance 
level} \code{alpha}.
\hlc{However, in a distributional model it also has to be considered that a 
minimal number of observations is required to allow for a reasonable 
estimation of parameters in each node. For that reason, the minimal number
of observations for which statistical tests are performed in the 
distributional forest in} Section~\ref{sec:precipitation} is set to
50 (\code{minbucket}) \hlc{and the minimal number of observations on which
parameters are estimated is set to 20} (\code{minsplit}).

As stated by \cite{Breiman:2001} and \cite{Hastie+Tibshirani+Friedman:2001} 
\hlc{random forests do not overfit, not even for an increasing number of 
trees} (\code{ntree}). \hlc{Hence, it is recommended to build forests consisting 
of a large number of trees as this generally leads to an increase 
in accuracy. However, it also comes along with increasing computational costs. 
Therefore, ``a good choice results from a trade-off between computational 
complexity and accuracy''} (\citealp{Biau+Scornet:2016}, p.~205).
\hlc{Following this advice we chose to build forests consisting of 100 trees which
is a commonly used value.}


\subsubsection*{East Tyrol}
Figure~\ref{fig:all-map} \hlc{shows that, contrary to the rest of Tyrol, 
for the majority of stations in East Tyrol the prespecified GAMLSS performs
best in this particular setting (learning data: 1985--2008, testing data:
2009--2012). East Tyrol is located south of the main Alpine Ridge and 
exhibits different climatological characteristics than the part north of 
the Alpine Ridge. It seems that the choice of covariates and their interactions 
we made for the prespecified GAMLSS method is particularly adequate for the 
region of East Tyrol for this specific analysis. However, the advantages of 
the prespecified GAMLSS models in this particular setting (training: 1985--2008, 
validation: 2009--2012) even out in a cross-validation framework for most of 
the stations in East Tyrol. These results are presented in Supplement B} 
\citep{Schlosser+Hothorn+Stauffer:2018b} \hlc{where the exact same evaluations as in} 
Section~\ref{sec:axams} \hlc{for station Axams have been carried out 
for 14 other stations.}

\subsubsection*{Choice of station Axams}
%% choice of station Axams mit Verweis auf Supplement stationwise
\hlc{Axams was chosen for the more extensive evaluations presented
in this paper as it is the observation station being closest 
...
the results of our evaluations. This is also confirmed by 
the extended analysis in Supplement B} \citep{Schlosser+Hothorn+Stauffer:2018b}
\hlc{where the 15 considered stations also include some of those where 
the forest does not perform better than the other models in the setting 
presented in} Figure~\ref{fig:all-map}. \hlc{It shows that the differences in 
performance even out in the cross-validation framework where the forest 
performs at least equally well compared to the best of the other models 
at most stations.}



\subsubsection*{Computational difficulties}
%% GAMLSS errors with references gamboostLSS Hofner et al.
As stated by \cite{Hofner+Mayr+Schmid:2016} \hlc{the AIC-based variable selection 
methods implemented in the} \textsf{R} package \textbf{gamlss} \hlc{``[...] can be 
...
applying the function} \textbf{gamlss} \hlc{in certain settings within the cross-validation
framework as it did not succeed in fitting the model. In these cases the method
was not taken into consideration in the comparison of all applied models.}


\section*{Computational details}

The proposed methods are in the \textsf{R} package \textbf{disttree}
(version~\Sexpr{packageVersion("disttree")}) based on the \textbf{partykit}
package (version~\Sexpr{packageVersion("partykit")}), both available 
on \textsf{R}-Forge at
...
(\citealp{Stasinopoulos+Rigby:2007}, version~\Sexpr{packageVersion("gamlss.dist")})
...
\citep[version~\Sexpr{packageVersion("crch")}]{Messner+Mayr+Zeileis:2016} for the EMOS models,
\textbf{gamlss}
\citep[version~\Sexpr{packageVersion("gamlss")}]{Stasinopoulos+Rigby:2007} for the prespecified GAMLSS, and
\textbf{gamboostLSS}
\citep[version~\Sexpr{packageVersion("gamboostLSS")}]{Hofner+Mayr+Schmid:2016} for the boosted GAMLSS.

The fitted distributional forest for July 24 and
observation station Axams (including Figure~\ref{fig:axams-dist})
is reproducible using 
\code{demo("RainAxams", package = "disttree")}.
...
\code{demo("RainTyrol", package = "disttree")}
requiring the companion \textsf{R} package \textbf{RainTyrol} (version~\Sexpr{packageVersion("RainTyrol")}),
also available within the \textsf{R}-Forge~project.
\hlc{The results presented in Supplement A} \citep{Schlosser+Hothorn+Stauffer:2018a} and 
Supplement B \citep{Schlosser+Hothorn+Stauffer:2018b} \hlc{can be reproduced using}
\code{demo("RainDistributions", package = "disttree")} and 
\code{demo("RainStationwise", package = "disttree")} respectively.
...
These hypotheses are assessed with permutation tests based on
the multivariate linear statistic
\begin{equation}
T_l = vec\left(\sum_{i=1}^n v_l(Z_{li}) \cdot s(\bm{\hat{\theta}}, Y_i)\right)
\end{equation}
\hlc{where $s(\bm{\hat{\theta}}, Y_i) \in \mathbb{R}^{1\times k}$ and the 
...
$v_l(Z_{li}) = e_H(Z_{li}) = (\I(Z_{li} = 1), \ldots, \I(Z_{li} = H))$ 
...
\hlc{The observed multivariate linear statistic $t_l$ which is either an
...
\end{equation}
\hlc{where $\Sigma_l^+$ is the Moore-Penrose inverse of $\Sigma_l$.} Alternatively,
...
is chosen as splitting variable. \hlc{Otherwise no further split is made 
...
a tree is sometimes also referred to as ``pre-pruning''. For forest models
...
\end{equation}
\hlc{for $q \in \{1,2\}$ where $\mathcal{B}_{1r}$ and $\mathcal{B}_{2r}$ are the 
...
<<rain_axams_pit_oosample, fig=TRUE, echo=FALSE, width = 7, eval=FALSE>>=
...
@
%%\end{subfigure}
%\caption{\label{fig:axams-pit}Out-of-sample PIT histograms (2009--2012) for station Axams and models learned on data from 1985--2008.}
%\end{figure}

%As an alternative to QQ~plots based on randomized quantile residuals,
%probability integral transform (PIT, \citealp{Gneiting+Balabdaoui+Raftery:2007}) 
%histograms are a commonly used tool to assess how well-calibrated distributional 
%fits are. Figure~\ref{fig:axams-pit} shows out-of-sample PIT histograms for the same
...
\subsection*{Supplement A: Different Response Distributions}
(doi: \href{http://dx.doi.org/10.1214/00-AOASXXXXSUPPA}{10.1214/00-AOASXXXXSUPPA}).
  ...
  %left-censored at zero to accomodate both the non-negativity of precipitation and
  %the point mass at zero. 
  To assess the goodness of fit of the Gaussian distribution, left-censored at zero,
  ...
  truncated at zero, for the positive precipitation observations. 
  %The latter could
...
\subsection*{Supplement B: Stationwise Evaluation}
(doi: \href{http://dx.doi.org/10.1214/00-AOASXXXXSUPPB}{10.1214/00-AOASXXXXSUPPB}).
  %In the case study presented in the main manuscript ``Distributional Regression
  %Forests for Probabilistic Precipitation Forecasting in Complex Terrain''
  ...
  %preformed for only one station, Axams. 
  To show that Axams is a fairly typical
