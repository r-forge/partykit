...
Figure \ref{Devel_treeforest} can often be too rough and impose abrupt changes on the model even in
case of smooth transitions. Random forest models such as Breiman and
Cutler's randomForest (\citealt{breiman2001random}) encounter this problem by combining an
ensemble of trees such that steps are smoothed out in case of smooth effects while abrupt changes
are still captured by its tree-based structure (see the top middle panel of Figure \ref{Devel_treeforest}).
...
models to distributional models will be embedded in the framework of regression trees and
...
In many modeling situations, especially when the types of effects are not known in advance,
...
forecasting. For example weather forecasting in mountainous regions various influencing factors 
can change rapidly and their effects are  difficult to be represented by a smooth parametric model. 
Additionally, large spacing of the grid applied in numerical weather prediction models over 
complex terrain can lead to abrupt changes within just one cell.

In the application discussed in this paper precipitation forecasts are made for a mountainous region
in the Alps covering mainly Tyrol in Austria and adjacent areas. Predictions are based on data from 
...
with censoring point 0. As covariates a large number of meteorological quantities are provided by 
ensembles of numerical weather prediction models. 
Next to distributional forests other methods commonly used in this research area are applied and their 
results are compared. In a first setting only one observation station is taken into consideration.
The models are trained on data from 24 years and probabilistic predictions of the consecutive 4 years 
are validated. A brief outlook on the results of this setting is given by Figure~\ref{axams_onestation}. 
For the 24th of July of 2009, 2010, 2011 and 2012 the density functions of the distributions estimated 
by a distributional forest are plotted together with the corresponding observations. This illustrates 
that appropriate probabilistic forecasts are provided by this model as they are close to the observed 
values. Beside the expected values the predictions made by distributional models offer a wide range 
of information as they specify the whole distribution and therefore enable further methods of inference 
and analysis. In section \ref{ProbabilisticForecasting} this application will be further discussed and it 
will be shown that the predictions provided by the distributional forest are reasonably well fitting, not 
only for the mean parameter of the distribution but also for its second parameter, the scale parameter, 
