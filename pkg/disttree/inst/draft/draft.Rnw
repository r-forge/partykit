...
numerical ensemble prediction systems (EPSs; see \cite{Bauer+Thorpe+Brunet:2015}).
...
interpretations} (see for example \cite{Wood+Scheipl+Faraway:2013}, 
\cite{Goicoa+Adin+Ugarte:2018}, \cite{Ugarte+Adin+Goicoa:2017}).
...
\hlc{a very flexible parametric distribution characterised by the transformation 
...
complexity and accuracy''} (\cite{Biau+Scornet:2016}, p.~205).
...
%\begin{supplement}[id=supp1]
%  \sname{Supplement A}
%  \stitle{Different Response Distributions}
...
%\begin{supplement}[id=supp2]
%  \sname{Supplement B}
%  \stitle{Stationwise Evaluation}
%  \slink[doi]{10.1214/00-AOASXXXXSUPP}
%  \sdatatype{.pdf}" 
%  \sdescription{
%  In the case study presented in the main manuscript ``Distributional Regression
