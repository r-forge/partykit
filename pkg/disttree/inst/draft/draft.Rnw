...
perform at least on par and sometimes clearly better while requiring no expert
knowledge for the model specification. The three alternatives are: a standard ensemble model
...
While Breiman and Cutler's random forests \citep{Breiman:2001} grow ensembles of trees
that pick up changes in the location of the response across the covariates,
\emph{distributional forests} employ an ensemble of $T$~\emph{distributional trees}. 
These pick up changes in the ``direction'' of any distribution parameter by
considering the full score vector for choosing splitting variables and split
points. \hl{For each distributional tree a different learning data set is obtained} 
through resampling (bootstrap or subsampling) and in each node only a
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  \hl{As stopping criteria the minimal number
  of observations to perform a split} (\code{minsplit}) \hl{is set to $50$, 
  the minimal number of observations in a terminal node} (\code{minbucket})
  \hl{is set to $20$ and the significance level for variable selection} (\code{alpha}) 
  \hl{is kept at the default value $1$.}
  % stopping criteria:
