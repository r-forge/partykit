...
The different methods are tested on generated data sets consisting of the values of a response variable $Y$ and two covariates $X_1$ and $X_2$.\\
...
\end{tabular}
\end{center}
The distribution of the response variable is a normal distribution where the corresponding distribution parameters are defined by parameter functions depending on the covariates. 
\vspace{-1cm}
\begin{center}
...
if(file.exists("simres.rda")){
  ...
  source("gensim.R")
  simres <- gensim(seedconst = 7, nrep = 20, ntree = 100L,
                   cov.sep = FALSE,
                   fix.mu = FALSE,
                   fix.sigma = FALSE,
