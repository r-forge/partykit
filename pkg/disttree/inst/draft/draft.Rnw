...
# predictions
pred1 <- c(Axams_24to4$testdata[pdays,"robs"][1]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][1], mean = df_mu[1], sd = df_sigma[1], left = 0))
pred2 <- c(Axams_24to4$testdata[pdays,"robs"][2]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][2], mean = df_mu[2], sd = df_sigma[2], left = 0))
pred3 <- c(Axams_24to4$testdata[pdays,"robs"][3]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][3], mean = df_mu[3], sd = df_sigma[3], left = 0))
pred4 <- c(Axams_24to4$testdata[pdays,"robs"][4]^(1.6), crch::dcnorm(Axams_24to4$testdata[pdays,"robs"][4], mean = df_mu[4], sd = df_sigma[4], left = 0))
...
@
\caption{\label{fig:axams-dist}Total precipitation predictions by a distributional forest
...
$$
Even though this parameter depends on the given $z$ it has to be noted that
...
\item \emph{Distributional forest:} All 80~predictor variables are
  ...
  in Section~\ref{sec:methods}. As stopping criteria the minimal number
  of observations to perform a split (\code{minsplit}) is set to 50, 
  the minimal number of observations in a terminal node (\code{minsbucket})
  is set to 20 and the value that must be exceeded in the statistical tests 
  (e.g. $1-\text{p-vlaue$}) for variable selection is kept at the default value $0$.
  % stopping criteria:
...
is chosen as splitting variable. Otherwise no further split is made is in this node.
