...
################################
### simulation function for one data set where only x1 varies
sim_oneset_vary_x1 <- function(parfun,
                       nobs = 500,
                       seedconst = 7, ntree = 100,
                       formula = y~x1+x2+x3+x4+x5+x6, 
                       tree_minsplit = 25, tree_minbucket = 10, tree_mincrit = 0.95, 
                       ...
                       nrcovform = 6)
{
  ...
  # data sets with fixed covariates except for x1
  data_x1 <- learndata[,-1]
  data_x1[,c("x3","x5","x6")] <-0
  data_x1[,c("x2", "x4")] <- cbind(rep.int(pred_fix_x2, NROW(learndata)),
                                   rep.int(pred_fix_x4, NROW(learndata)))
