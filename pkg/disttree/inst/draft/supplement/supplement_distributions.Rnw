...
censored Gaussian distribution (orange horizontal line). Gray lines correspond to the 15~stations and are aggregated in the boxplot.}
\end{figure}
...
Altogether it can be stated that for this application the left-censored at zero Gaussian
distribution seems to be an appropriate choice while the other two distributions are also
reasonable alternatives. However, they do not lead to an overall improvement in 
