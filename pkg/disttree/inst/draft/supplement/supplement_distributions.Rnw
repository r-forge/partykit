...
the underlying distribution is assumed to be a left-censored at zero Gaussian distribution.
In this supplement the exact same evaluations as in the paper are performed employing
other distribution assumptions. In particular, a left-censored at zero logistic distribution 
and a hurdle-model are applied and their results are compared to those of the left-censored 
at zero Gaussian distribution presented in the paper.}
...
This application study is supplementary material to the application presented
in Section~3 of the paper.
There, the presented distributional forest model is employed to obtain 
...
To compare the performance of the distributional forest to other commonly used
methods the exact same models as in the paper are evaluated on the same data set 
and applying the same settings while only the distribution is exchanged. Further 
details on the models can be found in Section~3.2. of the paper. In particular, 
Table~2 gives an overview over the models and their specifications. As a brief 
reminder the approaches are summarized below.

\begin{itemize}

\item \emph{Distributional forest:} All predictor variables are considered for 
learning a forest where subsampling is employed for each tree. Parameters are 
estimated by adaptive local likelihood based on forest weights.
...
a truncated (positive) part \citep[see][for an overview]{Winkelmann:2006}.
\begin{itemize}
...
For $Y>0$ the probability that it will rain $\nu$ is multiplied with the 
left-truncated at zero density function. Contrary to the censored model, 
here it is only the parameter $\nu$ controlling the point mass while $\mu$ 
and $\sigma$ only influence the distribution of the positive observations.
...
In the log-likelihood function in Equation~\ref{eq:hurdle} properties of the
...
by a separate model in the GAMLSS framework. The same approach is
...
\section{Stations}
Out of the 95 observation stations 15 are considered for this benchmark 
study (see Figure~\ref{fig:map}). 
They have been selected based on the region they are in, their altitude 
and the performance of the different models in the application in the 
paper such that all of these features are represented equally.
...
Innervillgraten, Matrei in Osttirol, St.Johann im Walde.
...
\section{Results}
In Figure~\ref{fig:lattice} each panel belongs to one of the 15 observation stations. 
The three distributions \textit{gaussian} (left-censored at zero Gaussian distribution), 
\textit{hgaussian} (hurdle model based on the Gaussian distribution) and 
\textit{logistic} (left-censored at zero logistic distribution) are compared based 
on the resulting CRPS values for each of the four applied methods that are represented 
by different colors and symbols. 
Overall, the forest model performs almost equally well for all three distributions. 
Therefore it seems to be very robust against missspecifications of the distribution.
For the two GAMLSS models the censored Gaussian and the hurdle model perform similarly 
at most of the stations while applying the censored logistic distribution leads to an 
increase in CRPS, i.e. to a decline in performance, especially for the prespecified 
GAMLSS (for example at stations Zuers and Jungholz).
For the EMOS model exchanging the distribution does not have a strong effect on its 
performance, similar to the forest model.
All in all, exchanging the underlying distribution does not lead to clear 
improvements of any of the methods, especially the censored Gaussian and the 
hurdle model achieve almost the same results. However, the robustness against 
missspecification of the distribution varies slightly over stations. For example 
at station Matrei in Osttirol all methods seem to be very stable while stronger 
influences can be seen at station Jungholz.

\textit{Note:} When applying a hurdle model for the stations Oetz and 
Innvervillgraten the prespecified GAMLSS computationally failed to build/fit 
a model. Therefore this method is not represented in the corresponding panels 
of Figure~\ref{fig:lattice} for these two cases.

In Figure~\ref{fig:box_crpss_gaussian} the tested distributions are compared 
for each method based on CRPS skill score with the censored Gaussian distribution 
as reference represented by the horizontal line at zero. Again, the forest model 
and the EMOS show only slight variations (on average not even one percent) over different 
distributions while the two GAMLSS models perform even or slightly better applying 
a hurdle model and loose strength when employing a logistic distribution compared 
...
@
\caption{\label{fig:lattice}CRPS values of all four methods, once applied with 
each of the three distributions for each of the 15 observation stations.}
\end{figure}
...
#abline(h = 0, col = pal[5], lwd = 2)
@
\caption{\label{fig:box_crpss_gaussian} CRPS skill score values of each of 
the 15 observation stations, separated by method and distribution. Left-censored 
at zero Gaussian is chosen as reference distribution.}
\end{figure}
...
and once based on CRPS values. Both confirm the conclusion that the censored Gaussian 
model and the hurdle model lead to almost the same results for all methods while only 
the logistic model shows small variations.

Altogether it can be stated that for this application the left-censored at zero Gaussian distribution seems 
to be an appropriate choice while the other two distributions, especially the hurdle model, 
are also reasonable alternatives. However, they do not lead to an overall improvement in 
...
distribution family. However, testing both versions of the forest model in this application 
study shows that they both perform almost equally. Figure~\ref{fig:box_crpss_forests} shows
...
improvement is not even one percent, hence neglectably small. Therefore, the two forest 
versions can be stated to perform almost equally well.
...
@
\caption{\label{fig:box_crpss_forests}CRPS skill score values of each of 
the 15 observation stations for the three-parametric forest model with 
the two-part forest model as reference model.}
\end{figure}
