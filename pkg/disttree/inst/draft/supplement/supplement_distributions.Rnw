...
\title{Supplement 1: Different Response Distributions}
...
with censoring point zerol; or zero-censored for short) is, two questions are considered:
...
data set. Table~2 of the manuscript provides
...
The main manuscript employes a Gaussian distribution with probability
...
\end{align}
%
Both distributions have to be adapted in our case study to accomodate that
...
a truncated (positive) part \citep[see][for an overview of both techniques]{Winkelmann:2006}.
\begin{itemize}
...
\end{cases}
\end{equation}
For $Y=0$ the probability for no rain is expressed by $1-\nu$. 
For $Y>0$ the probability that it will rain $\nu$ is multiplied with the 
left-truncated at zero density function. In contrast to the censored model, 
