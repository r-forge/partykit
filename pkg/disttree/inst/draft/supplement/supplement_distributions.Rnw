...
\item Similarly, for the EMOS model all distributions lead to comparable CRPS for all
stations. However, there is some more variation in CRPS across distributions compared to
the distributional forest.

\item In contrast, the two GAMLSS models typically perform better (i.e., have lower CRPS) for
the Gaussian distributions compared to the logistic distribution. The preference of
censored vs.\ hurdle specification is not unanimous across stations.
...
\item For EMOS, the median skill score for both Gaussian hurdle and zero-censored logistic
  are very close to the zero-censored Gaussian reference. However, the variation for the
  hurdle model is much higher than for the censored model.
  
\item For the two GAMLSS models the Gaussian hurdle is quite close to the zero-censored
  Gaussian reference, while the zero-censored logistic clearly performs worse (especially
  in the prespecified GAMLSS).
...
censored Gaussian distribution. Lines correspond to the 15 stations and are
aggregated in the boxplot. The y-axis range is limited
to $-10$\%, lower values for the two GAMLSS models are not displayed.}
\end{figure}
...
\begin{figure}
\setkeys{Gin}{width=0.99\textwidth}
<<results_box_crpss_EMOS, fig=TRUE, echo=FALSE, height=3, width=7.2>>=
...
\begin{figure}
\setkeys{Gin}{width=0.99\textwidth}
<<results_box_crps, fig=TRUE, echo=FALSE, height=3, width=7.2>>=
