...
censored Gaussian distribution (orange horizontal line). Gray lines correspond to the 15~stations and are
...
\emph{The same data already shown in Figures~\ref{fig:lattice} and~\ref{fig:box_crpss_gaussian}
can be displayed in further ways. However, these do not really add much and hence should probably
be omitted.}
...
the parameters of the model, then the two-part approach implemented in this supplement
