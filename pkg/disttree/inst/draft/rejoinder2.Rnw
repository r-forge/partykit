...
the associate editor, and the two referees. Based on your comments, the
manuscript has been revised and we feel that this substantially improved our
contribution.
%
...
TO DO: adaption in the manuscript

Thank you very much for your comment! East Tyrol lies in a different climat zone than the 
rest of Tyrol indicating different meteorological characteristics. For example, a covariate
...
Of course, these are just assumptions and there could be many other reasons. However,
looking at the results of the cross-validation in Supplement~B it can be observed that
the difference in performance between the GAMLSS and the distributional tree models as
seen in the single evaluation evens out, hence the specific choice of learning years seems
...
The missing information and reference have been added in the 
manuscript in Section~3.2 \textit{Models and evaluation: Distributional forest}.
The described parameters pertain to the forest-building
function \code{distforest}. But you are right, within the function \code{distforest} 
the arguments \code{minsplit}, \code{minbucket} and \code{alpha} are handed over to the 
tree-building function \code{disttree} which in this case applies the function \code{ctree}.
...
Thank you for pointing this out. We have rephrased this sentence in Section 3.3. of the manuscript. 
...
Depending on the whether a maximum or a quadratic form of the test statistic is considered
for the standardized test statistic its asymptotic conditional distribution is either a 
normal ($c_{max}$) or a $\chi^2$ distribution ($c_{quad}$). 
We have added this information to the addressed section of Appendix~A.
