...
Thank you very much for your comment! East Tyrol lies in a different climate zone than the 
...
the difference in performance between the GAMLSS and the distributional forest models as
...
Thank you very much for your feedback and the constructive comments regarding the paper. 
You are right that it still has to be tested how distributional trees and forests perform when
employing very complex distributions such as the Dagum distribution. 
However, contrary to the uncensored Gaussian distribution the parameters of the here applied 
censored Gaussian distribution are not orthogonal as changing the mean also influences the variance,
especially close-by the censoring point. Moreover, the three-parametric two-part hurdle model
employed in Supplement A is almost equivalent to the zero-inflated model you suggested. 
Hence, the distributional trees and forests have been tested for distributions with dependencies
among their parameters and have shown to perform well. But of course further investigation is
required in order to confirm these findings in general. This is now also mentioned in an additional 
subsection of the discussion in the manuscript.
