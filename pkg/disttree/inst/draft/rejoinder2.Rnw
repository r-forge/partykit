...
FIXME: adaption in the manuscript

FIXME: I think we should present the two arguments more side by side, i.e., mention both
early on and subsequently discuss both in more detail. (1) One factor is that only a single
split into 28 years of training and 4 years of test data is considered in the main manuscript.
When doing the full cross-validation the pronounced differences between the methods are much
diminished. (2) East Tyrol is in a different climate zone and hence effects could be more
additive and with less interactions than in North Tyrol.
