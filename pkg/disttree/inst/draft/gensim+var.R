                   ...
                   nrcovform = 6)
{
  ...
  ### simulation
  {
    ...
    if(eval_disttree){
      res_dt <- mclapply(1:(nsteps+1),
                         function(i){
                           ...
                           return(as.data.frame(rbind(rmse.exp.true.dt,
                                                      rmse.exp.obs.dt,
                                                      rmse.mu.dt,
                                                      rmse.sigma.dt,
                                                      loglik.dt,
    ...
    if(eval_distforest){
      res_df <- mclapply(1:(nsteps+1),
                         function(i){
                           ...
                           return(as.data.frame(rbind(rmse.exp.true.df,
                                                      rmse.exp.obs.df,
                                                      rmse.mu.df,
                                                      rmse.sigma.df,
                                                      loglik.df,
                                                      crps.df),
                                                row.names = c( "rmse.exp.true.df",
                                                               "rmse.exp.obs.df",
                                                               "rmse.mu.df",
                                                               "rmse.sigma.df",
                                                               "loglik.df",
    ...
    if(eval_gamlss){
      res_g <- mclapply(1:(nsteps+1),
                        function(i){
                          ...
                          return(as.data.frame(rbind(rmse.exp.true.g,
                                                     rmse.exp.obs.g,
                                                     rmse.mu.g,
                                                     rmse.sigma.g,
                                                     loglik.g,
    ...
    # not adapted to this version where all values are returned (not just mean over each step)
    if(FALSE) {
    if(eval_bamlss){
    ...
    simres <- list(reslist = reslist, x.axis = x.axis)
...
if(FALSE){
  #matrix with simresults
  # each row represents one repetition
  # each col represents one value of kappa
  crps_per_step <- matrix(nrow = length(simres$reslist$dt[[1]]["crps.dt",]), ncol = length(simres$reslist$dt))  
  for(k in 1:length(simres$reslist$dt)){
    crps_per_step[,k] <- as.numeric(simres$reslist$dt[[k]]["crps.dt",])
  }
  
  boxplot(crps_per_step[,1], crps_per_step[,2])
