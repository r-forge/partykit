   ...
   gamboost_cvr = FALSE)
{
  ...
  rainres <- mclapply(1:nrep_cross,
                      function(k){
                        ...
                        crps_dt <- crps_df <- crps_g <- crps_gb <- crps_mi <- crps_ml <- crps_mq <- numeric(length = 7L)
                        rmse_dt <- rmse_df <- rmse_g <- rmse_gb <- rmse_mi <- rmse_ml <- rmse_mq <- numeric(length = 7L)
                        ll_dt <- ll_df <- ll_g <- ll_gb <- ll_mi <- ll_ml <- ll_mq <- numeric(length = 7L)
                        cvr_opt <- numeric(length = 7L)
                        g_error_seed <- numeric(length = 7L)
                        mi_error_seed <- numeric(length = 7L)
                        ml_error_seed <- numeric(length = 7L)
                        mq_error_seed <- numeric(length = 7L)
                        evaltime_user <- matrix(0, ncol = 8, nrow = 7L)
                        evaltime_system <- matrix(0, ncol = 8, nrow = 7L)
                        evaltime_elapsed <- matrix(0, ncol = 8, nrow = 7L)
                        
                        # randomly split data in 7 parts each including 4 years
                        years <- c(min(raindata$year) : max(raindata$year))
                        testyears <- list()
                        for(i in 1:7){
                          testyears[[i]] <- sample(years, 4, replace = FALSE)
                          years <- years[!(years %in% testyears[[i]])]
                        }
                        
                        
                        for(i in 1:7) {
                          
                          testdata <- raindata[raindata$year %in% testyears[[i]], ]
                          learndata <- raindata[!(raindata$year %in% testyears[[i]]), ]
...
if(FALSE){
  ...
  # skills score mit Referenz-Modell EMOS quad:
  boxplot(1 - rain_crps / rain_crps[,7])
  abline(h = 0, col = 2, lwd = 2)
  
  # skills score mit Referenz-Modell EMOS log:
  boxplot(1 - rain_crps / rain_crps[,6])
  abline(h = 0, col = 2, lwd = 2)
  
  # skills score mit Referenz-Modell EMOS id:
  boxplot(1 - rain_crps / rain_crps[,5])
  abline(h = 0, col = 2, lwd = 2)
}
