...
in some cases (machine learning performance vs. interpretability).
We adapted our manuscript as we definitely want to emphasize that in our 
application the focus is mainly on prediction but not on interpretation. 
We hope we could point this out clearly. \\
\fixme{\textit{FIXME: Where should this be pointed out in the manuscript? Discussion?}}
