...
\item \emph{Supplement 1: Different response distributions.}\\
...
\item \emph{Supplement 2: Stationwise evaluation.}\\
The show that the more extensive cross-validation results -- obtained only
...
extended to regarding discussion of: variable selection, tuning parameters,
distributional specifications (referring to Supplement~1), and regional
differences (referring to Supplement~2).
...
Biau et al.\ 2016). Therefore, in principle, forests can be built with a very
...
upon non-significance) is set to 1, thus disabling any early stopping.
...
The exact results and times can be found in Supplement 2
...
obtaining the paramter vector that has been estimated in the 
corresponding node when learning the tree. This is now also
...
\fixme{This reply needs to be rewritten. It should be listed first
which variables correspond to each other between the models. Then
you can explain that the greater variation in the forest is not
surprising. Finally, it should be pointed out where in the manuscript
...
so that partitioning is stopped early upon non-signifcance
of the tests. This is sometimes also called ``pre-pruning'' in
the tree literature.

However, in our applications pre-pruning is disabled which
is explained in \dots \fixme{Improve Appendix~A to better
reflect this. Also align with the other comments about
alpha.}
...
precipitation forecasting literature \fixme{add reference}.
...
same evaluations as presented in the main manuscript and present
the full results in Supplement 1 (Different Response Distributions). 
The main insights from this are also discussed in Section~4:
At most stations the censored Gaussian manuscript from the main
manuscript performs at least as well as the two alternative
distributions, if not better. Moreover, the distributional forest
proves to be rather robust against distributional misspecifications.
Therefore, the discussion in the main manuscript is still based
...
power-transformation (Stidd 1973, Hutchinson 1998, Stauffer et al.\ 2017)
...
future research. \fixme{Include one or two sentences about this as an
outlook in the discussion of distributional specifications.}
...
toolbox. Much like random forests and GAMs are complements in
the conditional mean regression toolbox.
...
We have adapted our manuscript to reflect that we focus on forecasting
skill in a large number of models -- and not so much on interpretation.

\fixme{Where should this be pointed out in the manuscript? 
Z: Both in the introduction when the ``zoo'' of regression techniques
is introduced -- and again in the discussion (possibly along with
variable selection?).}
...
forest is very robust against distributional (mis)specifications
...
times are reported now in Supplement~2 (Stationwise Evaluation).
...
\fixme{Refer again to the corresponding point in the manuscript.}
...
in Supplement~2 to check for robustness across stations.
...
\fixme{This also needs to be rewritten (probably best by Reto).
It is a bit frustrating to start the response with comments how instable
all of this is. Instead focus on the insights that are quite
stable across stations.}

Figure 6 is only a visualization of the 10 most important covariates 
of the distributional forest in one particular setting (station Axams, learned 
