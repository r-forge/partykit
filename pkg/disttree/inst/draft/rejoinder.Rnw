...
present the full results in Supplement 1 (Different Response Distributions). 
...
future research. 
%\fixme{Include one or two sentences about this as an
...
We have adapted our manuscript in Section~1 and Section~4, \textit{Variable Selection}, 
to reflect that we focus on forecasting skill in a large number of 
models -- and not so much on interpretation.
...
forest is %very robust against distributional (mis)specifications and (FIXME: this is shown in Supplement 1)
almost always competitive with the best among the other methods 
