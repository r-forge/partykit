...
\newcommand{\fixme}[1]{{\sethlcolor{fixmecol}\hl{#1}}}
...
raster::image(raster("gfs_topo.nc"), col = cols,
              main=NA, ylab = "Latitude", xlab = "Longitude",
              xlim = c(9.8,13.2), ylim = c(46.6, 47.87), zlim = zlim,
              xaxt = "n", yaxt = "n")
box()
...
As you mention one way to deal with skewness is to use a distribution
with an additional skewness parameter (e.g., Scheuerer and Hamill 2015,
Baran and Nemoda 2016) which increases the complexity of the statistical
models.
Another frequently used approach is to apply a
power-transformation (Stidd 1973, Hutchinson 1998, Stauffer et al. 2017) to
remove the majority of the skewness from the data. The optimal transformation
depends on the application and data set, in our case we are following the work
of Stauffer et al. (2017) who presented a similar application on the very same
data set. As our results show (Figure~3 in the manuscript) this approach works 
well for the application presented in our manuscript.
Moreover, the focus of this paper is on the features of the presented
distributional forest regarding variable selection and the capability
to also model non-linear effects rather than optimizing the model
specification for this particular application.
...
%\fixme{\textit{FIXME: Should this explanation why there is skewness be included?}}

%As you mention one way how to deal with skewness would be to 
%apply a distributional family with an additional parameter to capture
%skewness. However, this can be very complex and thus lead to less well 
%fitting estimators of the location and scale parameter.
%Moreover, the focus of this paper is on the features of the presented 
%distributional forest regarding variable selection and the capability 
%to also model non-linear effects rather than optimizing the model 
%specification for this particular application.
%Transforming the data to remove large parts of the skewness is
%a frequently used approach. Therefore, employing such a transformation 
%allows for a better way of comparing our results to those presented in 
%literature. In this study we are thus following Stauffer et al. (2017) 
...
We completely agree with you as that other models such as
GAMLSS are clearly advantageous regarding interpretability.
These models provide a very powerful framework that works
well in a wide range of applications and research fields,
including meteorology. Therefore, our intention is not to present
distributional forests as a replacement of these commonly used methods
but rather as a well working alternative. Especially if one is mainly
interested in predictive skill and a good performing fully automated
weather forecasting tool the interpretability is not implicitly needed
in some cases (machine learning performance vs. interpretability, e.g.,
Rasp and Lerch 2018).
We adapted our manuscript as we want to emphasize that in our
application the focus is mainly on the predictive performance and less on
the interpretation of the different effects.
We hope we could point this out clearly. \\
\fixme{\textit{FIXME: Where should this be pointed out in the manuscript? 
In the discussion or after introducing trees and forests? (trade-off between
flexibility of the model and interpretability)}}
