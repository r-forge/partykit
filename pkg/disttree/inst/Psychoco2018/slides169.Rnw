...
\author{Achim Zeileis, Torsten Hothorn, Lisa Schlosser, Heidi Seibold}
...
\minipage{0.2\textwidth}
  ...
  \begin{center}
  \begin{tikzpicture}
  \draw[line width=0.2mm, black] (0,0) rectangle (2.25,2);
  \node[rectangle, align=center] (sq) at (1.12, 1) {\Huge{?}};
  \end{tikzpicture}
  \end{center}}
\endminipage
...
\begin{itemize}
\item Specified log-likelihood function $$\ell(\theta; y_i)$$ with distribution parameter $\theta$ and observation $y_i$ of a learning data set $\{y_i\}_{i = 1,\ldots,n}$.

\bigskip

\item Estimation of $\hat{\bold{\theta}}$ via maximum likelihood 
$$
...
{\bf Tree:}\\
\medskip
For a set of covariates $\bold{x}$, a learning data set $\{(y_i, \bold{x}_i)\}_{i =1,\ldots,n}$ and subgroups $\mathcal{B}_1, \ldots, \mathcal{B}_B$ calculate weights
$$
w^{\text{tree}}_i(\bold{x}) = \sum_{b=1}^B I((\bold{x}_i \in \mathcal{B}_b) \land (\bold{x} \in \mathcal{B}_b))
$$
which are then included in the estimation of $\hat{\theta}$
$$
\hat{\theta}_{\text{tree}}(\bold{x}) = \max_{\theta \in \Theta} \sum_{i=1}^n w^{\text{tree}}_i(\bold{x}) \cdot \ell(\theta; y_i).
$$
\end{frame}
...
{\bf Forest:}\\
\medskip
For a set of covariates $\bold{x}$, a learning data set $\{(y_i, \bold{x}_i)\}_{i =1,\ldots,n}$, $T$~trees and corresponding subgroups $\mathcal{B}_1^t, \ldots, \mathcal{B}_B^t$ calculate weights
$$
w^{\text{forest}}_i(\bold{x}) = \sum_{t=1}^T \sum_{b=1}^{B^t} I((\bold{x}_i \in \mathcal{B}^t_b) \land (\bold{x} \in \mathcal{B}^t_b))
$$
which are then included in the estimation of $\hat{\theta}$
$$
\hat{\theta}_{\text{forest}}(\bold{x}) =  \max_{\theta \in \Theta} \sum_{i=1}^n w^{\text{forest}}_i(\bold{x}) \cdot \ell(\theta; y_i).
$$
\end{frame}
...
\vspace{-0.5cm}
$$
\hat{\theta}(\bold{x}) =  \max_{\theta \in \Theta} \sum_{i=1}^n \ell(\theta; y_i)
$$
...
\setkeys{Gin}{width=0.55\linewidth, height=0.55\linewidth}
...
polygon(c(odata$x, rev(odata$x)), c(odata$mu + odata$sigma, rev(odata$mu - odata$sigma)),
  col = pallight["forest"], border = "transparent")
legend(x = -0.0715, y = 21.65, expression(mu  %+-%  sigma), bty = "n")
...
\begin{frame}
\frametitle{Choosing a distribution}

\medskip
{\bf Based on:}
\begin{itemize}
\item Recommendations by subject matter literature.
\item Experiences from other modelling situations.
\item Common simplifications/generalizations/assumptions.
\end{itemize}

\bigskip
{\bf Examples:}
\begin{itemize}
\item Survival times: Weibull distribution, exponential distribution.
\item Wages, precipitation: censored normal distribution.
\item Temperature: normal distribution.
\item Count of events/accidents (insurance): Poisson distribution.
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Alternative: Transformation trees and forests}

\medskip

{\bf General framework / Conditional model:}
$$
\mathbb{P}(Y\leq y|X=x) = \mathbb{P}_{Y|X=x} = \mathbb{P}_{Y, \theta(x)} 
$$

\bigskip

{\bf Transformation model:}
$$
\mathbb{P}(Y\leq y|X=x) = \Phi(\bold{a}_{Bs,d}(y)^T \theta(x)) 
$$

\begin{itemize} 
\item $\bold{a}_{Bs,d}(y)^T \theta(x)$ is a smooth, monotone Bernstein polynomial of degree $d$.
\item $d=1$ means $\mathbb{P}(Y\leq y|X=x) = \mathcal{N}(\mu(x),\sigma^2(x))$.
\item $d=5$ is surprisingly flexible.
\end{itemize}
%All ``classical'' distributions:  Distributional forests
\end{frame}
