...
control <-  disttree_control(type.tree = "ctree", teststat = "quad", testtype = "Bonferroni", mincriterion = 0.7, minbucket = 20L)
dt <- disttree(y~x1, data=learndata, family=dist_list_normal, control = control)
dt$fitted.par <- predict(dt, type = "parameter")

if(file.exists("sim_forest.rda")){
  load("sim_forest.rda")
} else {
  control <-  disttree_control(teststat = "quad", testtype = "Univ", mincriterion = 0, minbucket = 7L,type.tree = "ctree")
  df <- distforest(y~x1, data=learndata, family=dist_list_normal, ntree = 500L, control = control)
  df$fitted.par <- predict(df, type = "parameter")
