...
\begin{itemize}
\item Specify distribution family with distribution function 
$$F(y) = \mathbb{P}_{\theta}(Y\leq y)$$ 
and log-likelihood function $$\ell(\theta; y).$$ 
The full distribution is then defined by the parameter vector $\theta \in \Theta$ representing location, scale, and shape. 
...
$$
for a learning data set $\{y_i\}_{i = 1,\ldots,n}.$
...
How to calculate weights $\{w_i(\bold{x})\}_{i=1,\dots,n}$?\\

\bigskip

Two strategies:
\begin{itemize}
\item Build a tree and let $w_i(\bold{x}) \in \{0,1\}$ indicate whether $\bold{x}$ is in the same subgroup as the $i$-th learning observation.
\item Build an ensemble of trees on resampled versions of the learning data and let $w_i(\bold{x}) \in  \mathbb{N}_0$ count the number of trees in which $\bold{x}$ is in the same subgroup as the $i$-th learning observation.
%\item Via a tree model: $w_i(\bold{x}) \in \{0,1\}$ indicates whether $\bold{x}$ is in the same terminal node as the $i$-th learning observation.
%\item Via a forest model: $w_i(\bold{x}) \in  \mathbb{N}_0$ counts the number of trees in which $\bold{x}$ is in the same terminal node as the $i$-th learning observation. 
\end{itemize}
...
\hspace{0.55 em} disttree, distforest (100 trees), gamlss\\
...
\begin{tabular}{ll}
Response:         & $y \sim \mathcal{N}(\mu(x), \sigma(x))$ with\\
                  & \qquad $\mu(x) =  10 \cdot \exp\left(-(4 \cdot x - 2)^{4}\right)$\\
                  & \qquad $\sigma(x) = 0.5 + 2 \cdot |x|$\\
Covariate:        & $x \sim  \mathcal{U}([-0.4,1])$\\
\# observations:  & 300 
\end{tabular}
\end{frame}
...
\begin{tabular}{ll}
Response:         & $y \sim \mathcal{N}(\mu(x), \sigma(x))$ with\\
                 & \qquad $\mu(x) =  10 \cdot \exp\left(-(4 \cdot x - 2)^{2 \cdot \kappa}\right)$\\
                 & \qquad $\sigma(x) = 0.5 + 2 \cdot |x|$\\
                 & (increasing $\kappa$ leads to steeper transitions)\\
Covariate:        & $x \sim  \mathcal{U}([-0.4,1])$\\
\# observations:  & 1000 
\end{tabular}
...
\normalsize
{\bf Evaluation} over increasing $\kappa$:

\smallskip

\footnotesize
\begin{tabular}{ll}
Interval for $\kappa$:     & $[1, 71]$ \\
Stepsize for $\kappa$:     & 7 \\
\# replications per step:  & 150 
\end{tabular}
...
{\bf Based on:}
\begin{itemize}
\item Recommendations by specialist literature.
\item Expert subject matter knowledge.
\item Experiences from other modeling situations.
...
\begin{frame}
\frametitle{Alternative: Transformation models}
{\bf Advantages:}
\begin{itemize}
...
{\bf Distribution function:}
$$
F(y) = \Phi(\bold{a}_{Bs,d}(y)^\top \theta) 
$$
...
%\newgeometry{left=0.2cm, right=0.2cm, top = 0.5cm, bottom=0.5cm}
\begin{frame}
\frametitle{Alternative: Transformation model}
