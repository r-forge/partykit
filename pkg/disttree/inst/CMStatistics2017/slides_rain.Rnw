...
%Task:
\begin{itemize}
\item Goal: Predict total precipitation amount.
\item Input: Numerical Weather Prediction (NWP).
\item Problem: Uncertain initial conditions and \\
\hspace{1.7cm}unknown processes.
\item Idea: Statistical model with numerical predictions as \\
\hspace{1cm}covariates.
\item Distribution Assumption:\\
\vspace{0.2cm}
\hspace{1cm}$(\text{Total precipitation})^\frac{1}{1.6} \sim \textit{cens}\mathcal{N}(\mu,\sigma^2)$
\end{itemize}
\end{frame}
...
\item Daily 24h precipitation sums from July of 28 years (1985--2012)\footnote{provided by the National Hydrographical Service (EHYD)}.
\item Numerical ensemble predictions of meteorological quantities\footnote{provided by the {U.S.} National Oceanic and Atmospheric Administration (NOAA)\\
\hspace{0.32cm}using the Global Ensemble Forecast System (GEFS)}: \\
\hspace{0.5cm}11 members each using slightly different \\
\hspace{0.5cm}perturbed initial conditions.
\end{itemize}
...
Variable Selection:
\begin{itemize}
\item Complex terrain $\rightarrow$ unknown influences.
\item 82 Covariates deduced from 14 base variables:
\begin{itemize}
...
\item Mean sea level pressure difference
...
\end{itemize}
\item Variations such as ensemble minimum/maximum/mean/standard deviation.
\end{itemize}
\end{frame}
