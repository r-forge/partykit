...
\item Input: Numerical Weather Prediction (NWP) with gridsize ... .
\item 
\vspace{0.1cm}
\end{itemize}
\end{frame}
...
Data:
\begin{itemize}
\item Daily observations from 95 stations: \\
24h precipitation sums from July of 28 years.
\item Numerical ensemble predictions of meteorological quantities\\
\vspace{0.1cm}
$\rightarrow$ Covariates:\\
\begin{itemize}
