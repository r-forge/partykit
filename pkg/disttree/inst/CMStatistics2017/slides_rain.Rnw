...
\hspace{0.32cm}using the Global Ensemble Forecast System (GEFS)}: 11 members each using slightly different perturbed initial conditions.
\end{itemize}
...
(variations such as ensemble minimum/maximum/mean/standard deviation):\\
\begin{itemize}
