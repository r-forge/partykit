...
\item Idea: Statistical Model with numerical predictions as covariates.
\item Data:
\begin{itemize}
\item 95 observation stations in Tyrol.
\item Daily 24h precipitation sums from July of 28 years (1985--2012).
\item Numerical ensemble predictions of meteorological quantities.
\end{itemize}
\item Distribution Assumption: $(\text{Total precipitation})^\frac{1}{1/6} \sim \textit{c}\matchcal{N}$
\end{itemize}
\end{frame}
...
\item Learn forest model on data from 24 years (1985--2008).
\item Evaluate on 4 years (2009--2012).
\item Compare to commonly used methods:\\
\begin{itemize}
\item Prespecified GAM
\item Boosted GAM
\item Ensemble Model Output Statistics (EMOS) 
\end{itemize}
\end{itemize}
\end{frame}
