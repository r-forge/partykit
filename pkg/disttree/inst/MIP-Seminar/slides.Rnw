...
%{\small 
(what I've been working on the last 2 years)
}
\end{minipage}
...
\item<6-> Evaluate goodness of fit \\
(for each parameter and each observation).
\item<8-> Choose covariate $X$ with strongest influence on goodness of fit of $\mathcal{D}(Y; \hat{\theta})$ as split variable.
\end{enumerate}
\end{minipage}
\begin{minipage}{0.25\textwidth}
\vspace{-1cm}
