...
\visible<2-3>{
\node[inner sep=0pt] (today) at (-1.9,-0.2)
    {\includegraphics[width=.38\textwidth]{today.png}};
}
...
\visible<2-3>{
\node[inner sep=0pt] (future) at (5.4,-0.2)
    {\includegraphics[width=.38\textwidth]{future.png}};
}
\end{tikzpicture}
\end{center}
\visible<3>{
...
\begin{itemize}
\item Prediction.
\item Analysis of dependencies and interactions.
\end{itemize}
}
\vspace{0.5cm}
\visible<6->{
...
\visible<4>{
\begin{minipage}{0.65\textwidth}
\textbf{Example:} Generalized additive model\\
...
\begin{align*}
\mu(X) = \alpha_{\mu} + s_{\mu}(X)\\  
\sigma(X) = \alpha_{\sigma} + s_{\sigma}(X)
\end{align*}
\end{minipage}
\hfill
...
sp_humidity <- partysplit(4L, breaks = 80)

pn <- partynode(1L, split = sp_rain, kids = list(
  partynode(2L, split = sp_humidity, kids = list(
      partynode(3L, info = "5 mm/24h"),
      partynode(4L, info = "8 mm/24h"))),
  partynode(5L, info = "18 mm/24h")))
...
\begin{frame}[fragile]
\frametitle{Statistical modeling}
\textbf{Algorithmic modeling:}

\begin{center}
\begin{tikzpicture}
\node[] at (0,0.4) {X};
\draw[->] (0.3,0.4)--(0.8,0.4);
\draw[] (1,0) rectangle (2.5,0.8);
\node[font=\scriptsize] at (1.75,0.4) {nature};
\draw[->] (2.7,0.4)--(3.2,0.4);
\node[] at (3.5,0.4) {Y};
...
\begin{minipage}{0.65\textwidth}
\vspace{0.5cm}
\textbf{Example:} Tree model\\
\vspace{-0.8cm}
\begin{center}
\resizebox{0.6\textwidth}{!}{
<<plot_raintree, fig=TRUE, echo=FALSE, width=5, height=3>>=
...
\vspace{0.1 cm}
\begin{itemize}
  \item Probabilistic forecast\\
  $\rightarrow$ Distributional approach
  \item Automatic detection of steps and abrupt changes,\\
  automatic selection of variables and interactions\\
  $\rightarrow$ Tree approach
  \item Smoothing and regularization\\
...
{\bf Tree:}
\begin{enumerate}
\item<3-> Fit a global distributional model $\mathcal{D}(Y; \theta)$: \\ % to the whole data set:\\
Estimate model parameters $\hat{\theta}$.
\item<5-> Evaluate how well the model fits the data (for each parameter and each observation).
\item<6-> Choose the covariate $X$ with the strongest influence or association to
goodness-of-fit of $\mathcal{D}(Y; \theta)$ as split variable.
\end{enumerate}
\end{minipage}
...
\draw[-, line width=0.8pt] (n0) -- (n1) node [midway, left] {\scriptsize $X \leq p$};
\draw[-, line width=0.8pt] (n0) -- (n2) node [midway, right] {\scriptsize $X > p$};
}
\visible<9->{
...
\begin{enumerate}
\setcounter{enumi}{3}
\item<7-> Find the split point $p$ which leads to the highest improvement.
...
%{\bf Tree:}
%\begin{enumerate}
%\item Fit a global distributional model $\mathcal{D}(Y; \theta)$: \\ % to the whole data set:\\
%Estimate model parameters $\hat{\theta}$.
%\item Evaluate how well the model fits the data (for each parameter).
%\item Choose the covariate $X$ with the strongest influence or association to
%goodness-of-fit of $\mathcal{D}(Y; \theta)$ as split variable.
%\item Find the split which leads to the highest improvement.
%\item Repeat steps 1--4 recursively in the subgroups
%    until some stopping criterion is met.
%\end{enumerate}
...
GFS_t2m_mean[,1] <- rowMeans(GFS_t2m)
GFS_t2m_mean <- GFS_t2m_mean[,1]

# GFS_rain_mean <- rowMeans(GFS_rain)
GFS_rain_mean <- GFS_rain
GFS_rain_mean[,1] <- rowMeans(GFS_rain)
GFS_rain_mean <- GFS_rain_mean[,1]
@
...
\vspace{-0.3cm}
\setkeys{Gin}{width=1.1\linewidth}
<<ensemble_2_plot, fig=TRUE, echo=FALSE, width=9>>=
par(mar=c(0.4,0,0.2,0), oma=c(6,3.2,5,4))
layout( matrix(1:2,ncol=1) )
plot( GFS_t2m[,c(1,2)], screen=1, xaxs="i", xaxt="n", ylim = range(GFS_t2m)*1.05)
abline( v = main, lty = 2 )
mtext( side = 3, line = 1, cex = 1.2, font = 2, title )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Temperature [°C]" )
plot( GFS_rain[,c(1,2)], screen=1, xaxs="i", xaxt="n", yaxs="i", ylim=range(GFS_rain)*c(0,1.05) )
abline( v = main, lty = 2 )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Rain [mm/6h]" )
...
\vspace{-0.3cm}
\setkeys{Gin}{width=1.1\linewidth}
<<ensemble_3_plot, fig=TRUE, echo=FALSE, width=9>>=
par(mar=c(0.4,0,0.2,0), oma=c(6,3.2,5,4))
layout( matrix(1:2,ncol=1) )
plot( GFS_t2m[,c(1,2,3)], screen=1, xaxs="i", xaxt="n", ylim = range(GFS_t2m)*1.05)
abline( v = main, lty = 2 )
mtext( side = 3, line = 1, cex = 1.2, font = 2, title )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Temperature [°C]" )
plot( GFS_rain[,c(1,2,3)], screen=1, xaxs="i", xaxt="n", yaxs="i", ylim=range(GFS_rain)*c(0,1.05) )
abline( v = main, lty = 2 )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Rain [mm/6h]" )
...
\vspace{-0.3cm}
\setkeys{Gin}{width=1.1\linewidth}
<<ensemble_20_plot, fig=TRUE, echo=FALSE, width=9>>=
par(mar=c(0.4,0,0.2,0), oma=c(6,3.2,5,4))
layout( matrix(1:2,ncol=1) )
plot( GFS_t2m, screen=1, xaxs="i", xaxt="n", ylim = range(GFS_t2m)*1.05)
abline( v = main, lty = 2 )
mtext( side = 3, line = 1, cex = 1.2, font = 2, title )
mtext( side = 2, line = 2.3, cex = 1, font = 1, text = "Temperature [°C]" )
...
Ensemble predictions of various meteorological quantities (related to precipitation, temperature, air pressure, sunshine, \dots $\rightarrow$ 82 covariates)
\end{itemize}
\end{frame}
