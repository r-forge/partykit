...
\includegraphics{density_right.pdf}
%\end{center}
\end{minipage}
...
\item<10-> Repeat steps 1--3 recursively until some stopping criterion is met,\\
...
\textbf{Applied models and reference models:}
\begin{itemize}
...
\item Results are compared based on circular CRPS (Grimit et al., 2006)
\end{itemize}
...
\frametitle{Software}
%\vspace{0.4cm}
\textbf{Packages:} \emph{disttree} %and \emph{circmax} 
