...
change along with the available covariates. If so, by maximizing a partitioned likelihood
...
\caption{Fitted distributional tree for a circular response employing the von
Mises distribution. For each terminal node, the empirical density and the 
fitted density distribution is displayed in gray and red, respectively. The
estimated location parameter is depicted as red hand and conveys the direction from
...
presents an application of circular regression trees  on hourly wind direction
...
in the North with an approx. 2300\,m high mountain ridge and opens to the South
...
pressure gradient along the upper and lower Inn Valley, and the wind direction and
wind speed at Innsbruck Airport.

As an illustration of the tree-based wind direction forecasts at Innsbruck
Airport, a distributional tree of depth three is depicted in Figure~\ref{fig:tree}
showing the circular histogram and fitted response distribution for each terminal node.
According to the fitted location parameter the following wind regimes can be summarized:
...
11 captures the cases with rather low wind speeds which typically do not show a clear 
wind direction or distinct wind regime as implied by a small concentration 
parameter.
In general, winds typically blow from areas of high
...
identified by the fitted location parameter, and the additional splits in
...
concentration parameter. This can be seen in the terminal nodes with strong
...
This study shows that applying distributional trees on circular data provides
easily interpretable models clearly illustrating the impact of given covariates
and their possible interactions on the distribution of the dependent variable.
Moreover, covariates are selected automatically such that no prespecification
of the model is required which can be a challenging task, particularly for a
meteorological quantity such as wind direction with high variation and many
possible influencing factors. Additionally, the employed distributional trees
allow for abrupt changes in the response variable, such as e.g., a wind shift due to
a turn of the the pressure gradient within the Inn Valley.

A possible extension of the method as presented is to build an ensemble of
distributional trees to regularize and stabilize the model (see, e.g.,
distributional forests, Schlosser et.~al, 2018). 
Furthermore, in order to properly account not only for the circular properties of 
the response but also of the regressors, extensions in the search of split points 
would be feasible. For example, searching for two split points defining a split 
interval on the circular scale instead of a single split point on the linear scale 
could reduce the complexity of the model and possibly improve the results.
...
%required which can be a challenging task, particularly for a meteorological quantity such as wind 
...
%% In that way the impact of the chosen scale on which circular regressors are considered should be reduced.
