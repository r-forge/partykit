...
According to the fitted location parameters $\mu$ the following wind regimes can be summarized:
...
identified by the fitted location parameters, and the additional splits in
...
%Therefore, considering them on a circular scale could reduce the complexity of the model and
...
%meteorlogical quantity such as wind direction with high variation and many
...
%distributional forests, Schlosser et.~al, 2018). Additionally, in order to
%properly account not only for the ciruclar properties of the response but also
%of the regressors, some extension in searching the split points would be
%feasible. In that case two split points define a split interval on the circular
%scale instead of a single split point on the linear scale which would reduce
%the complexity of the model and possibly further improve the results.
%%% SUGGESTION MORITZ END
