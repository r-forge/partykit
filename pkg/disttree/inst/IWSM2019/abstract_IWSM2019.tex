...
are depicted along with the estimated location parameter (red hand). The covariates employed are
wind direction (dd; $\text{rad}$), wind speed (ff; $\text{ms}^{-1}$),
and pressure gradients along the upper (pdiff\_up; $\text{hPa}$) and lower
(pdiff\_low; $\text{hPa}$) Inn Valley.}
...
in the North by an approx.~2300\,m high mountain ridge and opens to the South
into the crest-perpendicular Wipp Valley. Hence, pressure gradients along the
upper and lower Inn Valley are considered as covariates along with other
meteorological measurements (lagged by one hour) such as wind direction and
wind speed at Innsbruck Airport.

As an illustration of the tree-based wind direction forecasts at Innsbruck
Airport, a distributional tree of depth three is depicted in 

Figure~\ref{fig:tree} depicts the resulting distributional tree, including
...
in covariates without prespecification of such effects. This is modeling
wind direction in mountainous terrain where wind shifts can occur due to turns
a turn of the pressure gradient along a valley.
...
for variable ``dd'' in Figure~\ref{fig:tree}).
