...
save(df, file = "~/svn/partykit/pkg/disttree/inst/circforest/df500.rda")

vi <- disttree:::varimp.distexforest(df, nperm = 1)
...
####################################################
# drop covariates from Obertauern, Montana, Mariazell, Guetsch and Altdorf

dropid <- (grepl("obertauern", names(d)) | grepl("montana", names(d)) |
             grepl("mariazell", names(d)) | grepl("guetsch", names(d)) |
             grepl("altdorf", names(d)))
