...
coltree <- hcl(c(128,100,30))
colforest <- hcl(c(10,100,30))
colgamlss <- hcl(c(260,100,30))
...
plot1 <- function(learndata, dt=NULL, df=NULL, g=NULL, all.par = FALSE, df.par = FALSE, g.par = FALSE, dt.par = FALSE) 
{
  ...
  if(df.par){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$fitted.mu.df, type = "l", col = colforest)
    lines(x = sp$x, y = sp$fitted.mu.df + sp$fitted.sigma.df, type = "l", col = 'orange')
    lines(x = sp$x, y = sp$fitted.mu.df - sp$fitted.sigma.df, type = "l", col = 'orange')
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')), 
           col = c('black','grey',colforest, 'orange'), lty = 1, bty = 'n')
  }
  ...
  if(g.par){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$fitted.mu.g, type = "l", col = colgamlss)
    lines(x = sp$x, y = sp$fitted.mu.g + sp$fitted.sigma.g, type = "l", col = 'lightblue')
    lines(x = sp$x, y = sp$fitted.mu.g - sp$fitted.sigma.g, type = "l", col = 'lightblue')
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')),  
           col = c('black','grey',colgamlss, 'lightblue'), lty = 1, bty = 'n')
  }
  ...
  if(all.par){
    ...
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\hat{\\mu}$ disttree'),
           ...
           col = c('black',coltree,colforest,colgamlss), lty = 1, bty = 'n')
  }
}
...
box(lwd=5)
lines(x = x, y = ytrue, col = "red", lwd = 5, main = "")
@
}
\end{center}
\endminipage
...
plot(y=y , x=x, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
box(lwd=5)
lines(x = x, y = ytrue, col = "red", lwd=5)
...
lines(x = x, y = ytree, col = "red", lwd=5)
@
\visible<1->{
...
box(lwd=5)
#lines(x = x, y = ytrue, col = "grey", lwd=5, main = "")
lines(x = x, y = yforest, col = "red", lwd=5, main = "")
@
}
\end{center}
...
plot(fig, inner_panel = node_inner_ext,
     tp_args = list(FUN = identity, width = 18, fill = pal[c(1, 3)]), 
...
plot_rmse <- function(simlist, type = c("exp", "par")){
  ...
  if(type == "exp"){
    ...
    par(mar=c(5.1,4.1,4.1,3.1))
    plot(x = simlist$x.axis, y = rmse[,"dt.true"], type = "l", col = coltree, ylim = ylim,
         xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
    lines(x = simlist$x.axis, y = rmse[,"dt.obs"], type = "l", lty = 2, col = coltree)
    lines(x = simlist$x.axis, y = rmse[,"df.true"], type = "l", col = colforest)
    lines(x = simlist$x.axis, y = rmse[,"df.obs"], type = "l", lty = 2, col = colforest)
    lines(x = simlist$x.axis, y = rmse[,"g.true"], type = "l", col = colgamlss)
    lines(x = simlist$x.axis, y = rmse[,"g.obs"], type = "l", lty = 2, col = colgamlss)
    #legend('left', c("disttree", "distforest", "gamlss"), col = c(coltree, colforest, colgamlss), lty = 1)
    ...
    mtext(text= "disttree", side = 2, col = coltree, las = 1, line = 0.2, padj = 6.5)
    mtext(text= "distforest", side = 2, col = colforest, las = 1, line = 0.2, padj = 13)
    mtext(text= "gamlss", side = 2, col = colgamlss, las = 1, line = 0.3, padj = 16)
    ...
    #add arrows below
    arrows(0.45,0.09,0.3,0.09,lwd=1) ## add arrow
    arrows(0.6,0.09,0.75,0.09,lwd=1) ## add arrow
  ...
  if(type == "par"){
    ...
    ylim <- c(min(rmse), max(rmse))
    par(mar=c(5.1,4.1,4.1,3.1))
    plot(x = simlist$x.axis, y = rmse[,"mu.dt"], type = "l", col = coltree, ylim = ylim,
         xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
    lines(x = simlist$x.axis, y = rmse[,"sigma.dt"], type = "l", lty = 2, col = coltree)
    lines(x = simlist$x.axis, y = rmse[,"mu.df"], type = "l", col = colforest)
    lines(x = simlist$x.axis, y = rmse[,"sigma.df"], type = "l", lty = 2, col = colforest)
    lines(x = simlist$x.axis, y = rmse[,"mu.g"], type = "l", col = colgamlss)
    lines(x = simlist$x.axis, y = rmse[,"sigma.g"], type = "l", lty = 2, col = colgamlss)
    #legend('topleft', c("disttree", "distforest", "gamlss"), col = c(coltree, colforest,colgamlss), lty = 1)
    mtext(text= "smooth", side = 1, line = 1, adj = 0)
    mtext(text= "steep", side = 1, line = 1, adj = 1)
    mtext(text= TeX('$\\kappa$'), side = 1, line = 1)
    mtext(text= "RMSE", side = 2, line = 1)
    mtext(text= "disttree", side = 2, col = coltree, las = 1, line = 0.2, padj = -4)
    mtext(text= "distforest", side = 2, col = colforest, las = 1, line = 0.2, padj = 8)
    mtext(text= "gamlss", side = 2, col = colgamlss, las = 1, line = 0.3, padj = 14)
...
plot_ll <- function(simlist){
  ...
  par(mar=c(5.1,4.1,4.1,3.1))
  plot(x = simlist$x.axis, y = ll[,"dt"], type = "l", col = coltree, ylim = ylim,
       xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
  lines(x = simlist$x.axis, y = ll[,"df"], type = "l", col = colforest)
  lines(x = simlist$x.axis, y = ll[,"g"], type = "l", col = colgamlss)
  #legend('topright', c("disttree", "distforest", "gamlss"), col = c(coltree, colforest,colgamlss), lty = 1)
  mtext(text= "smooth", side = 1, line = 1, adj = 0)
  mtext(text= "steep", side = 1, line = 1, adj = 1)
  mtext(text= TeX('$\\kappa$'), side = 1, line = 1)
  mtext(text= "Log-Likelihood", side = 2, line = 1)
  mtext(text= "disttree", side = 2, col = coltree, las = 1, line = 0.2, padj = 12)
  mtext(text= "distforest", side = 2, col = colforest, las = 1, line = 0.2, padj = -8.5)
  mtext(text= "gamlss", side = 2, col = colgamlss, las = 1, line = 0.3, padj = -16)
