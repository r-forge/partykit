...
plot1 <- function(learndata, dt=NULL, df=NULL, g=NULL, all.par = FALSE, df.par = FALSE, g.par = FALSE, dt.par = FALSE) 
{
  ...
  if(!(any(dt.par, df.par, g.par, all.par))){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", lty = 2, col = 'black')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", lty = 2, col = 'black')
  ...
  if(df.par){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$fitted.mu.df, type = "l", col = 'red')
    lines(x = sp$x, y = sp$fitted.mu.df + sp$fitted.sigma.df, type = "l", col = 'orange')
    lines(x = sp$x, y = sp$fitted.mu.df - sp$fitted.sigma.df, type = "l", col = 'orange')
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')), 
           col = c('black','grey','red', 'orange'), lty = 1, bty = 'n')
  }
  ...
  if(g.par){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$fitted.mu.g, type = "l", col = 'blue')
    lines(x = sp$x, y = sp$fitted.mu.g + sp$fitted.sigma.g, type = "l", col = 'lightblue')
    lines(x = sp$x, y = sp$fitted.mu.g - sp$fitted.sigma.g, type = "l", col = 'lightblue')
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')),  
           col = c('black','grey','blue', 'lightblue'), lty = 1, bty = 'n')
  }
  ...
  if(all.par){
    ...
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\hat{\\mu}$ disttree'),
           ...
           col = c('black','green4','red','blue'), lty = 1, bty = 'n')
  }
}



### plotting function for one data set and sigma only
plot1sigma <- function(learndata, dt=NULL, df=NULL, g=NULL) 
{
  plotdata <- cbind(learndata[,c("y","x1","mu","sigma")],
                    df$fitted.par,
                    dt$fitted.par,
  ...
  plot(y = sp$y-sp$true.mu, x = sp$x, type = "p", col="grey", xlab = "x", ylab = "y",  main = "disttree", xaxt='n', yaxt='n', xlab = "", ylab = "")
  lines(x = sp$x, y = sp$true.sigma, type = "l", col = 'black')
  lines(x = sp$x, y = -sp$true.sigma, type = "l", col = 'black')
  lines(x = sp$x, y = sp$fitted.sigma.dt, type = "l", col = 'green4')
  lines(x = sp$x, y = -sp$fitted.sigma.dt, type = "l", col = 'green4')
  lines(x = sp$x, y = sp$fitted.sigma.df, type = "l", col = 'red')
  lines(x = sp$x, y = -sp$fitted.sigma.df, type = "l", col = 'red')
  lines(x = sp$x, y = sp$fitted.sigma.g, type = "l", col = 'blue')
  lines(x = sp$x, y = -sp$fitted.sigma.g, type = "l", col = 'blue')
  legend('topleft', c(TeX('$\\sigma$'), TeX('$\\hat{\\sigma}$ disttree'),
                      TeX('$\\hat{\\sigma}$ distforest'), TeX('$\\hat{\\sigma}$ gamlss')), 
         col = c('black','green4','red','blue'), lty = 1)
}
...
par(mar=c(2,0,2,0))
plot(y=y , x=x, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
lines(x = x, y = ytrue, col = "red", lwd = 1.5, main = "")
@
}
\end{center}
\endminipage
...
par(mar=c(2,0,2,0))
plot(y=y , x=x, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
lines(x = x, y = ytrue, col = "red", lwd = 1.5)
lines(x = x, y = ytrue+0.1+var, col = "red", lty = 2)
lines(x = x, y = ytrue-0.1-var, col = "red", lty = 2)
@
}
\end{center}
...
plot(x = x, y = y, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
#lines(x = x, y = ytrue, col = "grey", lwd = 1.5, main = "")
lines(x = x, y = ytree, col = "red", lwd = 1.5)
@
\visible<1->{
...
plot(x = x, y = y, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
#lines(x = x, y = ytrue, col = "grey", lwd = 1.5, main = "")
lines(x = x, y = yforest, col = "red", lwd = 1.5, main = "")
@
\visible<2->{
...
\minipage{0.29\textwidth}
  ...
  \begin{center}
  \begin{tikzpicture}
  \draw[line width=0.01mm, black] (0,0) rectangle (2.6,2.24);
...
{\bf Tree:}
\begin{itemize}
  \item Specify the complete distribution in each subgroup.\\
...
\begin{itemize}
  ...
  \item Stabelization of the model.
\end{itemize}
\end{frame}
...
{\bf Tree:}\\
\vspace{0.1cm}
\hspace{0.7cm} Estimate $\hat{\theta}$ on the subsample of the learning data which \\
\hspace{0.7cm} ends up in the same terminal node as the new observation.
%\item Evaluate final node for new observation
%\item Return corresponding distribution (parameters)

\vspace{0.5cm}

{\bf Forest:}\\
\vspace{0.1cm}
\hspace{0.7cm} Estimate $\hat{\theta}$ on the whole learning data but weighted by \\
\hspace{0.7cm} the number of trees in which a learning observation ends up\\
\hspace{0.7cm} in the same terminal node as the new observation.
...
pal <- rgb(c(0.97, 0.64, 1), c(0.70, 0.83, 1), c(0.30, 0.99, 1))
plot(fig,
     tp_args = list(FUN = identity, width = 18.7, fill = pal[c(1, 3)]), 
     ip_args = list(fill = pal[c(2, 3)], pval = FALSE),
     drop_terminal = TRUE, tnex = 1.5)
@
\end{center}
\end{frame}
...
#FUN <- function(x) {
#                cf <- x$coefficients
#                cf <- matrix(cf, ncol = 1, dimames = list(c(expression(mu), expression(sigma)), ""))
...
#  cf <- x$coefficients
#  cf <- matrix(cf, ncol = 1, dimnames = list(names(cf), ""))
#  c(sprintf("n = %s", x$nobs), "Estimated parameters:", c(c(expression(mu), round(cf[1],3)), 
#                                                          c(expression(sigma), round(cf[2],3))))
#}
@
...
pal <- rgb(c(0.97, 0.64, 1), c(0.70, 0.83, 1), c(0.30, 0.99, 1))
plot(tree, drop = TRUE, tnex = 1.5, #FUN = FUN,
     tp_args = list(fill = pal[c(1, 3)]), 
     ip_args = list(fill = pal[c(2, 3)]))
@
\end{center}
\end{frame}
...
plot(as.constparty(tree), tnex = 1.5,
     tp_args = list(fill = pal[c(1, 3)]), 
     ip_args = list(fill = pal[c(2, 3)]))
@
\end{center}
...
<<prelimminaries2, echo=FALSE, results=hide>>=
set.seed(54)
nobs <- 500
x <- runif(nobs, 0, 1)
mu <- sigma <- ytrue <- numeric(length = nobs)
...
par(mar=c(5.1,4.1,4.1,3.1))
plot(y=odata$y, x=odata$x, ylab = "y", xlab = "x", col = "darkgrey")
#plot(y=odata$y, x=odata$x, xaxt='n', yaxt='n', ann=FALSE, col = "darkgrey")
lines(x = odata$x, y = odata$mu, col = "red", lwd = 1.5, main = "")
lines(x = odata$x, y = odata$mu + odata$sigma, col = "red", lwd = 1.5, lty = 2, main = "")
lines(x = odata$x, y = odata$mu - odata$sigma, col = "red", lwd = 1.5, lty = 2, main = "")
@
\end{center}
\end{frame}
...
<<preliminaries3, echo=FALSE, results=hide>>=
load("~/svn/partykit/pkg/disttree/presentation/simlist20170621.rda")
...
plot_rmse <- function(simlist, type = c("exp", "par")){
  ...
  if(type == "exp"){
    ...
    par(mar=c(5.1,4.1,4.1,3.1))
    plot(x = simlist$x.axis, y = rmse[,"dt.true"], type = "l", col = "forestgreen", ylim = ylim,
         xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
    lines(x = simlist$x.axis, y = rmse[,"dt.obs"], type = "l", lty = 2, col = 'forestgreen')
    lines(x = simlist$x.axis, y = rmse[,"df.true"], type = "l", col = 'red')
    lines(x = simlist$x.axis, y = rmse[,"df.obs"], type = "l", lty = 2, col = 'red')
    lines(x = simlist$x.axis, y = rmse[,"g.true"], type = "l", col = 'blue')
    lines(x = simlist$x.axis, y = rmse[,"g.obs"], type = "l", lty = 2, col = 'blue')
    #legend('left', c("disttree", "distforest", "gamlss"), col = c('forestgreen', 'red','blue'), lty = 1)
    ...
    mtext(text= "disttree", side = 2, col = "darkgreen", las = 1, line = 0.2, padj = 6.5)
    mtext(text= "distforest", side = 2, col = "red", las = 1, line = 0.2, padj = 13)
    mtext(text= "gamlss", side = 2, col = "blue", las = 1, line = 0.3, padj = 16)
    ...
    #add arrows below
    arrows(0.45,0.05,0.3,0.05,lwd=1) ## add arrow
    arrows(0.6,0.05,0.75,0.05,lwd=1) ## add arrow
  ...
  if(type == "par"){
    ...
    ylim <- c(min(rmse), max(rmse))
    par(mar=c(5.1,4.1,4.1,3.1))
    plot(x = simlist$x.axis, y = rmse[,"mu.dt"], type = "l", col = "forestgreen", ylim = ylim,
         xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
    lines(x = simlist$x.axis, y = rmse[,"sigma.dt"], type = "l", lty = 2, col = 'forestgreen')
    lines(x = simlist$x.axis, y = rmse[,"mu.df"], type = "l", col = 'red')
    lines(x = simlist$x.axis, y = rmse[,"sigma.df"], type = "l", lty = 2, col = 'red')
    lines(x = simlist$x.axis, y = rmse[,"mu.g"], type = "l", col = 'blue')
    lines(x = simlist$x.axis, y = rmse[,"sigma.g"], type = "l", lty = 2, col = 'blue')
    #legend('topleft', c("disttree", "distforest", "gamlss"), col = c('forestgreen', 'red','blue'), lty = 1)
    mtext(text= "smooth", side = 1, line = 2, adj = 0)
    mtext(text= "steep", side = 1, line = 2, adj = 1)
    mtext(text= TeX('$\\kappa$'), side = 1, line = 1)
    mtext(text= "RMSE", side = 2, line = 1)
    mtext(text= "disttree", side = 2, col = "darkgreen", las = 1, line = 0.2, padj = -4)
    mtext(text= "distforest", side = 2, col = "red", las = 1, line = 0.2, padj = 8)
    mtext(text= "gamlss", side = 2, col = "blue", las = 1, line = 0.3, padj = 14)
...
plot_ll <- function(simlist){
  ...
  par(mar=c(5.1,4.1,4.1,3.1))
  plot(x = simlist$x.axis, y = ll[,"dt"], type = "l", col = "forestgreen", ylim = ylim,
       xlab = "", ylab = "", main = "disttree vs distforest vs gamlss", xaxt='n', yaxt='n')
  lines(x = simlist$x.axis, y = ll[,"df"], type = "l", col = 'red')
  lines(x = simlist$x.axis, y = ll[,"g"], type = "l", col = 'blue')
  #legend('topright', c("disttree", "distforest", "gamlss"), col = c('forestgreen', 'red','blue'), lty = 1)
  mtext(text= "smooth", side = 1, line = 2, adj = 0)
  mtext(text= "steep", side = 1, line = 2, adj = 1)
  mtext(text= TeX('$\\kappa$'), side = 1, line = 1)
  mtext(text= "Log-Likelihood", side = 2, line = 1)
  mtext(text= "disttree", side = 2, col = "darkgreen", las = 1, line = 0.2, padj = 12)
  mtext(text= "distforest", side = 2, col = "red", las = 1, line = 0.2, padj = -8.5)
  mtext(text= "gamlss", side = 2, col = "blue", las = 1, line = 0.3, padj = -16)
  ...
  #add arrows below
  arrows(0.45,0.05,0.25,0.05,lwd=1) ## add arrow
  arrows(0.6,0.05,0.8,0.05,lwd=1) ## add arrow
...
\nocite{*}
%\nocite{hothorn2006unbiased, zeileis2008model, hothorn2015partykit, stasinopoulos2007generalized, seibold2016model}
{\footnotesize
\bibliography{ref.bib}}
\end{frame}
