...
plot(as.constparty(tree), tnex = 1.4, drop = TRUE,
...
plot(tree, tnex = 1.4, drop = TRUE,
     terminal_panel = node_density(horizontal = TRUE, col = pal["forest"], lwd = 2),
     tp_args = list(fill = paltrees[c(1, 3)]), 
     ip_args = list(fill = paltrees[c(2, 3)]))
@
\end{center}
...
\vspace*{-0.2cm}
\setkeys{Gin}{width=0.74\linewidth}
<<plottree_dens_v, fig=TRUE, echo=FALSE>>=
plot(tree, tnex = 1.4, drop = TRUE,
     terminal_panel = node_density(horizontal = FALSE, col = pal["forest"], lwd = 2),
     tp_args = list(fill = paltrees[c(1, 3)]), 
     ip_args = list(fill = paltrees[c(2, 3)]))
@
\end{center}
...
\begin{tabular}{ll}
\code{distfit}    & Distributional fits (ML, \code{gamlss.family} or custom \code{list}).\\
                  & No covariates. \\
