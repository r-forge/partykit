...
plot1 <- function(learndata, dt=NULL, df=NULL, g=NULL, all.par = FALSE, df.par = FALSE, g.par = FALSE, dt.par = FALSE) 
{
  ...
  if(dt.par){
    ...
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
           ...
           col = c('black','grey',pal["tree"], 'greenyellow'), lty = 1, bty = 'n')
  }
  ...
  if(df.par){
    ...
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')), 
           col = c('black','grey',pal["forest"], 'orange'), lty = 1, bty = 'n')
  }
  ...
  if(g.par){
    ...
    lines(x = sp$x, y = sp$true.mu + sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$true.mu - sp$true.sigma, type = "l", col = 'grey')
    lines(x = sp$x, y = sp$fitted.mu.g, type = "l", col = pal["gamlss"], lwd = 1.5)
    lines(x = sp$x, y = sp$fitted.mu.g + sp$fitted.sigma.g, type = "l", col = 'lightblue')
    lines(x = sp$x, y = sp$fitted.mu.g - sp$fitted.sigma.g, type = "l", col = 'lightblue')
    legend('topleft', c(TeX('$\\mu$'), TeX('$\\mu \\pm \\sigma$'), 
                        TeX('$\\hat{\\mu}$'), TeX('$\\hat{\\mu} \\pm \\hat{\\sigma}$')),  
           col = c('black','grey',pal["gamlss"], 'lightblue'), lty = 1, bty = 'n')
  }
...
FUN <- function (x) 
{
  ...
  c(sprintf("n = %s", x$nobs), "Estimated parameters:", parse(text = paste("mu == ", round(cf[1],3))), 
                                                        parse(text = paste("sigma == ", round(cf[2],3))))
}


# alternative to using FUN:
# change info slot in each terminal node and use FUN = identity
if(FALSE){
info2 <- tree$node$kids[[1]]$info 
info4 <- tree$node$kids[[2]]$kids[[1]]$info 
info5 <- tree$node$kids[[2]]$kids[[2]]$info 

cf <- info2$coefficients
info2 <- c(sprintf("n = %s", info2$nobs), "Estimated parameters:", parse(text = paste("mu == ", round(cf[1],3))), 
                                                          parse(text = paste("sigma == ", round(cf[2],3))))

cf <- info4$coefficients
info4 <- c(sprintf("n = %s", info4$nobs), "Estimated parameters:", parse(text = paste("mu == ", round(cf[1],3))), 
                                                          parse(text = paste("sigma == ", round(cf[2],3))))

cf <- info5$coefficients
info5 <- c(sprintf("n = %s", info5$nobs), "Estimated parameters:", parse(text = paste("sigma == ", round(cf[2],3))), 
                                                          parse(text = paste("mu == ", round(cf[1],3))))

tree$node$kids[[1]]$info <- info2
tree$node$kids[[2]]$kids[[1]]$info <- info4 
tree$node$kids[[2]]$kids[[2]]$info <- info5
...
## plot version using replaced info slots and tree of class 'party'
if(FALSE){
  plot(tree, drop = TRUE, tnex = 1.4,
       tp_args = list(fill = pal2[c(1, 3)], FUN = identity, width = 18), 
       ip_args = list(fill = pal2[c(2, 3)]))
}
@
\end{center}
...
plot_rmse <- function(simlist, type = c("exp", "par")){
  ...
  if(type == "exp"){
    ...
    mtext(text= "disttree", side = 2, col = pal["tree"], las = 1, line = 0.2, padj = 6.5)
  ...
  if(type == "par"){
    ...
    mtext(text= TeX('$\\kappa$'), side = 1, line = 1)
    mtext(text= "RMSE", side = 2, line = 1)
    mtext(text= "disttree", side = 2, col = pal["tree"], las = 1, line = 0.2, padj = -4)
...
plot_ll <- function(simlist){
  ...
  mtext(text= "disttree", side = 2, col = pal["tree"], las = 1, line = 0.2, padj = 12)
  mtext(text= "distforest", side = 2, col = pal["forest"], las = 1, line = 0.2, padj = -8.5)
  mtext(text= "gamlss", side = 2, col = pal["gamlss"], las = 1, line = 0.3, padj = -16)
