...
\textbf{Homogeneity:} Partitioning the data into subgroups aims at allowing for a better model fit in each resulting subset.
...
\textbf{Trade-off:} Complexity in the tree structure vs.\ complexity in the fitted models.\\
...
% \bigskip
% 
% \textbf{Examples of discrepancy measures:} 
% \begin{itemize}
...
\textbf{Examples of possible transformations of the response $Y$:} 
\begin{itemize}
...
\textbf{Possible association tests:} 
...
\textbf{Assess pairwise associations} (Titanic): \ Survived $\sim$ Gender
...
\begin{frame}[fragile]
\frametitle{Split variable selection}
\textbf{Assess pairwise associations} (Titanic): \ Survived $\sim$ Age
...
\begin{frame}[fragile]
\frametitle{Split variable selection}
\textbf{Assess pairwise associations} (Titanic): \ Survived $\sim$ Class
...
\textbf{Unbiased recursive partitioning:} Separation of split variable selection (based on $p$-values) and split point selection (only within the already chosen split variable).

\bigskip

\textbf{Alternative:} Directly compare all available split points in all possible split variables. In order to allow for an unbiased selection within this strategy, it has to be accounted for differences such as a varying number of available split points, different scales or variable types by correcting the measurement considered for comparison accordingly. However, this has often not been included in classic tree algorithms.
\end{frame}


\begin{frame}[fragile]
\frametitle{Split variable selection}
\textbf{Assess pairwise associations} (Wages, CPS1985): \ log(wage) $\sim$ education
...
\begin{frame}[fragile]
\frametitle{Split variable selection}
\textbf{Assess pairwise associations} (Wages, CPS1985): \ log(wage) $\sim$ gender
...
\textbf{Exhaustive search:} For each possible split point evaluate a defined objective function in the resulting subgroups and select the split point that minimizes the objective function, i.e., leads to the highest improvement of the model fit.

\vspace{0.4cm}

\textbf{Alternative:} Two-sample test statistics evaluating the discrepancy between resulting subgroups. 
\end{frame}
...
\textbf{Pre-pruning:} Internal stopping criterium. Stop splitting when there is no significant association to any of the possible split variable.
