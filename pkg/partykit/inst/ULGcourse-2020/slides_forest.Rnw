...
The predicted response $\hat{y}$ 
%or, more generally, the predicted model parameter (vector) $\hat{\theta}$ 
...
\[
\hat{y}(x) =  \argmin_{y} \sum_{i=1}^n w_i(x) \cdot f(y; y_i)
\]
...
\only<5->{
%\vspace{-3cm}
{\bf Note:} In model-based trees and forests a prediction is made not only for the response but for a model parameter (vector) $\theta$. 
This more general approach holds for classic random forests averaging over mean values of tree nodes by setting $f(\theta, y_i) = (\theta - y_i)^2$\\
