...
{\bf Simple tree model:} Obtain predictions for the response $y$ by averaging over observations in the same terminal node $\mathcal{B}_x$: 
$$
...
{\bf Model-based tree:} Obtain predictions for model parameter(s) $\theta$ by optimizing an objective function $f$ for all observations in the same terminal node $\mathcal{B}_x$:
$$
...
{\bf Note:} The model-based approach allows for fitting complex models but also includes the specific case of averaging over all observations which corresponds to minimizing the residual sum of squares $\sum (Y - \hat{Y})^2$.
...
that minimizes the weighted sum of the objective function $f$ evaluated for learning data $\{y_i\}_{i=1,\ldots,n}$:
...

{
  ...
  plot(x = predwage[,"age"], y = predwage[,"predicted log(wage)"], type = 'l',
       ylab = "predicted log(wage)", xlab = "age", main = "2 trees")
  
  cf <- cforest(f, data = CPS1985, ntree = 4)
  predwage <- cbind(c((18:64)/1), predict(cf, newdata = nd))
  colnames(predwage) <- c("age", "predicted log(wage)")
  plot(x = predwage[,"age"], y = predwage[,"predicted log(wage)"], type = 'l',
       ylab = "predicted log(wage)", xlab = "age", main = "4 trees")
  
  cf <- cforest(f, data = CPS1985, ntree = 20)
  predwage <- cbind(c((18:64)/1), predict(cf, newdata = nd))
  colnames(predwage) <- c("age", "predicted log(wage)")
  plot(x = predwage[,"age"], y = predwage[,"predicted log(wage)"], type = 'l',
       ylab = "predicted log(wage)", xlab = "age", main = "20 trees")
  
  cf <- cforest(f, data = CPS1985, ntree = 200)
  predwage <- cbind(c((18:64)/1), predict(cf, newdata = nd))
  colnames(predwage) <- c("age", "predicted log(wage)")
  plot(x = predwage[,"age"], y = predwage[,"predicted log(wage)"], type = 'l',
       ylab = "predicted log(wage)", xlab = "age", main = "200 trees")
}
@
\end{frame}
...
\item R package providing a faster implementation of classic random forests.
\end{itemize}

<<call_ranger, eval=FALSE, echo=TRUE>>=
...
For a ranger object:
<<call_predict_rf, eval=FALSE, echo=TRUE>>=
predict(rf, data = testdata)
@

\bigskip

For a cforest object:
<<call_predict_cf, eval=FALSE, echo=TRUE>>=
predict(cf, newdata = testdata)
@
