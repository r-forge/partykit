...
{\bf Idea:} Asses the influence of a covariate on the model by evaluating the change in performance of the model (mean decrease in accuracy) after permutating the selected covariate. 
