...
\visible<4->{
{\bf Model-based tree:} Obtain the predicted model parameter(s) $\hat{\theta}$ for an observed covariate~$x$ by optimizing a loss function for all observations in the same terminal node $\mathcal{B}_x$:
$$
\hat{\theta}= \argmin_{\theta} \sum_{i: y_i \in \mathcal{B}_x} loss(\theta; y_i)
