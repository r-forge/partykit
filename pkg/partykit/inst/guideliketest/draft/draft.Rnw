...
disadvantages of the different testing strategies can be elaborated in more detail and 
focus can be given to their power in terms of selecting the correct split variable.
...
Moreover, the intercept $\beta_0$ and/or the slope parameter $\beta_1$ of this linear equation
...
\end{enumerate}
%
The varying coefficient(s) $\beta_j$, $j\in\{0,1\}$, are binary taking either the positive or
