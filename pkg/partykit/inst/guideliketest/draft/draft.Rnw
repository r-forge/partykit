...
the best split variable. In particular, the approach of the GUIDE algorithm is
...
model-based unbiased recursive partitioning is presented in Section~\ref{sec:unify}.
...
response can be mapped to a unity vector by an indicator function $h(Y) = (0,\ldots,1,\ldots,0)^{\top}$.
...
the function $h$ can be a model-based transformation returning the corresponding model scores by 
setting $h(Y) = s(Y,X,\hat{\theta})$
...
$\boldsymbol{y} = (y_1,\ldots,y_n)^\top$ and 
regressor(s) $\boldsymbol{x} = (x_1,\ldots,x_n)^\top$ and 
a $k$-dimensional parameter vector $\boldsymbol{\theta} = (\theta_1,\ldots,\theta_k)^\top$ the 
score function $s$ returns a $n \times k$-matrix.
In that way a score value indicating how well the model fits the data is given separately
for each tuple of observation and parameter element $(y_i, x_i, \theta_l)$ with $i \in \{1,\dots,n\}$
and $l \in \{1,\ldots,k\}$. 
To estimate $\boldsymbol{\theta}$ many different M-type estimators can be applied including 
maximum likelihood or ordinary least squares. Therefore, these objective functions
are also possible options to obtain a model-based discrepancy measure in the tree algorithm.
...
For a categorical variable all possible splits are evaluated to find the best one while for $Z$ being 
a numeric variable a maximally selected split point search is employed. Hence, the number 
of possible breakpoints is not fixed unlike in the CTree algorithm which can make this operation 
...
For the statistical test applied to select a split variable in each node some type of 
residuals from the models is required such as for example 
...
in Section~\ref{sec:simulation} all three algorithms are applied using pre-pruning in order to put 
...
Since all three algorithms follow the same basic structure their approaches can be presented together
...
\subsection{Switches}
As explained before, the selection of a split variable is based on a statistical test 
investigating the dependency of the response $Y$ on each possible split variables $Z$.
The three following switches all refer to the way how the given data is prepared/transformed
before employing such a statistical test.
\begin{itemize}
\item  Transformations of Y:\\
A model-based transformation of $Y$ is applied to obtain a discrepancy measure giving 
information about the goodness of fit of the model. This can be residuals $r(Y,X,\hat{\theta})$
as in the GUIDE algorithm or scores $s(Y,X,\hat{\theta})$ as in the MOB algorithm or, 
more generally, a function $h(Y,X)$ as in the CTree algorithm including both of the foregoing cases.
...
hence the full model score matrix $S$.
...
\textit{Note:}
Apart from these three switches, the algorithms differ in their approximation of the underlying
null distribution of the test statistic. While the permutation tests in the CTree algorithm
work in a conditional framework the MOB and GUIDE algorithms both apply an unconditional approach. 
...
A very basic form of a discrepancy measure can be obtained by simply calculating deviations of the 
...
For a model-based approach residuals can be considered such as in the residual sum of squares (RSS)
$$
...
significance level is considered for all statistical tests, including the $\chi^2$-test in 
...
and \ref{sec:binarization}. Then the testing strategies are evaluated on the more
...
while GUIDE\_full performs reasonably well in this scenario. This is due to the fact that
the GUIDE algorithm only includes residuals, i.e. only the scores regarding the intercept,
such that changes in the slope parameter are not detected unless the corresponding scores 
are als included as in the GUIDE\_full algorithm.
...
both coefficients varying (right panel). The performances of four algorithms are compared: 
...
(CTree\_bin, MOB\_bin). Moreover, they are compared to the adapted GUIDE version which includes 
all available scores (GUIDE\_full).
...
To investigate the effect of categorizing the possible splitting variables the two algorithms 
CTree and MOB are applied, once in their original version without a categorization and once 
in an adapted version with a categorization of the possible split variables (CTree\_cat and MOB\_cat). 
Moreover, they are also compared to GUIDE\_full which includes all available scores.
The corresponding setting of the switches in the unifying framework is listed in 
Table~\ref{tab:switches_cat}.
...
($Z_1$ and $Z_2$), both with the same split point $\xi$ as illustrated in Figure~\ref{fig:dgp_tree}.

Figure~\ref{fig:tree_ari} shows that based on the resulting ARI values CTree and MOB perform 
almost equally well for the true split points $0$, $0.2$ and $0.5$ while GUIDE\_full performs
...
From comparing the effects of the three switches three main conclusions can be drawn: 
...
In the simulation study presented in Section~\ref{sec:simulation} a significance level is 
included in all of the algorithms such that a split is only performed if the lowest $p$-value
is smaller than this significance level. This strategy, usually referred to as pre-pruning, aims at 
avoiding overfitting.
However, ignoring this significance level within the tree algorithms yields the same results 
which is shown in Figure~\ref{fig:scores_stump_wosign} where the proportion of cases 
in which a split is performed in the true splitting variable, but not necessarily with a $p$-value 
smaller than the significance level, is illustrated.
...
is selected over increasing effect size $\delta$ for $\sigma = 1$, $\xi=0$, 250 observations and 
...
To show that similar results are obtained without any significance level but with the application
of post-pruning (building very large trees and employing cost-complexity pruning as in the CART algorithm) the models built in the second simulation scenario are built once with a pre-pruning and once
with a post-pruning approach. The results are compared based on the Adjusted Rand Index in 
Figure~\ref{fig:pruning_ari}.
...
@
\caption{\label{fig:pruning_nrsubgr}Number of terminal nodes over increasing effect size $\delta$ for
250 observations and 100 data sets per step.
Each panel represents a different value of the split point $\xi$ ($0$, $0.2$, $0.5$ and 
$0.8$ from left to right). The performances of four testing strategies are compared: CTree,
