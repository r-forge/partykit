...
Comparing the two panels suggests that unless one of the breaks used for binning 
...
unless the breaks used for the categorization are very close to the true split points.
...
of a continuous split variable. Therefore, categorizing possible split variables can have
advantages in practice, however, it has to be considered that in general
important effects might be neglected.

Both of the transformation functions $h$ and $g$ (applied to the response~$Y$ or a split 
variable~$Z$ respectively) can also be multivariate. For this case, the corresponding
statistical tests are available, in particular the \textsf{R} package \textbf{coin}
which is employed for the implementation of MOB and CTree provides 
the required toolbox for multivariate dependency tests, including a $\chi^2$~test. 
...
the parameter vector $\beta$ is shifted 
%to $\beta + \Delta$ 
by $\Delta = (0, \Delta_1)^{\top}$.
...
perform best. However, the cost in terms of power that comes with including all scores, in 
particular with testing more than one parameter, is negligibly small. 

Also in more general models residual-based and score-based procedures are expected to perform
equally well if all parameters are highly correlated. On the other hand, if parameters are 
orthogonal changes that are not connected to the mean might be missed when only considering
residuals. This can be prevented by including scores which are a standard measurement of goodness 
of fit that can be easily employed for all model types. Moreover, the score function $s$ can 
simply be seen as a  transformation to a different space in order to allow for a well structured
analysis of dependencies, regardless of the type of model and therefore applicable in a
wide variety of situations.
In order to benefit from the advantages of using scores 
some already existing models have been adapted, e.g., in \cite{Hothorn+Zeileis:2017} 
and \cite{Schlosser+Hothorn+Stauffer:2019}. Similarly, variations of the GUIDE algorithm
using different types of residuals are provided, e.g., in \citealp{Loh+Zheng:2013}, 
\citealp{Chaudhuri+Loh:2002} and \citealp{Loh+He+Man:2015}.
But even though it also depends on the type of the applied residuals whether parameter changes 
can be detected successfully, a one-dimensional measurement of goodness of fit such as residuals
provides less power than full model scores. 
