...
in the first scenario (stump with 1 split) the following criteria are considered:
...
@
\caption{\label{fig:scores_stump}Proportion of replications where the true split variable $Z_1$ is
...
level $\alpha=0.05$. Therefore, the proportion of replications in which the true split variable $Z_1$
...
corresponding scores are also included as in GUIDE+scores. This disadvantage of using 
only scores has already been elaborated in the literature of structural change
analysis \citep[see e.g.][]{Ploberger+Kramer:1992} and is also discussed 
...
when comparing GUIDE and GUIDE+scores. Moreover, from the fact that these two version are both
...
case where the intercept and slope parameter are both varying. In the left panel the true split point is in the center, i.e., $\xi=0$ and an effect size of $\delta = 0.3$ is considered. 
This choice has been made based on the results shown in Figure~\ref{fig:scores_stump} 
in order to make the differences particularly well observable.
In the right panel the true split point is close to the upper end of the considered interval
($\xi=0.8$) requiring a higher effect size $\delta = 1$ in order to visualize results properly.
(Additional evaluations for increasing $\delta$ as in Section~\ref{sec:scores} 
...
(100 replications of 250 observations per step).
...
The performances of four testing strategies are compared: CTree, MOB, GUIDE, GUIDE+scores. 
The dashed lines represent the results from applying pre-pruning and the solid lines 
the results from applying post-pruning.}
\end{figure}
