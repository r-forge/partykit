...
One specific feature of linear models is the orthogonality of the conditional expectation
$\mathbb{E}(Y|X)$ and the regressor $X$ which affects the capability of tests using only residuals
to capture shifts in the parameters. In particular, \cite{Ploberger+Kramer:1992} have shown that
residual-based tests are insensitive to parameter changes orthogonal to the mean regressor.
...
Testing procedures purely based on residuals perform best in situations with shifts only in the
intercept, however, the costs in terms of power that come with including all scores are 
negligibly small. 
...
In general residual-based and score-based procedures are expected to perform equally well if all
parameters are highly correlated. On the other hand, if parameters are orthogonal changes that are 
not connected to the mean might be missed when only considering residuals. This can be avoided by
including scores which are a standard measurement of goodness of fit that can be easily employed.
