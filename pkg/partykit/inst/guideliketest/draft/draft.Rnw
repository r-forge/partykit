...
goodness-of-fit measure (residuals vs.\ model scores), binarization of residuals/scores
at zero, binning of covariates. This shows that specifically the goodness-of-fit measure 
is crucial for the power of the procedures, with model scores without binarization performing
...
All these approaches have shown to work well for various situations, however, the relative
...
(2)~Binarization of these residuals or scores.
...
the performance of testing strategies with overall using model scores leading to clearly better
results than employing residuals only. Moreover, the original
values of the goodness-of-fit measure are preferred over binarized versions of them.
...
\begin{enumerate}
  ...
  \item Among all available covariates one is selected as the split variable,
...
also be based on a model for a response along with potential covariates.
...
\begin{enumerate}
  ...
  \item The dependency or association of this goodness-of-fit matrix with
        each possible split variable $Z_j$, $j \in \{1, \dots, m\}$, is assessed
        using some suitable test statistic. The corresponding $p$-values allow for
        a comparison of all $m$ covariates on a standardized or unified scale and in
        that way for an unbiased split variable selection.
  \item The covariate corresponding to the smallest $p$-value -- and thus the
...
(gradient contributions). Analogously, the log-likelihood and corresponding score function could be used.
...
response $h(Y)$ and each possible transformed split variable $g(Z_j)$ ($j = 1, \dots, m$).
...
$h(Y) = (Y, (Y-\bar{Y})^2)^{\top}$. 
If a parametric model is fitted to the response $Y$ with regressor variable(s)~$X$,
...
\citep[see][for a hands-on introduction]{Hothorn+Hornik+VanDeWiel:2006}.

If both variables $Y$ and $Z$ are numeric the default independence test corresponds to a Pearson 
correlation test. For one numeric and one categorical variable essentially a 1-way ANOVA
...
relies on classical unconditional inference. For more details see \cite{Zeileis+Hornik:2007}.
...
constant fits such as simple mean response $\hat{\beta} = \bar{y} = \frac{1}{n}\sum_{i=1,\ldots,n}^n y_i$.
...
(1)~The residuals $r(Y,X,\hat{\beta})$ are binarized at zero, yielding an indicator for positive vs.\ negative residuals.
...
Subsequently, a $\chi^2$~test of independence is performed for the binarized residuals and
...
not just the four bins that were constructed for the $\chi^2$~test.
...
\begin{itemize}
  \item \emph{Goodness-of-fit measure:}\\
        ...
        model for the response $Y$ and possible covariates $X$ can be considered. Either residuals
  ...
  \item \emph{Binarization of residuals/scores:}\\
        Rather than testing independence between the split variables and the residuals/scores
	themselves, it is possible	
	to binarize residuals/scores at $0$ so that only their signs are assessed (as proposed for GUIDE).
...
the model scores without binarization and assess their association with the untransformed split
...
binarization and maximally selects a score statistic over all potential split points in the
split variable. GUIDE employs the binarized residuals and assesses their association with
...
in the GUIDE approach, a 1-way ANOVA can be used for assessing the association of the residuals
(without binarization) with the categorized split variable. Or alternatively, a multivariate
1-way ANOVA can be used for the non-binarized scores as opposed to the residuals etc.
...
$$
with response variable $Y$, regressor variable $X$, and error term $\epsilon$. This model is
fitted, as usual, by ordinary least squares (OLS) to the observations in each notation,
yielding the parameter estimates $\boldsymbol{\hat{\beta}} = (\hat{\beta_0}, \hat{\beta_1})^\top$.
...
\frac{\partial \ell}{\partial \boldsymbol{\beta}}(y_i, x_i, \beta_0, \beta_1) = 
-2 \cdot r(y_i,x_i, \beta_0, \beta_1) \cdot (1,x_i)^\top
$$
Thus, up to a constant scaling factor of $-2$ (that could also be omitted) the first component
of the scores is in fact the residual. However, it is complemented by a second component
...
not only $Z_1$, and hence $\beta_0$ and $\beta_1$ are functions $\beta_i(Z_1,Z_2)$, $i=1,2$.}
\end{table}
...
changes ($\beta_j$ with $j\in\{0,1\}$) switches between two values at the split point $\xi$:
%
\begin{align*}
\beta_{i}(Z_1) = \begin{cases}
-\delta \cdot (-1)^{j}  \quad \text{if } Z_1 \leq \xi \\
+\delta \cdot (-1)^{j}  \quad \text{if } Z_1 > \xi
\end{cases}
\end{align*}
%
...
For the second simulation scenario (``tree'' scenario) the same basic setting with the same 
variables as in the ``stump'' scenario is applied (see Table~\ref{tab:variables}).
However, not only $Z_1$ but also $Z_2$ is used as a true split variable. Therefore, an additional split
...
probability (i.e., $Z_1$ being significant at 5\% level and having the
lowest $p$-value among all split variables). Section~\ref{sec:scores} begins by
highlighting the importance of using full model scores vs.\ residuals only
before Section~\ref{sec:3way} considers all building blocks (scores vs.\ residuals,
binarization of these, and categorization of the split variable).
...
scores. In the literature on structural change tests there is evidence
...
binarized scores rather than residuals.

In Figure~\ref{fig:scores_stump} the performances of the testing strategies are 
represented by the corresponding selection probability, hence, a significance level 
is incorporated. To show that in general the drawn conclusions do not change if no 
significance level is included, a different illustration of the exact same evaluation 
is provided in Appendix~\ref{app:significance} where the performances are compared based 
on the smallest $p$-values, regardless of whether they are beneath any significance level.

Moreover, as defined by the true tree structure used in the data generating process, the
parameter functions $\beta_i(Z_1)$, $i=1,2$, are step functions which is particularly 
advantageous for the maximum selection in the test statistic applied by MOB while the linear 
selection in CTree is preferable for detecting monotonous dependencies. To show that employing 
a continuous parameter function instead of a step function only has an impact on whether CTree or
MOB performs better the exact same simulation setting as used for Figure~\ref{fig:scores_stump} 
but with continuously changing parameters has been evaluated and is illustrated in 
Appendix~\ref{app:contbeta}.
...
Figure~\ref{fig:scores_stump} shows that overall CTree and MOB (both using full 
model scores) perform clearly better than the original GUIDE version (using residuals 
only) as they accomplish to select the true split variable $Z_1$ in a higher proportion
of replications. Even though the advantage of CTree and MOB evens out in certain situations
such as for a high effect size (see, e.g., for the median $0$ and varying $\beta_0$), 
GUIDE is never ahead of them in any of the considered settings. Moreover, the positive 
effect of including all available scores can be observed particularly well in the top middle 
panel where GUIDE+scores detects a shift in the slope parameter $\beta_1$ as opposed to GUIDE. 
%The fact that residual-based methods fail to detect changes that do not effect the
%expected value has already been elaborated in the literature of structural change
%analysis \citep[see, e.g.,][]{Ploberger+Kraemer:1992} and is also discussed in 
%Section~\ref{sec:discussion}. 
Furthermore, this advantage of including full models scores 
clearly outweighs the cost involved which is displayed by the small gain of GUIDE compared 
to GUIDE+scores in case of a fixed slope parameter (left panels).

For a central split point ($\xi=0$ corresponding to the median, upper panels) all strategies 
perform similarly well as long as the intercept varies (left and right panel) with CTree and 
MOB being only slightly ahead. However, for the split point at the $90$\% quantile $0.8$ 
(lower panels) the performance of all of the applied strategies decreases. 
...
As discussed before, this advantage of MOB over CTree is mainly due to the abrupt shift in the 
model parameters $\beta_0$ and $\beta_1$ and turns into an advantage of CTree over MOB for 
continuously changing parameters (see Appendix~\ref{app:contbeta}).
From the fact that GUIDE and GUIDE+scores are both performing worse than MOB and CTree it 
can be deduced that categorizing split variables and binarizing scores has a negative effect 
on the ability to detect the right split variable. To elaborate these effects more in detail 
the results for all possible combinations of the building blocks are discussed in a full 
factorial 3-way analysis in the following section.


\subsection{``Stump'' scenario: Full factorial analysis of building blocks}\label{sec:3way}
To investigate the effects of each of the building blocks separately the most general 
case of the ``stump'' scenario where the intercept and 
slope parameter are both varying is considered. In the underlying evaluation all 
possible combinations of the building blocks have been included. The different levels of 
each of the three building blocks are listed in Table~\ref{tab:levels} where 
$h$ and $g$ refer to the transformation functions applied to the response $Y$ or
a split variable $Z$ respectively, both as described in Section~\ref{sec:ctree}. 
(See Table~\ref{tab:combinations_appendix} in Appendix~\ref{app:combinations} 
for a more detailed overview of all 12 combinations.)
If quartiles are used as breakpoints to categorize a split variable the employed
function $g$ maps to a four-dimensional vector of zeros except for one value $1$
indication the bin to which an observation is assigned to. For a maximum selection
of a split variable $g$ maps to a vector containing only zeros until a certain
changepoint from which on the values are replace by $1$. This breakpoint indicates
for example a crossed quantile. In linear selection $g$ is simply the identity function.
...
\hline
Residuals vs.\ scores        & \textit{residuals}  & $r(Y,X,\hat{\beta})$\\
\quad ($h$ transformation)   & \textit{scores}     & $s(Y,X,\hat{\beta})$\\
\hline
Binarization                 & \textit{yes}        & $\mathds{1}_{[0,\infty)}(h)$\\
\quad (of $h$)               & \textit{no}         & $h$ without binarization\\
\hline
Categorization               & \textit{cat}        & $g(Z)=(0,1,0,0)$\\
\quad ($g$ transformation)   &                     & \quad indicating the assigned bin\\
                             & \textit{max}        & $g(Z)=(0,\ldots,0,1,\ldots,1)$\\
                             &                     & \quad indicating the crossed quantile\\
                             & \textit{lin}        & $g(Z)=Z$\\
\hline
\end{tabular}
...
Based on the results displayed in Figure~\ref{fig:3way} it can be stated that binarizing 
residuals/scores decreases the performance as it leads to higher $p$-values corresponding 
to the true split variable $Z_1$ and thus to lower power in the presented settings. 
For a central true split point ($\xi=0$, left panel) this effect remains at an almost constant 
level regardless of whether possible split variables are categorized or not. However, in case 
of the true split point at the $90$\% quantile $0.8$ (right panel), categorizing split 
variables increases the $p$-value of $Z_1$ even more. 
Here, the maximum selection performs best while the linear selection shows its disadvantage in 
detecting abrupt changes. Overall, it depends on the situation whether a linear or a maximum
selection of a split variable is advantageous. But unless one of the breakpoints used for binning 
possible split variables is close to the true split point (as for the median $0$, left panel),
this categorization weakens the performance of a testing strategy. 
While the effect of both of these transformations of the data (binarization of 
residuals/scores and categorization of split variables) can be observed
separately in Figure~\ref{fig:3way}, combining them increases the negative impact 
remarkably.
...
Note that the choice of the two specific settings 
(left panel: true split point at the median $0$,  effect size $0.3$; 
right panel: true split point at the $90$\% quantile $0.8$ requiring a higher effect 
size $1$)
has been made based on the results shown in Figure~\ref{fig:scores_stump} in order to make 
the differences particularly well observable. (Additional evaluations for increasing $\delta$ 
as in Section~\ref{sec:scores} can be found in Appendix~\ref{app:increasingdelta}.)
...
xyplot(pval_z1 ~ cat | xi, 
       ...
       strip = strip.custom(factor.levels = c(expression(xi==0~"(50%)"~" and"~delta==0.3), 
                                   ...
                                   bg = "gray90"),
       key = list(text = list(c("Goodness of fit:", "  residuals", "  scores", "Binarization:", "  yes", "  no")),
                  columns = 2,
...
\textit{binarization}, and \textit{categorization} in the ``stump'' scenario with the intercept
...
Figure~\ref{fig:dgp_tree}.
The results presented in the top row of Figure~\ref{fig:tree_all} have been obtained applying pre-pruning.
Therefore, based on the corresponding ARI values conclusion can again be drawn on the power of the testing strategies 
as significance tests. CTree and MOB perform almost equally well for the true split points $0$, $0.2$ and $0.5$ 
($50$\%, $60$\%, and $75$\% quantile). GUIDE+scores performs slightly worse but it still achieves an ARI above 
$0.5$ for the highest considered effect size of $\delta = 1$. 
However, the original version of GUIDE is clearly behind which is mainly due to the fact that it doesn't detect 
the first split in $Z_2$ corresponding to a changing slope parameter as it only considers residuals in its testing 
strategy. 
For a split point farther away from the center such as the $90$\% quantile $0.8$ MOB 
outperforms all other methods and both GUIDE versions fail to find the correct tree structure.
Hence, the insights provided by the top row of Figure~\ref{fig:tree_all} conform with the conclusions
drawn from the ``stump'' scenario: Including all available scores in the testing strategy leads to a better performance 
while binarizing scores and categorizing split variables both do not cause any improvement but in many situations even 
have a negative effect on the power of a testing strategy. 
...
To elaborate the impact of the choice of a pruning strategy the presented tree models
are built again in the exact same simulation setting but employing post-pruning (i.e., building very 
large trees without any significance level and applying cost-complexity pruning as in the CART 
algorithm \citep{Breiman+Friedman+Stone:1984}). 
The corresponding ARI values are illustrated in the bottom row of Figure~\ref{fig:tree_all}
allowing for a comparison to those obtained with pre-pruning (top row).
...
given by the tests for pre-pruning. As it can be seen in Figure~\ref{fig:tree_all}
in most cases the results of these two testing strategies do not vary significantly over 
a change of the applied pruning strategy.
On the contrary, overall the tests in GUIDE have less power. For that reason, the pruning
strategy should not be based on their results which argues for the application of post-pruning.
This is also supported by the results in Figure~\ref{fig:tree_all} showing that post-pruning improves
the performance of both GUIDE versions. Particularly if the true split point is close to a binning 
point (such that categorizing split variables has no negative effect) the post-pruning strategy 
compensates the lack of power of GUIDE+scores compared to CTree and MOB almost completely (bottom 
row, first and third panel).
In conclusion, the choice of a pruning strategy should be based on knowledge about the 
power of the applied testing strategy. With a well working significance test additional expensive
calculations such as needed for post-pruning may be avoided being one of the main advantages of 
pre-pruning. However, deficiencies of a testing strategy in terms of power can be at least partly 
evened out by employing a post-pruning strategy.
...
\cite{Ploberger+Kraemer:1992} have shown that residual-based tests can detect a change in the parameters
...
(see, e.g., \citealp{Loh+Zheng:2013}, \citealp{Chaudhuri+Loh:2002} 
and \citealp{Loh+He+Man:2015})
...
hardly be made. However, the results from previous literature such as \cite{Ploberger+Kraemer:1992} and
...
and GUIDE. The presented details have been extracted from the original papers 
...
\subsection{CTree}
To measure the association of a response $Y$ and each possible split variable $Z$ the
CTree algorithm applies a linear test statistic $T$ which is excerpted from Section~3.1.
(``Variable selection and stopping criteria'') of the original paper \citep{Hothorn+Hornik+Zeileis:2006} and is of the following form:
$$
T(Y, Z, w) = \text{vec}\left( \sum_{i=1}^n w_i g(Z_{i})h(Y_i, (Y_1,\ldots,Y_n))^{\top}\right)
$$
where $w$ are optional weights, $g: \mathcal{Z} \rightarrow \mathbb{R}^{p}$ is a nonrandom transformation of the split variable $Z$ and the influence function $h: \mathcal{Y} \times \mathcal{Y}^n \rightarrow \mathbb{R}^q$ depends on the responses $(Y_1, \ldots, Y_n)$ in a permutation symmetric way. Moreover, by applying the ``vec'' operator the resulting $p~\times~q$ matrix is converted into a $p q$ column vector.
Following \cite{Strasser+Weber:1999}
the conditional expectation $\mu$ and covariance $\Sigma$ of a linear test statistic can 
be calculated and used to standardize an observed linear test statistic $t$ within a 
function $c$ mapping into the real line.
For example, the maximum of the absolute values of the standardized linear statistic
$$
c_{\max}(t,\mu,\Sigma) = \max_{k=1,\ldots,pq} \left| \frac{(t-\mu)}{\sqrt{(\Sigma)_{kk}}} \right|
$$
or a quadratic form
$$
c_{\text{quad}}(t, \mu, \Sigma) = (t - \mu)\Sigma^{+} (t - \mu)^{\top}
$$
can be considered where $\Sigma^{+}$ is the Moore-Penrose inverse of $\Sigma$.
Since the asymptotic conditional distribution of a linear test statistic $T$ is a 
multivariate normal with parameters $\mu$ and $\Sigma$ \citep{Strasser+Weber:1999}, 
...
\subsection{MOB}
The MOB algorithm employs an empirical fluctuation process $W$ to measure deviations of
the model scores $s$ from zero. As described in detail in Section~3.2. (``Testing for 
parameter instability'') of the original paper \citep{Zeileis+Hothorn+Hornik:2008}
this process is of the following form:
$$
W(t) = \hat{J}^{-1/2} n^{-1/2} \sum_{i=1}^{\lfloor nt \rfloor} s_{\sigma(Z_{i})} \qquad (0 \leq t \leq 1)
$$
where the model scores $s$ are sorted by the split variable Z by including the ordering permutation $\sigma(Z_{i})$. 
To scale this partial sum process an estimate $\hat{J}$ of the covariance matrix $\text{cov}(s(Y,X,\hat{\beta}))$ is included.
Following \cite{Zeileis+Hornik:2007} $W(t)$ converges to a Brownian bridge $W^0$ under the null
hypothesis of parameter stability. To obtain a test statistic a scalar functional $\lambda(\cdot)$ capturing the fluctuation in the empirical process can be applied and the corresponding asymptotic distribution of $\lambda(W(t))$ can be obtained by employing the same functional to the limit process, i.e., $\lambda(W^0)$. 
For more details, particularly on calculating the corresponding $p$-values see Section~3.2. of the original paper \citep{Zeileis+Hothorn+Hornik:2008}.

\subsection{GUIDE}
The test statistic of the $\chi^2$~test as applied in the GUIDE algorithm is
$$
X^2 = \frac{(O_{11} - E_{11})^2}{E_{11}} + \ldots + \frac{(O_{11} - E_{11})^2}{E_{11}}
$$
where $O_{ij}$ are the observed frequencies of observations in a certain combination of binarized residuals ($i=1,2$) and categorized split variables ($j = 1,\ldots,4$). $E_{ij}$ are the corresponding expected frequencies under independence $E_{ij} = \frac{O_{i \cdot} \cdot O_{\cdot j}}{n}$. Under the null hypothesis of independence the asymptotic distribution of $X^2$ is a $\chi^2$~distribution allowing for a straight-forward calculation of $p$-values.
If full model scores are used instead of residuals a sum of the test statistic over the number of distribution parameters is considered such that each 
summand corresponds to one column of the score matrix.
...
``+res'' refers to using only residuals (instead of all scores),
``+scores'' to using all scores (instead of only residuals),
``+cat'' to applying a categorization of split variables,
``+bin'' to applying a binarization of scores/residuals.
...
MOB+bin+res     & residuals          & bin          & max \\
CTree+bin+res   & residuals          & bin          & lin \\
MOB+cat+res    & residuals          & lin          & cat \\
MOB+res          & residuals          & lin          & max \\
CTree+res        & residuals          & lin          & lin \\
GUIDE+scores     & scores             & bin          & cat \\
MOB+bin          & scores             & bin          & max \\
CTree+bin        & scores             & bin          & lin \\
MOB+cat          & scores             & lin          & cat \\
MOB               & scores             & lin          & max \\
CTree             & scores             & lin          & lin \\
\hline
\end{tabular}
\caption{Testing strategies applied (representing the 12 combinations
of building blocks).
...
subdata$test <- factor(subdata$test, 
                       levels = c("ctree", "ctree_max", "mfluc",
                                  "guide_sum_12", "guide_sum_1_cor"),
                       labels = c("CTree", "CTree+max", "MOB",
                                  "GUIDE+scores", "GUIDE"))
...
The testing strategies CTree, CTree+max, MOB, GUIDE, and GUIDE+scores are evaluated 
...
on the number of replications in which the true split variable $Z_1$ is detected with a $p$-value
...
In Figure~\ref{fig:scores_stump_wosign} the proportion of replications 
...
(denoted by CTree+max). 
Looking at the results presented in Figure~\ref{fig:stump_contbeta} it can be observed that in this setting the 
original version of CTree is ahead while CTree+max and MOB perform almost equally well. Hence, the comparison of 
Figures \ref{fig:scores_stump} and \ref{fig:stump_contbeta} points out that it clearly depends on the type of 
effect whether the maximal selection (MOB, CTree+max) or the linear selection (CTree) is advantageous.
...
scores at $0$ (CTree+bin, MOB+bin). Moreover, they are compared to the adapted GUIDE version 
...
of replications where the split variable was selected correctly.
...
Selection probability of the true split variable $Z_1$ for
testing strategies CTree, CTree+bin, MOB, MOB+bin, and GUIDE+scores in the ``stump''
scenario with both parameter varying. 
Probabilities are estimated over increasing effect size $\delta$
with 100 replications of 250 observations per step. 
...
lead to a similar proportion of replications where the true splitting variable is detected
