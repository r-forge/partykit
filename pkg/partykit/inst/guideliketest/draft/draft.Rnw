...
not only $Z_1$, and hence $\beta_0$ and $\beta_1$ are functions $\beta_j(Z_1,Z_2)$, $j=0,1$.}
\end{table}
...
parameter functions $\beta_j(Z_1)$, $j=0,1$, are step functions which is particularly 
...
can be deduced that categorizing split variables and binarizing scores also decreases the
ability to detect the right split variable. To elaborate these effects more in detail 
...
considered. In the underlying evaluation all possible combinations of the building blocks 
...
indication the bin to which an observation is assigned. For a maximum selection
of a split variable $g$ maps to a vector containing only zeros until a certain
changepoint from which on the values are replaced by $1$. This breakpoint indicates
for example a crossed quantile. In a linear selection strategy $g$ is simply the identity function.
...
detecting abrupt changes, but overall it depends on the situation whether a linear or a maximum
selection of a split variable is advantageous. 
Comparing the two panels suggests that unless one of the breakpoints used for binning 
possible split variables is close to the true split point (as for the median $0$, left panel), 
a categorization weakens the performance of a testing strategy. 
Moreover, while the effect of both of these transformations of the data (binarization of 
...
For MOB and CTree it has been shown that their tests work very well as significance tests.
Therefore, it can be recommended to take advantage of this power and use the information
provided by the tests for pre-pruning. As it can be seen in Figure~\ref{fig:tree_all}
...
scores clearly outweigh the price of testing all parameters.
...
$$
where $w$ are optional weights, $g: \mathcal{Z} \rightarrow \mathbb{R}^{p}$ is a nonrandom transformation of the split variable $Z$ and the influence function $h: \mathcal{Y} \times \mathcal{Y}^n \rightarrow \mathbb{R}^q$ depends on the response $Y = (Y_1, \ldots, Y_n)$ in a permutation symmetric way. Moreover, by applying the ``vec'' operator the resulting $p~\times~q$ matrix is converted into a $p q$ column vector.
...
\hline
GUIDE             & residuals          & yes          & cat \\
MOB+bin+res       & residuals          & yes          & max \\
CTree+bin+res     & residuals          & yes          & lin \\
MOB+cat+res       & residuals          & no           & cat \\
MOB+res           & residuals          & no           & max \\
CTree+res         & residuals          & no           & lin \\
GUIDE+scores      & scores             & yes          & cat \\
MOB+bin           & scores             & yes          & max \\
CTree+bin         & scores             & yes          & lin \\
MOB+cat           & scores             & no           & cat \\
MOB               & scores             & no           & max \\
CTree             & scores             & no           & lin \\
\hline
\end{tabular}
...
To elaborate over an increasing effect size whether a binarization of the residuals/scores at zero 
leads to an improvement or a deterioration of performance, CTree and MOB are applied, once in their 
original version without binarization and once in an adapted version with a binarization of 
scores at zero (CTree+bin, MOB+bin). Moreover, they are compared to the adapted GUIDE version 
...
lead to the same conclusions: Binarizing the score values decreases the selection probability
...
split point $\xi$ is close to the quartiles used as breakpoints for the categorizations both 
...
of the split variable does not lead to any advantages unless the true split point corresponds (or is at
least close) to one of the quartiles used for the categorization. In most situations it even causes the 
power of the testing strategy to decrease.
