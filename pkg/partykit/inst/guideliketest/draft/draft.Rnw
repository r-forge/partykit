...
data is generated is a stump, i.e., a tree with only one split (see Figure~\ref{fig:dgp_stump}). 
...
yielding a tree with three terminal nodes (see Figure~\ref{fig:dgp_tree}). It employs the same
basic structure as the first scenario but simply adds another split. This allows to evaluate
the power of the three testing strategies in a more complex setting, in combination with using
a post-pruning strategy.
...
\hline
\end{tabular}
\caption{Variables included in the data generating process of 
...
can depend on the true split variable $Z_1$. More specifically, three different scenarios
...
+\delta \cdot (-1)^{j}  \quad \text{if } Z_1 \geq \xi
\end{cases}
\end{align*}
%
...
@
\endminipage
\caption{\label{fig:dgp_stump}{Top left panel: True stump structure with the true model parameters $\beta_0$
...
in the first scenario (stump with one split) the following criteria are considered:
...
selected for splitting in a pre-pruning approach).
For the second scenario (tree with two splits) the adjusted Rand index~(ARI) is calculated 
...
as their original versions. Moreover, as pre-pruning is employed all of the considered statistical 
...
\subsection[Residuals vs. full model scores]{Residuals vs.\ full model scores}
\label{sec:scores}

To assess the impact of using only residuals or the full score matrix four testing
strategies are compared: CTree, GUIDE, MOB -- all three using the default specification
as shown in Table~\ref{tab:buildingblocks} -- and additionally the GUIDE strategy using
the full scores instead (denoted by GUIDE+scores).
...
CTree, MOB, GUIDE, and GUIDE+scores evaluated on the first simulation scenario (stump with one split). 
Their performances are compared based on the proportion of replications where the true split 
variable $Z_1$ is selected over increasing effect size $\delta$ (100 replications of 
250 observations per step), once for a true split point $\xi = 0$ (top row) and once 
for $\xi = 0.8$ (bottom row). 
Left: varying intercept $\beta_0$. Middle: varying slope parameter $\beta_1$.
Right: both coefficients varying.}
\end{figure}
...
such as for a high effect size (see, e.g., for $\xi=0$ and varying $\beta_0$), GUIDE is 
never ahead of them in any of the considered settings. Moreover, the positive effect of 
including all available scores can be observed particularly well in the top middle panel 
where GUIDE+scores detects a shift in the slope parameter $\beta_1$ as opposed to 
GUIDE. The fact that residual-based methods fail to detect changes that do not effect the
...
Section~\ref{sec:discussion}. Moreover, the advantage of including full models scores 
...
For a central split point ($\xi=0$, upper panels) all strategies perform similarly
well as long as the intercept varies (left and right panel) with CTree and MOB 
being only slightly ahead. However, for the split point being close to the upper 
edge of the considered interval ($\xi=0.8$, lower panels) the performance of all of 
the applied strategies decreases. 
While both GUIDE versions struggle to detect the correct split variable even for a 
high effect size, MOB is clearly ahead with CTree leading to the second best results.
...
on the ability to detect the right split variable. To elaborate these effects more in detail the results for all possible combinations of the building blocks are discussed in a full factorial 3-way analysis in the following section.

\textit{Note:} As explained before, the tests are applied as significance tests with 
significance level $\alpha=0.05$. To show that in general the drawn conclusions do not 
change if no significance level is included the same setting is evaluated again without 
any significance level and the results are presented in Appendix~\ref{app:significance}.
...
case of the first simulation scenario (stump with one split) where the intercept and 
...
each of the three building blocks are listed in Table~\ref{tab:levels}. A more detailed
overview of all 12 combinations can be found in Appendix~\ref{app:combinations}, 
Table~\ref{tab:combinations_appendix}.
...
Based on the results displayed in Figure~\ref{fig:3way} it can be stated that binarizing residuals/scores decreases the performance 
as it leads to higher $p$-values corresponding to the true split variable $Z_1$ 
and thus to lower power in the presented settings. 
For a central true split point (left panel) this effect remains at an almost constant level 
regardless of whether possible split variables are categorized or not. However, in case 
of the true split point being closer to the upper margin ($\xi=0.8$, right panel), categorizing
split variables increases the $p$-value of $Z_1$ even more. 
Here, the maximal selection performs best while the linear selection shows its disadvantage in 
detecting abrupt changes. Overall, it depends on the situation whether a linear or a maximal
selection of a split variable is advantageous. But unless one of the breakpoints used for binning possible split variables is close to the true split point (as for $\xi=0$, left panel),
...
(left panel: true split point $\xi=0$,  effect size $\delta = 0.3$; 
right panel: $\xi=0.8$ requiring a higher effect size $\delta = 1$)
...
\textit{binarization}, and \textit{categorization}. All possible combinations have been 
evaluated in two different settings of the first simulation study (stump with one split) 
and their performances are compared based on the mean $p$-values corresponding to the 
true split variable $Z_1$. 
...
almost equally well for the true split points $0$, $0.2$ and $0.5$. GUIDE+scores performs
...
CTree, MOB, GUIDE, and GUIDE+scores evaluated on the second simulation scenario (tree with two splits), all
applying pre-pruning. Their performances are compared based on the Adjusted Rand Index over
increasing effect size $\delta$ (100 replications of 250 observations per step).
...
\subsection[Pre-pruning vs. post-pruning]{Pre-pruning vs.\ post-pruning}
...
applying post-pruning (building very large trees without any significance level and employing 
cost-complexity pruning as in the CART algorithm \citep{Breiman+Friedman+Stone:1984}). 
In Figure~\ref{fig:pruning_ari} the results are compared to those obtained with pre-pruning,
...
given by the tests for pre-pruning. As it can be seen in Figure~\ref{fig:pruning_ari}
...
This can also be observed in Figure~\ref{fig:pruning_ari} showing that post-pruning improves
...
CTree, MOB, GUIDE, and GUIDE+scores evaluated on the second simulation scenario (tree), 
once applying pre-pruning (dashed lines) and once post-pruning (solid lines). 
Their performances are compared based on the Adjusted Rand Index over increasing effect 
size $\delta$ (100 replications of 250 observations per step).
Each panel represents a different value of the true split point $\xi$ ($0$, $0.2$, $0.5$ and 
$0.8$ from left to right).}
\end{figure}
...
(``Variable selection and stopping criteria'') of the original paper and is of the following form:
$$
...
% FIXME: adapt notation from here on (split variable Z and response Y, scores s)
\subsection{MOB}
...
parameter instability'') of the original paper this process is of the following form:
$$
...
Following \cite{Zeileis+Hornik:2007} $W(t)$ converges to a Brownian bridge under the null
hypothesis of parameter stability. To obtain a test statistic a scalar functional $\lambda(\cdot)$ capturing the fluctuation in the empirical process can be applied.
For more details, particularly on calculating the corresponding $p$-values see Section~3.2. of the original paper.
...
@
\caption{\label{fig:scores_stump_wosign}Proportion of replications where the true split variable $Z_1$ 
shows the smallest $p$-value, hence, where $Z_1$ is selected without any significance level, 
over increasing effect size $\delta$ (100 replications of 250 observations per step), once for a 
...
Proportion of replications where the true split variable $Z_1$ is
selected over increasing effect size $\delta$ (100 replications of 250 observations per step)
...
CTree, CTree+max, MOB, GUIDE, GUIDE+scores.}
\end{figure}
...
@
\caption{\label{fig:bin_stump}Proportion of replications where the true split variable $Z_1$ is
detected over increasing effect size $\delta$ (100 replications of 250 observations per step)
...
strategies are compared: CTree, CTree+bin, MOB, MOB+bin, GUIDE+scores.}
\end{figure}
...
@
\caption{\label{fig:cat_stump}Proportion of replications where the true split variable $Z_1$ is
detected over increasing effect size $\delta$ (100 replications of 250 observations per step)
with both parameter varying. Each panel represents a different value of the true 
split point $\xi$ (0, 0.2, 0.5 and 0.8 from left to right). The performances of five testing 
strategies are compared: CTree, CTree+cat, MOB, MOB+cat, GUIDE+scores.}
\end{figure}
