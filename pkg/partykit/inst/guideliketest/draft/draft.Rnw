...
Note that considering full model scores is particularly advantageous if the model parameters do not
depend on each other, hence are orthogonal, as it is the case for the intercept and slope parameter
of a linear model. On the other hand, if all parameters are closely correlated with the location parameter
a residual-based approach might be able to detect changes in other parameters than the mean as well,
however, the performance of a score-based search for split variables is not negatively effected.
