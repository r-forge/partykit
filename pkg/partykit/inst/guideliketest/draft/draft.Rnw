...
promising strategy. \hlc{The most popular algorithms adopting such an unbiased recursive
...
and MOB \citep{Zeileis+Hothorn+Hornik:2008}. \hlc{QUEST and CTree have been suggested as
...
In particular, the approach from the GUIDE algorithm \hlc{(which essentially encompasses QUEST)}
...
\hlc{Once a split variable has been selected, the selection of the corresponding
...
\hlc{while they differ somewhat more generally. As we focus on model-based partitioning
...
\hlc{Therefore, this aspect is not considered again here but focus is given to the building
...
Figure~\ref{fig:scores_stump} \hlc{also shows that for $\delta = 0$, i.e., no difference
...
Even with the limited number of replications in this simulation study the empirical size
of all testing strategies (i.e., for $\delta = 0$) is between 4 and 5~percent and thus
...
\hlc{which also shows that the asymptotic structural change tests are slightly more conservative}
