...
names(pal) <- c("CTree", "CTree_max", "MOB", "GUIDE", "GUIDE_scores")
...
that some objective function is optimized, they differ in their 
specific approaches to selecting a split variable and the corresponding split point. 
...
framework and MOB uses fluctuation tests based on asymptotic theory. All these approaches
have shown to work well for various situations, however, the specific advantages and
disadvantages of each of the testing strategies have not yet been investigated and compared
in detail.
...
algorithms is presented such that each of the three strategies can be obtained by a specific 
combination of the available building blocks. In that way, the effects of each of these 
features can be investigated separately by varying the building blocks in the tool box 
provided by this general framework. 
...
the values of possible split variables are investigated.
Moreover, the impact of applying either a pre- or postpruning strategy is analyzed.

In the first part of Section~\ref{sec:urp} a basic scheme on how to build a tree is given. 
Then, after explaining the investigated algorithms CTree, MOB, and GUIDE more in detail in 
Sections~\ref{sec:ctree}, \ref{sec:mob} and \ref{sec:guide}, the unifying framework for 
testing strategies in model-based unbiased recursive partitioning is presented in 
Section~\ref{sec:unify}.
...
have been developed, all following the same general structure starting at the root of the tree
which is simply the first node containing all available data:
\begin{enumerate}
...
or average value, but it can also be constructed based on a parametric model. For instance,
different types of residuals or their signs can be applied \citep[see e.g.,][]{Loh:2002} as 
well as rank sums or logrank scores as in \cite{Hothorn+Hornik+Wiel:2006}.
...
\begin{enumerate}
\item Based on the discrepancy measure an $n \times k$-matrix $D$ is obtained giving information
...
of $Z_j$ on $D$ and in that way the dependency of $Z_j$ on the response $Y$ as the discrepancy
measure (and therefore also $D$) depends on $Y$. 
...
%As the variables are usually not all measured at the same scale the resulting
...
%variable selection on this standardized/unified scale. 
...
response can be mapped to a unity vector by an indicator function 
...
corresponding model scores by setting $h(Y) = s(Y,X,\hat{\beta})$
where $\hat{\beta}$ is the estimated model parameter and $s$ is the score function
$$
s(Y, X, \hat{\beta}) = \frac{\partial \psi(Y, X, \hat{\beta})}{\partial \beta}
$$
with the sum of $\psi$ over all observations being the objective function that is optimized 
to estimate the model parameter $\beta$. For this purpose many different M-type estimators can 
be applied including maximum likelihood or ordinary least squares. Therefore, these objective 
functions are also possible options to obtain a model-based discrepancy measure for the tree 
algorithm.
Applying the score function $s$ on a data set of $n$ observations containing the 
response vector $\boldsymbol{y} = (y_1,\ldots,y_n)^\top$ and 
regressor(s) $\boldsymbol{x} = (x_1,\ldots,x_n)^\top$ and 
on a $k$-dimensional estimated parameter vector 
$\hat{\boldsymbol{\beta}} = (\hat{\beta_1},\ldots,\hat{\beta_k})^\top$ 
yields a $n \times k$-matrix $S$.
...
statistic given all permutations of the response variable can be calculated and used to 
...
statistic or a quadratic form is considered. The resulting values can then be transformed to 
the scale of $p$-values where they can be compared in an unbiased way to select a split variable.
...
The fitted model can be of a very basic and simple form such as 
an average value of the response vector
%a constant fit where the parameter $\beta$ is an
%average value of the response, hence 
%$\hat{\beta} = \bar{y} = \frac{1}{n}\sum_{i=1,\ldots,n}^n y_i$ for a response vector 
%$\boldsymbol{y} = (y_1,\ldots,y_n)^\top$ 
but can also be a more complex parametric model.
In \cite{Zeileis+Hothorn+Hornik:2008} GLMs and survival regression models are used to 
exemplify the algorithm. Subsequently various other types of models have been applied as well,
...
score matrix $S$ in order to test for any instability in the model parameters over each 
particular ordering induced by each of the possible partitioning variables $Z$.
If a variable $Z$ has no specific influence on the parameters the scores would always 
fluctuate randomly around their mean 0. Therefore, these deviations are investigated by 
measuring them using an empirical fluctuation process. Applying a scalar function which 
...
For a categorical variable all possible splits are evaluated to find the best one while 
for $Z$ being a numeric variable a maximally selected split point search is employed. Hence, 
...
a constant fit where the parameter $\beta$ is the mean of the response, hence 
$\hat{\beta} = \bar{y} = \frac{1}{n}\sum_{i=1,\ldots,n}^n y_i$ for a response vector 
$\boldsymbol{y} = (y_1,\ldots,y_n)^\top$, can be employed as well as more complex models.
For the statistical test applied to select a split variable some type of model-based
residuals is required. In \cite{Loh:2002} linear models are used as an example where the
...
regression trees for longitudinal and multiresponse data \citep{Loh+Zheng:2013}, %(http://www.stat.rice.edu/~jrojo/4th-Lehmann/slides/Loh.pdf)
Poisson regression trees \citep{Chaudhuri+Lo+Loh:1995}, %(adjusted Anscombe residual (Pierce and Schafer 1986))
and quantile regression models \citep{Chaudhuri+Loh:2002} among many others.
...
\subsection{Pruning}
\label{sec:pruning}
In early developed methods of unbiased recursive partitioning
such as FACT \citep{Loh+Vanichsetakul:1988}, QUEST \citep{Loh+Shih:1997}, 
...
pruning strategy namely prepruning: Only if the $p$-value corresponding to the selected split
...
For postpruning no significance level is included leading to very large trees which are then 
pruned afterwards, for example by employing cost-complexity pruning as in the 
...
expensive than the additional calculations needed for postpruning.
...
For this reason we are focusing on the testing strategies applied in tree algorithms and 
investigate how effective they are as significance tests.



%\bigskip
%\textit{Note:}
%While GUIDE originally constructs large trees and applies postpruning with the cross-validation 
%method of CART, CTree and MOB employ prepruning by setting a significance level for the 
%independence tests such that no split is performed if the lowest $p$-value is not smaller than 
%the significance level. As explained in Section~\ref{sec:evaluation}, within the simulation study 
%in Section~\ref{sec:simulation} all three algorithms are applied using prepruning in order to put 
%the focus on the power of the algorithms in terms of selecting the best split variable. 
%However, the impact of the choice of pruning strategy is further discussed in Section~\ref{sec:discussion}.
...
are already explained/determined by the testing strategy. Subsequently combinations
...
\hline 
      & Fit             & Test                     & Pruning \\ 
\hline
CTree & non-parametric  & conditional inference    & prepruning\\
MOB   & parametric      & score-based fluctuation  & prepruning (or AIC/BIC-based pruning)\\
GUIDE & parametric      & residual-based $\chi^2$  & postpruning (cost-complexity pruning)\\
\hline
\end{tabular}
\end{table}
...
basic structure. Thus they can be presented together in a unifying framework for testing strategies
...
This general toolbox covers all features of the here discussed testing strategies CTree, MOB, and
GUIDE. Furthermore, it allows for all possible combinations of these features and in that way also
for variations of the commonly known testing strategies. Moreover, by exchanging only one building
block its impact can be investigated in detail.
These building blocks are explained in Section~\ref{sec:buildingblocks} and their setting
for CTree, MOB, and GUIDE are listed in Table~\ref{tab:buildingblocks} which gives a more 
detailed overview of the differences in the second column of Table~\ref{tab:combinations}.
...
investigated regarding each parameter instead of just the mean/location parameter. 
Therefore, this facilitates the detection of changes in parameters other than the mean/location
parameter.
...
\textit{Note:}
Apart from these three building blocks, the algorithms differ in their approximation of the 
underlying null distribution of the test statistic. While the permutation tests in the CTree 
algorithm work in a conditional framework the MOB and GUIDE algorithms both apply an unconditional 
approach. 
...
setting of the building blocks in the unifying framework and the approximation type of the
...
For that reason, this model is also considered for the simulation study in 
Section~\ref{sec:simulation}.
However, the conclusions drawn from this example also apply for a wide range of other model
types. The models fitted in the nodes of the considered linear model tree are of the form
$$
...
which is minimized for the estimation of the coefficient vector
...
$$
s(y_i, x_i, \hat{\beta_0},\hat{\beta_1}) = 
\frac{\partial \psi}{\partial \boldsymbol{\beta}}(y_i, x_i, \hat{\beta_0},\hat{\beta_1}) = 
2 \cdot r(y_i,x_i,\hat{\beta_0},\hat{\beta_1}) \cdot (1,x_i)^\top
$$
where the factor $2$ is often omitted such that the first column of the score matrix $S$ 
corresponds to the residuals. $S$ is in this case a $n \times 2$-matrix
as $\boldsymbol{\beta}$ is two dimensional. In that way a score value indicating how well the model
fits the data is given separately for each tuple of observation and coefficient $(y_i,x_i,\beta_j)$
with $i \in \{1,\dots,n\}$ and $j \in \{1,2\}$.
...
the derivatives of $\psi$ regarding the intercept $\beta_0$ (first column) and the slope parameter 
...
mtext(expression(beta[0]==+delta), side = 4, col = "red", las = 1, line = 0.5, at = 3.2)
mtext(expression(beta[1]== 1), side = 4, col = "red", las = 1, line = 0.5, at = 2.2)
mtext(expression(beta[0]==-delta), side = 4, col = "blue", las = 1, line = 0.5, at = 0.2)
mtext(expression(beta[1]== 1), side = 4, col = "blue", las = 1, line = 0.5, at = -0.8)
@
\endminipage
...
points(x = d$x[d$z1>xi], y = d$y[d$z1>xi], ylim = c(-5,5), pch = 19, col = alpha("red", 0.4), cex = 0.7)
legend("top", c(expression(z[1]<=xi), expression(z[1]>xi)), col = c("blue", "red"), pch = c(19), bty = "n", cex = 0.9)
mtext(expression(beta[0]==0), side = 4, col = "red", las = 1, line = 0.5, at = -1)
mtext(expression(beta[1]==-delta), side = 4, col = "red", las = 1, line = 0.5, at = -2)
mtext(expression(beta[0]==0), side = 4, col = "blue", las = 1, line = 0.5, at = 2)
mtext(expression(beta[1]==+delta), side = 4, col = "blue", las = 1, line = 0.5, at = 1)
@
\endminipage
\minipage{0.45\textwidth}
...
legend("top", c(expression(z[1]<=xi), expression(z[1]>xi)), col = c("blue", "red"), pch = c(19), bty = "n", cex = 0.9)
mtext(expression(beta[0]==-delta), side = 4, col = "blue", las = 1, line = 0.5, at = 1.7)
mtext(expression(beta[1]==+delta), side = 4, col = "blue", las = 1, line = 0.5, at = 0.7)
mtext(expression(beta[0]==+delta), side = 4, col = "red", las = 1, line = 0.5, at = -0.7)
mtext(expression(beta[1]==-delta), side = 4, col = "red", las = 1, line = 0.5, at = -1.7)
@
\endminipage
...
abline(coef = c(0,1), col = "blue", lwd = 2)
abline(coef = c(-1,-1), col = "red", lwd = 2)
abline(coef = c(1,-1), col = "darkgreen", lwd = 2)
points(x = d$x[d$z2>xi & d$z1<=xi], y = d$y[d$z2>xi & d$z1<=xi], pch = 19, 
       col = alpha("red", 0.4), cex = 0.7)
points(x = d$x[d$z2>xi & d$z1>xi], y = d$y[d$z2>xi & d$z1>xi], pch = 19, 
       col = alpha("darkgreen", 0.4), cex = 0.7)
legend("top", c(expression(z[2]<=xi), 
                expression(z[2]>xi~" and "~z[1]<=xi),
                expression(z[2]>xi~" and "~z[1]>xi)), 
       col = c("blue", "red", "darkgreen"), pch = c(19), bty = "n", cex = 0.9)
mtext(expression(beta[0]==0), side = 4, col = "blue", las = 1, line = 0.5, at = 2)
mtext(expression(beta[1]==+delta), side = 4, col = "blue", las = 1, line = 0.5, at = 1.3)
mtext(expression(beta[0]==+delta), side = 4, col = "darkgreen", las = 1, line = 0.5, at = 0.1)
mtext(expression(beta[1]==-delta), side = 4, col = "darkgreen", las = 1, line = 0.5, at = -0.6)
mtext(expression(beta[0]==-delta), side = 4, col = "red", las = 1, line = 0.5, at = -2)
mtext(expression(beta[1]==-delta), side = 4, col = "red", las = 1, line = 0.5, at = -2.7)
@
\endminipage
\caption{\label{fig:dgp_tree}Left panel: True tree structure for the second scenario with the 
true model parameters $\beta_0$ and $\beta_1$ for each terminal panel.
...
each building block can be investigated on its own. In Table~\ref{tab:buildingblocks} the setting
of these building blocks is listed for CTree, MOB and GUIDE in their original form.
However, in order to be able to focus on particular features rather than the whole testing
strategies other combinations as presented in Table~\ref{tab:buildingblocks}, hence variations of
the original testing strategies, are evaluated as well. 

Moreover, as prepruning is employed all of the considered statistical tests are applied as significance tests answering two questions at once: 
...
%As mentioned before, the GUIDE algorithm usually builds large trees which are then pruned 
%afterwards. However, for the purpose of comparing the testing strategies rather than the whole
%tree algorithms in this simulation study a significance level is considered for all statistical 
%tests, including the $\chi^2$-test in GUIDE such that all algorithms apply prepruning rather 
%than postpruning.
%The effect of these two pruning strategies are investigated in Section~\ref{sec:discussion}.
...
useOuterStrips(xyplot(prop_Tsplit ~ delta | vary_beta + xi, groups = ~ test, 
                      ...
                      key =  list(text = list(levels(subdataxi$test)),
                                  lines = list(lty = c(1,1,1,1), type = "l", pch = 1, cex = 0.8,
                                               col = pal[c("CTree", "MOB", "GUIDE_scores", "GUIDE")], lwd = 1)),
                      layout= c(3,2),
...
selected over increasing effect size $\delta$ for $\sigma = 1$, $\xi=0$, 250 observations and 
100 data sets per step. Each panel represents one scenario for the coefficients: 
varying intercept $\beta_0$ (left panel), varying slope parameter $\beta_1$ (middle panel) and 
both coefficients varying (right panel). The performances of four testing strategies are compared: 
...
(Additional results over increasing $\delta$ as in Section~\ref{sec:scores} 
...
variable~$Z_1$.
The possible values for each of the three building blocks are listed in Table~\ref{tab:levels}.
In order to evaluate all 12 combinations the original testing strategies CTree, MOB, and GUIDE
are applied together with variations of them. For each combination the corresponding employed
strategy can be found in Appendix~\ref{app:combinations}, Table~\ref{tab:combinations_appendix}.
...
if(file.exists("3way_xi00.rda")){
  ...
  save(d00, file = "3way_xi00.rda")
}


if(file.exists("3way_xi08.rda")){
  load("3way_xi08.rda")
} else {
  simres <- sim(nobs = 250, nrep = 100, seed = 7, stump = TRUE, nrsteps = 1,
                delta = 0.5,
  ...
  save(d08, file = "3way_xi08_delta05.rda")
}
@
...
<<3way_xi00, fig=TRUE, echo=FALSE, height=4, width=4>>=
par(mar = c(3,5,1,5.7))
...
\endminipage
\minipage{0.45\textwidth}
<<3way_xi08, fig=TRUE, echo=FALSE, height=4, width=3.3>>=
par(mar = c(3,2,1,5.8))
with(d08, interaction.plot(res_scores, interaction(cat, bin), pval_z1, ylim = c(0,1),
                           ylab = expression("Mean"~p~"-value for"~Z[1])))
@
\endminipage
\caption{\label{fig:3way00_08} Mean $p$-values corresponding to the true split variable $Z_1$
for the true split point $\xi=0$ (left panel) and $\xi=0.8$ (right panel),
with both coefficients varying, effect size $\delta = 0.3$, 250 observations and 100 data sets.
All 12 combinations of the three building blocks are evaluated.}
\end{figure}
...
\begin{figure}%[t]
\setkeys{Gin}{width=\linewidth}
<<3way_xi00solo, fig=TRUE, echo=FALSE, height=5, width=9.2>>=
par(mar = c(3,5,1,5.7))
with(d00, interaction.plot(res_scores, interaction(cat, bin), pval_z1,
                           ylab = expression("Mean"~p~"-value for"~Z[1]),
                           xpd = TRUE))
@
\caption{\label{fig:3way} Mean $p$-values corresponding to the true split variable $Z_1$
for the true split point $\xi=0$,
with both coefficients varying, effect size $\delta = 0.3$, 250 observations and 100 data sets.
All 12 combinations of the three building blocks are evaluated.}
\end{figure}

The results presented in Figure~\ref{fig:3way} confirm the already discussed advantage of
including all available scores rather than just residuals as this leads to a lower mean $p$-value 
of $Z_1$ for all testing strategies.
Moreover, it can be observed that all combinations that binarize scores/residuals perform
worse than those who do not employ any binarization. 
Regardless of the transformation of scores/residuals categorizing split variables also leads
to higher $p$-values and therefore has a negative effect on the power of a testing strategy.
Consequentially, without categorization the $p$-values are lower, particularly for a linear
selection of split variables.
...
a bit worse but it still achieves an ARI above $0.5$ for the highest considered effect size
...
effect on the power of a testing strategies. 
...
cost-complexity pruning as in the CART algorithm). 
...
Each panel represents a different value of the split point $\xi$ ($0$, $0.2$, $0.5$ and 
$0.8$ from left to right). The performances of four testing strategies are compared: CTree,
MOB, GUIDE, GUIDE\_scores. The dashed lines represent the results from applying postpruning
and the continuous lines the results from applying prepruning.}
\end{figure}
...
(1)~In most cases including all available score values rather than just residuals leads to 
...
%FIX ME: Check implementation of pruning.
...
Each suffix represents a change of the original form of the considered testing strategy.
In particular, ``\_res'' refers to using only residuals (instead of all scores),
...
\begin{figure}
\setkeys{Gin}{width=\linewidth}
<<scores_stump_wosign, fig=TRUE, echo=FALSE, height=4, width=8>>=
subdata <- subset(sim_stump, test %in% c("ctree","mfluc",
                                         "guide_sum_12", "guide_sum_1_cor"))
subdata$test <- factor(subdata$test, 
                       levels = c("ctree","mfluc",
...
is selected over increasing effect size $\delta$ for $\xi=0$, 250 observations and 
100 data sets per step without any significance level. 
...
detected over increasing effect size $\delta$ for both parameter varying, 
...
testing strategies are compared: CTree, CTree\_cat, MOB, MOB\_cat, GUIDE\_scores.}
\end{figure}
