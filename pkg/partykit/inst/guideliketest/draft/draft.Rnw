...
association tests (GUIDE), conditional inference (CTree), and parameter instability 
tests (MOB). First, these are embedded into a common inference framework encompassing
...
(1)~Variations of a goodness-of-fit measure for the response: using residuals or full model scores.
...
In many of the considered scenarios it can be observed that the choice of goodness-of-fit
measure heavily influences the performance of testing strategies as overall using model 
scores leads to clearly better results than employing residuals only. Moreover, the original
values of the goodness-of-fit measure are to be preferred over binarized versions of them.
...
\begin{enumerate}
\item For the data set in the current node a model is fitted by optimizing
some objective function that reflects the goodness of fit.
...
for example be the sum of deviations from a typical or average value, but it can 
also be constructed based on the model fitted in each node and depend not only on 
the response but also on possible covariates. For instance, different 
types of residuals or their signs can be applied \citep[see e.g.,][]{Loh:2002} as 
well as rank sums or logrank scores as in \cite{Hothorn+Hornik+VanDeWiel:2006}.
...
tests following the same general strategy: 
\begin{enumerate}
...
measure is obtained. More formally, this is an $n \times k$ matrix $G$
where $n \in \mathbb{N}$ is the number of observations and $k \in \mathbb{N}$ 
the number of measures per observation.
\item The dependency or association of this goodness-of-fit matrix $G$ with
...
As explained in more detail below, the aggregated overall objective
function in the linear regression model would be the residual sum of
squares and the corresponding observation-wise goodness-of-fit measure is
given by the residuals or scores (gradient contributions). Analogously,
the log-likelihood and corresponding score function could be used.
...
$$
with the sum of $\psi$ over all observations being the aggregated overall objective 
function that is optimized to obtain the estimated model parameter $\hat{\beta}$. 
For this purpose many different M-type estimators can be applied including maximum 
likelihood or ordinary least squares such that the corresponding objective functions 
can also be employed for the tree algorithm. Evaluating the score function $s$ on a 
...
While CTree offers to apply model-based goodness-of-fit measures as one possible choice, the 
...
The possible number of breakpoints is determined by the categorization of the possible 
split variables and is in that way fixed.
...
algorithms, such as finding the best split point by optimizing an overall goodness-of-fit measure.
