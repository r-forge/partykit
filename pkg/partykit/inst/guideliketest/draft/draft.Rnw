...
$$
T(Y, Z, w) = \text{vec}\left( \sum_{i=1}^n w_i g(Z_{i})h(Y_i)^{\top}\right)
$$
where $w$ are optional weights, $g: \mathcal{Z} \rightarrow \mathbb{R}^{p}$ is a nonrandom 
transformation of the split variable $Z$ and the influence function 
...
$p~\times~q$ matrix is converted into a $p q$ column vector.
...
$$
with the model scores $s_i = s(Y_i, X_i, \hat{\beta}$ being sorted by the split variable Z by including the ordering permutation $\sigma(Z_{i})$. 
...
$$\lambda_{supLM}(W) = \max_{i = \underline{i},\ldots,\overline{i}}\left(\frac{i}{n}\cdot \frac{n-i}{n}\right)^{-1} \left|\left|W\left(\frac{i}{n}\right)\right|\right|_2^2
$$
...
$$
where $O_{kl}$ are the observed frequencies of observations in a certain combination of binarized residuals ($k=1,2$) and categorized split variables ($l = 1,\ldots,4$). $E_{kl}$ are the corresponding expected frequencies under independence $E_{kl} = \frac{O_{k \cdot} \cdot O_{\cdot l}}{n}$. Under the null hypothesis of independence the asymptotic distribution of $X^2$ is a $\chi^2$~distribution allowing for a straight-forward calculation of $p$-values.
