...
also be based on a model fro a response along with potential covariates.
For instance, different types of residuals (or the signs thereof) can be employd
...
multivariate. The actual test is carried out be mapping this standardized statistic to
...
Building on earlier work for the QUEST algorithm \citep{Loh+Shih:1997}, \citep{Loh:2002}
...
linear regression models could be fitted in the nodes of tree as well as
...
To construct a statistical test two additional transformation are carried out:
(1)~The residuals $r(Y,X,\hat{\beta})$ are binarized at zero, yielding an indicator for positive vs.\ negative residuals.
(2)~All possible split variable $Z$ are categorized, i.e., unless $Z$ is already categorical
...
cross-validation. This is also know as \emph{postpruning} (after growing the initial tree) and
more specifically cost-complexity pruning.

In the unbiased recursive partitioning literature this postpruning approach is also usef frequently
...
so-called \emph{prepruning} strategy which stops growing the tree as soon as no significant association
...
postpruning in model-based partitioning is to use information criteria such as AIC (Akaike information
criterion) or BIC (Bayes information criterion), following the ideas of \cite{Su+Wang+Fan:2004}.

Clearly, for inference-based prepruning it is crucial that the association tests employed for
split selection work well as statistical significance test, i.e., conform with their nominal size
and have high power. In contrast, when using postpruning (either based on cross-validation or
...
Subsequently we combine the tests with a cost-complexity postpruning approach in order to assess
...
\hline
\end{tabular}
\caption{Combinations of fitted model type, test type and pruning strategy
...
\begin{itemize}
\item Variations of the goodness-of-fit measure:\\
Different variations of the disaggregated, observation-wise goodness-of-fit measure of the 
model that is obtained for the response $Y$ and possible covariates $X$ can be 
considered. This can be residuals $r(Y,X,\hat{\beta})$ as in the GUIDE algorithm or 
scores $s(Y,X,\hat{\beta})$ as in the MOB algorithm or, more generally, a 
transformation function $h$ as in the CTree algorithm including both of the foregoing cases.
By including all available scores rather than just residuals the goodness of fit of the 
model is investigated regarding each parameter instead of just the mean. 
...
A very basic form of goodness-of-fit measure can be obtained for each observation by simply 
...
For a model-based approach one can consider residuals defined by
$$
r(y_i,x_i,\beta_0,\beta_1) = y_i - \beta_0 - \beta_1 \cdot x_i.
$$
The appropriate choice of the function $\ell$ is then
$$
\ell(y_i,x_i,\beta_0,\beta_1) = r(y_i,x_i,\beta_0,\beta_1)^2
$$
such that $\sum_{i=1}^n \ell(y_i,x_i,\beta_0,\beta_1)$ is the objective function
which is minimized to obtain the estimated coefficient vector
$\boldsymbol{\hat{\beta}} = (\hat{\beta_0}, \hat{\beta_1})^\top$.
The corresponding disaggregated, observation-wise goodness-of-fit measure is defined by
the score function 
$$
...
corresponds to including the derivatives of $\ell$ only regarding the intercept 
parameter~$\beta_0$.
On the contrary, by using full model scores (i.e. the whole $n \times 2$ matrix) 
the derivatives of $\ell$ regarding the intercept $\beta_0$ (first column) and the slope 
