...
if(file.exists("../simulation/results/sim_stump.rda")){
  ...
  save(sim_stump, file = "../simulation/results/sim_stump")
}
...
\Abstract{
A core step of every algorithm for learning regression trees is the selection 
...
directly comparing all possible split points in all available covariates. However, 
subsequent research showed that this is biased towards selecting covariates with 
...
based on statistical inference using $p$-values that are adjusted for the possible split 
points. In a second step a split point optimizing some objective function is selected in 
the chosen split variable. However, different unbiased tree algorithms obtain these 
$p$-values from different inference frameworks and their relative advantages or 
disadvantages are not well understood, yet.

Therefore, three different popular approaches are considered here: classical categorical 
association tests (as in GUIDE), conditional inference (as in CTree), and parameter 
instability tests (as in MOB). First, these are embedded into a common inference framework
encompassing parametric model trees, in particular linear model trees. Second, it is 
assessed how different building blocks from this common framework affect the power of the 
algorithms to select the appropriate covariates for splitting: observation-wise
goodness-of-fit measure (residuals vs.\ model scores), dichotomization of residuals/scores
at zero, and binning of possible split variables. This shows that specifically the 
goodness-of-fit measure is crucial for the power of the procedures, with model scores 
without dichotomization performing much better in many scenarios.
}
...
best split point only within this variable in a separate second step is a more 
promising strategy. The most popular algorithms adopting such an unbiased recursive
partitioning strategy are QUEST \citep{Loh+Shih:1997},
%% CHAID \citep{Kass:1980},
GUIDE \citep{Loh:2002}, CTree \citep{Hothorn+Hornik+Zeileis:2006}, 
and MOB \citep{Zeileis+Hothorn+Hornik:2008}. QUEST and CTree have been suggested as
nonparametric decision trees initially and have acquired more than 1,300 and 2,400~citations
on Google Scholar, respectively. GUIDE and MOB have been designed for general model-based
trees and have both received more than 400 citations.
All of these share the same basic concept of selecting that covariate for splitting which shows the
highest association to the response variable as captured by the $p$-value in the corresponding
statistical test. While QUEST
%%, CHAID,
and its successor GUIDE employ 
...
%\fixme{``Why CTree, GUIDE, and MOB?''}
%
\fixme{Focus on testing strategies} 

Therefore, \hlc{instead of comparing the complete algorithms,} in this paper the focus will be put on that first step of tree algorithms,
i.e., the task of selecting the best split variable in a given (sub)sample. 
In particular, the approach from the GUIDE algorithm (which essentially encompasses QUEST)
is compared to the one of the CTree and MOB algorithms by investigating the building blocks of 
their testing strategies in which they differ:
...
%% FIXME
\fixme{Provided theory, finite samples}

\hlc{As the underlying theory of the different testing strategies and the resulting properties and skills but also limitations are already discussed in previous literature} \citep[see, e.g.,][]{Ploberger+Kraemer:1992, Hothorn+Hornik+Zeileis:2006, Zeileis+Hothorn+Hornik:2008, Loh:2002}, \hlc{the presented simulation study aims at investigating the corresponding consequences in finite samples.}
...
Section~\ref{sec:urp} reviews unbiased tree models, starting with the general algorithmic idea~(Section~\ref{sec:generic_alg}) followed by the specific algorithms CTree~(Section~\ref{sec:ctree}), 
MOB~(\ref{sec:mob}), and GUIDE~(\ref{sec:guide}), before possible pruning strategies~(\ref{sec:pruning}) are discussed.
...
If both variables $Y$ and $Z$ are numeric the default independence test corresponds to a \mbox{Pearson} correlation test. For one numeric and one categorical variable essentially a one-way 
ANOVA~(analysis of variance) is employed while for two categorical variables a $\chi^2$~test is performed.
...
including beta regression \citep{Gruen+Kosmidis+Zeileis:2012}, psychometric item response theory models~\citep{Strobl+Kopf+Zeileis:2015}, or mixed effects models \citep{Fokkema+Smits+Zeileis:2018}.
...
\subsection{Selecting a split point}
%\fixme{``Why is the choice of the split point not part of this paper?''}

Once a split variable has been selected, the selection of the corresponding
split point is less controversial. The most natural strategy is to use an exhaustive
search over all possible split points and then select the one that optimizes the
objective function (or loss function). In fact, this is the strategy adopted by both
model-based approaches, GUIDE and MOB. Only CTree uses a slightly different approach,
based on maximizing a two-sample test statistic, because the algorithm as originally
proposed did not employ an explicit objective function. However, it can be shown
that in some settings the two approaches coincide \citep{Zeileis+Hothorn:2013}
while they differ somewhat more generally. As we focus on model-based partitioning
here we employ the exhaustive model-based search for split point selection in
all three algorithms considered.
...
In the unbiased recursive partitioning literature this post-pruning approach is also used 
frequently \citep[e.g., in][]{Loh+Vanichsetakul:1988,Loh+Shih:1997,Kim+Loh:2001,Loh:2002} 
and the $p$-values from the association tests are only employed for selecting the split 
variable on a unified scale.
However, \cite{Hothorn+Hornik+Zeileis:2006} proposed to also use these $p$-values for a 
so-called \emph{pre-pruning} strategy which stops growing the tree as soon as no significant
association can be found in a given subgroup. This approach is the default in CTree and also in
MOB. However, \cite{Zeileis+Hothorn+Hornik:2008} also pointed out that a natural strategy for
post-pruning in model-based partitioning is to use information criteria such as AIC (Akaike 
information criterion) or BIC (Bayes information criterion), following the ideas of \cite{Su+Wang+Fan:2004}.

Clearly, for inference-based pre-pruning it is crucial that the association tests employed for
split selection work well as statistical significance test, i.e., conform with their nominal
size and have high power. In contrast, when using post-pruning (either based on 
cross-validation or information criteria) it might not be as crucial that the significance 
test works well and has high power.

Due to these considerations we first evaluate the significance tests underlying CTree, MOB,
and GUIDE by themselves, i.e., without growing an actual tree in combination with a pruning 
strategy. Subsequently we combine the tests with a cost-complexity post-pruning approach in 
order to assess whether shortcomings of the tests are mitigated by pruning.
...
of the type of model fits, tests, and pruning strategy employed to grow the tree. 
Table~\ref{tab:combinations} provides an overview of the default combinations.
...
%\fixme{``Do the building blocks cover all possible options?''}
%
...
Therefore, this aspect is not considered again here but focus is given to the building
blocks selected above.
...
are considered for the coefficients $\beta_0$ and $\beta_1$ (see Figure~\ref{fig:dgp_stump} for an illustration):
%
\begin{enumerate}
...
\end{enumerate}
%
More precisely, the coefficient that
...
\fixme{again, focus on testing strategies}

\hlc{For both scenarios the employed testing framework and the considered evaluations allow for a direct comparison of the testing strategies only opposed to the complete tree algorithms.}
...
which are investigated in more detail in the next section.

%\fixme{Type I error}
%
Figure~\ref{fig:scores_stump} also shows that for $\delta = 0$, i.e., no difference
in the parameters and consequently a random split variable selection, the probability of 
selecting $Z_1$ as split variable is (close to)~$0$ (with the employed Bonferroni-adjustment of 
$p$-values keeping this value particularly low).
Even with the limited number of replications in this simulation study the empirical~size~of 
all testing strategies (i.e., for $\delta = 0$) is between 4 and 5~percent and thus
conforms well~with~the nominal significance level of $\alpha = 0.05$. The exact values
can be found in Table~\ref{tab:typeIerror} in Appendix~\ref{app:typeIerror}
which also shows that the asymptotic structural change tests are slightly more conservative
\citep[confirming previous results in][]{Hothorn+Zeileis:2008, Zeileis+Hothorn:2013}.
...
if(file.exists("../simulation/results/sim_3way.rda")){
  ...
  save(sim_3way, file = "../simulation/results/sim_3way.rda")
}
@
...
once with significance-based pre-pruning and once \mbox{with cost-complexity post-pruning.}
...
if(file.exists("../simulation/results/sim_tree.rda")){
  ...
  rm(sim_tree_ex)
  save(sim_tree, file = "../simulation/results/sim_tree.rda")  
}
...
interactions, see \citealp{Doove+Dusseldorp+VanDeun:2014}) it can easily occur \citep[see Figure~2 in][for an illustration]{Loh+He+Man:2015}.
Testing strategies being capable of detecting all kinds of changes including those orthogonal to the 
mean are also employed in an extension of (generalised) linear model trees introduced by 
\cite{Seibold+Hothorn+Zeileis:2019}
which allows for selected model parameters to be kept fixed at a globally estimated value while only 
changes in the remaining parameters are considered for splitting.
...
\fixme{Outlook: non-tree structure}

\hlc{To exploit the flexibility of tree models future investigations could also include situations where the underlying data generating process does not follow a (clear) tree structure. In this case, assessing the effects of different building blocks in the testing strategies should in general show similar results, however, they might vary depending on how well the true functional relationships can be approximated by a tree structure.}

\fixme{Reference on Fu and Simonoff (2015)}

As \cite{Fu+Simonoff:2015} \hlc{show, unbiased recursive partitioning algorithms can have a bias on their own. In particular, given that there are two possible split variables showing the same significant association to the (transformed) response the algorithms tend to select the one providing less possible split points. Therefore, this could be an additional specific setting in which the power of testing strategies and their building blocks could be compared.}

\newpage
\section*{Computational details}
\label{sec:comp}
The applied implementation is based on the \proglang{R} package \pkg{partykit}
%(version~\Sexpr{packageVersion("partykit")}) 
(version~1.2.4) 
which is available on \proglang{R}-Forge at 
\url{https://R-Forge.R-project.org/projects/partykit/}.
The code to reproduce the simulation study is available at \url{https://eeecon.uibk.ac.at/~zeileis/news/power_partitioning/}.
...
\begin{tabular}{ l l l l l}
\hline\noalign{\smallskip} 
Residuals/Scores   & Dich. & Cat. & \multicolumn{2}{l}{Via testing strategy}\\
\noalign{\smallskip}\hline\noalign{\smallskip}
residuals	   & yes	     & cat            & GUIDE & {(default)}\\
residuals	   & yes	     & max            & MOB   & {(modified)}\\
residuals	   & yes	     & lin            & CTree & {(modified)}\\
residuals	   & no 	     & cat            & MOB   & {(modified)}\\
residuals	   & no 	     & max            & MOB   & {(modified)}\\
residuals	   & no 	     & lin            & CTree & {(modified)}\\
scores  	   & yes	     & cat            & GUIDE & {(modified)}\\
scores  	   & yes	     & max            & MOB   & {(modified)}\\
scores  	   & yes	     & lin            & CTree & {(modified)}\\
scores  	   & no 	     & cat            & MOB   & {(modified)}\\
...
\label{app:typeIerror}
<<typeIerror, echo=FALSE, results=hide>>=
# Table: Type I error
subdata <- subset(sim_stump, test %in% c("ctree","mfluc",
                                         "guide_sum_12", "guide_sum_1_cor"))
subdata$test <- factor(subdata$test, 
...
sd <- subset(subdata, changetype == "abrupt" & delta == 0 & xi %in% c("0","0.8"))
sd$xi <- factor(sd$xi)
# sd[, c("test","xi", "variation", "prop_split")]

xtbl <- xtabs(prop_split ~ (xi + test) + variation, data = sd)

# sd$prop_split[sd$test == "CTree" & sd$variation == "all" & sd$xi == "0"]
@
...
\hline\noalign{\smallskip} 
Split point  & Strategy     & Varying $\beta_0$  & Varying $\beta_1$ & Varying $\beta_0$ and $\beta_1$ \\ 
\noalign{\smallskip}\hline\noalign{\smallskip} 
$\xi = 0$   & CTree         &  0.05 & 0.05 & 0.05 \\ 
($50$\%)    & MOB           &  0.04 & 0.04 & 0.04 \\
            & GUIDE+scores  &  0.05 & 0.05 & 0.05 \\
            & GUIDE         &  0.04 & 0.04 & 0.04 \\
\noalign{\smallskip}\hline\noalign{\smallskip} 
$\xi = 0.8$ & CTree         &  0.05 & 0.05 & 0.05 \\
($90$\%)    & MOB           &  0.04 & 0.04 & 0.04 \\
            & GUIDE+scores  &  0.05 & 0.05 & 0.05 \\
            & GUIDE         &  0.04 & 0.04 & 0.04 \\
\noalign{\smallskip}\hline
\end{tabular}
\caption{Probability of splitting in any of the available split variables,
i.e., probability of any of the available split variables showing a $p$-value
below the predefined level of significance $\alpha = 0.05$. 
The testing strategies CTree, MOB, GUIDE, and GUIDE+scores are considered in 
the ``stump'' scenario. Probabilities are estimated for effect size $\delta = 0$ 
(i.e., no difference in the parameters) with 100 replications of 250 observations.
The true split point $\xi$ in $Z_1$ is either the median~$0$~(top row) or the 90\%~quantile~$0.8$~(bottom row)
for either varying intercept~$\beta_0$, varying slope~$\beta_1$, or both coefficients varying.
\label{tab:typeIerror}}
\end{table}
