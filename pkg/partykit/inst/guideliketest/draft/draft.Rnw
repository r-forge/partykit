...
load("../sim/simres20181122_stump.rda")
sim_stump <- simres$res
#load("../sim/simres20180422_2step.rda")
#sim2step <- simres
rm(simres)
load("../sim/simres20181122_tree.rda")
sim_tree <- simres$res
rm(simres)

## HCL palette
pal <- hcl(c(140, 80, 260, 50, 10), 300, 60)
...
Section~\ref{sec:strategies} the unifying framework for model based unbiased
recursive partitioning is presented in Section~\ref{sec:unify}.
...
\label{sec:urp}
The basic idea of building a regression tree model is to partition the data into smaller and more 
homogeneous subgroups based on (a set of) covariates. Various different tree algorithms 
...
explained in detail in Section~\ref{sec:strategies}.
...
\subsection{Testing strategies}\label{sec:strategies}

\subsubsection{CTree}\label{sec:ctree}
The CTree algorithm is based on the idea of providing non-parametric regression tree models in 
a conditional inference framework by applying permutation tests. In its basic form linear test 
statistics are applied to measure the dependency between a covariate and the response variable. 
Optionally, maximally selected statistics can be employed as well. By allowing for transformations 
of the covariates and the application of so~-~called influence functions on the response variable 
within the test statistic a model-based approach is also possible. For example, the influence
function can calculate a model-based measure of goodness of fit as a discrepancy measure 
for each response value (in a permutation symmetric way) such that the test statistic evaluates 
the dependency of a covariate on the goodness of fit.
...
\subsubsection{MOB}\label{sec:mob}
...
and is hence purely intended for a model-based approach. 

An objective function $\psi(Y,X,\theta)$ depending on observations $(Y,X)$, with a response $Y$ and 
covariate(s) $X$, and a $k$-dimensional vector of parameters $\theta$ is minimized to obtain the 
parameter estimate $\hat{\theta}$ for the considered parametric model fitted in the current node of 
the tree. For this purpose many different M-type estimators can be applied including maximum likelihood 
...
To obtain additional information on the goodness of fit of the parametric model so~-~called scores are
calculated. Scores are the values of the derivative of the objective function with respect to the parameters 
with the estimated parameters and the observed values plugged in:
$$
S(y, x, \hat{\theta}) = \frac{\partial \psi(y, x, \hat{\theta})}{\partial \theta}
$$
which is a $n \times k$ - matrix for a $k$-dimensional parameter vector $\theta$ and
a data set of $n$ observations containing the response vector $y = (y_1,\ldots,y_n)^\top$
and regressor(s) $x = (x_1,\ldots,x_n)^\top$. 
...
and $l \in \{1,\ldots,k\}$. 
After calculating these scores the aim is to find out whether there is any instability in the parameters
over each particular ordering induced by the possible partitioning variables. If a covariate has
no specific influence on the parameters the scores would always fluctuate randomly around their 
mean 0. Therefore, these deviations are investigated by measuring them using an empirical fluctuation 
process. Applying a scalar function which captures the fluctuation in this process yields the
test statistic of this so called generalized M-fluctuation test. To obtain the distribution of
...
with the smallest p~-~value is selected as split variable. Then an exhaustive search over all possible
partitions can be applied to find the best split point. Hence, the number of possible breakpoints is
not fixed unlike in the CTree algorithm which can make this operation computationally expensive.


\subsubsection{GUIDE}\label{sec:guide}
Similar to the MOB algorithm, parametric models are fit to the nodes of a tree in the GUIDE algorithm.
From the model fitted to the data in the current node residuals are calculated. But before applying
a statistical test two additional steps are performed (to prepare(?) the data):
...
\subsubsection{Residuals vs full model scores}
Following the basic tree building strategy in Section~\ref{sec:urp}, a $n \times k$ - matrix is 
obtained based on the discrepancy measure. In the here discussed unifying framework this matrix 
is the score matrix $S$ as presented in Section~\ref{sec:mob} providing information on the 
goodness of fit for each observation and model parameter.
Since the first column of the score matrix is the vector of residuals the decision of using either 
residuals or full model scores for the test statistic is equivalent to the decision of using only 
the first column or the whole matrix $S$.

By including all available scores the goodness of fit of the model is investigated regarding each 
parameter rather than just regarding the mean/location parameter. In that way changes in parameters
other than the mean/location parameter should be detected more easily.

\textit{Example LM:} To exemplify this feature a linear model tree is considered as this allows 
for an easy and well structured description of the investigated subjects/issues. Therefore, from
here on this example will be used for illustrations.
(However, the conclusions drawn for this example also apply for a wide range of other model
types.)
The models fitted in the nodes of the considered linear tree model are of the form
$$
Y = \beta_0 + \beta_1 \cdot X + \epsilon
$$
for a response variable $Y$, a regressor variable $X$ and an error $\epsilon$. 
The coefficient vector $\boldsymbol{\beta} = (\beta_0, \beta_1)^\top$ is estimated
via minimizing the residual sum of squares (RSS) for the learning data set in 
the current node, for notational simplicity generally denoted by 
$\{(y_i, x_i)\}_{i=1\dots n}$. Hence, the discrepancy measure, i.e. the objective function is
$$
RSS(y_i, x_i, \boldsymbol{\beta}) = \sum_{i=1}^n (y_i - \beta_0 - \beta_1 \cdot x_i)^2
$$ 
such that the corresponding score matrix is defined by
$$
s(y_i, x_i, \hat{\boldsymbol{\beta}}) = \frac{\partial \textit{RSS}}{\partial \boldsymbol{\beta}}(y_i, x_i, \hat{\boldsymbol{\beta}})
$$
which is in this case a $n \times 2$ - matrix as $\boldsymbol{\beta}$ is two dimensional. 
(In that way a score value indicating how well the model fits the data is given separately
for each tuple of observation and coefficient $(y_i,x_i,\beta_j)$ with $i \in \{1,\dots,n\}$
and $j \in \{1,2\}$.)
In this example considering only residuals corresponds to including the derivatives of $RSS$
only regarding the intercept parameter $\beta_0$, i.e. the first column of $S$.
...
\subsubsection{Binarization of scores}
...
\subsubsection{Categorization of covariates}
Similarly, the covariates can also be categorized. For this purpose, the corresponding 1st, 2nd and 
3rd quartile are used to bin each possible split variable into four categories.
...
\hline
CTree & full model scores & off           & off             & conditional\\
MOB   & full model scores & off           & off             & unconditional\\
GUIDE & residuals         & on            & on              & unconditional\\
\hline
\end{tabular}
\end{table}
...
as illustrated in Figure~\ref{fig:true_tree}.
...
(can) depend on the true split variable $Z_1$ in the following way:
\begin{center}
...
the negative value of the effectsize, i.e. $\delta$ or $-\delta$.
...
can be found in Figure~\ref{fig:dgp}.
...
par(mar = c(4,4,2,1))
plot(x = d$x[d$z1<xi], y = d$y[d$z1<xi], ylim = c(-5,5), ylab = "y", xlab = "x", main = expression("vary"~beta[0]),
     pch = 19, col = alpha("blue", 0.4), cex = 0.7)
points(x = d$x[d$z1>=xi], y = d$y[d$z1>=xi], ylim = c(-5,5), pch = 19, col = alpha("red", 0.4), cex = 0.7)
legend("top", c(expression(z[1]<0), expression(z[1]>=0)), col = c("blue", "red"), pch = c(19), bty = "n", cex = 0.9)
plot(x = d$z1, y = d$beta0, ylab = expression(beta[0]), xlab = expression(z[1]), pch = 21, bg = "black", cex = 0.7, ylim = c(-1,1)) 
plot(x = d$z1, y = d$beta1, ylab = expression(beta[1]), xlab = expression(z[1]), pch = 21, bg = "black", cex = 0.7, ylim = c(-1,1))

d <- dgp_stump(nobs = nobs, binary_beta = TRUE, binary_regressor = FALSE, only_intercept = FALSE,
               vary_beta = "beta1", beta0 = beta0, delta = delta, xi = xi, sigma = sigma)

par(mar = c(4,4,2,1))
plot(x = d$x[d$z1<xi], y = d$y[d$z1<xi], ylim = c(-5,5), ylab = "y", xlab = "x", main = expression("vary"~beta[1]),
     pch = 19, col = alpha("blue", 0.4), cex = 0.7)
points(x = d$x[d$z1>=xi], y = d$y[d$z1>=xi], ylim = c(-5,5), pch = 19, col = alpha("red", 0.4), cex = 0.7)
legend("top", c(expression(z[1]<0), expression(z[1]>=0)), col = c("blue", "red"), pch = c(19), bty = "n", cex = 0.9)
plot(x = d$z1, y = d$beta0, ylab = expression(beta[0]), xlab = expression(z[1]), pch = 21, bg = "black", cex = 0.7, ylim = c(-1,1)) 
...
plot(x = d$x[d$z1<xi], y = d$y[d$z1<xi], ylim = c(-5,5), ylab = "y", xlab = "x",
     main = expression("vary"~beta[0]~"and"~beta[1]),
     pch = 19, col = alpha("blue", 0.4), cex = 0.7)
...
plot(x = d$z1, y = d$beta1, ylab = expression(beta[1]), xlab = expression(z[1]), pch = 21, bg = "black", cex = 0.7, ylim = c(-1,1)) 
@
\caption{\label{fig:dgp}{Generated data set of 250 observations for binary coefficients 
$\beta_0$ and $\beta_1$, continuous regressor $X$, variance parameter $\sigma = 0.5$, 
effect size $\delta = 1$ and split point $\xi = 0$. 
Left panels: response $Y$ on regressor $X$; middle panels: intercept $\beta_0$ on split 
variable $Z_1$; middle panels: slope coefficients $\beta_1$ on split variable $Z_1$; each 
for varying $\beta_0$ (top), varying $\beta_1$ (middle) and both coefficients varying 
(bottom).}}
\end{figure}
...
split (in $Z_1$, at $\xi$) induces a change in the slope parameter $\beta_1$ while the second
...
Figures~\ref{fig:true_tree} and \ref{fig:true_tree_biv}.
...
d <- dgp_tree(nobs = 250, xi = 0, binary_regressor = FALSE, vary_beta = "all", 
              binary_beta = TRUE, nrsplitvar = 2, sigma = 0.001)
...
<<true_tree, fig=TRUE, echo=FALSE, width=5.4, height=4>>=
...
@
\caption{\label{fig:true_tree}True tree structure for split point $\xi = 0$} with the
true model parameters $\beta_0$ and $\beta_1$ for each terminal node.
\end{center}
\end{figure}
...
However, in order tobe able to focus on particular features rather than the whole algorithms
...
to a predefined level of significance $\alpha$. Only if the smallest p~-~value is smaller than
$\alpha$ a split is performed and the covariate corresponding to this p~-~value is selected.
Including this criterion of significance in the building process of a tree is often refereed
to as \textit{pre-pruning}, however, this is not always applied in practice as significance
is often not considered. As mentioned before, the GUIDE algorithm usually builds large 
trees which are then pruned afterwards. However, for the purpose of comparing the testing
strategies rather than the whole tree algorithms in this simulation study a significance
level is considered for all statistical tests, including the $\xi^2$-test in the GUIDE algorithm
such that all algorithms apply pre-pruning rather than post-pruning.
...
(for a given significance level $\alpha = 0.05$) is considered for the first scenario (stump).
...
MOB         & full model scores & off           & off             & unconditional\\
GUIDE       & residuals         & on            & on              & unconditional\\
GUIDE\_full & full model scores & on            & on              & unconditional\\ \hline
\end{tabular}
\end{table}
...
xyplot(prop_Tsplit ~ delta | vary_beta, groups = ~ test, 
       ...
       type = "b", lty = c(1,1,1,1),
       key =  list(text = list(levels(subdata$test)),
                   lines = list(lty = c(1,1,1,1), type = "b", pch = 1, cex = 0.8,
...
\hline
CTree       & full model scores & off           & off             & conditional\\
CTree\_cat  & full model scores & off           & on              & conditional\\
MOB         & full model scores & off           & off             & unconditional\\
MOB\_cat    & full model scores & off           & on              & unconditional\\
GUIDE\_full & full model scores & on            & on              & unconditional\\
\hline
\end{tabular}
\end{table}
...
selected over increasing effect size $\delta$ for a
varying slope parameter $\beta_1$, $\sigma = 1$, 250 observations and 100 data sets per 
step. Each panel represents a different value of the split point $\xi$ ($0$, $0.2$, $0.5$ 
and $0.8$ from left to right). The performances of five testing strategies are compared: 
CTree, CTree\_cat, MOB, MOB\_cat, GUIDE\_full.}
%\end{figure}
%\begin{figure}[h!]
\setkeys{Gin}{width=\linewidth}
<<cat_stump_all, fig=TRUE, echo=FALSE, height=4, width=8>>=
subdata <- subset(sim_stump, test %in% c("ctree", "ctree_cat",
                                         #"ctree_max", "ctree_max_cat",
                                         "mfluc", "mfluc_cat",
                                         "guide_sum_12"))
...
@
\caption{\label{fig:cat_stump_all}Proportion of cases where the true split variable $Z_1$ is
...
\hline
CTree       & full model scores & off           & off             & conditional\\
CTree\_bin  & full model scores & on            & off             & conditional\\
MOB         & full model scores & off           & off             & unconditional\\
MOB\_bin    & full model scores & on            & off             & unconditional\\
GUIDE\_full & full model scores & on            & on              & unconditional\\
\hline
\end{tabular}
\end{table}

Figure~\ref{fig:bin_stump} shows the effect of binarizing the score varlues over
...
detected over increasing effect size $\delta$ for a 
varying slope parameter $\beta_1$, $\sigma = 1$, 250 observations and 100 data sets per 
step. Each panel represents a different value of the split point $\xi$ (0, 0.2, 0.5 and 0.8
from left to right). The performances of five testing strategies are compared: CTree, CTree\_bin,
MOB, MOB\_bin, GUIDE\_full.}
%\end{figure}
%
%\begin{figure}[h!]
\setkeys{Gin}{width=\linewidth}
<<bin_stump_all, fig=TRUE, echo=FALSE, height=4, width=8>>=
subdata <- subset(sim_stump, test %in% c("ctree", "ctree_bin", 
                                         #"ctree_max", "ctree_max_bin",
                                         "mfluc", "mfluc_bin",
                                         "guide_sum_12"))
...
@
\caption{\label{fig:bin_stump_all}Proportion of cases where the true split variable $Z_1$ is
...
slope parameter as it only consideres residuals in its testing strategy.
...
Optionally compare GUIDE with prepruning to GUIDE with postpruning on tree structure?
