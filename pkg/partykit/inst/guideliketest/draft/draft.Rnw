...
\hlc{Therefore, in this paper the focus will be put on that first step of tree algorithms,
i.e., the task of selecting the best split variable in a given (sub)sample. 
For this purpose, three leading unbiased recursive partitioning algorithms are considered, 
namely GUIDE~(Google Scholar: 484), CTree~(2444) and MOB~(421). Moreover, through this 
selection the approach of QUEST (1306) is included as well due to GUIDE having been introduced 
as a descendant/extension of QUEST (and others). %\citep{} User Manual for GUIDE ver. 31.0.
In particular, GUIDE, CTree, and MOB are compared by investigating the building blocks of 
