...
at zero, and binning of covariates. This shows that specifically the goodness-of-fit measure 
...
\begin{enumerate}
  ...
  \item The dependency or association of this goodness-of-fit matrix with
        ...
        a comparison of all $J$ covariates on a standardized or unified scale and in
...
one-way ANOVA can be used for the non-binarized scores as opposed to the residuals etc.
...
parameter functions $\beta_{k-1}(Z_1)$, $k=1,2=K$, are step functions which is particularly 
...
factorial three-way analysis in the following section.
...
compensates the lack of power of GUIDE+scores compared to CTree and MOB almost completely 
(bottom row, first and third panel).
...
of the investigated subjects. However, the obtained results also hold for a variety of other 
models as the introduced building blocks can be applied in the same way for all model types.
...
$$
W_j(t) = \hat{\Sigma}^{-1/2} N^{-1/2} \sum_{i=1}^{\lfloor Nt \rfloor} s_{\pi(Z_{ji})} \qquad (0 \leq t \leq 1)
$$
...
To scale this partial sum process an estimate $\hat{\Sigma}$ of the covariance matrix $\text{cov}(s(Y,X,\hat{\beta}))$ is included.
...
$$
where $O_{lm}$ are the observed frequencies of observations in a certain combination of binarized residuals ($l=1,2$) and categorized split variables ($m = 1,\ldots,4$). $E_{lm}$ are the corresponding expected frequencies under independence $E_{lm} = \frac{O_{l \cdot} \cdot O_{\cdot m}}{N}$. Under the null hypothesis of independence the asymptotic distribution of $X^2$ is a $\chi^2$~distribution allowing for a straight-forward calculation of $p$-values.
