...
\hlc{GUIDE and MOB, which are both model-based approaches, employ the model estimated 
in the considered node for an exhaustive search of the best split point over all possible 
split points in the already selected split variable.
CTree has not been developed specifically for model-based partitioning, hence, a 
different approach to selecting a split point is suggested in the original paper. 
However, as an adaption of CTree employing models for the split variable selection 
is considered in this study, it is also combined with the same exhaustive search 
strategy as GUIDE and MOB.}
%\newpage
...
\hlc{Therefore, in this study the focus is put on the selected building blocks as they are assumed 
(and have shown) to have the highest influence on the difference in performance between the testing 
strategies.}
...
Even though it has to be noted that the number of replications in this simulation study is of
course limited, the results for this setting indicate that all testing strategies conform well with 
the predefined level of significance $\alpha = 0.05$ as the probability of splitting in any 
of the available covariates is between 4 and 5~percent for $\delta = 0$ 
(the exact values can be found in} Table~\ref{tab:typeIerror} in Appendix~\ref{app:typeIerror})
\hlc{with the tests for structural changes being slightly more conservative} 
\citep[see, e.g.,][for more details on this typical behaviour]{Hothorn+Zeileis:2008, Zeileis+Hothorn:2013, Hansen:1997}.
