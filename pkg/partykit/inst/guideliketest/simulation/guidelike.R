...
##
## Content: test functions (modifications of CTree and MOB, reimplementation of GUIDE, also with modifications)
#################################


##### reimplementation of testfunctions for GUIDE
...
.guide_test <- function(model, trafo, data, subset, weights, j, SPLITONLY = FALSE, ctrl) {
  ...
  if(!ctrl$weighted.scores && !model$unweighted) Y <- Y/weights  
  x <- data[[j]]
  if(!is.null(subset)) {
...
##### ctree and mfluc tests with binarization of scores

## ctree_bin
.ctree_bin_select <- function(...)
  function(model, trafo, data, subset, weights, whichvar, ctrl) {
    args <- list(...)
    ctrl[names(args)] <- args
    partykit:::.select(model, trafo, data, subset, weights, whichvar, ctrl, FUN = .ctree_bin_test)
  }

.ctree_bin_test <- function(model, trafo, data, subset, weights, j, SPLITONLY = FALSE, ctrl) {
  ...
  if (!is.null(iy)) {
    stopifnot(NROW(levels(iy)) == (NROW(Y) - 1))
    return(partykit:::.ctree_test_2d(data = data, j = j, Y = Y, iy = iy, 
                                     subset = subset, weights = weights, 
                                     SPLITONLY = SPLITONLY, ctrl = ctrl))
  }
  ...
  return(partykit:::.ctree_test_1d(data = data, j = j, Y = Y, subset = subsetNArm, 
                                   weights = weights, SPLITONLY = SPLITONLY, ctrl = ctrl))
}
...
## mfluc_bin
.mfluc_bin_select <- function(...)
  function(model, trafo, data, subset, weights, whichvar, ctrl) {
    args <- list(...)
    ctrl[names(args)] <- args
    partykit:::.select(model, trafo, data, subset, weights, whichvar, ctrl, FUN = .mfluc_bin_test)
  }



.mfluc_bin_test <- function(model, trafo, data, subset, weights, j, SPLITONLY, ctrl)
{
  ...
  if(is.null(ctrl$ygroups)){
    ...
    if(length(ctrl$ygroups)>1) ybreaks <- ctrl$ygroups
    if(length(ctrl$ygroups)==1) ybreaks <- quantile(estfun, c(0:ctrl$ygroups)/ctrl$ygroups)
    
    # split estfun according to ybreaks
    estfun <- matrix(cut(estfun, breaks = ybreaks, include.lowest = TRUE), 
                     ncol = NCOL(estfun), dimnames = list(rownames(estfun), colnames(estfun)))
...
##### ctree and mfluc tests with categorization of explanatory variables and binarization of scores

## ctree_cat_bin
.ctree_cat_bin_select <- function(...)
  function(model, trafo, data, subset, weights, whichvar, ctrl) {
    args <- list(...)
    ctrl[names(args)] <- args
    partykit:::.select(model, trafo, data, subset, weights, whichvar, ctrl, FUN = .ctree_cat_bin_test)
  }

.ctree_cat_bin_test <- function(model, trafo, data, subset, weights, j, SPLITONLY = FALSE, ctrl) {
...
## mfluc_cat_bin
.mfluc_cat_bin_select <- function(...)
  function(model, trafo, data, subset, weights, whichvar, ctrl) {
    args <- list(...)
    ctrl[names(args)] <- args
    partykit:::.select(model, trafo, data, subset, weights, whichvar, ctrl, FUN = .mfluc_cat_bin_test)
  }



.mfluc_cat_bin_test <- function(model, trafo, data, subset, weights, j, SPLITONLY, ctrl)
{
