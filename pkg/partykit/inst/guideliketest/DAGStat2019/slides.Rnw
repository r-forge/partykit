...
The testing strategies of CTree, MOB, and GUIDE can be obtained from a\\
general framework by combining building blocks as follows:
...
\vspace{0.5cm}
\textit{Note:}\\
\begin{itemize}\item Adaptions of standard versions are provided.\\
\item Further differences (e.g., approximation of null distribution) remain.
\end{itemize}
\end{frame}
...
subdata <- subset(sim_tree, test %in% c("ctree","mfluc",
                                        "guide_sum_12", "guide_sum_1_cor"))
#subdata <- subset(subdata, xi %in% c(0,0.5,0.8))
subdata$test <- factor(subdata$test, 
                       levels = c("ctree", "mfluc",
                                  "guide_sum_12", "guide_sum_1_cor"),
                       labels = c("CTree","MOB",
...
#prex <- subset(prex, xi %in% c(0,0.5,0.8))
...
\item Include full model scores better than residuals only.
\item Use original values of scores/residuals better than binarized values.
