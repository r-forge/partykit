...
## HCL palette (light)
pallight <- hcl(c(140, 80, 260, 70, 10), 300, 60, alpha = 0.25)
names(pallight) <- c("CTree", "CTree_max", "MOB", "GUIDE", "GUIDE_scores")
...
points(x, y2)
abline(lm(y~c(x,x)), col = "red", lwd = 2)
dev.off()
...
\begin{tikzpicture}
\visible<1>{
\node[ellipse, fill=HighlightBlue!70, align=center, scale = 1, minimum width=90pt, minimum height = 30pt] (n0) at (4, 5) {$Y,X,Z$};
}
\visible<2>{
\node[ellipse, fill=HighlightBlue!70, align=center, scale = 1, minimum width=90pt, minimum height = 30pt] (n0) at (4, 5) {$\mathcal{M}(Y,X;\hat{\beta}$)};
}
\visible<3->{
\node[inner sep=0pt] (lm1) at (4, 4.5)
    {\includegraphics[width=0.25\textwidth]{lm1.jpeg}};
}
\visible<4>{
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n1) at (2, 1) {?};
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n2) at (6, 1) {?};
\draw[-, line width=0.4pt] (3.5, 3.5) -- (2.3,1.6);
\draw[-, line width=0.4pt] (4.5, 3.5) -- (5.7,1.6);
}
\visible<5-6>{
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n1) at (2, 1) {$Y_1,X_1$};
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n2) at (6, 1) {$Y_2,X_2$};
\draw[-, line width=0.4pt] (3.5, 3.5) -- (2.3,1.6) node [midway, left] {\scriptsize $Z \leq \xi$};
\draw[-, line width=0.4pt] (4.5, 3.5) -- (5.7,1.6) node [midway, right] {\scriptsize $Z > \xi$};
}
\visible<6>{
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n1) at (2, 1) {$\mathcal{M}(Y_1,X_1;\hat{\beta_1}$)};
\node[rectangle, fill=HighlightOrange!70, align=center, scale = 1, minimum width=80pt, minimum height = 30pt] (n2) at (6, 1) {$\mathcal{M}(Y_2,X_2;\hat{\beta_2}$)};
}
\visible<7->{
\node[inner sep=0pt] (lm2) at (2,0.5)
    {\includegraphics[width=0.25\textwidth]{lm2.jpeg}};
\node[inner sep=0pt] (lm3) at (6,0.5)
    {\includegraphics[width=0.25\textwidth]{lm3.jpeg}};
\draw[-, line width=0.4pt] (3.5, 3.5) -- (2.3,1.6) node [midway, left] {\scriptsize $Z \leq \xi$};
\draw[-, line width=0.4pt] (4.5, 3.5) -- (5.7,1.6) node [midway, right] {\scriptsize $Z > \xi$};
}
\end{tikzpicture}
...
\medskip
\begin{enumerate}
\item Fit a model to the response $Y$ (and possible covariates $X$).
...
\item Apply a statistical test assessing dependency of the discrepancy measure on each possible split variable $Z_j$.
...
As an example linear models $\mathcal{LM}(Y,X;\beta_0, \beta_1)$
...
$${\color{red}\Downarrow}$$
$$
{\color{red} 2 \cdot r(Y,X,\hat{\beta_0},\hat{\beta_1})}
$$
...
\item {\only<1-3,7->{\color{lightgray}} Binarization of residuals/scores}
\item {\only<1-6>{\color{lightgray}} Categorization of possible split variables}
\end{itemize}
...
\begin{frame}%[fragile]
\frametitle{Simulation Setting (Scenario 1)}
True tree structure:
                ...
                height = NULL, width = NULL) 
{
  ...
  if (align == "centre") 
    align <- "center"
  rval <- function(node) {
    if(node$id == 2) fill <- c("white", "blue")
    if(node$id == 3 | node$id == 4) fill <- c("white", "red")
    if(node$id == 5) fill <- c("white", "darkgreen")
...
\begin{frame}%[fragile]
\frametitle{Results}
Full factorial analysis (with both parameters varying):
\begin{figure}%[t]
\setkeys{Gin}{width=1\linewidth}
...
\begin{frame}%[fragile]
\frametitle{Simulation Setting (Scenario 2)}
True tree structure:
...
\frametitle{Software}
\vspace{0.4cm}
\textbf{Package:} \emph{partykit} available on CRAN\\

\bigskip

\textbf{Reproduction code:} available on R-Forge at\\
