...
\definecolor{dred}{RGB}{119,16,26}
...
\section{The Power of Unbiased Recursive Partitioning}
...
\begin{itemize}
  \item Proposed as unbiased recursive partitioning for nonparametric modeling.
  \item Based on conditional inference (or permutation tests).
...
\begin{itemize}
  \item Model-based recursive partitioning using M-estimation (ML, OLS, CRPS, \dots).
  \item Based on parameter instability tests.
  \item Adapted to various psychometric models: Rasch, PCM, Bradley-Terry, MPT, SEM, networks, \dots.
\end{itemize}
...
\vspace{-0.3cm}
$$\hspace{-0.3cm}{\color{dred}\Downarrow} \hspace{3.4cm} {\color{dred}\Downarrow} $$
$$
{\color{dred} -2 \cdot r(Y, X, \hat{\beta_0},\hat{\beta_1})}  \hspace{0.7cm} {\color{dred} -2 \cdot r(Y, X, \hat{\beta_0},\hat{\beta_1}) \cdot X}
$$
...
\item If the significance tests perform well pre-pruning works well,\\
...
Schlosser L, Hothorn T, Zeileis A (2019).
  ...
  arXiv:1906.10179, \emph{arXiv.org E-Print Archive}.
