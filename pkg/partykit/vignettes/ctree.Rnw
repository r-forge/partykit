...
%\VignetteDepends{coin, TH.data, survival, strucchange, Formula, sandwich}
...
\item[\code{multiway}]: Implements multiways splits in unordered factors,
...
covariates are removed from the user interface.
