...
plot(bh_tree)
@
\includegraphics[width=18cm,keepaspectratio,angle=90]{mob_R1-BostonHousing-plot}
...
@
%
With this function a censored Weibull-regression-tree can
...
%%% NOTE: R CMD build tries, with R1 in .Rbuildignore, to build this file
%%% which results in an error (because Formula::model.part(mobobj$info$Formula, mf, lhs = 1L, rhs = 0L)
%%% won't work in R2. I commented this specific figure only.
...
%
%\setkeys{Gin}{width=\textwidth}
<<GBSG2-scatter, echo=FALSE, eval = FALSE, results=hide, fig=TRUE, height=6, width=9>>=
