...
\resizebox{0.9\textwidth}{!}{
  \begin{tikzpicture}
    ...
    \visible<11>{
    \node[rectangle, fill=HighlightOrange!70, align=center, scale = 0.7, minimum width=60pt, minimum height = 20pt] (n1) at (0, 0) {$\text{vM}(Y_1;\hat{\theta_1}$)};
    \node[rectangle, fill=HighlightOrange!70, align=center, scale = 0.7, minimum width=60pt, minimum height = 20pt] (n2) at (1.7, 0) {$\text{vM}(Y_2;\hat{\theta_2}$)};
    }
    \visible<11->{
    \draw[-, gray, line width=0.5pt] (0.7, 1.1) -- (n1) node [midway, left] {\scriptsize $X \leq p$};
    \draw[-, gray, line width=0.5pt] (1.0, 1.1) -- (n2) node [midway, right] {\scriptsize $X > p$};
...
\item Circular GLM: Based on current wind speed and wind vector $(u, v)^{\top}$.
\end{itemize}
...
\textbf{Correspondingly:} \emph{disttree} for any \code{gamlss.family} or custom \code{family}.\\
