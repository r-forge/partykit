...
# - PURPOSE:
# -------------------------------------------------------------------
# - L@ST MODIFIED: 2019-09-12 on thinkmoritz
# -------------------------------------------------------------------

## Script version for output files
my_vers <- "v1"
...
## Save predictions
saveRDS(pred_pers, file = sprintf("results/circforest_pred_pers_vie_lag6_%s.rds", my_vers))
...
## Save predictions
saveRDS(pred_clim, file = sprintf("results/circforest_pred_clim_vie_lag6_%s.rds", my_vers))
...
for(cv in unique(cvID)) { 
  ...
  saveRDS(m_ct, 
    file = sprintf("results/circforest_model_tree_vie_lag6_%s_cv%s.rds", my_vers, cv))
  saveRDS(pred_ct.tmp, 
    file = sprintf("results/circforest_pred_tree_vie_lag6_%s_cv%s.rds", my_vers, cv))
  saveRDS(pred_cf.tmp, 
    file = sprintf("results/circforest_pred_forest_vie_lag6_%s_cv%s.rds", my_vers, cv))
...
for(cv in unique(cvID)) {
  pred_ct.tmp <- readRDS(file = sprintf("results/circforest_pred_tree_vie_lag6_%s_cv%s.rds", my_vers, cv))
  pred_cf.tmp <- readRDS(file = sprintf("results/circforest_pred_forest_vie_lag6_%s_cv%s.rds", my_vers, cv))
  pred_ct[[cv]] <- pred_ct.tmp
  pred_cf[[cv]] <- pred_cf.tmp
  rm(pred_cf.tmp, pred_ct.tmp); gc()
}

pred_pers <- readRDS(file = sprintf("results/circforest_pred_pers_vie_lag6_%s.rds", my_vers))
pred_clim <- readRDS(file = sprintf("results/circforest_pred_clim_vie_lag6_%s.rds", my_vers))
...
save(pred, pred_naomit, obs, obs_naomit, 
  file = sprintf("results/circforest_results_vie_lag6_%s.rda", my_vers))


# -------------------------------------------------------------------
# Validate predictions
# -------------------------------------------------------------------
load(file = sprintf("results/circforest_results_vie_lag6_%s.rda", my_vers))
...
save(crps, crps.boot, crps_skill, crps.boot_skill, 
  file = sprintf("results/circforest_validation_vie_lag6_%s.rda", my_vers))
...
if(FALSE){
  ...
  load(file = sprintf("results/circforest_validation_vie_lag6_%s.rda", my_vers))
  ...
  dev.print(pdf, sprintf("results/_plot_circforest_validation_crpsraw_vie_lag6_%s.pdf", my_vers))
  ...
  dev.print(pdf, sprintf("results/_plot_circforest_validation_crpsraw_boot_vie_lag6_%s.pdf", my_vers))
  ...
  dev.print(pdf, sprintf("results/_plot_circforest_validation_crpsskill_vie_lag6_%s.pdf", my_vers))

  X11(width = 8, height = 4.5)
  par(mar = c(3.1, 4.1, 2.1, 2.1))
  boxplot(crps.boot_skill, ylim = c(-60, 110), col = gray(0.6), 
    ylab = "CRPS skill ccore [%]", main = "Boot-strapped mean values")
  dev.print(pdf, sprintf("results/_plot_circforest_validation_crpsskill_boot_vie_lag6_%s.pdf", my_vers))

  ### Plot single tree
  #cv <- 1
  #m_ct <- readRDS(file = sprintf("results/circforest_model_tree_vie_lag6_%s_cv%s.rds", my_vers, cv))
