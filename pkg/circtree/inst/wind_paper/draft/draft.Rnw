...
potential forecast range of the different methods. For the specific task of
wind forecasting not only the wind direction, but also the wind speed and
especially their combination is of high interest. To account for both
quantities simultaneously, distributional tree-based models could be built for
wind vectors employing the bivariate normal distribution as performed by
\citet{Lang+Mayr+Stauffer:2019} in an additive regression framework.
Alternatively, circular regression trees could be combined with distributional models for
wind speed employing the truncated normal distribution
\citep{Thorarinsdottir+Gneiting:2010}. This could be pursued either in a
two-step approach with two independent distributional models or in a more
sophisticated way employing a mixed response distribution in the presented
tree-building algorithm. Another possible improvement for obtaining more
