...
regress its parameters on covariates, a circular distributional tree is
proposed as an alternative to more commonly-used additive models. The resulting
circular trees are easy to interpret, can detect non-additive effects, and
select covariates and their interactions automatically.  By combining an
ensemble of circular trees a more stable and regularized circular
distributional forest can be built which allows for modeling smooth effects as
well.

To evaluate the proposed circular trees and forests, a case study on
...
covariates. \cite{Johnson+Wehrly:1978} took this idea one step further by
plugging in a link function such that the values of the linear predictor are
transformed to a restricted interval of length $2\pi$. This generalized linear
...
function which maps the values of the real line to the unit circle. Moreover,
the presented methodology allows for both distribution parameters, the mean
direction and the concentration parameter of the von Mises distribution, to be
modeled depending on covariates, not only in separate models but also in a
mixed model. While being built on well elaborated theory this methodology has
shown to be very challenging in their application, mainly due to the complexity
encountered in optimizing the log-likelihood function which is not globally
concave. Therefore, highly informative starting values are crucial for the
model to converge at all \citep{Pewsey+Neuhaeuser+Ruxton:2013,
...
regression models. While these generally provide a powerful toolbox for
modeling particularly smooth effects, they may not be able to capture all types
of effects adequately. Especially abrupt changes might not be detected
properly owing to the enforced additive structure and model assumptions, such
as a possibly highly non-linear link function and the form of linear
combinations.
...
model can be obtained for each observation $y_i$ and fitted parameter.
...
% -------------------------------------------------------------------
% Circular distributional trees and forests
% -------------------------------------------------------------------
...
effects more precisely and hence can result in a overall better fitting model.
...
corresponds to the abrupt splits from a tree. Thus, circular distributional
...
circular distributional trees and forests, the case study employs various
...
the their vicinity. For Innsbruck, measurements at the airport and along the
intersecting valleys are used, whereas, for Vienna, measurements at the airport
and within an area of about $10$\,km$^2$ are used. A topographical overview of
...
%%
\begin{table}[t!]
  \caption[Table caption text]{Overview of the employed data sets for the case
...
\begin{itemize}
  \item \emph{Climatological model:} 
    ...
    \citet{vogel.etal:2018} and is discussed in a comprehensive summary on
    different time-adaptive training schemes in \citet{lang.etal:2019c}.
    %%
  ...
  \item \emph{Generalized linear model:}
    ...
    Section~\ref{sec:introduction}, circular regression models can experience the
  ...
  \item \emph{Circular distributional tree:}
    For the circular distributional tree introduced in Section~\ref{sec:circtree},
...
by the circular distributional tree and, except for the $3$\,h forecast at
Innsbruck, the persistency model. In comparison to the circular tree and
...
lower performance for longer lead times; this is probably due to minimum
3-hourly instead of 1-hourly lagged response values employed as covariates in
the persistency model. The circular distributional tree and forest seem to
...
\begin{figure}[t]
  ...
  \caption{CRPS skill scores for wind direction forecasts based on the full
    ...
    per for each of the $12$~months averaged over the $5$~validation years which
...
shows that even if the persistency model's performance is lower than the
reference (Panel~c) the tree-based models are significantly superior to the
climatological model used as reference.
%%
\begin{figure}[t]
  \centering
  \setkeys{Gin}{width=1.0\textwidth}
...
\label{sec:summary}
Circular distributional trees for circular responses are established by
...
particularly useful for modeling a highly fluctating response such as wind
direction. \dots

\begin{itemize}
  \item Add summary/conclusion of case study;
  \item Add summary/conclusion of circular trees and forests.
...
package \textbf{circtree} (version~\Sexpr{packageVersion("circtree")}). The
package is based on the \textbf{disttree} package
...
\cite{Mulder+Klugkist:2017}. In particular the function \textbf{circGLM} is
