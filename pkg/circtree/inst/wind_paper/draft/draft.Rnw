...
To fit a probabilistic model $\text{vM}(Y;
...
$2\pi$.  Typically $g(x) = 2\cdot arctan(x)$ is employed, as suggested by
...
\begin{itemize}
  ...
  \item \emph{Generalized linear model:}
    ...
    2-dimensional wind vector $\bm{vec} = (vec_1, vec_2)^{\top}$ and the lagged
