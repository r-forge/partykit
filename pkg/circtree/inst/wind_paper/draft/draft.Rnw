...
\citet{Hara+Chellappa:2017} presented a distribution-based tree
building algorithm by applying $k$-means clustering to the target space where
...
approach that considers both splits in all distribution parameters and yields a
...
together with a comparison with commonly-used alternative approaches.
...
this application. An alternative tree-building framework is provided by the MOB
