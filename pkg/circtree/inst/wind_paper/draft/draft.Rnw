...
as an inuitive alternative approach that is relatively easy to fit. Building on
...
introduced a circular regression tree algorithms where binary splits are
...
In contrast, here we introduce a distributional tree approach that considers
...
the airport as well as spatial and temporal differences.
...
The data used in this study consists of four subsequent years from January 2015
