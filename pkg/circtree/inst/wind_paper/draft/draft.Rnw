...
\fixme{refer to data, Innsbruck airport}
}
...
bear the risk of not converging if not applied thoughtfully. \fixme{citation} %\citealp{}.
...
As the linear version of the CRPS it measures the difference between an observation
and the corresponding predicted distribution function in order to assess the probabilistic goodness 
of fit. In order to adjust this general methodology to circular data an additional auxiliary function
is employed which measures the angular distance between two circular variables. 
The resulting values are summed up over each observation contained in the learning data and the 
corresponding predicted distribution parameters of each of the models.
...
\fixme{information on applied implementation}
...
As winds with very low speed tend to vary their direction strongly they are not expected to be very 
useful/meaningful for predictions. Moreover, the performance and the stability of the employed reference 
models might suffer from the influence of these ``noise'' observations. Therefore, only observations
corresponding to a wind speed of at least $1$ m/s are considered.

\fixme{information on NAs}
