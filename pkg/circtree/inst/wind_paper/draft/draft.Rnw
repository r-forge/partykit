...
directions, periodic measurements of neuronal activity or wind directions has shown
to be very challenging. Commonly applied generalized linear and additive models 
can provide a useful toolbox, however, non-additive effects might not be detected 
and difficulties may arise in modeling smooth effects due to highly non-linear link 
...
%models. 
The resulting circular regression trees are easy to interpret, can detect non-additive effects, 
and select covariates and their interactions automatically. By combining an
ensemble of circular regression trees a more stable and regularized circular
...
(see Section~\ref{sec:prob_circ} for more details). This is also the basis
which early regression models for circular data are built on as they regress
the parameter(s) of the von Mises distribution on covariates. One of the first
...
parameter constant. Additionally, they introduce a combined hetereoscedastic version 
...
Therefore, highly informative starting values are crucial for the model to converge at all 
...
present an alternative approach to regression modeling of circular data by employing a GLM in a 
Bayesian framework, however, only the mean direction is regressed on covariates.
...
however, in the case of circular data a possibly highly non-linear link function
...
Moreover, not all types of effects may be modeled adequately. Especially abrupt changes 
might not be detected properly owing to the enforced additive structure and selected model 
specification such as the form of linear combinations.
%% FIXME: clear explanation of difficulties close to 0

In order to avoid these difficulties in probabilistic regression modeling of circular data 
(and also the requirement of initial values) we propose an alternative and very flexible 
...
The resulting circular regression trees and forests 
(for short: \emph{circular trees and forests}) 
do not require any model assumptions as covariates and their possible interactions are
selected automatically. Non-additive effects are captured by the tree structure while 
circular forests allow for modeling smooth changes as well. Thus, a high level of 
flexibility is provided while a full distributional model is specified in each terminal 
node offering a wide range of inference. Moreover, due to the parameters being estimated 
as constants for each subgroup difficulties connected to ``circular shifts'' close to $0$ 
are avoided.
...
%In particular, 
\cite{Lund:2002} have already introduced the concept of circular regression 
trees where binary splits are made based on a circular distance which is evaluated between 
...
While this approach and also further extensions \citealp{Larraondo+Inza+Lozano:2018} allow not only for 
linear but also for a circular covariate, the tree splits are performed based on effects in the expected 
value only, i.e., only the location parameter $\mu$ is regressed on covariates.
...
In already existing generalized linear or additive approaches to circular regression modeling 
usually the location parameter $\mu$ of is regressed on 
covariates in the general form 
...
\mu = \mu_0 + g(\beta^{\top} x).
\end{equation}
with a circular intercept $\mu_0$ and $x$ being a vector of covariates
with the corresponding vector of coefficients $\beta$ that are to be estimated.
For example, \cite{Johnson+Wehrly:1978} guarantee for the link function to return values
on an interval of length $2\pi$ by setting $g(x) = 2\pi \cdot F(x)$ for a cumulative 
distribution function $F$ while \cite{Fisher+Lee:1992} suggest to use
$g(x) = 2 \cdot arctan(|x|^{\lambda} \cdot sgn(x))$. 
The concentration parameter $\kappa$ is mostly estimated as a constant, however,
\cite{Fisher+Lee:1992} also present a model where $\kappa$ is regressed on covariates
employing the exponential function as a link function while $\mu$ is assumed to be a fixed
value. But regardless of whether $\mu$ or $\kappa$ depend on covariates,
both variations have shown to be very complex in their optimization procedure and hence
challenging in their application. Consequently, these difficulties intensify even more in 
a combined model where the sets of regression 
coefficients for the two distribution parameters $\mu$ and $\kappa$ respectively are reestimated 
alternatingly in an iterative optimization algorithm. Hence, regardless of difficulties in practice, 
the presented theory provides a first approach to probabilistic regression modeling of circular data, 
however, the parameters are not regressed simultaneously on covariates as for example in a more general
case by generalized additive models for location, scale, and shape 
\citep[GAMLSS, ][]{Rigby+Stasinopoulos:2005}.
Moreover, the additive structure of the discussed models might impose a smooth effect even if the 
true underlying effect is an abrupt shift.
One way how to deal with or even avoid the mentioned difficulties in circular regression modeling
is to employ tree-based models as presented in the following Section.
%% FIXME: better description of difficulties?
...
%% FIXME: what are we aiming at? which problems do we want to avoid/solve?
%% mu and kappa as constants in subgroups, no initial values
%% Lund: what do they present (including formulas)
%% compare, what is different, how do we improve this approach
...
particularly of CTree, can be found in Appendix~\ref{app:tree}.
...
\begin{appendix}
\section{Tree-building algorithms}
\label{app:tree}
CTree, MOB
\end{appendix}
\end{document}
