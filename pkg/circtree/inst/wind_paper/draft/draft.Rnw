...
due to the highly non-linear link function. Consequently, the representation of smooth transitions 
on the unit circle is not straightforward. In addition, the same rotation can be obtained in either 
...
of the group-specific distribution parameters.
Moreover, by incorporating the framework of distributional modeling we allow for detecting 
...
Furhtermore, covariates and their possible interactions do not need to specified in advance as they are
