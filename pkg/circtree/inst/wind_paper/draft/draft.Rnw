...
trees where binary splits are made based on an angular distance measure employed to evaluate node homogeneity.
...
usually the location parameter $\mu$ is regressed on covariates in the general form 
...
\mu = \mu_0 + g(\beta^{\top} z).
\end{equation}
with a circular intercept $\mu_0$, a vector of covariates $z$
with the corresponding vector of coefficients $\beta$ that are to be estimated,
and a link function $g$.
...
parameters depends on covariates a combination of them is introduced in a joint model. There, the 
...
for only one parameter being regressed on covariates and consequently even more so in the combined model.
This is mainly due to characteristics of the log-likelihood function of the employed von Mises distribution
with the plugged-in link function. In particular, the function is not globally concave and possibly very 
sharp local maxima can prevent the algorithm from converging.
...
%% FIXME: difficulties in modeling smooth effects on the unit circle
