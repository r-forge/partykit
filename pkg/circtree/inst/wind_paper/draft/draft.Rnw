...
\begin{itemize}
  \item \emph{Climatological model:} 
    ...
    direction.
    \newline
    For simplicity, rather to fit one single climatological model, a single model is estimated 
    rolling over all observation time points, 
    using the $31$~days centered around the day of interest for all available years in the training data.
