...
\citep{Breiman:2001} to conditional mean models for cicular data. But rather
than considering only binary splits, they employed $k$-means clustering
to determine multi-way splits with the number $k$ adaptivaly determined
...
ensemble aggregation of the predictions across trees in a forest. Therefore,
this yields a full probabilistic predictive model as proposed by
\citet{Gneiting:2008}.
