...
As a motivating example Figure~\ref{fig:tree_ibk} displays a distributional
tree applied to predict the wind direction at the airport of Innsbruck in the
following hour.  Innsbruck lies at the bottom of a deep valley in the Alps.
...
Vienna is presented in Section~\ref{sec:wind} where their performances are
benchmarked to alternative commonly used approaches.  Beforehand,
distributional models for circular data are discussed in
Section~\ref{sec:prob_circ} and the methodology of circular regression trees
and forest and its features are presented in Section~\ref{sec:tree_forest}.
After the case study, a comprehensive summary and conlusion is given in
...
A common way of including covariates is to regress the model parameters on the covariates.
...
appropriate link function plugged in as in GLMs \citep{Fisher+Lee:1992, Fisher:1993}.
In a more general setting this idea can be extended by generalized additive models for location, scale, 
and shape \citep[GAMLSS, ][]{Rigby+Stasinopoulos:2005} where each distribution parameter is estimated
by its own generalized additive model (GAM). However, an adaption of this methodology to circular data
can not be obtained by the straight forward approach of simply wrapping the modelled additive effects
around the unit circle since the parameters of the von Mises distribution and their dependence/interaction 
differs strongly from the linear case and therefore requires specific handling.
Moreover, difficulties might arise in case of non-additive changes and specifying a model properly can 
be very challenging, particularly for a high number of covariates and no information on possible 
interactions.
