...
allowing for the mean direction or the dispersion to be modeled not only in separate models 
but also in a mixed model. However, applying these models has shown to be very challenging, 
mainly due to the complexity encountered in optimizing the log-likelihood function.
Moreover the named approaches and also its further developments 
...
selected automatically. Non-additive effects are captured by the tree structure while combining an ensemble
of trees to a forest model allows for modeling smooth changes as well. 
In that way, a high level of flexibility is provided while a full distributional model is specified by distributional trees and forests such that a wide range of inference is made available.
...
Along with distributional trees for circular data and forests three other models have been applied 
and their performances have been compared based on a circular version of the continuous ranked probability 
score (CRPS).
...
Five-fold cross validation (four years as learning data, one year as testing data) ...
