...
additive effects and their interpretation are not intuitive for circular responses.
Due to the highly non-linear link function, the representation of smooth transitions on the unit circle
is not straightforward; additionally, the same rotation
...
\newline
Referring to additive models in general and hence also to their application on circular data, it has to
be considered that a proper model specification can be very challenging, particularly for a high number 
of covariates and no information on possible interactions. Moreover, the additive structure might impose 
a smooth effect even  if the true underlying effect is an abrupt shift.
\newline
...
easy-to-apply modeling approach which allows to detect additive as well as abrupt changes while
accounting for a proper representation on the unit circle. 
