...
of any continuous distribtuion around the unit circle applying the modulo operation on the 
interval length $2\pi$ \citep{Mardia+Jupp:2009}.
In that way, for example the wrapped Cauchy distribution or the wrapped normal distribution can be employed to model symmetric unimodal circular data.
...
One way how to deal with these problems is to apply a tree-structured model.

\subsection{Circular Distributional Tree}
\label{sec:circtree}
...
Once a circular distributional tree model is fit it can be employed to obtain probabilistic predictions
for a possibly new set of observations of the covariates $\bm{z} = (z_1, \ldots, z_m)$.
Starting at the root node the tree structure leads the observation to a terminal node depending on the values of the covariates and in that way to the corresponding estimated parameter pair $(\hat{\mu}, \hat{\kappa})$ of this node. Therefore, the same parameter pair is estimated for all observations
ending up in the same terminal node. This facilitates the application as the parameter estimates do not need to be recalculated for each (new) observation via maximum likelihood but can be extracted directly
from the learning sample and the fitted model. This can also be expressed by employing weights
...
\subsection{Circular Distributional Forest}
...
%\citep{Lin+Jeon:2006}
is obtained that is based on how often $\bm{z}$ is assigned to the same terminal node as
...
its specific parameter estimates
\begin{equation}
...
Next to circular distributional trees and forests three other models have been applied and their 
