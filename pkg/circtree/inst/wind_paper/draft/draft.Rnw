...
on covariates, not only in separate models but also in a mixed model. There, within an iterative 
...
While being built on well elaborated theory this methodology and the contained 
...
\citep{Pewsey+Neuhaeuser+Ruxton:2013, Gill+Hangartner:2010}. Therefore, the selection of 
starting values is crucial and bears a high risk of the model not to converge if not done thoughtfully. 
In order to avoid this strong dependence on choosing well-fitting initial values \cite{Mulder+Klugkist:2017} presented an alternative approach to regression modeling of circular data by employing a GLM 
...
While they generally provide a powerful toolbox for modeling particularly smooth effects, 
it has to be considered that representing these smooth effects on the unit circle
can be challenging. Moreover, the discussed additive models may not be able to capture all 
types of effects adequately. For example, abrupt changes might not be detected properly owing 
to the enforced additive structure and model assumptions such as the type of link 
function and the form of linear combinations.
...
which was further explored by \cite{Larraondo+Inza+Lozano:2018}. This methodology
enables splits leading to subsets which contain the ``origin point'' $0$ equal to $2\pi$
and in that way allows for a proper representation of periodicity. 

Following the idea of employing tree-based methods but including also the probabilistic concept
...
As a motivating example Figure~\ref{fig:tree_ibk} displays a distributional tree applied to 
predict wind directions at the airport of Innsbruck in the following hour. Wind direction is 
a classical circular quantity and accurate forecasts are of great importance for decision-making 
processes and risk management in air traffic management. Innsbruck lies at the bottom of a 
deep valley in the Alps where topography channels wind along the west-east valley axis or along 
a tributary valley intersecting from the south. Hence, pressure gradients to which valley wind 
regimes react are considered as covariates along with other meteorological measurements at the 
airport (lagged by one hour), such as wind direction and wind speed at the airport.

Together with the tree structure itself Figure~\ref{fig:tree_ibk} also depicts the 
...
A more extensive case study of circular regression trees and forests applied to 
probabilistic forecasting of wind direction at the airports of Innsbruck and Vienna 
is presented in Section~\ref{sec:wind} where their performances are benchmarked to 
alternative commonly used approaches.  Beforehand, distributional models for circular 
data are discussed in Section~\ref{sec:prob_circ} and the methodology of circular 
regression trees and forest and its features are presented in Section~\ref{sec:tree_forest}.
After the case study, a comprehensive summary and conclusion is given in
...
in the mixed model of \cite{Fisher+Lee:1992} and \cite{Fisher:1993}. While applying the mixed model 
...
Moreover, difficulties might arise in case of non-additive changes and specifying a model 
properly can be a very tough task, particularly for a high number of covariates and no information
...
\subsection{Data}
As response variables, $10$\,min mean wind direction observations are employed at
the airports Innsbruck and Vienna on an hourly temporal resolution. As
predictor variables, we use observations of various meteorological quantities
such as wind direction, temperature, air pressure and humidity directly at the
respective airports, as well as within the vicinity of the airports. For
Innsbruck, meteorological variables are observed at XXX different locations
along the intersecting valleys; whereas for Vienna, located in the flat,
meteorological variables are observed at XXX different locations within an area
of about $10$\,km$^2$. In addition, we employ temporal differences, averages, minima, maxima over the last 3\,h, 

of ias well as tempat each of the weather stations also differences in these 
...
For a fair evaluation of circular regression trees and forest and to
...
on existing approaches in the field of meteorological forecasting while the
third is a state-of-the-art model to forecast circular response variables. The
next paragraphs introduce the reference methods and provide the corresponding
model specifications as employed in this study for all considered model
including the circular tree and forest. 

\paragraph{Circular regression tree}
...
\paragraph{Circular regression forest}
...
\fixme{not done like this, always using the whole data set. wrong?}

\paragraph{Climatological model}
...
In addition to the raw CRPS, CRPS-based skill scores are computed to assess
differences in the improvement of the various statistical models over the
climatological model used as a reference:
\begin{equation} 
  \text{CRPSS}_{\text{method}} = 1 -
\frac{\text{CRPS}_{\text{method}}}{\text{CRPS}_{\text{climatology}}}.
\end{equation}

All scores presented in the next section are out-of-sample: For the persistency
model only time points prior to the time of interest are used, for all other
models a five-fold cross-validation is employed using up to four calendar years
for model training and the remaining single calendar year for validation. Due
to the large sample size of 24 hourly values per day over five years, some kind
of temporal aggregation is needed to ensure a correct visual comparison of the
individual methods. As observed for the employed models, the variability of the
predictive performance over the five years is lower than over a single day or
over a single year. Hence, CRPS and CRPS skill scores are aggregated over the
...
performance, followed by the circular regression tree and, except for the
...
climatological model and the linear model show clearly higher CRPS scores and
hence a lower predictive performance.

The different site characteristics between the airports Innsbruck
(Figure~\ref{fig:boxplot_crpsraw}a,\,c) and Vienna
(Figure~\ref{fig:boxplot_crpsraw}b,\,d) seem to have an effect on the absolute
level of the model performances and on their respective performance
variation. At Innsbruck, due to the surrounding mountains a limited number of
possible wind directions exists, namely the three wind regimes discussed for
...
potential high prediction errors in case of a paradigm shift seem to lead to
a higher variation in the predictive performance for all models in comparison
to Vienna; this variation is especially high for the persistency model due to
its strong vulnerability to abrupt wind shifts. On the contrary, at Vienna the
...
regression tree and forest seem to partially compensate for the lower skill of
...
\begin{figure}[t]
  ...
  \caption{CRPS skill scores for wind direction forecasts based on the full
    ...
    persistency and the linear model as well as for the circular regression tree and
    forest.}
...
model is lower for Innsbruck due to the few number of predominant wind regimes
