...
follows a circular distribution, in particular the von Mises distribution (which will be discussed 
in more detail in Section~\ref{sec:prob_circ}). This was also the basis on which early regression
models for circular data were built on as they regress the distribution parameter(s) on covariates.
One of the first regression models with a circular response variable and linear covariates
was presented by \cite{Gould:1969} where a linear combination of the covariates is employed to model
the circular mean, hence the location parameter of the von Mises distribtuion. 
\cite{Johnson+Wehrly:1978} took this idea one step further 
by plugging in a link function such that the values of the linear predictor are transformed 
to an restricted interval of length $2\pi$. 
...
unit circle. Moreover, the presented methodology allows for the mean direction or the dispersion 
to be modeled not only in separate models but also in a mixed model. However, applying these models 
has shown to be very challenging, mainly due to the complexity encountered in optimizing the 
log-likelihood function.
Additionally, the named approaches and also its further developments 
\citep[see, e.g.,][]{Jammalamadaka+Sengupta:2001, Mulder+Klugkist:2017}
are built on additive regression models which may not be able to capture all types of effects.
For example abrupt changes might not be detected properly owing to the fixed structure enforced
by model assumptions such as the type of link function and the type of linear combination.
Therefore, we propose an adaption of regression trees and random forests to circular data by employing 
distributional trees and distributional forests. 
...
In that way, a high level of flexibility is provided while a full distributional model is specified 
by distributional trees and forests such that a wide range of inference is made available.
These features and the corresponding methodology are presented in Section~\ref{sec:tree_forest} after discussing 
distributional models for circular data in Section~\ref{sec:prob_circ}.
In Section~\ref{sec:wind} the presented models a applied to probabilistic forecasting of wind direction
...
For circular data the mixed model by \cite{Fisher:1993} follows the idea of modeling both
parameters of the von Mises distribution, however, in an iterative optimization procedure
requiring well fitting initial values for the parameters that are recommended to be calculated 
in advance in two separate models. Using the obtained initial value the model parameters are 
estimated and improved stepwise until convergence. While providing a well elaborated theory 
this methodology and the contained optimization problem have shown to be very complex and 
bear the risk of not converging if not applied thoughtfully \textit{FIXME: citation}. %\citealp{}.
Furthermore, these additive strategies work well for smooth effects but difficulties can arise 
in case of non-additive changes. Additionally, specifying a model properly can be very challenging,
...
As a motivational example Figure~\ref{fig:tree_ibk} illustrates a distributional
...
Next to the tree structure itself Figure~\ref{fig:tree_ibk} also depicts the empirical (gray) 
...
\subsection{Data}
For both airports observations of various meteorological quantities measured
either directly at the airports or at surrounding weather stations are used as 
covariates in order to predict the wind direction at the corresponding airport 
in the following hour.
Next to the current wind direction, temperature, air pressure, humidity, ... at each 
of the weather stations also differences in these quantities between the considered 
stations are included. For example, differences in air pressure between Innsbruck and 
either a western or eastern location are expected to have a high influence on wind 
directions. Moreover, changes of these quantities over time are included as well as 
daytime. In total, 550 covariates are provided for Innsbruck and 522 covariates for Vienna. 
...
\subsection{Evaluation Setting and Models Applied}
...
ranked probability score (CRPS) is calculated and summed up over each observation contained in
the learning data and the corresponding predicted distribution parameters of each of the models.

To test whether distributional trees and forests can be applied as a reasonable alternative to already
existing approaches to modeling circular data, particularly in the field of meteorological forecasting,
three commonly used models are selected as reference models.
...
Since a high number of possible split points lead to a high computational effort that is 
...
% TODO: references
To establish a distributional model of persistence for this study a set of six observations
is considered to make a prediction for a selected day and time point. In particular,
the exact same time points of the previous six days are considered. In order to account
for a stronger influence of observations closer to the prediction time point exponentially
increasing weights are included with the highest weight being assigned to the time point of the
previous day. For this selected set of observations the distribution parameters of the von Mises 
distribution can be estimated as presented in Section~\ref{sec:prob_circ} such that a full
parametric model is specified allowing for probabilistic predictions and an evaluation of the CRPS.
...
for a selected day of the year weather conditions will always be the same over all years 
considered.
...
% TODO: references
Similar to the applied persistence model observations are selected to fit a distributional climatology
model. For a specified day and time point observations from the exact same time point and the same day 
as well as the 30 surrounding days from all other years contained in the learning data are selected and
parameters are estimated following again Section~\ref{sec:prob_circ}.

\paragraph{Generalized Linear Model}
Another commonly applied approach is to regress the parameter(s) of the von Mises distribution
on covariates by a linear combination of selected covariates connected to the parameter(s) via
a specified link function. The toolbox of generalized linear models ...
% TODO: references
