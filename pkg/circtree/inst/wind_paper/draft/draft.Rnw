...
Moreover, not all types effects of may be modeled adequately. Especially abrupt changes 
...
Already existing generalized linear or additive approaches to circular regression 
models employ linear combinations of the covariates to connect them to one (or more) 
distribution parameters. 
%For the mean direction~$\mu$ \cite{Gould:1969} proposes the model 
%\begin{equation}
%\mu = \mu_0 + \sum_j \beta_j\cdot x_j(\textit{mod } 2\pi)
%\end{equation}
%with a circular intercept $\mu_0$ and $x$ being a vector of covariates
%with the corresponding vector of coefficients $\beta$.
By following this idea and including a link function $g$ many other models
have been developed in the general form 
\begin{equation}
\mu = \mu_0 + g(\beta^{\top} x).
\end{equation}
For example, \cite{Johnson+Wehrly:1978} guarantee for the link function to return values
on an intervall of length $2\pi$ by setting $g(x) = 2\pi \cdot F(x)$ for a cumulative 
distribution function $F$ while \cite{Fisher+Lee:1992} suggest to use
$g(x) = 2 \cdot arctan(|x|^{\lambda} \cdot sgn(x))$. The concentration parameter


%% FIXME: information on already existing generalized linear and additive approaches
%% with formulas (\mu = g(X \beta) + \mu_0)   FIXME: add mu_0?
%% clear description of difficulties 
