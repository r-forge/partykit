...
challenging to optimize and to interpret. Therefore, we suggest circular
...
regression forest yields a local adaptive likelihood estimator for the
von~Mises distribution that can regularize and smooth the covariate effects.
...
\end{equation}
%%
Figure~\ref{fig:densities} depicts von~Mises model for circular data in $[0, 2
...
resulting forests allow for modeling smooth effects and stabelize the model.
