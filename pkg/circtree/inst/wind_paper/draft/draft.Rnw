...
distributional trees and forests and their features are introcuded in
...
management in various fields such as agriculture, industry or energy
production. Thus, for example in order to direct airplanes to a safe landing,
precise knowledge of wind direction at the respective airport for the next hour(s) is highly
desirable. This section provides a comprehensive case study on probabilistic
wind direction forecasting at two airports in Austria with fundamentally
...
%
%highly fluctating, intersting variable for trees,,, requires high flexibility
%many covariables, few variables have influence, hihgly non-linear non-known interactions
%airports vienna and innsbruck
%lead times 1 and 3 hours
%
% plus figures table and fixmes
...
climatological model by using maximimum likelihood estimation and fitting the distribution
