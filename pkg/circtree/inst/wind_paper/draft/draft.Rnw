...
\title{Distributional Trees and Forests for Circular Data with an Application to Probabilistic Wind Direction Forecasting}
...
This was also the basis which early regression models for circular data were built on as they 
regress the distribution parameter(s) on covariates.
...
\cite{Johnson+Wehrly:1978} took this idea one step further by employing generalized linear 
...
on covariates. While providing a well elaborated theory this methodology and the contained optimization 
problem have shown to be very challenging, mainly due to the complexity encountered in optimizing the 
log-likelihood function that is not globally concave, and thus bear a high risk of not converging if 
not applied thoughtfully \citep{Pewsey+Neuhaeuser+Ruxton:2013, Gill+Hangartner:2010}.
...
While they provide a powerful toolbox, particularly for modeling smooth effects, they may not be 
able to capture all types of effects. For example, abrupt changes might not be detected properly owing 
to the fixed structure enforced by model assumptions such as the type of link function and the 
...
forests to circular data, specifically by employing distributional trees and distributional forests
following \cite{Schlosser+Hothorn+Stauffer:2019}. 
...
These features and the corresponding methodology are presented in Section~\ref{sec:tree_forest} after
...
A common way to including covariates as regressors in distributional models is to apply
...
and no information about possible interactions.
...
\section{Case Study: Wind Direction Forecasts}
\label{sec:wind}
...
Just as the linear version of the CRPS \citep[see][for more details]{Hersbach:2000} it measures the 
difference between an observation and the corresponding predicted distribution function in order 
to assess the probabilistic goodness of fit of the assessed model. Hence, the lower the CRPS value 
...
To measure the improvement of the trees, forests, persistence and linear models 
over the climatology model the skill score based on the circular CRPS is evaluated
with the climatology model used as reference:
\begin{equation}
...
its lagged values. For the here discussed application this corresponds to establishing a linear model
with wind direction as the outcome variable and wind direction of the previous time point as covariate.
...
Using the corresponding implementation provided in the \textsf{R} package \textbf{circglmbayes}
by \cite{Mulder+Klugkist:2017} the function \textbf{circGLM} estimates the
intercept and regression coefficient along with the the concentration parameter $\kappa$.

\fixme{add references and further information on the estimation process within circGLM}

\fixme{information on applied implementation here or in separate chapter?}
...
\fixme{Summary of results}

\fixme{Possible extensions of models, use of NWP data}
