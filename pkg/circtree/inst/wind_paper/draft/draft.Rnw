...
%
%NEW VERSION:
Circular data occur in a wide range of scientific fields, but the methodology
for distributional modeling and probabilistic forecasting of circular response
variables is rather limited. While most of the existing methods are built on
the framework of generalized linear and additive models, which are often
challenging to optimize and interpret, we propose a distributional approach for
regression trees yielding probabilistic forecasts based on the von~Mises
distribution. The resulting tree-based models use the available covariates only
for partitioning the data into sufficiently homogeneous subgroups, allowing for
a simple circular distributional fit in each subgroup. These circular
...
are associated with changes in any distribution parameter. Ensembles or
forests of such circular trees can further improve the forecasts by
regularizing and stabilizing the model. The newly developed methodology is
illustrated in an application on short-term probabilistic wind direction
forecasting for Innsbruck and Vienna Airport in order to direct airplanes to a
safe landing. The predictive skill of the novel approaches is benchmarked with
an additive regression model plus a persistency and a climatology model,
...
until convergence \citep{Fisher+Lee:1992, Fisher:1993}. While all of these models are built on well-elaborated
...
inference. In addition, the employed tree structure can capture
...
literature on tree-based circular modeling. \cite{Lund:2002} already introduced a
circular regression tree algorithm where binary splits are made based on an
...
be estimated \citep{Gneiting:2008}. 
%However, this only models
...
%in forecasts \citep{Gneiting:2008}. 
In the field of image based object direction estimation,
\citet{Hara+Chellappa:2017} presented a distribution-based tree-building 
algorithm by applying $k$-means clustering to the target space where
the number of clusters $k$ is adaptively determined by the Bayes
information criterion employing the von~Mises distribution. Despite their
distribution-based splitting approach to circular response variables, they
solely predict the mode of the distribution, thus, providing no full
probabilistic forecasts. Combining these previous ideas for circular trees with the
concept of distributional modeling, we introduce a fully distributional tree
approach that not only considers splits in all distribution parameters, but also yields a
full probabilistic predictive model as proposed by \citet{Gneiting:2008}. In
addition, as a natural extension, ensembles or forests of circular regression
trees are presented in this study, showing that these can further improve
predictive power.
...
the log-likelihood function and the strongly nonlinear link function. In general,
for additive models a proper model specification can be very challenging, particularly for a high number of
...
\begin{itemize}
  ...
  \item \emph{Generalized linear model:}
    ...
    implementation of \cite{Mulder+Klugkist:2017} the accommodation of circular covariates would require
    an additional manual adjustment, such as an approximation by a finite Fourier expansion
    (cf., \citealt{Lund:1999}), we use only the linear components of the lagged
    2-dimensional wind vector~$(u, v)^{\top}$ and the lagged wind speed $spd$ as
    predictor variables. Thus, the model formula for the location
...
in an application on shortterm probabilistic wind direction forecasting at two
...
such effects. As supported by the application on probabilistic wind direction
...
The computation time on a standard computer is in the order of 10\,milliseconds
for the circular fit (used for the persistency and the climatological model),
seconds for the estimation of the linear model and the circular tree, and from
a few minutes to an hour for the estimation of the circular forest; the latter
strongly depends on the number of covariates employed and, hence, varies
between Innsbruck and Vienna Airport. However, for a single station and
forecast step, the persistency and linear models must be re-estimated for each
time of interest, and independent climatological models have to be fitted for
all times of interest within a single calendar year, whereas for all times of interest only one 
circular tree and circular forest model is required. 
...
%a few minutes to an hour for the estimation of the circular forest; the latter
%strongly depends on the number of covariates employed and, hence, varies
%between Innsbruck and Vienna Airport. However, the persistency and linear
%models must be re-estimated for each time point in the validation data set, and
%independent climatological models have to be estimated for all the time points
%within one calendar year which exist in the validation period, whereas only one
%model is needed for the circular tree and the circular forest valid for all
%times points in the validation data set. 
