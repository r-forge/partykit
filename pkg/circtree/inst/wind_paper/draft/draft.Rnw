...
%% FIXME: first attempt to include reference Gneiting+Balabdaoui+Raftery:2007
which would allow for also considering uncertainty in forecasts \citep{Gneiting+Balabdaoui+Raftery:2007}.
%%
...
that this quantity is highly related to wind speed. For example, low wind speeds
usually leads to a higher fluctuation in wind direction. One way how to account for
