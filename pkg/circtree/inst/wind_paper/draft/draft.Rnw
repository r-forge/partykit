...
%To stabilize and regularize the model an ensemble of distributional trees can be combined 
%yielding a distributional forest which allows for modeling smooth effects as well.
...
%In order to assess the predictive performance of the proposed circular trees and forests,
%a case study on probabilistic forecasting of wind direction for two airports in Austria is presented
%with alternative commonly used approaches being employed as benchmarks.
...
\caption[Table caption text]{Overview of the employed data set.}
\label{tab:data}
\begin{tabular}{l  l}
\hline
Data components              & Description \\
\hline
Meteorological variables     & Wind direction, wind speed, air pressure, \\
...
Weather stations (Innsbruck) & 21 stations at the airport, \\
                             & Landeck, Kufstein, Steinach, \dots \\
Weather stations (Vienna)    & 22 stations at the airport, \\
                             & \ldots \\
