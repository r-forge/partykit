...
airport of Innsbruck which lies at the bottom of a narro valley within the European Alps.
...
As benchline models, probabilistic climatology and perstistency models, as well as a state-of-the-art
...
highly skillful estimates leading to good performance of the persistency model for shortterm wind direction forecasts. While in these cases the trees and forests also benefit from the highly informative lagged response, they can compensate for a lower information of this covariate by incorporating other quantities and possible interactions of these, contrary to the persistency model (see Figure~\ref{fig:boxplot_crpsskill}).
Hence, the tree-based models provide reliable forecasts in all tested meterological settings.
...
this should not affect the performance  of circular forest, as these still typically 
consist of very large trees with many different splits.

To conlcude, the tree structure can capture non-linear changes, shifts, and potential interactions in 
