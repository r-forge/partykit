...
the resulting subgroups until some stopping criterion is reached. Complementing
the pre-pruning based on the significance level of the
statistical tests, as mentioned above, further criteria such as maximal tree depth or a minimal
number of observations in a node can be employed.
