...
homogeneity.
While this approach and further extensions \citep{Larraondo+Inza+Lozano:2018} do not only
...
\begin{figure}[p!]
  ...
  with the estimated location parameter (red hand). The covariates used for splitting
...
parameter~$\kappa$, i.e. a high estimated variance. 
...
$0$ (see, e.g., \citealt{Jammalamadaka+Sengupta:2001} or \citealt{Ley+Verdebout:2017} for a more detailed overview).
...
\begin{itemize}
  \item \emph{Climatological model:} 
    ...
    using the $31$~days centered around the day of interest over all available years in the training data.
  ...
  \item \emph{Persistency model:}
    ...
    the regime changes takes place in the response variable during the forecast
...
by the circular regression tree and the persistency model; except for the $3$\,h~forecast 
