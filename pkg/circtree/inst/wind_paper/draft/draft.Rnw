...
For a complete tree model, the same procedure is applied to all resulting child nodes with the 
...
In order to obtain a goodness-of-fit measurement of $\text{vM}(Y; \mu,\kappa)$ for each parameter 
and each observation the score function $s(\mu, \kappa, Y)$ is evaluated at the estimated location 
and concentration parameter $\hat{\mu}$ and $\hat{\kappa}$.
In a next step, a statistical test is employed to detect dependencies between the calculated score 
values
\begin{equation}
...
\end{pmatrix}
\end{equation}
and each possible split variable $Z_l \in \{Z_1, \ldots, Z_m\}$.  
In particular, a permutation test is applied to asses the following hypotheses:
\begin{align}
H_0^l:  s(\hat{\mu}, \hat{\kappa}, Y) \qquad \bot \qquad Z_l.
\end{align}
%
The permutation test is based on the multivariate linear statistic
\begin{equation}
T_l = vec\left(\sum_{i=1}^n v_l(Z_{li}) \cdot s(\hat{\mu}, \hat{\kappa}, Y_i)\right)
\end{equation}
where $s(\hat{\mu}, \hat{\kappa}, Y_i) \in \mathbb{R}^{1\times 2}$. The form 
of the transformation function $v_l$ depends on the type of the split 
variable $Z_l$. If $Z_l$ is numeric then $v_l$ is simply the identity 
function $v_l(Z_{li}) = Z_{li}$ and therefore $T_l \in \mathbb{R}^2$ as 
the ``vec'' operator converts the $1 \times 2$ matrix into a $2$ column vector. 
...
the value of $Z_{li}$ is $1$. In this case the statistic $T_l \in \mathbb{R}^{H \cdot 2}$
as the ``vec'' operator converts the $H \times 2$ matrix into a $H \cdot 2$ column vector 
by column-wise combination. Observations with missing values are excluded from the sums.

Using the conditional expectation $\mu_l$ and the covariance 
$\Sigma_l$ of $T_l$ as derived by \cite{Strasser+Weber:1999} 
the test statistic can be standardized. 
The observed multivariate linear statistic $t_l$ which is either a
$2$- or $2 \cdot H$-dimensional vector, depending on the scale of $Z_l$,
...
$\mu_l$ and $\Sigma_l$. Hence, the asymptotic conditional distribution of
