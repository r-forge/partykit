...
Furthermore, covariates and their possible interactions do not need to specified in advance as they are
...
\begin{equation}
\mu = \mu_0 + g(\mathbf{\beta}^{\top} \mathbf{z})
\end{equation}
with a circular intercept $\mu_0$ and a coefficient vector $\mathbf{\beta}$; whereby the 
the additive predictor is transformed to an interval of length $2\pi$  by a link function $g$. 
...
homogeneity, \citet{Fisher+Lee:1992} also provide a heteroscedastic version by combining two  
...
alternative to commonly used circular regression models for circular data.
