...
%
%NEW VERSION:
Circular data occur in a wide range of scientific fields, however the methodology
for distributional modeling and probabilistic forecasting of circular response
variables is rather limited. While most of the existing methods are built within
...
distribution. The resulting tree-based models use the available covariates 
...
\begin{itemize}
  ...
  \item \emph{Generalized linear model:}
    ...
    (cf., \citealt{Lund:1999}), we only use the linear components of the lagged
...
a few minutes up to an hour for the estimation of the circular forest; the latter
strongly depends on the number of covariates employed and, hence, varies
between the two locations included in the application. However, for a single station and
...
all times of interest within a single calendar year, whereas for all times of interest only a single 
circular tree and a single circular forest model is required. 
