...
station and forecast step, the persistency and linear models must be
re-estimated for each time of interest, and independent climatological models
have to be fitted for all times of interest within a single calendar year,
whereas for all times of interest only a single circular tree and a single
circular forest model is required. 
