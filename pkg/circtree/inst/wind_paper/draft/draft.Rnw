...
As a motivational example Figure~\ref{fig:tree_ibk} displays a distributional
...
A more extensive case study of circular regression trees and forests applied to probabilistic 
forecasting of wind direction at the airports of Innsbruck and Vienna is presented in 
Section~\ref{sec:wind} where their performance is also compared to other commonly used models.
Before that, distributional models for circular data are discussed in Section~\ref{sec:prob_circ} 
and the methodology of circular regression trees and forest and its features are presented in 
Section~\ref{sec:tree_forest}.
...
investigate whether circular trees and forests can be applied as a reasonable
alternative to already existing approaches, three additional statistical models are
applied in this study to probabilistically forecast wind directions. Two
approaches are based on existing approaches in the field of meteorological
forecasting, one approach is a state-of-the-art model to forecast circular
variables.  The next paragraphs introduce the alternative approaches and
provide for all statistical models, including the circular tree and forest, the
model specification employed in this study.
...
In addition to the raw CRPS, to assess differences in the
improvement of the various statistical models over the climatological model
used as a reference, CRPS-based skill scores are computed according to
\begin{equation} 
...
individual methods. As for the employed models the variability of the
predictive performance over the five years is lower than over a single day or
over a single year, CRPS and CRPS skill scores are aggregated over the
respective five validation years; this yields 24 hourly scores per 12 months
...
level of the respective model performances and on their respective performance
...
an higher variation in the predictive performance for all models in comparison
...
\begin{figure}[t]
  ...
  \caption{CRPS skill scores for wind direction forecasts based on the full
    ...
    per $12$~months averaged over the $5$~validation years; this yields a total of
    $288$~yearly mean values. The scores are shown for the reference models climatology, 
    persistence and the linear model, as well as for the circular regression tree and
    forest.}
