...
\Abstract{
This vignette introduces the \pkg{glmertree} package for fitting a wide range of generalized linear mixed-effects model trees (GLMM trees or glmertrees). In hands-on examples based on artificial datasets, emphasis is given to three special cases of fitting GLMM trees to clustered or longitudinal data: trees with constant fits in the terminal nodes (Section~\hyperref[sec:regression_trees]{\ref{sec:regression_trees}}), detection of treatment-subgroup interactions (Section~\hyperref[sec:treatment_subgroups]{\ref{sec:treatment_subgroups}}), and subgroup detection in growth curve models (Section~\hyperref[sec:growth_curves]{\ref{sec:growth_curves}}). 
}
...
Generalized linear mixed-effects model trees (GLMM trees or glmertrees) have initially been proposed by \cite{FokkySmit18} for detecting treatment-subgroup interactions in clustered datasets. GLMM trees can however be used to address a much wider range of research questions. Using hands-on (artificial) examples, this vignette describes how to fit GLMM trees: Section 2 will describe how to fit (G)LMM trees with constant fits in the terminal nodes on clustered (multilevel) data. Section 3 will describe how to assess main and interaction effects of a categorical variable (treatment) on a continuous response (treatment outcome) on clustered data. Section 4 will describe how subgroups can be detected with respect to the parameters of a growth curve model. These are just examples; \proglang{R} package \pkg{glmertree} can be used to detect predictors and moderators in a wide range of GLMMs-type models.    
...
\section{Fitting a mixed-model tree with constant fits in terminal nodes}
...
@
%
Because the response is a continuous variable, we employ function \code{lmertree()} for fitting a mixed-effects model tree. In the model formula, we specify \code{outcome} as the response variable, followed by a tilde. Next, we specify the node-specific model, which comprises only an intercept in this case, as we want to identify subgroups which differ on their value of the response variable. Next, we specify the predictor for the global model. We only want to account for possible outcome differences between the service providers, so only want to estimate a random intercept with resepct to the \code{cluster_id} variable. Finally, we specify the demographic variables and case characteristic as the potential partitioning variables: 
%
<<echo = TRUE, eval = TRUE>>=
...
@
%
Note that we specified only \code{cluster_id} as a predictor variable for the global model, to estimate the random intercept with respect to \code{cluster_id}. Here we used short-hand notation for \code{(1|cluster_id)}. Function \code{lmertree} and \code{glmertree} assume a single random intercept term, by default, which can be specified by only providing the cluster indicator.
%
More complex random-effects structure can be specified, using the customary formulation employed in package \pkg{lme4}. For example, if we would want to account for a global linear fixed effect of age, we can incorporate it in the specification of the global model (results not presented or discussed further):
%
...
@
\caption{\label{fig:tree_1} LMM tree for predicting with constant fits in the terminal nodes.}
\end{figure}
%
Using the argument \code{which}, we can also specify which part of the model should be plotted: \code{which = "tree"} plots only the tree, \code{which = "ranef"} plots only the predicted random effects, \code{which = "all"} (the default) plots the tree as well as the random effects, \code{which = "tree.coef"} yields caterpillar plots of the estimated coefficients in each terminal node with error bars (i.e., $\pm$ 1.96 times the standard error; note that these standard errors do not account for the searching of the tree structure and are likely too low/optimistic). 

The plotted tree is depicted in Figure~\ref{fig:tree_1}. In every inner node of the plotted tree, the splitting variable and corresponding $p$-value from the parameter stability test is reported. To control for multiple testing, the $p$-values are Bonferroni corrected, by default. This can be turned off by adding \code{bonferroni = FALSE} to the function call, yielding a less conservative criterion for the parameter stability tests, but note that this will increase the likelihood of overfitting. The significance level $\alpha$ equals .05 by default, but a different value, say .01, can be specified by including \code{alpha = .01} in the function call.  

The plotted tree shows the distribution of the observated values of the response in each of the terminal nodes. Four subgroups were found: terminal 3 indicates that for female patients with lower age, the highest values for the response are observed and thus for this group, poorer treatment outcomes are predicted. Slightly better treatment outcomes are observed for those in node 4 (female gender and higher age) and node 6 (male gender, no emotional disorder). The best treatment outcomes (lowest response variable values) are observed among those in node 7 (male gender, presence of emotional disorder).  
...
To obtain numerical results, \code{print}, \code{coef}, code{fixef}, \code{ranef} and \code{VarCorr} methods are available (results omitted here for space considerations):
%
<<echo = TRUE, eval = FALSE>>=
print(MH_tree)
coef(MH_tree)
...
Residuals of the fitted mixed-effects tree can be obtained with the \code{residuals} method. Residuals can be used for assessing potential misspecification of the model or violation of assumptions (e.g., heteroscedasticitytapply()):
%
<<eval = FALSE, echo = TRUE>>=
...
\end{figure}
%
The plotted residuals are depicted in Figure~\ref{fig:residuals_1}. The left panel indicates some differences in residual variances across the levels of \code{cluster_id} (which are not statistically significant when tested with functions \code{bartlett.test()} or \code{fligner.test()}). The right panel indicates no association between fitted values and residuals.
...
@
%
Note that the left hand side of the model formula (preceding the tilde symbol) specifies the response variable (\code{depression}). The right hand side of the model formula consists of three parts, separated by vertical bars: The first part specifies the predictor variable(s) of the (generalized) linear model (\code{treatment}, in this example). The second part specifies the predictors of the global model (comprising only a random intercept with respect to \code{cluster}, in this example). Third part specifies the potential partitioning variables. All partitioning variables are continuous in this example, but (ordered) categorical partitioning variables may also be specified. Also, we specified a single variable in the random-effects part, resulting in estimation of a random intercept with respect to \code{cluster}. More complex random effects can also be specified; for example, specifying the model formula as 
%
<<eval = FALSE, echo = TRUE>>=
...
@
%
would yield a model with globally estimated random intercept and slope terms for \code{age} estimated with respect to \code{cluster}. Note that we also included a fixed-effect term for age, as is customary in speciying a random effect. Note that the round brackets are necessary to protect the vertical bars in the formulation of the (global) random effects.
%
Alternatively, we could encounter datasets with nested designs of more than tow levels, for example when we have treatment centers nested within geographical areas. Using the indicators for treatment center and geographical areas (e.g., \code{center} and \code{area}), we could have specified random intercept terms for \code{center} nested within \code{area} as is customary with package \pkg{lme4}:
%
<<eval = TRUE, echo = TRUE>>=
...
Alternatively, we can obtain caterpillar plots of the estimated node-specific coefficients by specifying \code{which = "tree.coef"}:
%
<<eval = FALSE, echo = TRUE>>=
plot(lmm_tree, which = "tree.coef")
@
%
\begin{figure}[t!]
\centering
...
\end{figure}
%
The plotted results depict the node-specific parameter estimated with error bars ($\pm 1.96$ times the standard error). Note that these standard errors do not account for the searching of the tree structure and are likely too small. 
%
...
\end{figure}
%
The plotted residuals are depicted in Figure~\ref{fig:residuals}. The left panel indicates some variation in error variances across levels of the random effects, but these differences appear not statistivally significant: 
%
<<>>=
...
@
%
The right panel of Figure~\ref{fig:residuals} shows fitted values plotted against residuals and also does not reveal a pattern indicating model misspecification.
...
For partitioning longitudinal data, functions \code{lmertree()} and \code{glmertree()} requires data to be in the long format. An artificially generated longitudinal dataset is included in package \pkg{glmertree} and can be loaded as follows: 
%
<<echo = TRUE, eval = TRUE>>=
...
The default fitting procedure as employed in \code{lmertree()} assumes potential predictor variables are measured on the observation level. In this example, potential partitioning variables measured on the cluster level (i.e., time-invariant covariates) and the observation-level stability tests will likely have inflated type-I error. We can account for the level of the partitioning variables through specification of the \code{cluster} argument. As a result, parameter stability tests will be performed on the cluster instead of the observation level:
%
<<eval=TRUE, echo=TRUE>>=
...
@
%
Note that we additionally specified \code{whcih = "tree"}, to obtain a plot of the tree only, and \code{fitted = "marginal"}. Through the latter, we specified the plotted fitted values to be computed by fixing all remaining (fixed- an random-effects) predictor variables at their means (or majority class, for categorical predictors). By default, \code{fitted = "combined"} yields plotted fitted values, computed based on the observed values of the remaining (random and fixed-effects) predictor variables, and their estimated effects. We employed the marginal approach here, as this yields straight lines plotted in the terminal nodes, which may better reflect the average trajectories.  
%
\begin{figure}[t!]
\centering
\setkeys{Gin}{width=0.8\textwidth}
