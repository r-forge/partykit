...
outcome ~ 1 | (age + (1 + age | cluster_id)) | age + gender + emotional + 
  autism + impact + conduct
@
%
would yield a model in which a random intercept and slope for \code{age} would be estimated with respect to \code{cluster_id}. Note that the parentheses are necessary to protect the vertical bars, which separate the (global) random-effects from the other parts of the model. Note also, that when we specify a random slope (for \code{age}), we also specify a fixed slope (for \code{age}) in the global model, as is customary in specifying mixed-effects models. 
...
%
<<eval = FALSE, echo = TRUE>>=
depression ~ treatment | (age + (1 + age | cluster)) | age + duration + anxiety
@
%
would yield a model with a random intercept and slope for \code{age} estimated with respect to \code{cluster}. The brackets are necessary to protect the vertical bars in the formulation of the (global) random effects. Note also, that when we specify a random slope (for \code{age}), we also specify a fixed slope (for \code{age}) in the global model, as is customary in specifying mixed-effects models. 
