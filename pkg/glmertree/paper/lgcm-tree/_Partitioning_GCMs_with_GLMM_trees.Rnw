...
Convergence of the algorithm is monitored through the log-likelihood of the mixed-effects model fitted in Step~3. Typically, this converges when the partition~$j(r)$ from the GLM tree is the same as $j(r-1)$ from the previous step.
