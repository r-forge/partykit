...
Development over time is of prime interest in psychological research. For example, in educational studies researchers may want to model student's academic development over time; in clinical studies researchers may want to model patients' symptoms over time. Mixed-effects or latent-variable models can be used to model such trajectories and allow for explaining heterogeneity with covariates of a-priori known relevance \citep[e.g., ][]{NeisyMatt18}. However, when the covariates that can explain heterogeneity are not known in advance, methods for identifying them are needed. 

For example, Figure~\ref{fig:global_trajectories} shows trajectories of science knowledge and skills from 250 children who were assessed at three timepoints throughout grades 3 through 8\footnote{Further details on the source of these data and scaling of the response variable are provided in Study~III.}. The grey lines depict individual trajectories, while the red line depicts the estimated average trajectory. Although the red line describes the average trajectory well, the grey lines indicate substantial intra-individual variability in average levels and growth over time. A common research aim might be to identify which covariates can explain or predict this heterogeneity. 
...
lines(x = c(min(data$months), max(data$months)), 
     ...
     lwd = 3, col = 2)
@
\\\footnotesize{\textit{Note.} Grey lines depict observed individual trajectories. The red line depicts the average growth curve as estimated with a linear mixed-effect model, comprising a fixed effect of time and a random intercept with respect to individuals.}
\end{subfigure}%
...
\begin{figure}%
\caption{Partitioned growth curves of science abilities.}
\begin{subfigure}{0.8\textwidth}%
<<fig=TRUE, height=9, width=12, echo=FALSE>>=
...
@
\\\footnotesize{\textit{Note.} The $x$-axes represent the number of months after the baseline assessment, $y$-axes represent science ability. Grey lines depict observed individual trajectories. Red lines depict the average growth curve within each terminal node, as estimated with a linear mixed-effect model comprising node-specific fixed effects of time and a random intercepts with respect to individuals.}
\end{subfigure}%
...
library("kableExtra")
coefs <- coef(lmm_tree_c)
coefs <- cbind(Node = as.numeric(rownames(coefs)), coefs)
...
Recursive partitioning methods (RPMs), also known as ``trees``, allow for identifying relevant predictors from a potentially (very) large number of covariates. Figure~\ref{fig:LMM_tree_c} shows an example tree, which identified socio-economic status (SES) and internalizing problems (INTERN) from a set of 11 socio-demographic and behavioral characteristics of these children, assessed at baseline. Four subgroups were identified, corresponding to the terminal nodes of the tree, each with a different estimate of the fixed intercept and slope (see Table~\ref{tab:local_coefs}). For children in the group with lower levels of SES, the intercept is also lowest, indicating lower science ability. For intermediate levels of SES (between -0.41 and 1.23), there is also an effect of internalizing problems, where children with higher internalizing problems have a lower intercept. Highest science ability intercepts are observed in the group with highest SES. It appears that nodes with higher intercepts also tend to have higher slopes, indicating that children with relatively higher ability also show larger ability gains over time.   

The tree in Figure~\ref{fig:LMM_tree_c} has been estimated with generalized linear mixed-effects model trees \citep[GLMM trees; ][]{FokkySmit18}. GLMM trees were originally proposed for subgroup detection in clustered cross-sectional studies, where subjects are for example nested in treatment centers, classrooms and/or geographical areas. In the current paper we aim to extend GLMM trees, so that they can be applied to partitioning LGCMs. The general idea of GLMM trees is appealing for subgroup detection in almost any type of mixed-effects model. Compared to clustered cross-sectional data, however, longitudinal data may require a different estimation approach: The variance of random effects tends to be higher with longitudinal data, and predictors of interest tend to be measured at higher levels (e.g., time-invariant covariates). In this paper, we propose and test two extensions of GLMM trees that account for these characteristics. We focus on the specific use case of partitioning LGCMs, but the extensions are critical for a wider range of settings where covariates are measured at higher levels and/or where the random effects have substantial variance.
...
The following two subsections describes an alternative initialization strategy for Step~0 and an alternative approach for fitting the GLM tree from Step~1. Each subsection first reviews the well-established methods and then proceeds to discuss modifications that may improve performance when partitioning longitudinal data.
...
<<fig=TRUE, height=7, width=12, echo=FALSE, out.width=12>>=
lmm_tree <- lmertree(LMM_form, data = data, maxdepth = 6)
...
@
\\\footnotesize{\textit{Note.} The $x$-axes represent the number of months after the baseline assessment, the $y$-axes represent science ability. Grey lines depict observed individual trajectories. Red lines depict the average growth curve within each terminal node, as estimated with a linear mixed-effect model with a node-specific fixed effect of time and random intercepts estimated with respect to individuals.}
\label{fig:LMM_tree}
\end{figure}%


\begin{figure}%
\caption{GLMM tree initialized by estimating the random effects.}
...
@
\\\footnotesize{\textit{Note.} The $x$-axes represent the number of months after the baseline assessment, $y$-axes represent science ability. Grey lines depict observed individual trajectories. Red lines depict the average growth curve within each terminal node, as estimated with a linear mixed-effect model with a node-specific fixed effect of time and random intercepts estimated with respect to individuals.}
...
To illustrate how initialization may make a difference we applied both approaches to the dataset from Figures~\ref{fig:global_trajectories}~and~\ref{fig:LMM_tree_c}. Initializing estimation assuming zero random effects yielded the tree in Figure~\ref{fig:LMM_tree}, with \Sexpr{width(lmm_tree$tree)} subgroups and an estimated random-intercept variance of \Sexpr{formatC(data.frame(VarCorr(lmm_tree$lmer))$vcov[1], digits = 2)}. Additional splits were implemented based on gender, race, internalizing problems and fine motor skills. Considering the relatively large number of subgroups and relatively small sample sizes in some of the terminal nodes, this tree may not generalize well to other samples. Initializing with estimation of the random effects yielded the tree depicted in Figure~\ref{fig:LMM_tree_r}, with \Sexpr{width(lmm_tree_r$tree)} subgroups and an estimated random-intercept variance of \Sexpr{formatC(data.frame(VarCorr(lmm_tree_r$lmer))$vcov[1], digits = 2)}. Compared to the tree in Figure~\ref{fig:LMM_tree_c}, an additional split was implemented based on gross motor skills (GMOTOR): children with further developed motor skills at baseline have a higher intercept. 
...
In fact, the tree in Figure~\ref{fig:LMM_tree_c} was obtained by application of GLMM trees with cluster-level parameter stability tests. Application of GLMM trees with both cluster-level parameter stability tests and random-effects initialization yielded the exact same tree structure and parameter estimates. It provides the most parsimonious tree structure of the three estimation approaches employed thus far. The estimated random-intercept variance was \Sexpr{formatC(data.frame(VarCorr(lmm_tree_c$lmer))$vcov[1], digits = 2)}, the highest value obtained thus far, illustrating how some variation in the response variable can be captured by either the random effects or by the tree structure. In the next sections, we assess the performance of the original algorithm and proposed adaptations through empirical evaluations.
...
{\textit{Note.} M~= mean number of splits; SD~= standard deviation of the number of splits; MAD~= Mean absolute deviation from true tree size. Gray circles represent counts, dark gray horizontal lines represent true number of splits~3. Panel labels indicate whether variances of the random intercept and slope were fixed to 0 or freely estimated. Distances on $y$-axis are on the log scale. cl.cov.~= cluster-level covariances employed in parameter stability tests; ran.eff.~= estimation initialized with the random effects on the full sample. }
\label{fig:LMM_sizes}
\end{figure}



Figure~\ref{fig:LMM_sizes} depicts the number of splits implemented by each partitioning approach. The default fitting approach tends to overfit, irrespective of the random-effects specification, implementing $> 3$ splits in \Sexpr{format(100*mean(c(props_more_splits["lm"], props_more_splits["lmer"], props_more_splits["lmer_s"])), digits = 2L)}\% datasets, on average. The use of cluster-level covariances in the parameter stability tests successfully mitigated overfitting, irrespective of the random-effects specification. LMM trees with clustered covariances implemented an average number of splits that was closest to the true tree size, with little difference between random-effects specifications (i.e., with and without random slopes).

In terms of MAD, however, the LMM tree initializing estimation with the random effects performed best, if only random intercepts (not random slopes) were estimated. Likely, initializing estimation with the random effects may be successful only when the random-effects specification is kept relatively simple. With a more complex random-effects specification (i.e., both random intercept and slope estimated; right panel in Figure~\ref{fig:LMM_sizes}, initializing estimation with the random effects resulted in underfitting. Subgroup differences are then more likely to be captured by the random effects than by the tree structure.

The combined use of random-effects initialization and cluster-level covariances was not very effective, irrespective of the random-effects specification. All in all, use of clustered covariances appears to provide the most reliable improvement in performance.

Distributions of the number of splits, separated according to the levels of the data-generating design are presented in Figure~\ref{fig:LMM_sizes_interact} and discussed in Appendix~\ref{sec:AppendixA}. The results show a pattern very similar to Figure~\ref{fig:LMM_sizes}; no substantial interactions between data-generating and model-specification parameters were observed. Main effects of the data-generating parameters were as expected: The strongest effects were for $\sigma_{b_1}$ and $N$, with higher values resulting in a higher number of splits. The number of noise variables and $\sigma^2_{b_1}$ had smaller effects, and were involved in minor interactions: With the default fitting approach, they had a positive effect on the number of implemented splits; with clustered covariance or random-effects initialization, they had a negative effect on the number of splits. The correlation between partitioning variables hardly affected the number of implemented splits.
...
\vspace{-0.5cm}%
{
\footnotesize \textit{Note.} M~= mean number of splits; SD~= standard deviation of the number of splits; MAD~= Mean absolute deviation from true tree size. Grey circles represent counts, dark gray horizontal lines indicate true number of splits~3. Distances on the $y$-axis are on the log scale; $x$-axis labels indicate whether variances of the random intercept and slope were fixed to 0 or freely estimated.}
...
@
\end{subfigure}%
\\ \textit{Note.} $y$-axis is on the log scale; M = mean computation time in seconds. 
...
\section{Study~III: Cross-validated predictive accuracy on real data}
...
Our analyses focus on predicting trajectories of reading, math and science abilities, based on baseline characteristics. Time was measured as the number of months since the baseline assessment. In order to obtain approximately linear trajectories, we chose the timing metric based on visual inspection of the observed data: $\mathrm{months}^{1/2}$ was used as the timing metric for reading and math trajectories, and $\mathrm{months}^{2/3}$ for science trajectories.
...
We fitted four LM(M) trees using the approaches which showed most promise in the simulations: 
% 
\begin{itemize}
  \item An LM tree ($\hat{\sigma}_{b_0} = \hat{\sigma}_{b_1} = 0$) using clustered covariances in the parameter-stability tests.
  \item An LMM tree using clustered covariances, with a random intercept freely esimtated ($\hat{\sigma}_{b_0} > 0$, $\hat{\sigma}_{b_1} = 0$).
  \item An LMM tree using clustered covariances, with both random intercept and slope freely estimated ($\hat{\sigma}_{b_0} > 0$, $\hat{\sigma}_{b_1} > 0$). 
  \item An LMM tree using random-effect initialization with the full-sample estimate, with a random intercept freely estimated ($\hat{\sigma}_{b_0} > 0$, $\hat{\sigma}_{b_1} = 0$). 
\end{itemize}
%
As a benchmark, we also estimated two standard LMMs with a fixed effect of time: One LMM with a random intercept freely estimated, one LMM with both random intercept and slope freely estimated. The LMMs did not include additional predictors in the model, only timepoint and the child identifier.
...
The ECLS-K datasets have exceptionally large sample sizes. We therefore employed random sampling to obtain training samples of $N=250$ children, likely more representative of real-world studies in psychology. For each combination of response variable (math, reading, or science) and sample size, we performed 100 repetitions. We evaluated predictive accuracy by computing the mean squared difference between predicted and observed response variable values for all children not included in the training sample in the current repetition. This separation of train and test observations does not allow for using the random effects in computing predictions, these were thus assumed 0; only accuracy of the fixed-effects parameters is thus qualified by the MSEs. Tree size was measured by the number of splits in each tree.
...
Figure~\ref{fig:application_MSEs} presents MSE distributions for each of the estimation approaches and Table~\ref{tab:application_MSEs} presents the means and standard deviations. %As expected, all methods show higher accuracy with larger sample sizes. LM(M) trees tend to show better predictive accuracy than longRPart trees. Pairwise $t$-tests indicated significantly better performance for each of the LM(M) tree approaches employing clustered covariances, compared to both longRPart approaches (Bonferroni corrected $p$-values all $< .01$). However, these performance differences are small; the effects of sample size are much more obvious than the differences between methods in Figure~\ref{fig:application_MSEs}.
Performance of the benchmark LMMs indicates that a substantial amount of variance can be explained by the single fixed linear effect of time. Further, all LM(M) trees outperform the benchmark LMMs. Differences in performance between LM(M) trees are small. Table~\ref{tab:application_MSEs} and Figure~\ref{fig:application_MSEs} indicate that LMM trees employing cluster-level covariances performed best for predicting math and reading abilities. Trees estimating only a random intercept performed best, but performance differed only about 1 SE compared to trees estimating both random intercept and slope.

For predicting science abilities, LMM trees initializing estimation with the random effects and estimating only a random intercept performed best, while this was the worst-performing approach for math and reading. LM trees with clustered covariances never performed best, illustrating the benefit of estimating random effects.
...
\end{subfigure}%
\vspace{-0.5cm}%
{\footnotesize \\ \textit{Note. } Top, middle and bottom panels depict math, reading and science ability trajectories, respectively. The secondary $y$-axis on the right quantifies the proportion of variance explained, computed as $1 - \frac{\text{MSE}}{\text{var}(y)}$.}
...
kable_styling(add_header_above(#add_header_above(
  ...
  kable(MSE_df, format = "latex", booktabs = TRUE, label = "application_MSEs", align = "c", 
        ...
        caption = "Cross-validated mean squared errors for each of the response variables."),
  "\\footnotesize \\\\ \\textit{Note.} Means and standard deviations computed over 100 repetitions. Standard deviations of the MSE can be interpreted as standard errors. $R^2$ was computed as $\\frac{MSE}{var(y)}$. $^c$~cluster-level covariances; $^r$~estimation initialized with random effects; $^i$~random-intercept variance freely estimated; $^s$~random-slope variance freely estimated.", 
  notation="none", threeparttable = TRUE, escape = FALSE),
...
\begin{figure}[!bt]%
\caption{Tree sizes for trees fitted to math, reading, and science ability trajectories.}
\begin{subfigure}{\textwidth}%
<<echo=FALSE, fig=TRUE, height=2, width=5>>=
## Tree size
...
The Simulations in Study~I showed that the proposed extensions of GLMM trees are effective for partitioning linear growth curve models. Use of clustered covariances in the parameter stability tests mitigated the strong overfitting that was observed using the standard covariances. Initializing estimation with the random effects, instead of assuming these to be zero was also successfull in mitigating overfitting, but only when the random-effects specification comprised only a random intercept. The performance of trees fitted using clustered covariances appeared largely unaffected by (mis-)specification of the random effects; use of clustered covariances may therefore be the safest choice in practice. Combining cluster-level covariances and random-effects initialization worsened performance and thus not recommended. The results also indicated that the optimal estimation approach for LMM trees does not depend on sample size, variance of the random effect(s), the number of potential partitioning variables or the intercorrelation between partioning variables.

Study~II showed comparable performance of LM(M) and SEM trees in partitioning LGCMs. SEM trees appeared more sensitive to mis-specification of the random effects, with under-specification yielding too many splits being implemented. In line with results of \cite{ArnoyVoel21}, we found score-based SEM trees to have somewhat lower power than LRT-based SEM trees, but with much lower computational burgen. LongCART trees often selected the wrong partitioning variable for the first split, and were outperformed by LM(M) and SEM trees. The LongCART parameter stability tests \citep{KundyHare19} may be underpowered for detecting instability of the fixed intercept, or for detecting instability with respect to categorical covariates. As in Study~I, these results were not strongly affected by sample size, variance of the random effect(s), the number of potential partitioning variables or the intercorrelation between partioning variables.

Study~III assessed performance of LM(M) trees in partitioning children's trajectories of math, reading and science abilities. Similar to the results of the simulation studies, we found very similar performance between LM(M) trees which employed clustered covariances but with different random effects specifications, though best performance was observed for a random-effects specification comprising only a random intercept. However, when partitioning science trajectories, for which there were a smaller number of observations per child, initializing estimation with the random effects showed the best performance. A possible explanation is that the higher the number of observations per subject, the more the clustered covariances will differ from covariances assuming independent observations, creating greater potential for improving the parameter stability tests. With a smaller number of observations per subject, initializing estimation with the random effect may be more beneficial. However, initializing estimation with the random effects also resuled in a substantially higher number of splits in all three datasets.  

As expected, Study~II revealed a clear computational advantage for LM(M) trees. This is in large part due to the local-global estimation approach employed by LMM trees, where fixed-effects parameters are estimated locally within a node and random-effects parameters are estimated globally, using all observations. In contrast, SEM trees and LongCART fit the full mixed-effects model in each node, which substantially increases computational load. The local-global estimation approach also reduces model complexity, because a lower number of random-effects parameters need to be estimated. The good performance of LMM trees in the current study suggest the lower complexity may also yield better stability and generalizability.

Yet, a possible downside of the local-global estimation approach is that it does not allow for recovering subgroups with differences in random-effects parameters. When there is a specific interest in partitioning the random effects, score-based SEM trees may be preferred. Alternatively, researchers may want to use the parameter stability tests for mixed-effects models developed by \cite{WangyMerk18} \citep[see also ][]{WangyMerk21,WangyGrav22}. This will be useful, for example, when the number and/or distances between timepoints differs between respondents and SEM-based growth curve models can thus not be applied \citep{NeisyMatt18}.

The current evaluations were limited to Gaussian responses and LGCMs. Future studies should assess performance of GLMM trees in partitioning longitudinal data with, for example, binomial or count responses. We expect that the good performance of clustered covariances in the parameter stability tests generalizes to other settings where covariates are measured at higher levels, either in longitudinal and/or otherwise nested data structures. Yet, whether our conclusions generalize to settings beyond LGCMs remains to be evaluated. Finally, we used the outer-product-of-gradients (OPG) estimator for computing (clustered) covariances. Though computationally more burdensome, future studies could assess whether use of the full sandwich estimator would further improve performance.
