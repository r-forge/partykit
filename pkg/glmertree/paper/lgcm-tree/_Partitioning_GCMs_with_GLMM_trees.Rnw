...
For example, Figure~\ref{fig:global_trajectories} shows trajectories of science knowledge and skills from 250 children who were assessed at three timepoints throughout grades 3 through 8\footnote{Further details on the source of these data and scaling of the response variable are provided in Study~III.}. The gray lines depict individual trajectories, while the red line depicts the estimated average trajectory. Although the red line describes the average trajectory well, the gray lines indicate substantial intra-individual variability in average levels and growth over time. A common research aim might be to identify which covariates can explain or predict this heterogeneity. 
...
lines(x = c(min(data$months), max(data$months)), 
     ...
     lwd = 3, col = 2)
@
\\\footnotesize{\textit{Note.} Gray lines depict observed individual trajectories. The red line depicts the average growth curve as estimated with a linear mixed-effect model, comprising a fixed effect of time and a random intercept with respect to individuals.}
\end{subfigure}%
...
\vspace{-0.5cm}%
{
\footnotesize \textit{Note.} M~= mean number of splits; SD~= standard deviation of the number of splits; MAD~= Mean absolute deviation from true tree size. Gray circles represent counts, dark gray horizontal lines indicate true number of splits~3. Distances on the $y$-axis are on the log scale; $x$-axis labels indicate whether variances of the random intercept and slope were fixed to 0 or freely estimated.}
