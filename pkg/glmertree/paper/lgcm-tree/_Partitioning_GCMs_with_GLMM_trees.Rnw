...
\abstract{Growth curve models are popular tools for studying the development of a response variable within subjects over time. Between-subject heterogeneity is commonly encountered and often of specific research interest. We show how generalized linear mixed effects model (GLMM) trees can be used to identify heterogeneous subgroups in linear growth curve models. GLMM trees were originally proposed for clustered cross-sectional data; here we introduces extensions for longitudinal data. The resulting extended GLMM trees are directly applicable to growth curve models as an important special case. We evaluate the performance of GLMM trees in partitioning growth curve models on simulated datasets and existing datasets on children's development of reading, math and science abilities. We compare the performance of GLMM trees with the SEM trees, LongCART and longRPart methods. GLMM trees provide similar accuracy compared to SEM trees, and better accuracy compared to LongCART and longRPart. In addition, GLMM trees allow for analysing discrete as well as continuous time series, are less sensitive to (mis-)specification of the random-effects structure and require substantially lower computation times.}
...
Modeling development over time is of prime interest in psychological research. For example, researchers may want to model student's reading or math development in an educational study, or symptom reduction over the course of a clinical trail. Inter-individual heterogeneity is likely encountered in such studies. For example, some (groups of) participants may show higher or lower rates of growth over time. Development over time is generally studied with mixed-effects or latent-variable models, which allow for capturing inter-individual heterogeneity and explaining this heterogeneity with covariates of a-priori known relevance \citep{NeisyMatt18}.

However, the covariates that predict or affect developmental trajectories may not be known in advance and often researchers may have a specific interest in identifying these covariates. Recursive partitioning methods (RPMs) are exploratory methods that allow for identifying relevant predictors from a potentially (very) large number of covariates. At the same time, RPMs allow for direct identification of heterogeneous subgroups. Several existing RPMs allow for subgroup detection in growth-curve models: GUIDE \citep{Loh02}, longRPart \citep{AbdoyLeBl02}, GEE-based decision trees \citep{Lee05}, longitudinal interaction trees \cite[IT; ][]{SuyMene11}, structural equation model (SEM) trees \citep{BranyOert13, ArnoyVoel21}, mixed-effects longitudinal trees \citep[MELT; ][]{EoyCho14} and LongCART \cite{KundyHare19} allow for partitioning linear growth curve models (LGCMs). Further, longRPart2 \citep{StegyJaco18} and IT-LT \citep{WeiyLiu20} methods allow for subgroup detection in non-linear growth curve models \footnote{Both IT methods have a specific aim of detecting subgroups for which the time-by-treatment interaction differs, so are not general methods for partitioning growth curve models.}.

In this paper, we propose the use of generalized linear mixed-effects model trees \citep[GLMM trees; ][]{FokkySmit18} for subgroup detection in LGCMs. GLMM trees were originally proposed for subgroup detection in clustered cross-sectional studies, where subjects are for example nested in treatment centers, classrooms and/or geographical areas. Yet, GLMM trees provide a much more general method, allowing for subgroup detection in almost any type of mixed-effects model. Compared to clustered cross-sectional data, however, longitudinal data often come with two central characteristics that may require a different estimation approach: Higher random-effects variance (i.e., higher intra-class correlations) and predictors of interest measured at higher levels (e.g., time-constant covariates). In this paper, we introduce and test two recent extensions of GLMM trees that can account for these characteristics. We focus on the specific use case of partitioning LGCMs, but methods for dealing with stronger intra-class correlations and higher-level covariates of interest are obviously of relevance beyond the realm of LGCMs. 


\subsection{Recursive partitioning methods for longitudinal data}

Compared to the aforementioned RPMs for partitioning LGCMs \citep{Loh02, AbdoyLeBl02, Lee05, SuyMene11, BranyOert13, ArnoyVoel21, EoyCho14, KundyHare19}, GLMM trees differ in that they do not fit a full parametric model in each of the terminal nodes. GLMM trees estimate the fixed-effects parameters locally, using only the observations within a terminal node, and estimate the random-effects parameters globally, using all observations. Not only does this reduce computation time, it also reduces model complexity, which in turn may reduce instability and overfitting. This local-global estimation approach was first proposed by \cite{HajjyBell11} and \cite{SelaySimo12} for trees with constant fits in the terminal nodes. In effect, the approach replaces the fixed-effects part of the mixed-effects model with a decision tree with constant fits in the terminal nodes, while the random effects are estimated as usual. \cite{FokkySmit18} generalized this approach to allow for GLMs with any number of parameters in the terminal nodes, thus allowing for non-Gaussian responses and targeted detection of interaction effects.

The computational advantage of GLMM trees is highest compared to longRPart, longRPart2, IT-LT and LRT-based SEM trees. These latter methods employ an exhaustive split detection procedure: For every possible split in the current node, the full parametric model is re-estimated in the two nodes that would result from the split. To choose the optimal split, a likelihood ratio test is computed for every possible cut point. In addition to a heavy computational burden, this also introduces a selection bias towards covariates with a larger number of possible cut points \citep{ShihyTsai04, Shih04}. LongCART, MELT, GEE-based decision trees and score-based SEM trees also fit full parametric models in each of the nodes, but do not refit models for cutpoint selection; they employ the predictions or residuals from the fitted model in the current node for selecting the best split. This reduces the computational burden, and allows for separating variable and cutpoint selection, thus preventing variable selection bias. The GLMM tree algorithm is more similar to the latter group of RPMs, in that it employs a two-step approach to split detection.


\subsubsection{Current paper}

In what follows, the next section explains how GLMM trees are estimated, and how it can be adapted to better account for longitudinal data structures. In three empirical evaluations, we assess and compare performance of GLMM trees in partitioning LGCMs: In Simulation Study I, we assess performance of the proposed adaptations to GLMM trees. In Simulation Study II, we compare performance of GLMM trees with that of two other methods: SEM trees and LongCART. In the Application, we compare the performance of GLMM trees and longRPart in partitioning existing datasets on children's development of reading, math and science abilities. In the Discussion, we summarize our findings and their implications.
...
\end{eqnarray}
%
In the current paper, $g$ is the identity function and we assume a continuous response with normally distributed errors and constant variance $\sigma_\epsilon$. In principle, generalizations to other distributions within the GLM. Further, $X_i$ is the $N_i \times (p+1)$ fixed-effects design matrix for subject $i$ ($i = {1, \dots, N}$). We furter assume time is the predictor variable of interest, thus $p=1$ and $X_i$ comprises two columns: a column of 1s for the intercept and a column of observed timepoints for subject $i$. The number ($N_i$) and values of observed timepoints may differ between subjects. While in a traditional GLMM, the fixed-effects parameters are assumed constant over all observations, but in the GLMM tree model, $\beta_j$ is a column vector of node-specific fixed-effects parameters, its value depending on terminal node $j$ of which observation $i$ is part. As in a traditional GLMM, $Z_i$ is the random-effects design matrix for subject $i$, comprising a subset of columns of $X_i$ and $b_i$ is a vector of random effects for subject $i$. Finally, we assume that $b$ follows a (possibly multivariate) normal distribution with mean zero and (co)variance $\Sigma_{b}$. 
...
Studies to date indicate that initializing estimation with zero random effects yields an accurate final model \citep{HajjyBell11, HajjyBell14, HajjyLaro17, SelaySimo12, FuySimo15, FokkySmit18}. \cite{SelaySimo12} assessed found only minor impact of different initialization values on the final model, which became smaller with increasing sample size. In \cite{FokkySmit18}, we found initializing estimation of GLMM trees with zero random effects performed well in cross-sectional clustered data. With longitudinal data, repeated measures on the same subjects tend to be correlated more strongly than observations nested withiin the same unit in cross-sectional data. If the random effects are in fact sizable, assuming them to be zero could provide an unrealistic starting point that is difficult to overcome in subsequent iterations. Our first hypothesis is that for partitioning LGCMs, initializing estimation with the random effects instead of the subgroup structure improves accuracy of GLMM trees.
...
where $\hat{J}$ is a suitable estimate of the covariance matrix of the parametric model fitted in Step~(a), and $n$ gives the number of observations in the current node. Further, $\hat{\psi}_{\sigma(U_{ik})}$ denotes estimated scores (denoted $\hat{\psi}$), with the subscript {$\sigma(U_{ik})$} denoting their ordering by the values of partitioning variable $U_k$. Note that $0 \leq t \leq 1$, thus $nt = 1$ for an observation with a unique minimum on the partitioning variable, and $nt = n$ for an observation with a unique maximum. 

\textcolor{red}{Perhaps replace $n$ with $N_j$? This is more line with notation of GLMM tree model, but might be confusing with cov mat $J$?}
...
When partitioning longitudinal data, covariates will often be measured at the subject level. This clustering of observations within subjects can be accounted for in computing a so-called \textit{clustered} covariance matrix $\hat{J}$ \citep[e.g., ][]{ZeilyKoll20}. This resembles a GEE-type approach with an independence correlation structure. Our second hypothesis is that for partitioning LGCMs, use of clustered covariances improves accuracy of GLMM trees.
...
We employed a full factorial design yielding $2^5 = 32$ cells and performed 100 repetitions per cell. We used R \citep[version 4.1.2; ][]{R22} to generate and analyze all data. 



\subsubsection{Partitioning approaches}

We fitted ten trees to each dataset:

\begin{itemize}
  \item Two linear model (LM) trees: 
  \begin{itemize}
    \item A tree employing default settings, where the parameter stability tests use observation-level covariances.
    \item A tree where the parameter stability tests use clustered covariances. 
  \end{itemize}
  \item Eight linear mixed model (LMM) trees: 
  \begin{itemize}
    \item Two trees fitted using default settings, where parameter stability tests use observation-level covariances and estimation is initialized with the tree structure. 
    \item Two trees where the parameter stability tests use cluster-level covariances.
    \item Two trees where estimation is initialized with the random effects. 
    \item Two trees where the parameter stability tests use cluster-level covariances and estimation is initialized with the random effects.
  \end{itemize}
\end{itemize}  
    
Each of the two LMM trees were fitted with different specifications of the random effects: One of the LMM trees estimated random intercepts only, one of the LMM trees estimated both random intercepts and slopes. Because variances of random intercepts and slopes were $> 0$ in all cells of the design, the three different random-effects specifications (LM trees assume 0 random effects) allow for assessing the effect of mis-specifying the random effects. To fit LM trees, we used package \textbf{partykit} \citep[version 1.2-15; ][]{HothyZeil15}. To fit LMMs, we used package \textbf{lme4} \cite[version 1.1-29; ][]{BateyMach15}. To fit LMM trees we used package \textbf{glmertree} \citep[version 0.2-0; ][]{FokkySmit18}.
...
Figure~\ref{fig:LMM_sizes} depicts the number of splits implemented by each partitioning approach. Irrespective of the random-effect specification, the default fitting approaches overfit, implementing $> 3$ splits in a large majority of datasets. Irrespective of the random-effects specification, the overfitting is successfully mitigated by the use of cluster-level covariances in the parameter stability tests (mean number of splits \Sexpr{round(M_cluster, digits = 2)}), which appears most effective for LMM trees: the mean number of splits was \Sexpr{round(means["lmer_c"], digits = 2)} for LMM trees with random intercepts only and \Sexpr{round(means["lmer_s_c"], digits = 2)} for LMM trees estimating both random intercepts and slopes.

Initializing estimation with the random effects was also effective in reducing tree size, bput the effect depended on the random-effects specification. When random slopes were estimated in addition to random intercepts, too few splits were implemented on average: the mean number of splits was \Sexpr{round(means["lmer_r"], digits = 2)} for LMM trees with random intercepts only, and \Sexpr{round(means["lmer_s_r"], digits = 2)}) for LMM trees estimating both random intercepts and slopes. Combined use of random-effects initialization and cluster-level covariances does not appear beneficial, yielding too many splits, irrespective of the random-effects specification (mean number of splits \Sexpr{round(M_both, digits = 2)}).

Distributions of the number of splits, separated according to the levels of the data-generating design are depicted in Figure~\ref{fig:LMM_sizes_interact} (Appendix~\ref{sec:AppendixA}). The plots indicate little interaction between data-generating parameters and model-fitting approached. Figure~\ref{fig:LMM_sizes_interact} shows a pattern very similar to Figure~\ref{fig:LMM_sizes}, where the use of cluster-level covariances provides the most accurate split recovery. Main effects of the data-generating design were as expected: Increasing values of $\sigma_{b_0}$, $N$, $p$ and $\sigma_{b_1}$ yield higher numbers of splits. The effect of $\rho$ was minor. The only evidence of an interaction between data-generating parameters and model-fitting approaches is that increased variance of the random effects (higher $\sigma_{b_0}$ and/or $\sigma_{b_1}$) yields lower numbers of splits if random slopes are estimated and estimation is initialized with the random effects. Thus, with stronger random effects and a more complex random-effects specification, differences in fixed effects (which should be captured by the tree) tend to be captured by the random effects when estimation is intialized with the random effects. 
