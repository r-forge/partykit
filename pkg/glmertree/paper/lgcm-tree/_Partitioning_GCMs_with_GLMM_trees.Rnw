...
\emph{FIXME: The `Note` below Table~1 (and~2) is too close to the lower horizontal rule, making it hard to read. The order of of the rows should match the order of the panels in
Figure~2, possibly aided by horizontal rules (then each combination of sigmas only has to be spelled out once).}
...
\emph{README: Are these power transformations for months by 1/2 or 2/3 standard in the literature? Should this be cited in some way or is this uncontroversial?}
...
\emph{FIXME: Maybe better clarify what `existing data' in the next sentence means. I think you are referring to data where we have to evaluate out-of-sample rather than with the `true' effects in-sample, right?}
...
\emph{README: The conclusion not to consider SEM trees which performed rather well in the simulation studies is a bit unsatisfying. Also, longRPart was not included in the simulation studies. Overall, the new insights gained compared to the simulation studies are not so huge. So maybe we should discuss again how to best present this?}
...
Table~\ref{tab:application_MSEs} presents means and standard deviations of the MSEs. Only few differences in performance between LM(M) and longRPart trees are larger $> 1$ standard error: For reading with $N=250$ and for science with both sample sizes. Note that the standard errors in the pairwise $t$-tests get smaller with a larger number of replications, while the standard errors in Table~\ref{tab:application_MSEs} do not. The latter thus provide a more useful measure of effect size, while the former have higher power to detect differences between methods.
