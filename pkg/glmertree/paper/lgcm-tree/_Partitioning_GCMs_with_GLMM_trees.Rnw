...
In the following two subsections provide some more details on alternative initialization strategies for Step~0 and details of the GLM tree algorithm from Step~1, respecitvely. Each subsection first reviews the well-established methods and the proceeds to discuss modifications that may improve the performance when partitioning LGCMs using longitudinal data.


\subsection{Initialization}

Previous publications on recursive partitioning of models with mixed effects find that initializing the random-effect estimates with zero yields accurate estimates of the subgroup memberships and thus of the final models  \citep{HajjyBell11, HajjyBell14, HajjyLaro17, SelaySimo12, FuySimo15, FokkySmit18}. \cite{SelaySimo12} assessed the impact of different initialization values and found only minor differences that became decreased further with increasing sample size. In \cite{FokkySmit18}, we found initializing estimation of GLMM trees with zero random effects performed well in cross-sectional clustered data. With longitudinal data, however, random effects tend to be more pronounced: Repeated measures on the same subjects tend to be correlated more closely than observations nested within the same unit in cross-sectional data. If the random effects are sizable, the inital assumption of zero random effects could provide an unrealistic starting point that may be difficult to overcome in subsequent iterations. Our expectation is that for partitioning LGCMs, initializing estimation with the random effects instead of the subgroup structure may improve subgroup recovery and predictive accuracy. More specifically, that means the algorithm starts by estimating the classic version of the mixed-effects model from Equation~\ref{eq:GLMMtree} with just one set of fixed-effects coefficients $\beta$ and all subjects in a single group.


\subsection{Partitioning}

The subgroup structure in Step~1 is estimated by a GLM tree using the general model-based recursive partitioning (MOB) algorithm by \cite{ZeilyHoth08}. Here, we give a short general overview and then comment on the aspects of the algorithm that may be particularly relevant for LGCMs. In the case of GLMs (with random effects held constant in an offset), the MOB algorithm cycles iteratively through the following steps: 
