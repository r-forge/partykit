...
Development over time is of prime interest in psychological researchers. For example, researchers may want to model and predict student's reading or math development in educational studies \citep{MorgyFark09}, or patient's symptom reduction over the course of clinical trails \citep{Fein09}. Mixed-effects or latent-variable models can be used to model development over time, and allow for explaining possible heterogeneity with covariates of a-priori known relevance \citep{NeisyMatt18}. However, the covariates that are predictive of development may often not be known in advance, and identifying them may be of prime interest. 

Recursive partitioning methods (RPMs) allow for identifying relevant predictors from a potentially (very) large number of covariates. Several RPMs that allow for partitioning growth-curve models have been developed: GUIDE \citep{Loh02}, longRPart \citep{AbdoyLeBl02}, GEE-based decision trees \citep{Lee05}, longitudinal interaction trees \cite[IT; ][]{SuyMene11}, SEM trees \citep{BranyOert13}, mixed-effects longitudinal trees \citep[MELT; ][]{EoyCho14} and LongCART \cite{KundyHare19} allow for partitioning linear growth curve models (LGCMs). Further, the longRPart2 \citep{StegyJaco18} and IT-LT \citep{WeiyLiu20} methods allow for subgroup detection in non-linear growth curve models\footnote{Both IT methods have a specific aim of detecting subgroups for which the time-by-treatment interaction differs, so are not general methods for partitioning growth curve models.}.

Here we propose the use of generalized linear mixed-effects model trees \citep[GLMM trees; ][]{FokkySmit18} for partitioning LGCMs. GLMM trees were originally proposed for subgroup detection in clustered cross-sectional studies, where subjects are for example nested in treatment centers, classrooms and/or geographical areas. Yet, GLMM trees provide a more general method 2for subgroup detection in almost any type of mixed-effects model. Compared to clustered cross-sectional data, however, longitudinal data may require a different estimation approach: The variance of random effects tends to be higher with longitudinal data, and predictors of interest tend to be measured at higher levels (e.g., time-invariant covariates). In this paper, we propose and test two recent extensions of GLMM trees that account for these characteristics. We focus on the specific use case of partitioning LGCMs, but the extensions are critical for a wide range of settings where covariates are measured at higher levels and/or random-effects variance is substantial.

The main characteristic that sets GLMM trees apart from other methods for partitioning LGCMs is its local-global estimation approach: GLMM trees do not fit a full parametric model in each of the terminal nodes. Instead, the fixed-effects parameters are estimated \textit{locally}, using the observations within a terminal node, and the random-effects parameters are estimated \textit{globally}, using all observations. This local-global estimation approach was first proposed by \cite{HajjyBell11} and \cite{SelaySimo12} for trees with constant fits in the terminal nodes. With GLMM trees, the approach was generalized to allow for GLMs with any number of parameters in the terminal nodes, thus allowing for non-Gaussian responses and targeted detection of a wide range of possible interaction effects \cite{FokkySmit18}.

All other methods for partitioning LGCMs take a fully local estimation approach: Within every terminal node, a full parametric model is estimated based the observations in that node only. This fully local estimation approach brings higher computational load and increased model complexity. GLMM trees require estimation of a (much) lower number of random-effects parameters, which likely reduces overfitting and improves stability and generalizability of the fitted model. Furthermore, the fully local estimation approach requires possible partitioning variables to be measured at the highest level of the multilevel structure, while the local-global estimation approach allows for partitioning variables to be measured at any level.

The computational advantage of GLMM trees may be highest compared to longRPart, longRPart2, IT-LT and LRT-based SEM trees. These methods employ an exhaustive split detection procedure: For every possible split in the current node, the full parametric model is re-estimated in the two nodes that would result from the split. To choose the optimal split, a likelihood ratio test is computed for every possible cut point. Not only does this bring a heavy computational load, it also introduces a selection bias towards covariates with a larger number of possible cut points \citep{ShihyTsai04, Shih04}. LongCART, MELT, GEE-based decision trees and score-based SEM trees also fit full parametric models in each of the nodes, but do not refit models for cutpoint selection; they employ the predictions or residuals from the fitted model in the current node for selecting the best split. This not only reduces computational load, it also allows for separating variable and cutpoint selection, thus preventing selection bias. The GLMM tree algorithm also employs a two-step approach to split selection.

In summary, compared to other methods for partitioning LGCMs, GLMM trees may provide lower computational load, lower model complexity and instability, and unbiased variable selection.

In what follows, we first describe estimation of GLMM trees, and possible adjustments to better account for longitudinal data structures. Next, we assess and compare performance of GLMM trees for partitioning LGCMs: In Simulation~Study~I, we assess performance of the proposed adaptations to GLMM trees. In Simulation~Study~II, we compare performance of GLMM trees with that of two other methods: SEM trees and LongCART. In the Application, we compare the performance of GLMM trees and longRPart in existing datasets on children's development of reading, math and science abilities. In the Discussion, we summarize our findings and implications.
...
\end{eqnarray}
%
Thus, in the current paper, $g$ is the identity function and we assume a continuous response with normally distributed errors and constant variance $\sigma_\epsilon$. Generalizations to other response variable types within the GLM are straightforward. Further, $X_i$ is the $n_i \times (p+1)$ fixed-effects design matrix for subject $i$ ($i = {1, \dots, N}$). We furter assume time is the predictor variable of interest, thus $p=1$ and $X_i$ comprises a column of 1s for the intercept and a column of observed timepoints for subject $i$. Note that the number and spacing of observed timepoints may differ between subjects. $\beta_j$ is a column vector of node-specific fixed-effects parameters, its value depending on terminal node $j$ of which observation $i$ is part. As in a traditional GLMM, $Z_i$ is the random-effects design matrix for subject $i$, comprising a subset of columns of $X_i$, and $b_i$ is a vector of random effects for subject $i$. Finally, $b$ is assumed to follow a (possibly multivariate) normal distribution with mean zero and (co)variance $\Sigma_{b}$. 

The parameters of a traditional GLMM can be estimated by, for example, (restricted) maximum likelihood. The GLMM tree model is estimated by alternating between estimating the partition (i.e., subgroups or terminal nodes $j$), and estimating the random- and fixed-effects parameters, as per the following algorithm: 
...
The algorithm initializes by assuming zero random effects. In every iteration, the partition is (re-)estimated using a GLM-based recursive partition in Step~1, while the fixed- and random-effects parameters are (re-)estimated in Step~2. Thus, the random effects are not partitioned, but estimated \textit{globally}, using all observations in the dataset. The fixed effects are estimated \textit{locally}, using the observations in the current node. Convergence of the algorithm is monitored through the log-likelihood of the mixed-effects model fitted in Step~3. Typically, this converges if the tree does not change from one iteration to the next. 


\subsection{Initialization}

Studies to date indicate that initializing estimation with zero random effects yields accurate final models \citep{HajjyBell11, HajjyBell14, HajjyLaro17, SelaySimo12, FuySimo15, FokkySmit18}. \cite{SelaySimo12} assessed the impact of different initialization values and found minor impact that became smaller with increasing sample size. In \cite{FokkySmit18}, we found initializing estimation of GLMM trees with zero random effects performed well in cross-sectional clustered data. With longitudinal data, however, random effects tend to be stronger: Repeated measures on the same subjects tend to be correlated more strongly than observations nested within the same unit in cross-sectional data. If the random effects are sizable, the inital assumption of zero random effects could provide an unrealistic starting point that is difficult to overcome in subsequent iterations. Our expectation is that for partitioning LGCMs, initializing estimation with the random effects instead of the subgroup structure may improve subgroup recovery and predictive accuracy.


\subsection{Partitioning}

The subgroup structure in Step~1 is estimated using model-based recursive partitioning \citep[MOB; ][]{ZeilyHoth08}. The MOB algorithm cycles iteratively through the following steps: 

\begin{enumerate}
\setlength\itemsep{0em}
\renewcommand{\labelenumi}{\alph{enumi})}
	\item Fit the parametric model to all observations in the dataset. 
	\item Test for parameter instability with respect to each of the partitioning variables.
...
Parameter stability in Step~(b) is tested using the the \textit{scores} (gradient contributions) from the model fitted in Step~(a). Under mild regularity conditions, the scores have an expected value of 0. The parameter stability tests evaluate whether the scores fluctuate randomly around their mean of 0, or exhibit systematic deviations when ordered by the values of a covariate. For continuous covariates (or ordered covariates with a large enough number of unique values), this involves computing the cumulative score process $W_k(t)$ with respect to each potential partitioning variable $U_k$ \citep{ZeilyHoth08}:
...
where $\hat{J}$ is a suitable estimate of the covariance matrix of the parametric model fitted in Step~(a), and $n_j$ gives the number of observations in the current node. Further, $\hat{\psi}_{\sigma(U_{ik})}$ denotes the estimated scores, with subscript {$\sigma(U_{ik})$} denoting their ordering by the values of partitioning variable $U_k$. Note that $0 \leq t \leq 1$, thus $n_jt = 1$ for an observation with a unique minimum on the partitioning variable, and $n_jt = n_j$ for an observation with a unique maximum. 
...
We performed a simulation study to compare the performance GLM(M) trees estimated by initializing estimation with the subgroup structure versus the random effects, and employing observation- versus cluster-level covariances for the parameter stability tests.
...
We applied ten partitioning approaches to every generated dataset, each employing one of three different random-effects specifications: 

  \begin{itemize}
    
    \item $\hat{\sigma}_{b_0} = \hat{\sigma}_{b_1} = 0$. That is, random effects and thus their variance were fixed to 0, and two fixed-effects linear model (LM) trees were fitted.
    \begin{itemize}
        \item The default observation-level covariances were used to compute the parameter stability tests.
        \item Clustered covariances were used to compute the parameter stability tests. 
    \end{itemize}
    
    \item $\hat{\sigma}_{b_0} > 0; \hat{\sigma}_{b_1} = 0$. That is, four linear mixed-effects model (LMM) tree were fitted in which the variance of the random intercept was freely estimated and the variance of the random slope was fixed to 0: 
    \begin{itemize}
      \item Default settings were used, in which parameter stability tests employed observation-level covariances and estimation is initialized with the tree structure. 
    \item Instead, clustered covariances were used to compute the parameter stability tests.
    \item Instead, estimation was initialized with the random effects. 
    \item Instead, clustered covariances were used to compute the parameter stability tests and estimation was initialized with the random effects.
  \end{itemize}
    
    \item $\hat{\sigma}_{b_0} > 0; \hat{\sigma}_{b_1} > 0$. That is, four LMM trees were fitted in which the variance of both the random intercept and slope were freely estimated. The same four approaches were applied (default, clustered covariances, random-effects initialization, clustered covariances and random-effects initialization) as with the previous LMM tree specification.
  \end{itemize}

To fit LM trees, we used package \textbf{partykit} \citep[version 1.2-15; ][]{HothyZeil15}. To fit LMMs, we used package \textbf{lme4} \cite[version 1.1-29; ][]{BateyMach15}. To fit LMM trees we used package \textbf{glmertree} \citep[version 0.2-0; ][]{FokkySmit18}. To compute clustered covariance, we used package \textbf{sandwich} \citep[version 3.0-1; ][]{ZeilyKoll20}. We employed outer product of gradients method to compute covariances, thus employing only the meat of the sandwich estimator to estimate the covariances.
...
plot0 <- ggplot(tmp[LMM_ids, ]) +
  ...
  geom_count(aes(x=setting, y=tree_size), col = "grey", alpha = .75) +
...
{\textit{Note.} Grey circles represent counts, grey lines represent true number of splits. Panel labels indicate whether variances of the random intercept and slope were fixed to 0 or freely estimated. Distances on $y$-axis are on the log scale. cl.cov. = cluster-level covariances employed in parameter stability tests; ran.eff. = estimation initialized with the random effects. MAD = Mean absolute deviation from true tree size.}
\label{fig:LMM_sizes}
\end{figure}

Figure~\ref{fig:LMM_sizes} depicts the number of splits implemented by each partitioning approach. Irrespective of the random-effect specification, the default fitting approaches (left-most box in every panel) tended to overfit, implementing $> 3$ splits in \Sexpr{format(100*mean(c(props_more_splits["lm"], props_more_splits["lmer"], props_more_splits["lmer_s"])), digits = 2L)}\% datasets, on average. This overfitting was successfully mitigated through use of cluster-level covariances in the parameter stability tests, irrespective of the random-effects specification. The average number of splits of LMM trees employing clustered covariances were closest to the true number of splits. In terms of mean absolute deviation (MAD) from the true tree size, LM trees with clustered covariances performed best, closely followed by the LMM trees employing clustered covariances.

In terms of MAD, however, the LMM tree estimating random intercepts only performed even slightly better than the latter two approaches.  Thus, initializing estimation with the random effects can successfully mitigate the overfitting observed for the default LMM trees, but the effect depens strongly on the random-effects specification. With estimation of both a random intercept and slope, random-effects initialization may lead to subgroup differences being captured by the random effects instead of the tree structure. Use of random-effects initialization and cluster-level covariances in tandem was less effective in reducing the number of splits than using either of the two approaches.

Distributions of the number of splits, separated according to the levels of the data-generating design are presented and discussed in  Figure~\ref{fig:LMM_sizes_interact} and Appendix~\ref{sec:AppendixA}, because they show a pattern very similar to Figure~\ref{fig:LMM_sizes}. Use of cluster-level covariances provides the most accurate split recovery. Of the data-generating parameters, $\sigma_{b_1}$ and $N$ showed the strongest effects, with higher values resulting in a higher number of splits. The number of noise variables and $\sigma^2_{b_1}$ had smaller effects, and were involved in minor interactions: With the default fitting approach, they had a positive effect on the number of implemented splits; with clustered covariance or random-effects initialization, they had a negative effect on the number of splits. The correlation between partitioning variables hardly affected the number of implemented splits.
...
## Print the table
kable_styling(add_footnote(kable(splits[1:10, -1], format = "latex", booktabs = TRUE, 
                                 label = "first_splits", align = "llccccc", 
        escape=FALSE, linesep = "", # linesep command suppressess addlinesep every 5 rows
        caption = "Variables selected for the first split by each LM tree (top two rows) and LMM tree (bottom eight rows) estimation approach.", row.names = FALSE, digits = 3L),
...
plot1 <- ggplot(tmp2) +
  ...
  geom_count(aes(x=random, y=tree_size), col = "grey", alpha = .75) +
  geom_hline(yintercept=3, col = "darkgrey") +
...
\vspace{-0.5cm}%
{
\footnotesize \textit{Note.} Grey circles represent counts, grey lines indicate true number of splits. Distances on the $y$-axis are on the log scale. $x$-axis labels indicate whether variances of the random intercept and slope were fixed to 0 or freely estimated. MAD = Mean absolute deviation from true tree size.}
...
Table~\ref{tab:first_splits2} presents variable selection frequencies for the first split in the fitted trees. For SEM trees, the LRT criterion yields almost perfect accuracy for the first split. The score-based approach selected the wrong variable for the first split in about \Sexpr{round(100*(1 - mean(splits$`$U_1$`[grep("s_sem", rownames(splits))])))}\% of datasets, but only when the random-effects structure was misspecified. When random effects were correctly specified, score-based SEM trees also provided perfect recovery of the first split. Closer inspection of stability tests seperately per model parameter suggested that the the score-based tests for SEMs are more sensitive to instability in the fixed slope than in the fixed intercept.

LongCART trees exhibit low accuracy for recovering the first split, selecting the wrong variable in all datasets where at least one split was implemented. LongCART showed an even stronger tendency than score-based SEM trees to select $U_2$ or $U_3$ for the first split. Of note, in the datasets where LongCART did not implement any splits, in \Sexpr{round(100*prop.table(table(Lcrt_splitvars[tree_size$Lcrt == 1L] == "x1"))["TRUE"])}\% percent of datasets $U_1$ was the strongest splitting candidate, but the parameter stability test simply did not reach the criterion for significance. This suggests the parameter stability tests proposed by \cite{KundyHare19} are less sensitive to instabilities with respect to the fixed intercept, or less sensitive to instability with respect to a categorical covariate. 
...
ggplot(times, aes(Var2, value)) + 
  geom_violin(, alpha = .5, fill = "grey", width=.5) +
...
Figure~\ref{fig:comp_times} presents computation time distributions for the partitioning algorithms. A clear computational advantage is observed for LM trees. LMM trees require longer computation times because of the estimation of random effects. Yet, this increase is minor compared to the computation times required by LongCART and SEM trees: LRT-based SEM trees required strikingly computation times, while score-based SEM trees were computationally more efficient, as expected. Computation times for LongCART were in between LRT- and score-based SEM trees. 
...
We analyzed longitudinal assessments of children's reading, math and science abilities from the Early Childhood Longitudinal Study-Kindergarten class of 1998-1999 \citep[ECLS-K; ][]{NCES10}. Data were collected from 21,304 children from 1,018 schools across the USA were collected. Assessments took place from kindergarten in 1998 through 8th grade in 2007. In the current analyses, we focus on the assessments from kindergarten, 1st, 3rd, 5th and 8th grade.
...
Our analyses focus on predicting trajectories of reading, math and science abilities, based on baseline characteristics. Time was measured as the number of months since the baseline assessment. In order to obtain approximately linear trajectories, $\mathrm{months}^{1/2}$ was used as the timing metric for reading and math trajectories, and $\mathrm{months}^{2/3}$ for science trajectories.
...
The ECLS-K datasets have exceptionally large sample sizes. We therefore employed random sampling to obtain training samples of $N=250$ and $N=1,000$ children, likely more representative of real-world studies in psychology. For each combination of response variable (math, reading or science) and sample size, we performed 100 repetitions. We evaluated predictive accuracy by computing the mean squared difference between predicted and observed response variable values for all children not included in the training sample of the current repetition. Note that this measures the accuracy of the fixed-effects predictions only. Tree size was measured by the number of splits in each tree, and computation time was measured in seconds. 
...
ggplot(tmp3) +
  geom_violin(aes(x=random, y=MSE, fill = N), 
               position=position_dodge(1), alpha = .5, width = .7) +
...
\end{subfigure}%
\vspace{-0.5cm}%
{\footnotesize \textit{Note. }The second $y$-axis (far right) gives an estimate of the proportion of variance explained, computed as $1 - \frac{MSE}{var(y)}$. Distances on $y$-axis are on the log scale.}
...
LongRPart trees comprising both random intercepts and slopes yielded estimation errors in \Sexpr{(table(is.na(math_MSEs$longRPart_s))["TRUE"]/200)*100}\% of samples from the Math data and in \Sexpr{(table(is.na(scie_MSEs$longRPart_s))["TRUE"]/200)*100}\% of samples from the Science data. All other model-fitting attempts converged successfully. Results are presented for successful model estimations.

Figure~\ref{fig:application_MSEs} presents MSE distributions for each of the partitioning approaches, separated by sample size. As expected, all methods show higher accuracy with larger sample sizes. LM(M) trees tend to show better predictive accuracy than longRPart trees. Pairwise $t$-tests indicated significantly better performance for each of the LM(M) tree approaches employing clustered covariances, compared to both longRPart approaches (Bonferroni corrected $p$-values all $< .01$). However, these performance differences are small; the effects of sample size are much more obvious than the differences between methods in Figure~\ref{fig:application_MSEs}. 

Table~\ref{tab:application_MSEs} presents means and standard deviations of the MSEs. Only few differences in performance between LM(M) and longRPart trees are larger $> 1$ standard error: For Reading with $N=250$ and for Science with both sample sizes. Note that the standard errors in the pairwise $t$-tests are get smaller with a larger number of replications, while the standard errors in Table~\ref{tab:application_MSEs} do not. The latter thus provide a more useful measure of effect size, while the former have higher power to detect differences between methods.

Table~\ref{tab:application_MSEs} indicates that LMM trees estimating only a random intercept and employing cluster-level covariances performed best for predicting Math and Reading abilities. For predicting science abilities, LMM trees initializing estimation with the random effects and estimating only a random intercept performed best. Random-effects initialization was the worst-performing LMM tree approach for Math and Reading. 
...
ggplot(tmp6) +
  geom_violin(aes(x=random, y=splits, fill = N), 
               position=position_dodge(1), alpha = .5, width = .7) +
  facet_grid(~method2, scales = "free", space = "free") +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank()) +
...
<<echo=FALSE, fig=TRUE, height=3.25, width=3>>=
...
## Plot
ggplot(ECLSK_times) +
  geom_violin(aes(x=ind, y=values), alpha = .5, fill = "grey", width=.7) +
...
In line with expectations, the simulation study showed that the default estimation approach for GLMM trees yields too many splits when partitioning LGCMs. As hypothesized, performing parameter-stability tests using clustered covariances mitigated this overfitting. Also as expected, initializing estimation with the random effects mitigated overfitting, but too strongly when both random intercepts and slopes were estimated. The performance of of LM(M) trees using clustered covariances appeared largely unaffected by (mis-)specification of the random effects, and could therefore be the safest choice in practice. Combining cluster-level covariances and random-effects initialization worsened performance, os in practice users should use either one of the two approaches.

The simulations showed comparable performance of LM(M) and SEM trees in partitioning LGCMs. SEM trees did however show stronger sensitivity to misspecification of the random effects, with underspecification yielding a higher number of splits. Furthermore, in line with results of \cite{ArnoyVoel21}, we found score-based SEM trees to have somewhat lower power than LRT-based SEM trees, but computationally much less burdensome. LongCART trees often selected the wrong partitioning variable for the first split, and were outperformed by LM(M) and SEM trees. The LongCART parameter stability tests \citep{KundyHare19} may be underpowered for detecting systematic instability of the fixed intercept, or for detecting instability with respect to categorical covariates.  

In the application, we found higher predictive accuracy for LM(M) trees than for longRPart, but differences in performance were small. In line with the simulation studies, we found performance of LM(M) trees did not depend on (mis-)specification of the random effects, which may provide an important practical advantage. 

Both simulation and application studies indicated a substantial computational advantage of GLM(M) trees. This is likely due to the local-global estimation approach employed by GLMM trees, where fixed-effects parameters are estimated locally within a node and random-effects parameters are estimated globally, using all observations. In contrast, SEM trees, LongCART and longRPart fit the full mixed-effects model in each node, which may substantially increase computational burden. The local-global estimation approach also reduces model complexity and the good performance of GLMM trees in the current study suggests this may be beneficial for stability and generalizability.

Yet, a possible downside of the local-global estimation approach is that it does not allow for recovering subgroups with differences in random-effects parameters. When there is a specific interest in partitioning the random effects, score-based SEM trees should be preferred. Alternatively, researchers may want to use the parameter stability tests for mixed-effects models developed by \cite{WangyMerk18} \citep[see also ][]{WangyMerk21,WangyGrav22}. This will be useful, for example, when the number and/or distances between timepoints differs between respondents and SEM-based growth curve models can thus not be applied \cite{NeisyMatt18}.

The focus of the current study was limited to Gaussian responses and LGCMs. Future studies should assess performance of GLMM trees in partitioning longitudinal data with, for example, binomial or count responses. We expect that the good performance of clustered covariances in the parameter stability tests generalizes to other settings where covariates are measured at higher levels, either in longitudinal and/or otherwise nested data structures. Yet, whether our conclusions generalize to settings beyond LGCMs should be assessed in future studies. Finally, we used the outer-product-of-gradients (OPG) estimator for computing (clustered) covariances. Future studies could assess whether computation and use of the full sandwich estimator further improves performance of the parameter stability tests.
