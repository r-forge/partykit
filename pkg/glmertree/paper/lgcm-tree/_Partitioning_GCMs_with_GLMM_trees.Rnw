...
lines(x = c(min(data$months), max(data$months)), 
     ...
     lwd = 3, col = 2)
@
\\\footnotesize{\textit{Note.} Gray lines depict observed individual trajectories, red line depicts average growth curve as estimated with a linear mixed-effect model, comprising a fixed effect of time and a random intercept with respect to individuals. The $x$-axis is not linear in the number of months, because time was scaled as $(\text{\# of months})^\frac{2}{3}$ in order to obtain the approximately linear trajectories shown here. To facilitate interpretation, the labels on the $x$-axes represent values in the original time metric.}
\end{subfigure}%
...
Throughout this paper we focus on the case with a continuous, normally-distributed response $y_i$ with constant variance $\sigma_\epsilon^2$. Therefore, the identity function can be used for the link $g$ but generalizations to other response variable types within the GLM are straightforward. In the general notation above, $X_i$ is the $n_i \times (p + 1)$ fixed-effects design matrix for subject i $(i = 1, \dots, N)$, comprising $p$ regressors plus one column of 1s for the intercept. In this paper, we assume that time is the only time-varying covariate of interest (i.e., $p = 1$), but other time-varying covariates can also be included in $X_i$. The number $n_i$ and spacing of observed timepoints may differ between subjects. Ttime-constant covariates are not contained in $X_i$, but enter the model through the subgroup indicator $j$. The value of the fixed-effects parameters $\beta$ (here, intercept and time slope) in GLMM trees are node-specific, their value depending on the subgroup/node $j$ into which subject $i$ falls. Subgroup membership is defined by the values of time-constant covariates\footnote{In principle, it is also possible to use time-varying covariates for partitioning with GLMM trees, but this might not be appropriate for partitioning growth curves, as participants changing subgroups over time yields discontinuous growth curves.}.

As in the standard GLMM \citep[e.g., ][]{PinhyBate00}, GLMM, $Z_i$ is the random-effects design matrix for subject $i$, comprising a subset of columns of Xi, and bi is the corresponding vector of random effects for subject $i$. Finally, $b_i$ is assumed to follow a (possibly multivariate) normal distribution with mean zero and (co)variance $\Sigma$. 
...
The tree in Figure~\ref{fig:LMM_tree_c} was estimated using cluster-level parameter stability tests. Combining cluster-level parameter stability tests and random-effects initialization yielded the exact same tree structure and parameter estimates. Compared to the trees in Figures~\ref{fig:LMM_tree_r} and \ref{fig:LMM_tree}, the cluster-level parameter stability tests provided the most parsimonious tree structure thus far. Table~\ref{tab:ranef} presents the estimated random-effects parameters for the models fitted thus far. Not surprisingly, the estimated residual standard deviation ($\hat{\sigma}_\epsilon$) differs only very slightly between models. Yet, substantial differences can be seen for the standard deviation of the random intercept ($\hat{\sigma}_\epsilon$): With more subgroups or terminal nodes, less variance is captured by the random effects and more variance is captured by the tree structure. 
...
The variances in Table~\ref{tab:ranef} can also be used to quantify variability associated with the trajectories in the terminal nodes of the trees. The third column of Table~\ref{tab:ranef} presents pooled standard deviations that can be computed from variance estimates \citep{WestyKenn14}. These allow, for example, to gauge the magnitude of subgroup differences in the subgroup-specific intercepts in Table~\ref{tab:local_coefs}, which range from $d = \Sexpr{round(min(ds), digits = 2)}$ to $d = \Sexpr{round(max(ds), digits = 2)}$. Yet, these estimates are based on a modest sample size, and are based on training data only, so can be overly optimistic. In the next sections, we evaluate performance on simulated (Studies~I~and~II) and real data (Study~III).
...
tab$pool_sd <- c(sqrt(sum(vcv$vcov)), sqrt(sum(vcov_r$vcov)),
                 sqrt(sum(vcov_d$vcov)), sqrt(sum(vcov_c$vcov)))
colnames(tab)[4:7] <- c("# of nodes", "$\\hat{\\sigma}_b$", "$\\hat{\\sigma}_\\epsilon$", "$\\hat{\\sigma}_{\\text{pooled}}$")
kable_styling(
  kable(tab, format = "latex", linesep = "", row.names = FALSE, 
        label = "ranef", digits = 3, align = c("cllccc"), booktabs = TRUE, escape =FALSE, 
        caption = "Estimated random-effects parameters for fitted mixed-effects models in Figures 1 through 4."),
  position = "left")
@

<<echo=FALSE, eval=FALSE>>=
test_dat <- sciedata[!sciedata$CHILDID %in% bag_ids[ , 39], ]
test_dat$CHILDID <- factor(test_dat$CHILDID)
test_dat$GENDER <- factor(test_dat$GENDER)
test_dat <- test_dat[test_dat$RACE != 6, ]
lmmt_preds <- predict(lmm_tree, re.form = NA, newdata = testdat)
lmmt_c_preds <- predict(lmm_tree_c, re.form = NA, newdata = testdat)
lmmt_r_preds <- predict(lmm_tree_r, re.form = NA, newdata = testdat)
mean((test_dat$score - lmmt_preds)^2)
mean((test_dat$score - lmmt_r_preds)^2)
mean((test_dat$score - lmmt_c_preds)^2)
var(test_dat$score)
lmm_preds <- predict(LMM, newdata = test_dat, re.form = NA)
mean((test_dat$score - lmm_preds)^2)
@
...
Chosen values of $\sigma_{b_0}$ are reflective of intra-class correlations of \Sexpr{round(1/(1+5), digits = 3)} and \Sexpr{round(4/(4+5), digits = 3)} in designs with zero random-slope variance. Values chosen for $\sigma_{b_1}$ are reflective of effective curve reliabilities of \Sexpr{round(ecr(model2), digits = 3)} and \Sexpr{round(ecr(model1), digits = 3)} for $\sigma_{b_0}^2 = 1$, and \Sexpr{round(ecr(model4), digits = 3)} and \Sexpr{round(ecr(model3), digits = 3)} for $\sigma_{b_1}^2 = 4$ \citep{BrandyVonO18}. 
...
Table~\ref{tab:first_splits} shows the variables selected for the first split, and indicates high accuracy for recovery of the first split for all LM(M) tree fitting approaches. Only very rarely is $u_1$ not selected for the first split, if a first split was implemented.
...
<<ech0=FALSE, eval=FALSE>>=
## Identify which LongCART trees perform very badly. Latest version of LongCART
## May not perform as bad and may have tgvars argument fixed.
rng_lcrt <- range(tmp2$tree_size[tmp2$method_id == "Lcrt"])
lcrt_min_size <- rng_lcrt[1]
lcrt_max_size <- rng_lcrt[2]
length(which(tmp2$tree_size == lcrt_min_size & tmp2$method_id == "Lcrt")) ## too many
length(which(tmp2$tree_size == lcrt_max_size & tmp2$method_id == "Lcrt"))
tmp2[which(tmp2$tree_size == lcrt_max_size & tmp2$method_id == "Lcrt"), ]
## dataset_id has 3200 levels, so unique number for each repetition and row of design
as.numeric(tmp2[which(tmp2$tree_size == lcrt_max_size & tmp2$method_id == "Lcrt"), ]$dataset_id) %% 32
## So, this is the 30th dataset from the 
ceiling(as.numeric(tmp2[which(tmp2$tree_size == lcrt_max_size & tmp2$method_id == "Lcrt"), ]$dataset_id) / 32)
## 28th repetition, before the 28th repetition, there were 27:
27*32+30
...
We applied five LM(M) trees to the data, focusing on the original GLMM tree approach and approaches that performed well in the simulations:
% 
\begin{itemize}
  ...
  \item An LMM tree using observation-level covariances, with a random intercept freely estimated ($\hat{\sigma}_{b_0} > 0$, $\hat{\sigma}_{b_1} = 0$) and random-effect initialization with the full-sample estimate. 
\end{itemize}
%
...
Figure~\ref{fig:application_sizes} presents tree size distributions. Similar to the simulation study's results, default LMM trees implement the largest number of splits. Cluster-level covariances provide a robust reduction in the number of splits. Given their non-distinguishable predictive performance, LM trees with clustered covariances may be preferred if a obtaining a sparse result is critical. LMM trees with clustered covariances may provide less sparsity but better predictive accuracy, irrespective of whether random intercepts and/or slopes were estimated. In contrast to the simulation study results, random-effects initialization here yields tree sizes similar to the default fitting approach. 
...
Yet, a possible downside of the local-global estimation approach is that it does not allow for recovering subgroups with differences in random-effects parameters. When there is a specific interest in partitioning the random effects, score-based SEM trees should likely be preferred. Alternatively, researchers may want to use the parameter stability tests for mixed-effects models developed by \cite{WangyMerk18} \citep[see also ][]{WangyMerk21,WangyGrav22}. This will be useful, for example, when the number of or distances between timepoints differ between respondents so SEM-based growth curve models cannot be applied \citep{NeisyMatt18}.
...
\begin{figure}[!ht]
\caption{Observed trajectories before and after transforming time for reading ($N = \Sexpr{nrow(readdata)/5}$), math ($N = \Sexpr{nrow(mathdata)/5}$) and science ($N = \Sexpr{nrow(sciedata)/3}$) abilities in the Application.}
\begin{subfigure}{1.2\textwidth}%
