...
In the following two subsections provide some more details on alternative initialization strategies for Step~0 and details of the GLM tree algorithm from Step~1, respecitvely. Each subsection first reviews the well-established methods and then proceeds to discuss modifications that may improve the performance when partitioning LGCMs using longitudinal data.
