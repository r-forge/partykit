...
\emph{FIXME: Figure~2 and its caption are inconsistent because LM trees are not on the left but on the right. Also, the following paragraphs are inconsistent with respect to the discussion of MAD. The first occurrence of MAD is maybe M?}
...
In terms of MAD, however, the LMM tree estimating random intercepts only performed even slightly better than the latter two approaches.  Thus, initializing estimation with the random effects can successfully mitigate the overfitting observed for the default LMM trees, but the effect depends strongly on the random-effects specification. With estimation of both a random intercept and slope, random-effects initialization may lead to subgroup differences being captured by the random effects instead of the tree structure. Use of random-effects initialization and cluster-level covariances in tandem was less effective in reducing the number of splits than using either of the two approaches.
