...
To assess the performance of GLMM tree, we carried out three simulation studies. In Study I, we compared the accuracy of GLMM tree with that of GLM tree in datasets with treatment-subgroup interactions. In Study II, we compared the accuracy of GLMM tree with that of GLM tree in datasets without treatment-subgroup interactions, allowing for assessment of the Type-I error rate. In Study III, we compared the accuracy of GLMM tree with that of a more classical approach of interaction detection: a GLMM with pre-specified interactions. 


\subsection{Software}

R \cite{R} was used for the generation and analysis of all datasets. The \verb|partykit| package (version 1.0-2; \citeNP{HothyZeil15}) was employed for estimating GLM trees, using the \verb|lmtree| function for normal linear regressions and \verb|glmtree| is available for other response distributions. For estimation of GLMMs the \verb|lmer| function (or \verb|glmer| function, respectively) from the \verb|lme4| package (version 1.1-7; \citeNP{BateyMeac12}) was employed, using restricted maximum likelihood (REML) estimation.
%
...
\readme{Z: My impression is that the description of the data-generating process is too far ahead of the simulation itself. I would recommend to first explain what is the same in all three simulation studies. And then what is specific could maybe be included at the beginning of each of the individual studies so that readers have it in mind when looking at the results. So in this section here I would just briefly explain the idea and defer all details until later.}
...
\readme{Z: Also include the formula for the linear predictor in this setup so that this can be easily matched with the GLMM estimated.}
...
\readme{Z: Wouldn't it be more natural to just look at P(tree size greater than 1), i.e., the probability of splitting at all. Then it could be worked out more clearly that GLMM trees approximately keep the significance level while GLM trees generally have an inflated irror.}
