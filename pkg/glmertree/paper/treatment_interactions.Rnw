...
\readme{SD rather than var is used in the manuscript up to this point.}
...
\readme{Practitioners might still worry that the CAD variable is not selected more often -- only in a bit more than 50\% of the cases. Maybe this would be better in bootstrap sampling rather than resampling because the sample size is not reduzed?}
...
\readme{This summary is quite extensive. Personally, I would prefer a more condensed summary of `highlights'.}
