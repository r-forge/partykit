...
For the datasets with both piecewise and continuous interactions, the partition as depicted in Figure~\ref{fig:modelC} was also used. In addition, the fixed-effects part $x_{i}^{\top}\beta_{j}$ in each of the terminal nodes now comprised continuous main and (treatment) interaction effects of the partitioning variables. In other words, the partitioning variables $U_2$, $U_1$ and $U_5$ appear are also part of the linear predictor $x_i$, as part of the terms presented in Table~\ref{tab:continuous_terms}. The corresponding node-specific $\beta_j$ parameters are also presented in Table~\ref{tab:continuous_terms}. The $\beta_j$ values were chosen to yield the same treatment-subgroup means as in Figure~\ref{fig:modelC}. The interaction terms were created using centered $U_k$ variables, calculated by subtracting their variable means. Again, the outcome variable was calculated as $y_i = x_{i}^{\top}\beta_{j} + z_{i}^{\top}b + \epsilon_i$, with the value of $\beta_j$ depending on the values of $U_2$, $U_1$ and $U_5$. 

In datasets with purely continuous interactions, $\beta$ has a global value and no subscript, comprising only purely continuous main and interaction effects, as shown by the single $\beta$ column in Table~\ref{tab:continuous_terms}. The outcome variable was calculated as $y_i = x_{i}^{\top}\beta + z_{i}^{\top}b + \epsilon_i$.  
...
Finally, it should be noted that in terms of interpretability, LMM trees often provide simpler models: The LMMs included 12.30 significant terms on average, whereas LMM trees had 3.38 inner nodes on average, requiring only about 3-4 variables to be evaluated for making predictions. 
...
GLMM trees accurately recovered the subgroups in 90\% of simulated datasets with treatment-subgroup interactions, outperforming GLM trees without random effects. In terms of predictive accuracy, GLMM trees outperformed GLM trees as well as MERTs, predicting treatment-effect differences in test data with .94 accuracy, on average. In datasets without treatment-subgroup effects, GLMM trees showed a Type-I error rate 4\$, compared with a Type-I error rate of 33\% for GLM trees. 

Better performance of GLMM trees was mostly observed when random effects were sizeable and correlated with potential partitioning variables. In those circumstances, GLM trees are likely to detect spurious splits and subgroups. At the same time, GLM trees showed less power to detect subgroups in the presence of random effects. As expected, accuracy of MERTs was not affected by the presence of random effects, but only approached GLMM trees' accuracy in datasets with the largest sample and effect sizes. Especially with smaller effect and sample sizes (i.e., Cohen's $d=.5$ and/or $N = 200$), GLMM tree outperformed the other tree methods. Such effect and sample sizes are quite common in multi-center clinical trials, and GLMM trees may provide a useful tool for subgroup detection in those instances. 

In the absence of random effects, GLM and GLMM trees yielded very similar predictive accuracy. This finding is of practical importance, indicating that application of GLMM trees will not `hurt' predictive accuray when in fact, random effects are absent.

Compared to linear mixed-effects models with pre-specified interactions, GLMM trees provided somewhat better accuracy, on average. GLMM trees performed poorly in datasets with purely continuous interactions, but much better than GLMMs when interactions were at least partly piecewise. GLMM trees performed better in the presence of larger numbers of potential moderator variables, indicating that GLMM trees are much better suited for exploration that GLMMs. Also, GLMM trees may be easier to interpret and apply: The number of terms in a GLMM increases quadratically with the number of predictors, yielding complex models. GLMM trees in our simulations had a maximum tree depth of three, requiring evaluation of at most three variables to make a prediction for new observations. 

In the Application, the GLMM tree yielded somewhat higher predictive accuracy, while using a smaller number of variables than the other two methods (GLM tree and a GLMM with pre-specified interactions). The GLMM trees obtained over repeated subsamples of the training data proved to be relatively stable. 


\subsubsection{Limitations}

\readme{I would emphasize early on that GLMM trees rely on the properties of their building blocks, namely GLMMs and MOBs. The effect of misspecifications are studied to some degree for both methods. For GLMM one could cite papers discussing this. For MOB, or trees more broadly, there are also some references. One of these could be the `To Split or to Mix?' paper by Frick, Strobl, and Zeileis.}

Recursive partitioning methods were origfinally developed as a non-parametric tool for classification and regression, assuming the mechanism that generated the data unknown \cite<e.g., >{Brei01TwoCult}. GLMM trees take MOB and GLMMs as building blocks, thereby relying on the properties of these building blocks. As a result, GLMM trees may inherit their sensitivities to model misspecification as well as their robustness against it. We will address potential types of misspecification in fitting GLMM trees:

\readme{MF: GLMM tree at least tackles one type of random-effects misspecification: ignoring them. \cite{BarryLevy13} and \cite{Barr13} advice to keep it maximal is at repeated measures designs. \cite{McCuyNeuh11}: "A key conclusion is the large degree of robustness of maximum likelihood for a wide variety of commonly encountered situation". .... fit fixed-effects linear models in the nodes of the tree and a global model for the random effects, in turn introducing several distributional assumptions about the random effects and errors. Misspecification of these distributions will likely have a negative effect on the accuracy of the estimated GLMM trees.}

Furthermore, misspecification of partitioning and fixed- and random-effects variables will also reduce accuracy of the resulting GLMM trees. If relevant variables are omitted or incorrectly specified, GLMM trees can only approximate the true subgroups using the specified variables. Our simulations indicate that LM trees detect spurious subgroups as a result of misspecifying (that is, not including) the random effects. Reduced accuracy and spurious splits can also be expected to occur when relevant random effects are not included when specifying the GLMM tree model. Furthermore, as the random effects are estimated globally, misspecification of the random effects may have a strong impact on the resulting GLMM tree model.   

Another source of misspecification is the inclusion of irrelevant variables. Although our simulations indicate that the performance of GLMM trees was not negatively affected by increasing the number of noise variables specified for partitioning from 2 to 12, the power to detect subgroups may be reduced with much larger numbers of noise variables. Including irrelevant variables in the random or fixed effects may also negatively affect accuracy of GLMM trees, but we have not assessed this in our study. 

Users can evaluate potential misspecification by inspecting residuals and predictions, and assessing tree stability. The vigette included as Supplementary Material shows how residuals andrandom-effects predictions can be plotted to assess potential misspecification. In the Application we have shown how tree stability can be assessed using the methods suggested by \cite{PhilyZeil16}. Future studies may focus on the effects of possible misspecification on the performance of GLMM trees.

As GLMM trees fit more complex models than non-parametric tree-based methods, like CART for example, larger sample sizes are needed to fit the model. How much larger likely depends on the complexity of the specified model: Our simulations show that with fixed and random-effects specifications with only a single predictor, a sample size of 200 is sufficient to detect subgroups with moderate differences in treatment effect. More complex fixed- and random-effects specifications will require larger sample sizes; how these affect the performance of GLMM trees requires further research.


\subsubsection{Future directions}

\readme{MF: Add something about implementing non-normal non-parametric or mixture ranef}

In the Introduction we mentioned several existing tree-based methods for treatment-subgroup interaction detection. These methods have different objectives and there is not yet an agreed-upon single best method. In a simulation study, \citeA{SiesyVanM16} found the method of \citeA{ZhanyTsia12Stat} to perform best, followed by MOB. However, the method of Zhang et al. performed worst under some conditions of the simulation study in terms of the Type I error rate. Further research comparing tree-based methods for treatment-subgroup interaction detection is needed, especially  for clustered datasets, as our simulations and comparisons only focused on GLMM trees and GLM trees.
