...
did very well thrughout the semester (at this point we should state that the
...
Dependence plots with the group effect (treatment effect) on the y-achsis and
...
The R package \emph{model4you} is focussed on ease of use and interpretability.
...
functionalities is provided by the \emph{partykit} package which is a widely
