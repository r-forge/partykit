...
assigned treatments} with a focus on ease of use and
...
Currently the R package \emph{model4you} can handle only
...
as all variables not given in the model formula and can be defined using
...
@
\new{To investigate the correlation between exam group and exam performance,
we compute a simple linear model regressing the percentage points of
correct answers on the exam group.}
<<math_bmod>>=
...
@
\new{
We restrict the depth of the tree to two (\Sexpr{'maxdepth = 2'}) for
illustration purposes. If this setting is not used a more complex tree would
be estimated.
}
...
\new{Note that confidence intervals shown in the tree are not adjusted and should
hence be interpreted as a measure for variablity and not be used for inference.}
...
student characteristics and the treatment.
\new{
...
patient in the given data set.
}
Since the percentage of successful online tests is measured on a grid, a
\new{beeswarm} plot possibly shows the relationships
even better than the scatter plot (both shown in Figure~\ref{fig:math_dp1},
\new{
Figure~\ref{fig:math_dp11} includes a loess curve). We see a nonlinear relation
between the percentage of successful online \Sexpr{'tests'} and the exam group
effect. Students with great performance during the semenster are estimated to
...
exam (\Sexpr{'attempt'}) and \Sexpr{'gender'} we can use box plots,
\new{beeswarm} plots or a combination thereof can be used (as in
Figure~\ref{fig:math_dp21}). Figure~\ref{fig:math_dp22} shows the number of
previous attempts on the x-axis and the color indicates the gender of each student.
This way we can see that for students writing the exam for the first
...
compelling visualisations that can be used to communicate these models.



\section*{Implementation and architecture}
The R package \emph{model4you} is focused on ease of use and interpretability.
Users can take a simple model that they know and understand as basis.
\new{
Models currently available for use are linear models (function \Sexpr{'lm()'}
in R), generalised
linear models (\Sexpr{'glm()'}) and survival models (\Sexpr{'survreg()'} and
\Sexpr{'coxph()'} from the R package \emph{survival}). Models are restricted
to those with a single binary covariate (e.g. two treatment options).
The user can simply plug the model object into \Sexpr{'pmtree()'} or
\Sexpr{'pmforest()'} depending on whether they want subgroup wise or
personalised models.
}
