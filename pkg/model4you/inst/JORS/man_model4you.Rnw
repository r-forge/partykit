...
\section*{Abstract}
Typical models estimating treatment effecs assume that the treatment effect is
the same for all individuals observed and for possible future individuals.
Model-based recursive partitioning allows to relax this assumption.  With
...
similarity estimation between individuals in terms of model estimates and
through this we can estimate personalised models.  The R package
\emph{model4you} (available on CRAN) implements these stratified and
...
If you do not believe that all individuals have the same intercept and
treatment effect the R package \emph{model4you} gives you two options to loosen
up this strict model: \paragraph{(1) Model-based trees} identify subgroups
...
In R, to get from the initial base model to personalised models for all
observations in the training data, type:
<<eval=FALSE>>=
...
reproducibility -- set a random seed. Also for plots we will need the
\emph{ggplot2} package and we use a black and white theme.
<<prep>>=
library("model4you")
set.seed(2017)
