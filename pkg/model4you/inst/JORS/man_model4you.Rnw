...
@
Note that \Sexpr{'pmtree()'} uses the data given in the call of the base model.
It automatically uses variables not used in the model formula (in the example
above \Sexpr{'response ~ treatment'}) as potential subgroup defining variables.
This can be edited using the \Sexpr{'zformula'} argument.
...
\Sexpr{'zformula'} argument in \Sexpr{'pmforest()'}.
...
view the tree in the R console, the command \Sexpr{'print()'} function can be
used.
}
<<math_printtree>>=
print(tr_math)
...
\new{Note that confidence intervals shown in the tree are not adjusted and
...
\new{beeswarm} plot possibly shows the relationships even better than the
scatter plot (both shown in Figure~\ref{fig:math_dp1},
\new{
...
With the tools provided by the \emph{model4you} package it is very simple to
create understandable stratified and personalised models and compelling
visualisations that can be used to communicate these models.
...
in R), generalised linear models (\Sexpr{'glm()'}) and survival models
(\Sexpr{'survreg()'} and \Sexpr{'coxph()'} from the R package \emph{survival}).
Models are restricted to those with a single binary covariate (e.g. two
treatment options).  The user can simply plug the model object into
\Sexpr{'pmtree()'} or \Sexpr{'pmforest()'} depending on whether they want
subgroup wise or personalised models.
}

The basis for these functionalities is provided by the \emph{partykit} package
which is a widely used R package for trees and forests
\citep{hothorn_partykit_2015, hothorn_partykit_2017}.  The \emph{model4you}
package provides wrappers for the well implemented and tested functions
