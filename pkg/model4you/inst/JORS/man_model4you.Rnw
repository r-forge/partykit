...
stratified and personalised models with a focus on ease of use and
interpretability so that clinicians and other users can take the model they
usually use for the estimation of the average treatment effect and with a few
lines of code get a visualisation that is easy to understand and interpret.
...
\emph{treatment indicator}, which always takes values $0$
(individual in first group) and $1$ (individual in second group).
...
@
Note that \Sexpr{'pmtree()'} uses the data given in the call of the base 
model. It automatically uses variables not used in the model formula 
(in the example above \Sexpr{'response ~ treatment'}) as potential subgroup 
defining variables. This can be edited using the \Sexpr{'zformula'} argument.
...
personalised models can be found in \citep{seibold_individual_2017}.  
Computing personalised models for all observations in the training data is
simple:
<<eval=FALSE>>=
pm_forest <- pmforest(base_model)
pers_models <- pmodel(pm_forest)
@ 
Again here the potential effect-modifying variables are taken by default
as all variable not given in the model formula and can be defined using 
the \Sexpr{'zformula'} argument in \Sexpr{'pmforest()'}.
...
reproducibility -- set a random seed. Also for visualisations we need packages
\emph{ggplot2}, \emph{ggbeeswarm} and \emph{gridExtra}.
<<prep>>=
library("model4you")
set.seed(2017)
...
Innsbruck were divided into two examination groups. Group 1 wrote the exam in
...
To investigate whether the exam was fair, we asses whether the two groups
differ in terms of the percentage of correctly answered exam questions. This
can be done using a simple linear model regressing the percentage points of
correct anwers on the exam group. 
<<math_bmod>>=
...
@
% The estimtates and confidence intervals of this model are shown in 
% Table~\ref{tab:bmod}.
% <<math_bmod_coef, results="asis", echo=FALSE>>=
% library("Hmisc")
% cf <- round(cbind(estimate = coef(bmod_math), confint(bmod_math)), 2)
% colnams <- dimnames(cf)[[2]]
...
%       rowlabel = "", file = "", 
%       colheads = colnams,
%       label = "tab:bmod", caption = "Estimate and confidence intervals for the base model."
%       )
% @
%% <TH> warum brauchen wir die Abb 2x? Bitte nur einmal richtig ohne die
%% gg-Freuerei. /TH>
...
lm_plot(bmod_math)
@
Both the estimates and confidence intervals and the density curves suggest
that there is almost no difference between the two groups. But does this
really hold for all types of students?
% As the plot is a \emph{ggplot2} graphic it can be maniulated with the usual
% \emph{ggplot2} layers, e.g.\ with
% <<eval=FALSE>>=
% lm_plot(bmod_math) + labs(y = "Estimated Density", x = "% Correct")
% @
% or equivalently with 
% <<eval=FALSE>>=
% myplot <- function(model, ...) {
%   lm_plot(model, ...) + 
%     labs(y = "Estimated Density", x = "% Correct")
% }
% myplot(bmod_math)
% @
% The result is shown in Figure~\ref{fig:math_bmod_vis} (b).
% <<math_bmod_vis, echo=FALSE, fig.cap = "Base model for Mathematics Exam data.", fig.subcap=c("Figure made with \\texttt{lm\\_plot()}.", "Figure made with manipulated \\texttt{lm\\_plot()}."), out.width="0.45\\textwidth", fig.height=3, fig.width=5>>=
% lm_plot(bmod_math)
% myplot <- function(model, ...) {
%   lm_plot(model, ...) + 
%     labs(y = "Estimated Density", x = "% Correct")
...
percentage of successful online \Sexpr{'tests'}. These online tests were
...
than \Sexpr{round(tr_math[[5]]$node$split$breaks, 1)} percent correct). The
...
Dependence plots with the group effect (treatment effect) on the y-axsis and
...
student characteristics and the treatment. Since the percentage of successfull
online tests is measured on a grid, a bee plot possibly shows the relationships
even better than the scatter plot (both shown in Figure~\ref{fig:math_dp1}).
...
ggplot(dpdat_math, aes(x = tests, y = group2)) +
  ...
  ylab("individual estmated group effect")
...
bee plots or a combination thereof can be used (Figure~\ref{fig:math_dp2}).
...
With the tools provided by the \emph{model4you} package it is
very simple to create understandable stratified and personalised models and 
compelling visualisations that can be used to communicate theses models.



\section*{Implementation and architecture}
The R package \emph{model4you} is focused on ease of use and interpretability.
Users can take a simple model that they know and understand as basis and simply
plug it into \Sexpr{'pmtree()'} or \Sexpr{'pmforest()'} depending on whether
they want subgroup wise or personalised models. The basis for these
functionalities is provided by the \emph{partykit} 
package which is a widely
used R package for trees and forests \citep{hothorn_partykit_2015, hothorn_partykit_2017}. 
The \emph{model4you} package provides
wrappers for the well implemented and tested functions
\Sexpr{'partykit::ctree()'} and \Sexpr{'partykit::cforest()'} and extends the
functionalities to allow for the computation of personalised models and to 
improve usability and interpretability. 

The \emph{partykit} package provides the basis for functionalities in other
packages namely \emph{glmertree}, \emph{psychotree}, \emph{betareg} (all on
CRAN), \emph{trtf}, \emph{disttree}, \emph{lagsarlmtree} and \emph{palmtree}
...
(\Sexpr{'pmtree()'}) can be used; For personalised treatment effects
model-based forests (\Sexpr{'pmforest()'}) provide a way of estimating
similarity between patients and using this similarity measure to estimate
personalised models (\Sexpr{'pmodel()'}).  The target audience are people who
