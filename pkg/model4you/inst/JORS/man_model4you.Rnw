...
\Sexpr{'lm()'} in R), generalised linear models (\Sexpr{'glm()'}) and
survival models (\Sexpr{'survreg()'} and \Sexpr{'coxph()'} from the R
...
simply plug the model object into \Sexpr{'pmtree()'} or
...
implemented and tested functions \Sexpr{'partykit::ctree()'} and
\Sexpr{'partykit::cforest()'} and extends the functionalities to
