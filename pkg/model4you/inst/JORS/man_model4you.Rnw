...
%% <TH> bei mir fehlt hier die tilde </TH>
\Sexpr{'response ~ treatment'}) as potential subgroup 
...
@
%% <TH> hier koennte man ggf. die Parameter einmal herzeigen und
%% interpretieren </TH>
...
(see Figure~\ref{fig:math_bmod_vis} (a)):
<<eval=FALSE>>=  
lm_plot(bmod_math)
@
...
plot(tr_math, terminal_panel = node_pmterminal(tr_math, plotfun = myplot))
@
...
%% <TH> ich wuerde die Achsenbeschriftung ein bisschen besser machen, also
%% keine Variablennamen sondern lesbare Beschreibungen </TH>
...
%% <TH> hier fehlt das Zitat (CRAN + MLOSS) </TH>
package which is a widely
used R package for trees and forests. The \emph{model4you} package provides
...
%% <TH> Gib noch einen Link zum SMMR paper rein, das findet man sonst nicht
%% </TH>
