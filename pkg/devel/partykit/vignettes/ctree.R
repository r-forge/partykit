...
###################################################
### code chunk number 49: nf-save
###################################################
(airct_party_1 <- party::ctree(Ozone ~ ., data = airq, 
     controls = party::ctree_control(maxsurrogate = 3, savesplitstats = FALSE)))
object.size(airct_party_1)
object.size(airct_party_1@tree)
object.size(airct_partykit_1)
object.size(node_party(airct_partykit_1))
(airct_partykit_2 <- partykit::ctree(Ozone ~ ., data = airq, 
     control = partykit::ctree_control(maxsurrogate = 3, saveinfo = FALSE)))
object.size(airct_partykit_2)
object.size(node_party(airct_partykit_2))


###################################################
### code chunk number 50: nf-nmax
###################################################
...
###################################################
### code chunk number 51: nf-multiway
###################################################
...
###################################################
### code chunk number 52: nf-cluster
###################################################
...
###################################################
### code chunk number 53: nf-ytrafo-1
###################################################
...
###################################################
### code chunk number 54: nf-ytrafo-2
###################################################
...
h <- function(formula, data, ...) {
    ...
    return(function(subset, ...) {
        ...
        w[-subset] <- 0L
        mf$w <- w
        m <- glm(f, data = mf, family = gaussian(), weights = w)
        ef <- matrix(0, ncol = length(coef(m)), nrow = nrow(mf))
        ef[w > 0,] <- estfun(m)[w > 0,,drop = FALSE]
        list(estfun = ef, converged = m$converged, coef = coef(m))
    })
}
